# shellcheck shell=sh
# Update @openai/codex into a fresh versions/<pin> directory, then switch `current`.
#
...
mkdir -p "$target"
dep_log "Updating @openai/codex from ${MEMOH_DEP_CURRENT_VERSION:-unknown} to $MEMOH_DEP_VERSION in $target"

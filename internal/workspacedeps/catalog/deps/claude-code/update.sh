# shellcheck shell=sh
# Update @anthropic-ai/claude-code into a fresh versions/<version> directory, then switch `current`.
#
...
# NPM_MIRROR (§5.4). The previous version stays under versions/ for rollback.

# shellcheck shell=sh
# Install @anthropic-ai/claude-code into a fresh versions/<pin> directory, then switch `current`.
#
...
mkdir -p "$target"
dep_log "Installing @anthropic-ai/claude-code@$MEMOH_DEP_VERSION into $target"

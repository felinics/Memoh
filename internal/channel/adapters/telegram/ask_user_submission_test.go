...
func TestAskUserSummaryRequiresDurableAcceptance(t *testing.T) {
	for _, state := range []string{"handler-error", userinput.StatusPending, userinput.StatusSubmitted} {
		t.Run(state, func(t *testing.T) {
			...
			adapter := &TelegramAdapter{userInput: &submissionStore{status: state}}
			...
			if (err != nil) != (state == "handler-error") {
				t.Fatalf("result: %v", err)
			}
			if (edits.Load() == 1) != (state == userinput.StatusSubmitted) {

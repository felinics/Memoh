...
// DependencyItem is one dependency reference of a Package as seen on a
// workspace target.
...
// ConnectorItem is one connector reference of a Package.
...
// Item is one Package on a workspace target: an installation record with its
// components, or a dependency the workspace already carries shown through
// its canonical Package.
type Item struct {
	// Installation is nil for a discovered Package.
	Installation *Installation
	RegistryID   string
	PackageID    string
	Revision     string
	Version      string
	// Release is the materialized release; nil for discovered Packages and
	// for records whose cached release cannot be decoded.
	Release      *supermarket.SkillPackageDescriptor
	Discovered   bool
	Dependencies []DependencyItem
	Connectors   []ConnectorItem
}

// ListResult is the Package view of one workspace target.
...
// List returns every installed Package of a workspace target together with
// the canonical Packages of dependencies no installation references.
func (s *Service) List(ctx context.Context, botID, targetID string, refresh bool) (ListResult, error) {
	...
	if err != nil {
		return ListResult{}, fmt.Errorf("packages: list installations: %w", err)
	}
...
func (s *Service) assemble(ctx context.Context, botID, targetID string, installations []Installation, deps workspacedeps.ListResult) (ListResult, error) {
	...
	if err != nil {
		return ListResult{}, fmt.Errorf("packages: list dependency references: %w", err)
	}
	...
	for _, entry := range deps.Entries {
		...
		entry := entry
		result.Items = append(result.Items, Item{
			RegistryID: DependencyRegistryID, PackageID: entry.Dependency.ID, Discovered: true,
	...
	sort.SliceStable(result.Items, func(i, j int) bool {
		...
		return result.Items[i].PackageID < result.Items[j].PackageID
	})
	return result, nil
}
...
func (s *Service) item(ctx context.Context, inst Installation, entries map[string]workspacedeps.Entry, refCounts map[string]int, connectionsByID map[string]connectors.Connector) (Item, error) {
	...
	item := Item{
		Installation: &installation,
		RegistryID:   inst.RegistryID, PackageID: inst.PackageID,
	...
	if len(inst.Release) > 0 {
		...
		if err := json.Unmarshal(inst.Release, &release); err == nil {
			...
			s.logger.Warn("decode cached Package release", slog.String("installation_id", inst.ID), slog.Any("error", err))
		}
	}
	depRefs, err := s.store.ListDependencyRefs(ctx, inst.ID)
	if err != nil {
		return Item{}, fmt.Errorf("packages: list dependency references: %w", err)
	}
	...
	if err != nil {
		return Item{}, fmt.Errorf("packages: list connector references: %w", err)
	}
...
// CheckUpdates compares every installed Package with the registry's current
...
func (s *Service) CheckUpdates(ctx context.Context, botID, targetID string) (ListResult, error) {
	...
	for _, inst := range installations {
		...
		case supermarket.ErrorKindOf(err) == supermarket.ErrorNotFound || isNotFound(err):
			// The Package left the registry; nothing newer can be offered.
			_, err = s.store.SetCheck(ctx, botID, inst.ID, "", "", now)
		default:
			return ListResult{}, err
		}
		if err != nil {
			return ListResult{}, fmt.Errorf("packages: record update check: %w", err)
		}
	}
	var deps workspacedeps.ListResult
	if s.dependencies != nil {

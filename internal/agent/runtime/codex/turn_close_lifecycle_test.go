...
func TestClosedApprovalDoesNotParkFinishedRun(t *testing.T) {
	for _, terminalDelivered := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			st := newTurnState(ctx, external.PromptInput{BotID: h.BotID, ThreadID: h.SessionID, RunID: h.RunID, CanRequestUserInput: true, Sink: external.EventSinkFunc(func(ev event.StreamEvent) {
			...
			})}, "native-thread", flow, nil, nil, nil, slog.Default())
			defer st.close()
			done := make(chan struct{})
			go func() { defer close(done); st.decide(st.ctx, "pwd", "exec", map[string]any{"command": "pwd"}, nil) }()
			select {
			case s := <-seen:

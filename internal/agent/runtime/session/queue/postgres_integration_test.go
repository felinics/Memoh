...
func TestPostgresQueuesFIFOIsolationReplayAndTerminalRace(t *testing.T) {
	...
	if _, err := store.ClaimNextSteer(ctx, sessionID, run); err != nil {
		t.Fatal(err)
	}
	if err := queries.InTx(ctx, func(txq dbstore.Queries) error {
		if _, err := txq.LockSessionForQueueAdmission(ctx, dbsqlc.LockSessionForQueueAdmissionParams{BotID: mustUUID(t, botID), SessionID: mustUUID(t, sessionID)}); err != nil {
...
func TestPostgresClaimReclaimFencingAndSingleOutstanding(t *testing.T) {
	...
	first, _ := enqueue("fence-1"), enqueue("fence-2")
	store := queue.NewPostgresStore(queries)
	...
	if _, err := store.ClaimNextSteer(ctx, sessionID, stale); !errors.Is(err, queue.ErrNotPending) {
		t.Fatalf("claim with stale owner = %v, want ErrNotPending", err)
	}
	claimed, err := store.ClaimNextSteer(ctx, sessionID, current)
	if err != nil {
		t.Fatal(err)
	}
	oldRef := queue.SteerClaimRef{ItemID: claimed.ID, RunID: runID, OwnerID: current.OwnerID, FencingToken: current.FencingToken}
	if _, err := store.ClaimNextSteer(ctx, sessionID, current); !errors.Is(err, queue.ErrNotPending) {
	...
	if _, err := store.ApplySteerItem(ctx, oldRef); !errors.Is(err, queue.ErrInvalidReference) {
	...
	if _, err := store.ApplySteerItem(ctx, successorRef); err != nil {
		t.Fatal(err)
	}
	var status string
	...
	if _, err := store.ClaimNextSteer(ctx, sessionID, successor); err != nil {
...
func TestPostgresRejectClaimClearsClaimColumns(t *testing.T) {
	...
	run := sessionruntime.RunHandle{BotID: botID, SessionID: sessionID, RunID: runID, OwnerID: "owner", FencingToken: 1}
	if _, err := queue.NewPostgresStore(queries).ClaimNextSteer(ctx, sessionID, run); err != nil {
		t.Fatal(err)
	}
	if err := queries.InTx(ctx, func(txq dbstore.Queries) error {
...
func TestPostgresContinuationWaitsForParentRuntimeAndAppliesFollowUp(t *testing.T) {
	...
	go func() {
		...
		if err := manager.WaitSessionSlotFor(ctx, botID, sessionID, handoff.ContinuationRunID); err != nil {
...
func openQueuePostgres(t testing.TB, ctx context.Context) *pgxpool.Pool {
	t.Helper()
...
func createQueueFixture(t testing.TB, ctx context.Context, pool *pgxpool.Pool) (string, string, string) {
	t.Helper()
...
func mustUUID(t testing.TB, value string) pgtype.UUID {
	t.Helper()

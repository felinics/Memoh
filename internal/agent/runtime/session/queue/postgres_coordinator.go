...
func (c *PostgresCoordinator) CommitStep(ctx context.Context, req CommitStepRequest) (result CommitStepResult, err error) {
	...
	err = c.tx(ctx, func(txq dbstore.Queries) error {
		...
		if e = validateCommitClaims(ctx, store, req); e != nil {
			return e
		}
		...
		if req.Steer != nil {
			appliedSteer, lookupErr := store.SteerByID(ctx, req.Steer.ItemID)
			if lookupErr != nil {
				return lookupErr
			}
			if e = store.ApplySteer(ctx, *req.Steer); e != nil {
				return e
			}
		...
		pendingFollowUps, e := store.PendingFollowUp(ctx, req.Run.SessionID)
		if e == nil && len(pendingFollowUps) > 0 {
...
func validateCommitClaims(ctx context.Context, store *PostgresStore, req CommitStepRequest) error {
	if req.Steer != nil {
		item, err := store.SteerByID(ctx, req.Steer.ItemID)
		if err != nil {
			return ErrInvalidReference
		}
		claim := item.Claim
		if item.Status != Claimed || claim == nil || *claim != *req.Steer || item.TargetRunID != req.Run.RunID {
			return ErrInvalidReference
		}
	}
	if req.FollowUp != nil {
		item, err := store.FollowUpByID(ctx, req.FollowUp.ItemID)
		if err != nil {
			return ErrInvalidReference
		}
		claim := item.Claim
		if item.Status != Claimed || claim == nil || *claim != *req.FollowUp || item.AssignedRunID != req.Run.RunID {
			return ErrInvalidReference
		}
	}
	return nil
...
// Persist is the transaction-aware history callback used by CommitStep.
type StepPersistence func(context.Context, dbstore.Queries) error

...
func (s *Service) recoverQueuedSteerRun(ctx context.Context, candidate sessionruntime.LeaseCandidate) (bool, error) {
	...
	if err != nil {
		if rejector, ok := s.queueCoordinator.(sessionqueue.LostQueuedRunRejector); ok {
			_ = rejector.RejectLostQueuedRun(context.WithoutCancel(ctx), candidate.RunID, handle.FencingToken)
		}
		return true, nil
	}
	...
	if err != nil {
		if rejector, ok := s.queueCoordinator.(sessionqueue.LostQueuedRunRejector); ok {
			_ = rejector.RejectLostQueuedRun(context.WithoutCancel(ctx), candidate.RunID, started.FencingToken)
		}
		return true, nil
	}
...
func (s *Service) runQueueContinuation(parent context.Context, runID string, item sessionqueue.FollowUpItem) {
	...
	if parentRunID := strings.TrimSpace(item.EnqueuedDuringRunID); parentRunID != "" {
		if err := s.sessionManager.WaitRunRelease(ctx, metadata.BotID, metadata.SessionID, parentRunID); err != nil {
			if s.logger != nil && !errors.Is(err, sessionruntime.ErrManagerClosed) {
				s.logger.Warn("continuation parent release wait failed",

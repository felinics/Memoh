...
func (c *Client) StartTurn(ctx context.Context, cmd agentdomain.StartTurnCommand) (agentdomain.RunHandle, error) {
	...
	h := &runHandle{
		...
		events: make(chan agentdomain.Event, 16), errs: make(chan error, 1),
...
func (c *Client) RespondToolApproval(ctx context.Context, input agentdomain.ToolApprovalResponse, eventCh chan<- json.RawMessage) error {
...
func (c *Client) RespondUserInput(ctx context.Context, input agentdomain.UserInputResponse, eventCh chan<- json.RawMessage) error {
...
func (c *Client) AdvancePlainTextUserInput(ctx context.Context, input agentdomain.AdvanceTextInput) (agentdomain.AdvanceTextResult, error) {
	...
	if err != nil {
		return agentdomain.AdvanceTextResult{}, mapClientError(err)
	}
	var result agentdomain.AdvanceTextResult
...
func mapClientError(err error) error {
	...
	case codes.AlreadyExists:
		return agentdomain.ErrDuplicateTurn
	case codes.PermissionDenied:
		return agentdomain.ErrTeamNotServed
	case codes.Canceled:

...
type channelInput struct {
	...
	Config      gateway.UpsertConfigRequest
	Disabled    bool
	Webhook     gateway.SetWebhookEndpointRequest
	Send        gateway.SendRequest
	React       gateway.ReactRequest
}

func (c *Client) UpsertBotChannelConfig(ctx context.Context, botID string, typ gateway.ChannelType, req gateway.UpsertConfigRequest) (gateway.ChannelConfig, error) {
	var out gateway.ChannelConfig
	return out, c.call(ctx, MethodUpsertConfig, channelInput{BotID: botID, ChannelType: typ, Config: req}, &out)
}

func (c *Client) SetBotChannelStatus(ctx context.Context, botID string, typ gateway.ChannelType, disabled bool) (gateway.ChannelConfig, error) {
	var out gateway.ChannelConfig
	return out, c.call(ctx, MethodSetStatus, channelInput{BotID: botID, ChannelType: typ, Disabled: disabled}, &out)
}

func (c *Client) DeleteBotChannelConfig(ctx context.Context, botID string, typ gateway.ChannelType) error {
	return c.call(ctx, MethodDeleteConfig, channelInput{BotID: botID, ChannelType: typ}, nil)
}

func (c *Client) SetWebhookEndpoint(ctx context.Context, botID string, typ gateway.ChannelType, req gateway.SetWebhookEndpointRequest) (gateway.SetWebhookEndpointResponse, error) {
	var out gateway.SetWebhookEndpointResponse
	return out, c.call(ctx, MethodSetWebhook, channelInput{BotID: botID, ChannelType: typ, Webhook: req}, &out)
}

func (c *Client) Send(ctx context.Context, botID string, typ gateway.ChannelType, req gateway.SendRequest) error {
	return c.call(ctx, MethodSend, channelInput{BotID: botID, ChannelType: typ, Send: req}, nil)
}

func (c *Client) React(ctx context.Context, botID string, typ gateway.ChannelType, req gateway.ReactRequest) error {
	return c.call(ctx, MethodReact, channelInput{BotID: botID, ChannelType: typ, React: req}, nil)
}

func (c *Client) ConnectionStatusesByBot(botID string) []gateway.ConnectionStatus {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	var out []gateway.ConnectionStatus
...
func (c *Client) Status() webhook.Status {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	var out webhook.Status
	if c.call(ctx, MethodTunnelStatus, nil, &out) != nil {
		return webhook.Status{Enabled: false, Mode: "unavailable", Status: webhook.StatusError}
	}
	return out
}
...
var reasonSentinels = map[string]error{
	reasonConfigNotFound:     gateway.ErrChannelConfigNotFound,
	reasonDiscoveryFailed:    gateway.ErrChannelDiscoveryFailed,
	reasonEnableFailed:       gateway.ErrEnableChannelFailed,
	reasonInvalidWebhook:     gateway.ErrInvalidWebhookEndpoint,
	reasonWebhookUnsupported: gateway.ErrWebhookEndpointUnsupported,
}
...
func safeChannelError(err error) error {
	...
	case errors.Is(err, gateway.ErrInvalidWebhookEndpoint):
		return reasonStatus(codes.InvalidArgument, reasonInvalidWebhook, err)
	case errors.Is(err, gateway.ErrWebhookEndpointUnsupported):
...
func Handlers(channelRuntime gateway.Runtime, emailRuntime email.Runtime, tunnel webhook.Manager) map[string]runtimeRpc.Handler {

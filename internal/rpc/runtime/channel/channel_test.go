...
func TestSafeChannelErrorRoundTripsSentinelAndCause(t *testing.T) {
	wireErr := safeChannelError(errors.Join(gateway.ErrEnableChannelFailed, errors.New("adapter cause")))
	...
	if !errors.Is(restored, gateway.ErrEnableChannelFailed) {
...
func TestRestoreChannelErrorBareReason(t *testing.T) {
	...
	if !errors.Is(restored, gateway.ErrChannelConfigNotFound) {

...
func provideAgentService(log *slog.Logger, a *native.Agent, modelsService *models.Service, queries dbstore.Queries, msgService *message.DBService, settingsService *settings.Service, accountService *accounts.Service, botService *bots.Service, mediaService *media.Service, containerdHandler *handlers.ContainerdHandler, workspaceManager *workspace.Manager, memoryRegistry *memprovider.Registry, channelStore *channel.Store, _ *route.DBService, sessionService *sessionpkg.Service, eventHub *event.Hub, compactionService *compaction.Service, pipeline *timeline.Pipeline, cfg config.Config, rc *boot.RuntimeConfig, bgManager *background.Manager, toolApproval *toolapproval.Service, userInput *userinput.Service, acpPool *acpagent.SessionPool, hookService *hookspkg.Service, sessionRuntime *sessionruntime.Manager) *application.Service {
	...
	service.SetAuxiliaryVisionConfig(application.AuxiliaryVisionConfig{
		Model:      cfg.Agent.AuxiliaryVisionModel,
...
func provideToolGatewayService(log *slog.Logger, fedGateway *handlers.MCPFederationGateway, oauthService *mcp.OAuthService, mcpConnService *mcp.ConnectionService, containerdHandler *handlers.ContainerdHandler, nativeSource *agenttools.NativeToolSource, toolContexts *mcp.ToolSessionContextStore, queries dbstore.Queries, cfg config.Config) *mcp.ToolGatewayService {
	...
	svc := mcp.NewToolGatewayService(
		log,
		[]mcp.ToolSource{nativeSource, fedSource},
		mcp.WithToolOutputLimit(limits.ToolOutputLimit()),
		mcp.WithChannelPolicyResolver(channelpolicy.NewResolver(queries)),
	)

...
func NewSDKChatModel(cfg SDKModelConfig) *sdk.Model {
	...
	switch modeldomain.ClientType(cfg.ClientType) {
	case modeldomain.ClientTypeOpenAICompletions:
	...
	case modeldomain.ClientTypeGitHubCopilot:
		return memohcopilot.NewCopilotSDKModel(cfg.APIKey, cfg.ModelID, cfg.HTTPClient)
...
func BuildReasoningOptions(cfg SDKModelConfig) []sdk.GenerateOption {
	...
	ct := modeldomain.ClientType(cfg.ClientType)
	...
	if ct == modeldomain.ClientTypeOpenAICompletions &&
		(isDeepSeekChatCompletionsCompat(cfg.ChatCompletionsCompat) || isMiniMaxChatCompletionsCompat(cfg.ChatCompletionsCompat)) {
		switch {
		case rc.Disabled:
			return []sdk.GenerateOption{sdk.WithReasoningEffort(modeldomain.ReasoningEffortNone)}
	...
	case modeldomain.ClientTypeOpenAIResponses, modeldomain.ClientTypeOpenAICodex, modeldomain.ClientTypeOpenAICompletions:
...
func openAIEffortOptions(clientType modeldomain.ClientType, rc *ReasoningConfig) []sdk.GenerateOption {
	switch {
	case rc.Active:
		effort := openAIWireEffort(clientType, rc.Effort)
		if effort == "" {
			effort = modeldomain.ReasoningEffortMedium
		}
...
func openAIWireEffort(clientType modeldomain.ClientType, effort string) string {
	if clientType != modeldomain.ClientTypeOpenAICodex && effort == modeldomain.ReasoningEffortMax {
		return modeldomain.ReasoningEffortXHigh
	}
	return effort
}
...
var anthropicLegacyBudget = map[string]int{
	modeldomain.ReasoningEffortLow:    5000,
	modeldomain.ReasoningEffortMedium: 16000,
	modeldomain.ReasoningEffortHigh:   50000,
}
...
func legacyAnthropicBudgetFor(effort string) int {
	...
	return anthropicLegacyBudget[modeldomain.ReasoningEffortMedium]
}
...
func ResolveClientType(model *sdk.Model) string {
	if model == nil || model.Provider == nil {
		return string(modeldomain.ClientTypeOpenAICompletions)
	}
	name := model.Provider.Name()
	switch {
	case strings.Contains(name, "anthropic"):
		return string(modeldomain.ClientTypeAnthropicMessages)
	case strings.Contains(name, "google"):
		return string(modeldomain.ClientTypeGoogleGenerativeAI)
	case strings.Contains(name, "github-copilot"), strings.Contains(name, "copilot"):
		return string(modeldomain.ClientTypeGitHubCopilot)
	case strings.Contains(name, "codex"):
		return string(modeldomain.ClientTypeOpenAICodex)
	case strings.Contains(name, "responses"):
		return string(modeldomain.ClientTypeOpenAIResponses)
	default:
		return string(modeldomain.ClientTypeOpenAICompletions)
	}
}

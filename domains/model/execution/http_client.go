...
var (
	defaultProviderTransport = newDefaultProviderTransport()
	providerUAVersion        = "dev"
	providerUACommit         = "unknown"
)

// ConfigureProviderUserAgent sets version tokens used by DefaultProviderUserAgent.
// cmd should call this once during composition with build-time version info.
func ConfigureProviderUserAgent(version, shortCommit string) {
	if token := sanitizeUserAgentToken(version); token != "" {
		providerUAVersion = token
	}
	if token := sanitizeUserAgentCommentValue(shortCommit); token != "" {
		providerUACommit = token
	}
}
...
func DefaultProviderUserAgent() string {
	versionToken := providerUAVersion
	if versionToken == "" {
		versionToken = "dev"
	}
	buildToken := providerUACommit
	if buildToken == "" {
		buildToken = "unknown"
	}

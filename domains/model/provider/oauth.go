...
import (
	...
	provideroauth "github.com/memohai/memoh/domains/model/internal/provider/oauth"
)
...
var (
	...
	validateOpenAICodexACPAuthURLFunc = func(raw string) error {
		return validateOAuthTokenURLFunc(modeldomain.ClientTypeOpenAICodex, raw)
	}
)

type oauthTokenRecord = OAuthTokenRecord
...
func (s *Service) oauthConfigForProvider(provider ProviderRecord) oauthConfig {
	...
	case modeldomain.ClientTypeGitHubCopilot:
		result := oauthConfig{
			ClientType:    modeldomain.ClientTypeGitHubCopilot,
	...
	default:
		result := oauthConfig{
			ClientType:              modeldomain.ClientTypeOpenAICodex,
...
func supportsOAuth(provider ProviderRecord) bool {
	switch modeldomain.ClientType(provider.ClientType) {
	case modeldomain.ClientTypeOpenAICodex, modeldomain.ClientTypeGitHubCopilot:
		return true
	default:
		return false
	}
...
func (s *Service) StartOAuthAuthorization(ctx context.Context, providerID string) (*OAuthAuthorizeResponse, error) {
	...
	if modeldomain.ClientType(provider.ClientType) == modeldomain.ClientTypeOpenAICodex {
	...
	if modeldomain.ClientType(provider.ClientType) == modeldomain.ClientTypeGitHubCopilot {
...
func (s *Service) HandleOAuthCallback(ctx context.Context, state, code string) (string, error) {
	...
	provider, err := s.providers.GetProvider(ctx, token.ProviderID)
	if err != nil {
	...
	if err := s.saveOAuthToken(ctx, provider.ID, oauthTokenRecord{
...
func (s *Service) GetOAuthStatus(ctx context.Context, providerID string) (*OAuthStatus, error) {
	...
	if modeldomain.ClientType(provider.ClientType) == modeldomain.ClientTypeOpenAICodex ||
		modeldomain.ClientType(provider.ClientType) == modeldomain.ClientTypeGitHubCopilot {
		status.Mode = "device"
		status.CallbackURL = ""
	}
...
func (s *Service) pollGitHubCopilotProviderAuthorization(ctx context.Context, provider ProviderRecord) (*OAuthStatus, error) {
	providerID := provider.ID
	token, err := s.getOAuthToken(ctx, providerID)
	if err != nil {
		if errors.Is(err, ErrOAuthTokenNotFound) {
...
func (s *Service) pollOpenAICodexProviderAuthorization(ctx context.Context, provider ProviderRecord) (*OAuthStatus, error) {
	providerID := provider.ID
	token, err := s.getOAuthToken(ctx, providerID)
	if err != nil {
		if errors.Is(err, ErrOAuthTokenNotFound) {
...
func (s *Service) RevokeOAuthToken(ctx context.Context, providerID string) error {
	...
	return s.oauth.DeleteOAuthToken(ctx, provider.ID)
}
...
func (s *Service) loadOAuthProvider(ctx context.Context, providerID string) (ProviderRecord, error) {
	provider, err := s.providers.GetProvider(ctx, providerID)
	if err != nil {
		return ProviderRecord{}, fmt.Errorf("get provider: %w", err)
	}
	if !supportsOAuth(provider) {
		return ProviderRecord{}, errors.New("provider does not support oauth")
	}
	return provider, nil
}

func (s *Service) getOAuthToken(ctx context.Context, providerID string) (*oauthTokenRecord, error) {
	record, err := s.oauth.GetOAuthToken(ctx, providerID)
	if err != nil {
		return nil, err
	}
	return &record, nil
...
func (s *Service) getOAuthTokenByState(ctx context.Context, state string) (*oauthTokenRecord, error) {
	record, err := s.oauth.GetOAuthTokenByState(ctx, state)
	if err != nil {
		return nil, err
	}
	return &record, nil
...
func (s *Service) updateOAuthState(ctx context.Context, providerID, state, codeVerifier string, metadata map[string]any) error {
	return s.oauth.UpdateOAuthState(ctx, OAuthStateUpdate{
		ProviderID:       providerID,
		State:            state,
...
func (s *Service) saveOAuthToken(ctx context.Context, providerID string, token oauthTokenRecord) error {
	...
	return s.oauth.SaveOAuthToken(ctx, token)
}
...
func validateOAuthTokenURL(clientType modeldomain.ClientType, raw string) error {
	return provideroauth.ValidateTokenURL(clientType, raw)
}
...
func generateState() (string, error) {
	return provideroauth.GenerateState()
}

func generateCodeVerifier() (string, error) {
	return provideroauth.GenerateCodeVerifier()
}

func computeCodeChallenge(verifier string) string {
	return provideroauth.ComputeCodeChallenge(verifier)
}

func expiresAtFromNow(expiresIn int64) time.Time {
	return provideroauth.ExpiresAtFromNow(expiresIn)
}

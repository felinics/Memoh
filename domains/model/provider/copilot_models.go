...
func (s *Service) fetchGitHubCopilotModels(ctx context.Context, provider ProviderRecord) ([]RemoteModel, error) {
...
func (s *Service) listGitHubCopilotRemoteModels(ctx context.Context, baseURL, githubToken string) ([]RemoteModel, error) {
	...
	for _, model := range catalog.Data {
		...
		if modelType != "" && modelType != string(modeldomain.ModelTypeChat) {
			continue
		}
		...
		if model.Capabilities.Supports.ToolCalls {
			compatibilities = append(compatibilities, modeldomain.CompatToolCall)
		}
		if model.Capabilities.Supports.Vision {
			compatibilities = append(compatibilities, modeldomain.CompatVision)
		}
		...
		for _, effort := range model.Capabilities.Supports.ReasoningEffort {
			...
			if modeldomain.IsValidReasoningEffort(effort) && !containsFold(reasoningEfforts, effort) {
		...
		remoteModels = append(remoteModels, RemoteModel{
			...
			Type:              string(modeldomain.ModelTypeChat),

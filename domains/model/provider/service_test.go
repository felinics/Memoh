...
`), 0o600); err != nil {
	...
	svc := NewService(nil, nil, nil, "", dir)
	models, ok := svc.fetchTemplateModels(context.Background(), "", []byte(`{"preset":{"source":"zai.yaml"}}`))
	if !ok {
...
func TestListCodexRemoteModels(t *testing.T) {
	...
	if model.ThinkingMode != modeldomain.ThinkingModeToggle {
...
func TestOAuthConfigForGitHubCopilotUsesFixedDeviceFlowSettings(t *testing.T) {
	...
	service := &Service{}
	cfg := service.oauthConfigForProvider(ProviderRecord{
		ClientType: string(modeldomain.ClientTypeGitHubCopilot),
...
func overrideOpenAICodexDeviceAuthTestHooks(t *testing.T, issuer string) func() {
	...
	validateOAuthTokenURLFunc = func(modeldomain.ClientType, string) error { return nil }
...
func TestFetchRemoteModelsViaSDK(t *testing.T) {
	...
	t.Run("anthropic", func(t *testing.T) {
		...
		s := &Service{}
		remoteModels, err := s.fetchRemoteModelsViaSDK(context.Background(), ProviderRecord{
		...
		if remoteModels[0].Type != string(modeldomain.ModelTypeChat) {
	...
	t.Run("google gemini", func(t *testing.T) {
		...
		s := &Service{}
		remoteModels, err := s.fetchRemoteModelsViaSDK(context.Background(), ProviderRecord{
			ClientType: string(modeldomain.ClientTypeGoogleGenerativeAI),
		...
		if remoteModels[1].Type != string(modeldomain.ModelTypeEmbedding) {

...
SELECT id, provider_id, user_id, access_token, refresh_token, expires_at, scope, token_type, state, pkce_code_verifier, metadata, created_at, updated_at, team_id FROM model.user_provider_oauth_tokens
WHERE team_id = iam.memoh_current_team_id() AND provider_id = $1
  AND user_id = $2
`
...
func (q *Queries) GetUserProviderOAuthToken(ctx context.Context, arg GetUserProviderOAuthTokenParams) (ModelUserProviderOauthToken, error) {
	row := q.db.QueryRow(ctx, getUserProviderOAuthToken, arg.ProviderID, arg.UserID)
	var i ModelUserProviderOauthToken
	err := row.Scan(
		&i.ID,
		&i.ProviderID,
		&i.UserID,
...
SELECT id, provider_id, user_id, access_token, refresh_token, expires_at, scope, token_type, state, pkce_code_verifier, metadata, created_at, updated_at, team_id FROM model.user_provider_oauth_tokens
WHERE team_id = iam.memoh_current_team_id() AND state = $1
  AND state != ''
`

func (q *Queries) GetUserProviderOAuthTokenByState(ctx context.Context, state string) (ModelUserProviderOauthToken, error) {
...
func (q *Queries) UpsertUserProviderOAuthToken(ctx context.Context, arg UpsertUserProviderOAuthTokenParams) (ModelUserProviderOauthToken, error) {

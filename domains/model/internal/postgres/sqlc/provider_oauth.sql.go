...
DELETE FROM model.provider_oauth_tokens WHERE team_id = iam.memoh_current_team_id() AND provider_id = $1
`
...
SELECT id, provider_id, access_token, refresh_token, expires_at, scope, token_type, state, pkce_code_verifier, metadata, created_at, updated_at, team_id FROM model.provider_oauth_tokens WHERE team_id = iam.memoh_current_team_id() AND provider_id = $1
`

func (q *Queries) GetProviderOAuthTokenByProvider(ctx context.Context, providerID pgtype.UUID) (ModelProviderOauthToken, error) {
	row := q.db.QueryRow(ctx, getProviderOAuthTokenByProvider, providerID)
	var i ModelProviderOauthToken
	err := row.Scan(
		&i.ID,
		&i.ProviderID,
		&i.AccessToken,
...
SELECT id, provider_id, access_token, refresh_token, expires_at, scope, token_type, state, pkce_code_verifier, metadata, created_at, updated_at, team_id FROM model.provider_oauth_tokens WHERE team_id = iam.memoh_current_team_id() AND state = $1 AND state != ''
`

func (q *Queries) GetProviderOAuthTokenByState(ctx context.Context, state string) (ModelProviderOauthToken, error) {
...
func (q *Queries) UpsertProviderOAuthToken(ctx context.Context, arg UpsertProviderOAuthTokenParams) (ModelProviderOauthToken, error) {

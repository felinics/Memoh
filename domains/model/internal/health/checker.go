...
const (
	...
	defaultTimeout           = modelexecution.DefaultProviderProbeTimeout
)
...
type ModelProber interface {
	Test(ctx context.Context, id string) (modelcatalog.TestResponse, error)
}
...
func (c *Checker) ListChecks(ctx context.Context, botID string) []healthcheck.CheckResult {
	...
	for i, slot := range slots {
		...
		go func(idx int, s modelSlot) {
			defer wg.Done()
			slotCtx := oauth.WithUserID(probeCtx, botModels.OwnerUserID)

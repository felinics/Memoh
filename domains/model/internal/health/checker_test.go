...
func (p testProber) Test(ctx context.Context, id string) (modelcatalog.TestResponse, error) {
	if got := oauth.UserIDFromContext(ctx); got != p.wantUserID {
		p.t.Fatalf("expected oauth user id %q, got %q", p.wantUserID, got)
	}
	if id != "model-chat-1" {
		p.t.Fatalf("expected model id %q, got %q", "model-chat-1", id)
	}
	return modelcatalog.TestResponse{
		Status:    modelcatalog.TestStatusOK,
		Reachable: true,
		Message:   "ok",
	}, nil
}

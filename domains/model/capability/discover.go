...
import modeldomain "github.com/memohai/memoh/domains/model"
...
func derive(e litellmEntry) Capabilities {
	...
	case reasoningSupported:
		if boolVal(e.SupportsAdaptiveThinking) {
			caps.ThinkingMode = modeldomain.ThinkingModeAdaptive
		} else {
			caps.ThinkingMode = modeldomain.ThinkingModeToggle
		}
	...
	case e.SupportsReasoning != nil && !*e.SupportsReasoning:
		...
		caps.ThinkingMode = modeldomain.ThinkingModeNone
	default:
...
func deriveEffortLevels(e litellmEntry) []string {
	present := map[string]bool{
		modeldomain.ReasoningEffortMedium: true,
		modeldomain.ReasoningEffortHigh:   true,
	}
	if e.SupportsLowReasoningEffort == nil || *e.SupportsLowReasoningEffort {
		present[modeldomain.ReasoningEffortLow] = true
	}
	if boolVal(e.SupportsNoneReasoningEffort) {
		present[modeldomain.ReasoningEffortNone] = true
	}
	if boolVal(e.SupportsMinimalReasoningEffort) {
		present[modeldomain.ReasoningEffortMinimal] = true
	}
	if boolVal(e.SupportsXHighReasoningEffort) {
		present[modeldomain.ReasoningEffortXHigh] = true
	}
	if boolVal(e.SupportsMaxReasoningEffort) {
		present[modeldomain.ReasoningEffortMax] = true
	}

// Package projection publishes Model-owned read projections for other domains.
//
// These are business translations, not wiring: they join Model catalogs and
// decide which fields another domain's contract receives. They live in a named
// public package rather than an assembly seam because they depend only on
// Model's public surface, and because burying a join behind a name like
// "assembly" hides real logic in a package readers expect to be trivial.
package projection
...
func NewUsageModelReader(models *catalog.Service, providers *provider.Service) usagepersistence.ModelProjectionReader {
	return newUsageModelReader(models, providers)
}

func newUsageModelReader(models usageModelCatalog, providers usageProviderCatalog) usagepersistence.ModelProjectionReader {
	return &usageModelReader{models: models, providers: providers}
}

func (r *usageModelReader) GetModelProjections(ctx context.Context, modelIDs []string) (map[string]usagepersistence.ModelProjection, error) {
	if len(modelIDs) == 0 {
		return map[string]usagepersistence.ModelProjection{}, nil
	}
	...
	projections := make(map[string]usagepersistence.ModelProjection, len(wanted))
	for _, row := range modelRows {
		if _, ok := wanted[row.ID]; !ok {
			continue
		}
		projections[row.ID] = usagepersistence.ModelProjection{

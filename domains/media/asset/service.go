...
func (s *Service) Ingest(ctx context.Context, input domainmedia.IngestInput) (domainmedia.Asset, error) {
	if s.provider == nil {
		return domainmedia.Asset{}, domainmedia.ErrProviderUnavailable
	}
	if strings.TrimSpace(input.BotID) == "" {
		return domainmedia.Asset{}, errors.New("bot id is required")
	}
	if input.Reader == nil {
		return domainmedia.Asset{}, errors.New("reader is required")
	}

	maxBytes := input.MaxBytes
	if maxBytes <= 0 {
		maxBytes = domainmedia.MaxAssetBytes
	}
	contentHash, sizeBytes, tempFile, err := spoolAndHashWithLimit(input.Reader, maxBytes)
	if err != nil {
		return domainmedia.Asset{}, fmt.Errorf("read input: %w", err)
	}
	defer func() {
		_ = tempFile.Close()
	...
	if err := s.provider.Put(ctx, routingKey, tempFile); err != nil {
		return domainmedia.Asset{}, fmt.Errorf("store media: %w", err)
	}
...
func (s *Service) Resolve(ctx context.Context, botID, contentHash string) (domainmedia.Asset, error) {
	if s.provider == nil {
		return domainmedia.Asset{}, domainmedia.ErrProviderUnavailable
	}
...
func (s *Service) Stat(ctx context.Context, botID, contentHash string) (domainmedia.Asset, error) {
...
func (s *Service) Open(ctx context.Context, botID, contentHash string) (io.ReadCloser, domainmedia.Asset, error) {
	if s.provider == nil {
		return nil, domainmedia.Asset{}, domainmedia.ErrProviderUnavailable
	}
	asset, err := s.resolveByContentHash(ctx, botID, contentHash)
	if err != nil {
		return nil, domainmedia.Asset{}, err
	}
	routingKey := path.Join(botID, asset.StorageKey)
	reader, err := s.provider.Open(ctx, routingKey)
	if err != nil {
		return nil, domainmedia.Asset{}, fmt.Errorf("open storage: %w", err)
	}
	return reader, asset, nil
}

// GetByStorageKey returns an asset derived from a known storage key.
func (s *Service) GetByStorageKey(ctx context.Context, botID, storageKey string) (domainmedia.Asset, error) {
	if s.provider == nil {
		return domainmedia.Asset{}, domainmedia.ErrProviderUnavailable
	}
	...
	if err != nil {
		return domainmedia.Asset{}, domainmedia.ErrAssetNotFound
	}
	_ = rc.Close()
...
func (s *Service) AccessPath(ctx context.Context, asset domainmedia.Asset) string {
...
func (s *Service) EnsureAccessPath(ctx context.Context, asset domainmedia.Asset) (string, error) {
	if s.provider == nil {
		return "", domainmedia.ErrProviderUnavailable
	}
...
func (s *Service) IngestContainerFile(ctx context.Context, botID, containerPath string) (domainmedia.Asset, error) {
	if s.provider == nil {
		return domainmedia.Asset{}, domainmedia.ErrProviderUnavailable
	}
	opener, ok := s.provider.(storage.ContainerFileOpener)
	if !ok {
		return domainmedia.Asset{}, storage.ErrContainerFileNotSupported
	}
	f, err := opener.OpenContainerFile(ctx, botID, containerPath)
	if err != nil {
		return domainmedia.Asset{}, fmt.Errorf("open workspace file: %w", err)
	}
	defer func() { _ = f.Close() }()
	ext := path.Ext(containerPath)
	mime := mimeFromExtension(ext)
	return s.Ingest(ctx, domainmedia.IngestInput{BotID: botID, Mime: mime, Reader: f, OriginalExt: ext})
}
...
func (s *Service) resolveByContentHash(ctx context.Context, botID, contentHash string) (domainmedia.Asset, error) {
...
func deriveAssetFromKey(botID, storageKey string) domainmedia.Asset {
...
func spoolAndHashWithLimit(reader io.Reader, maxBytes int64) (contentHash string, size int64, f *os.File, err error) {
	...
	if written > maxBytes {
		cleanup()
		return "", 0, nil, fmt.Errorf("%w: max %d bytes", domainmedia.ErrAssetTooLarge, maxBytes)
	}
	if written == 0 {
		cleanup()

...
func TestParseToolInputBundles(t *testing.T) {
	resolveUnderData := func(rel string) string {
		return "/data/" + strings.TrimLeft(rel, "/")
	}
...
func TestParseToolInputBundles_NilResolverKeepsRelative(t *testing.T) {
	bundles, ok := ParseToolInputBundles("screenshot.png", nil)
	if !ok {
		t.Fatal("expected tool input parsing to succeed")
	}
	if len(bundles) != 1 || bundles[0].Path != "screenshot.png" {
		t.Fatalf("expected relative path left unchanged, got %#v", bundles)
	}
}

func TestParseToolInputBundles_InvalidTopLevelType(t *testing.T) {
	if _, ok := ParseToolInputBundles(42, nil); ok {

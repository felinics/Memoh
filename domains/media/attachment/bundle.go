...
// internal bundles. resolveRelativePath, when non-nil, rewrites relative
// path/URL refs that are neither absolute nor http(s)/data URLs; nil leaves
// those refs unchanged. Container data-mount policy belongs to the caller.
func ParseToolInputBundles(raw any, resolveRelativePath func(string) string) ([]Bundle, bool) {
	switch v := raw.(type) {
	case nil:
		return nil, true
	case string:
		bundle, ok := parseToolInputBundle(v, resolveRelativePath)
		if !ok {
			return []Bundle{}, true
		}
	...
	case []string:
		...
		for _, item := range v {
			bundle, ok := parseToolInputBundle(item, resolveRelativePath)
			if ok {
...
func parseToolInputBundle(raw any, resolveRelativePath func(string) string) (Bundle, bool) {
	switch v := raw.(type) {
	case string:
		return normalizeToolBundle(Bundle{URL: v}, resolveRelativePath), strings.TrimSpace(v) != ""
	case map[string]any:
		...
		return normalizeToolBundle(bundle, resolveRelativePath), true
	default:
		return Bundle{}, false
	}
}

func normalizeToolBundle(bundle Bundle, resolveRelativePath func(string) string) Bundle {
	if path := strings.TrimSpace(bundle.Path); path != "" {
		bundle.Path = normalizeToolInputRef(path, resolveRelativePath)
		bundle.URL = ""
	}
	if bundle.Path == "" {
		...
		case IsHTTPURL(rawURL):
			bundle.URL = rawURL
		case rawURL != "":
			bundle.Path = normalizeToolInputRef(rawURL, resolveRelativePath)
			bundle.URL = ""
		}
	}
...
func normalizeToolInputRef(raw string, resolveRelativePath func(string) string) string {
	raw = strings.TrimSpace(raw)
	if raw == "" || IsHTTPURL(raw) || IsDataURL(raw) || isLocalPath(raw) {
		return raw
	}
	if resolveRelativePath == nil {
		return raw
	}
	return resolveRelativePath(raw)
}

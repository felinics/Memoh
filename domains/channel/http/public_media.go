...
// publicMediaAssetOpener is the narrow open surface PublicMediaHandler needs.
type publicMediaAssetOpener interface {
	Open(ctx context.Context, botID, contentHash string) (io.ReadCloser, media.Asset, error)
}

type PublicMediaHandler struct {
	logger  *slog.Logger
	media   publicMediaAssetOpener
	enabled bool
	signer  *publicmedia.Signer
}

func NewPublicMediaHandler(log *slog.Logger, mediaService publicMediaAssetOpener, signingSecret string) *PublicMediaHandler {
	if log == nil {
		log = slog.Default()
	}
...
func NewConfiguredPublicMediaHandler(log *slog.Logger, cfg config.Config, mediaService publicMediaAssetOpener) *PublicMediaHandler {
...
func configuredPublicBaseURL(cfg config.Config) string {
	base, err := webhook.NormalizeConfiguredPublicBase(cfg.WebhookTunnel.PublicBaseURL)
	if err != nil {
		return ""
	}
	return base

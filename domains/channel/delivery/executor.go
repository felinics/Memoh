...
func (e *Executor) sendWithMode(
	...
	if err := e.Sender.Send(ctx, plan.botID, plan.channelType, SendRequest{
		...
		if e.Logger != nil {
			e.Logger.WarnContext(ctx, "outbound send failed",
...
func (e *Executor) resolveOutboundAttachments(
	...
	bundles, ok := attachmentpkg.ParseToolInputBundles(rawAttachments, runtimedomain.DataMountPath)
	if !ok {
...
func dropUnresolvedDataPathAttachments(attachments []Attachment) []Attachment {
	...
	for _, att := range attachments {
		if strings.TrimSpace(att.ContentHash) == "" &&
			...
			runtimedomain.IsDataPath(att.Path) {
			continue
		}
...
func (e *Executor) React(ctx context.Context, session SessionContext, args map[string]any) (*ReactResult, error) {
	...
	if err := e.Reactor.React(ctx, botID, channelType, ReactRequest{
		...
		if e.Logger != nil {
			e.Logger.WarnContext(ctx, "react failed", slog.Any("error", err), slog.String("bot_id", botID), slog.String("platform", string(channelType)))
		}
		return nil, err
	}
	action := "added"
...
func (e *Executor) resolveAttachmentBundle(ctx context.Context, botID string, bundle attachmentpkg.Bundle) *Attachment {
	...
	if storageKey := media.StorageKeyFromAccessPath(runtimedomain.DefaultDataMount, ref); storageKey != "" && e.AssetResolver != nil {
	...
	if runtimedomain.IsDataPath(ref) && e.AssetResolver != nil {

...
SELECT id, bot_id, session_id, event_kind, event_data, external_message_id, sender_channel_identity_id, received_at_ms, created_at, team_id FROM channel.bot_session_events
WHERE team_id = iam.memoh_current_team_id() AND bot_id = $1
ORDER BY received_at_ms ASC, id ASC
`

func (q *Queries) ListSessionEventsByBot(ctx context.Context, botID pgtype.UUID) ([]ChannelBotSessionEvent, error) {
	rows, err := q.db.Query(ctx, listSessionEventsByBot, botID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ChannelBotSessionEvent
	for rows.Next() {
		var i ChannelBotSessionEvent
		if err := rows.Scan(
			&i.ID,
			&i.BotID,
			&i.SessionID,
...
func (q *Queries) ListSessionEventsBySession(ctx context.Context, sessionID pgtype.UUID) ([]ChannelBotSessionEvent, error) {
...
func (q *Queries) ListSessionEventsBySessionAfter(ctx context.Context, arg ListSessionEventsBySessionAfterParams) ([]ChannelBotSessionEvent, error) {

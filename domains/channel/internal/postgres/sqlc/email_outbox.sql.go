...
func (q *Queries) CreateEmailOutbox(ctx context.Context, arg CreateEmailOutboxParams) (ChannelEmailOutbox, error) {
...
SELECT id, provider_id, bot_id, message_id, from_address, to_addresses, subject, body_text, body_html, attachments, status, error, sent_at, created_at, team_id FROM channel.email_outbox WHERE team_id = iam.memoh_current_team_id() AND id = $1
`

func (q *Queries) GetEmailOutboxByID(ctx context.Context, id pgtype.UUID) (ChannelEmailOutbox, error) {
...
SELECT id, provider_id, bot_id, message_id, from_address, to_addresses, subject, body_text, body_html, attachments, status, error, sent_at, created_at, team_id FROM channel.email_outbox
WHERE team_id = iam.memoh_current_team_id() AND bot_id = $1
...
func (q *Queries) ListEmailOutboxByBot(ctx context.Context, arg ListEmailOutboxByBotParams) ([]ChannelEmailOutbox, error) {
	rows, err := q.db.Query(ctx, listEmailOutboxByBot, arg.BotID, arg.Off, arg.Lim)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ChannelEmailOutbox
	for rows.Next() {
		var i ChannelEmailOutbox
		if err := rows.Scan(
			&i.ID,

...
DELETE FROM channel.email_oauth_tokens WHERE team_id = iam.memoh_current_team_id() AND email_provider_id = $1
`
...
SELECT id, email_provider_id, email_address, access_token, refresh_token, expires_at, scope, state, created_at, updated_at, team_id FROM channel.email_oauth_tokens WHERE team_id = iam.memoh_current_team_id() AND email_provider_id = $1
`

func (q *Queries) GetEmailOAuthTokenByProvider(ctx context.Context, emailProviderID pgtype.UUID) (ChannelEmailOauthToken, error) {
	row := q.db.QueryRow(ctx, getEmailOAuthTokenByProvider, emailProviderID)
	var i ChannelEmailOauthToken
	err := row.Scan(
		&i.ID,
		&i.EmailProviderID,
		&i.EmailAddress,
...
SELECT id, email_provider_id, email_address, access_token, refresh_token, expires_at, scope, state, created_at, updated_at, team_id FROM channel.email_oauth_tokens WHERE team_id = iam.memoh_current_team_id() AND state = $1 AND state != ''
`

func (q *Queries) GetEmailOAuthTokenByState(ctx context.Context, state string) (ChannelEmailOauthToken, error) {
...
func (q *Queries) UpsertEmailOAuthToken(ctx context.Context, arg UpsertEmailOAuthTokenParams) (ChannelEmailOauthToken, error) {

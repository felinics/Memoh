...
func (*Adapter) SelfIdentityPolicy() gateway.SelfIdentityPolicy {
	return gateway.SelfIdentityPolicy{
...
func (*Adapter) Type() gateway.ChannelType { return Type }

func (*Adapter) Descriptor() gateway.Descriptor {
	return gateway.Descriptor{
		...
		OutboundPolicy: gateway.OutboundPolicy{
			...
			ChunkerMode:    gateway.ChunkerModeText,
		...
		UserConfigSchema: gateway.ConfigSchema{
			Version: 1,
			Fields: map[string]gateway.FieldSchema{
				userConfigKeyUserID: {Type: gateway.FieldString, Required: true, Title: "User ID"},
			},
		},
...
func (*Adapter) MatchBinding(config map[string]any, criteria gateway.BindingCriteria) bool {
	return matchBinding(config, criteria)
}

func (*Adapter) BuildUserConfig(identity gateway.Identity) map[string]any {
...
func (*Adapter) Connect(_ context.Context, cfg gateway.ChannelConfig, _ gateway.InboundHandler) (gateway.Connection, error) {

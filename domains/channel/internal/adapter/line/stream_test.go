...
func TestOpenStreamRequiresTarget(t *testing.T) {
	...
	if _, err := adapter.OpenStream(context.Background(), testLineConfig(), "  ", gateway.StreamOptions{}); err == nil {
...
func TestLineStreamAccumulatesDeltasAndFiltersReasoning(t *testing.T) {
	...
	stream, err := adapter.OpenStream(context.Background(), testLineConfig(), "Uuser", gateway.StreamOptions{})
	if err != nil {
		t.Fatalf("OpenStream returned error: %v", err)
	}
	if err := stream.Push(context.Background(), gateway.PreparedStreamEvent{Type: gateway.StreamEventDelta, Delta: "hello"}); err != nil {
		t.Fatalf("Push delta returned error: %v", err)
	}
	if err := stream.Push(context.Background(), gateway.PreparedStreamEvent{Type: gateway.StreamEventDelta, Phase: gateway.StreamPhaseReasoning, Delta: " secret reasoning "}); err != nil {
		t.Fatalf("Push reasoning returned error: %v", err)
	}
	if err := stream.Push(context.Background(), gateway.PreparedStreamEvent{Type: gateway.StreamEventDelta, Delta: " world"}); err != nil {
		t.Fatalf("Push delta returned error: %v", err)
	}
...
func TestLineStreamFinalUsesFinalMessage(t *testing.T) {
	...
	if err := stream.Push(context.Background(), gateway.PreparedStreamEvent{Type: gateway.StreamEventDelta, Delta: "draft"}); err != nil {
		t.Fatalf("Push delta returned error: %v", err)
	}
	err = stream.Push(context.Background(), gateway.PreparedStreamEvent{

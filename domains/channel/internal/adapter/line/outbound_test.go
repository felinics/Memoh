...
func TestAllowedLineImageURL(t *testing.T) {
	...
	tests := []struct {
		name string
		att  gateway.PreparedAttachment
		want bool
	}{
		{
			name: "public png",
...
func TestLineImageMessageUsesPublicMediaURLForPersistedImage(t *testing.T) {
	...
	image, ok := adapter.lineImageMessage(gateway.ChannelConfig{
		BotID: "bot-1",
		ID:    "cfg-1",
	}, gateway.PreparedAttachment{
		Kind: gateway.PreparedAttachmentUpload,
		Logical: gateway.Attachment{
...
func TestLineImageMessageRejectsPersistedImageWithoutPublicBaseURL(t *testing.T) {
	...
	adapter := NewAdapter(nil)
	_, ok := adapter.lineImageMessage(gateway.ChannelConfig{BotID: "bot-1"}, gateway.PreparedAttachment{
		Kind: gateway.PreparedAttachmentUpload,
		Logical: gateway.Attachment{
			Type:        gateway.AttachmentImage,
			ContentHash: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
...
func TestSendPreparedReturnsErrorWhenOnlyUnsupportedAttachments(t *testing.T) {
	...
	_, err := adapter.sendPrepared(context.Background(), testLineConfig(), gateway.PreparedOutboundMessage{
		Target: "Uuser",
		Message: gateway.PreparedMessage{
			Message: gateway.Message{
				Attachments: []gateway.Attachment{{Type: gateway.AttachmentFile, URL: "memoh://file"}},
			},

...
func (a *Adapter) HandleWebhook(ctx context.Context, cfg gateway.ChannelConfig, handler gateway.InboundHandler, r *http.Request, w http.ResponseWriter) error {
...
func (a *Adapter) handleCallbackEvent(ctx context.Context, cfg gateway.ChannelConfig, handler gateway.InboundHandler, rawEvent webhook.EventInterface, budget *callbackBudget) eventResult {
	...
	if err := handler(ctx, cfg, msg); err != nil {
		if gateway.IsInboundQueueFull(err) {
			a.forgetEvent(dedupeKey)
...
func lineSelfUserID(cfg gateway.ChannelConfig) string {
...
func buildInboundMessage(cfg gateway.ChannelConfig, ev webhook.MessageEvent, userID string, budget *callbackBudget) (gateway.InboundMessage, bool) {
	...
	return gateway.InboundMessage{
		...
		BotID:       cfg.BotID,
		ReplyTarget: userID,
		RouteKey:    gateway.GenerateRoutingKey(Type.String(), cfg.BotID, userID, gateway.ConversationTypePrivate, userID),
...
func buildMessageContent(raw webhook.MessageContentInterface, budget *callbackBudget) (gateway.Message, bool) {
	switch msg := raw.(type) {
...
func lineTextMessage(msg webhook.TextMessageContent) gateway.Message {
...
func lineImageMessage(msg webhook.ImageMessageContent, budget *callbackBudget) (gateway.Message, bool) {
	...
	return gateway.Message{
		ID:          messageID,
		Format:      gateway.MessageFormatPlain,
		Attachments: []gateway.Attachment{att},
	}, true
}

func lineFileMessage(msg webhook.FileMessageContent, budget *callbackBudget) (gateway.Message, bool) {
	messageID := strings.TrimSpace(msg.Id)
	if messageID == "" {
		return gateway.Message{}, false
	}

...
func (a *Adapter) Send(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.PreparedOutboundMessage) error {
	_, err := a.sendPrepared(ctx, cfg, msg)
	return err
}

func (a *Adapter) sendPrepared(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.PreparedOutboundMessage) (int, error) {
...
func (a *Adapter) lineImageMessage(cfg gateway.ChannelConfig, att gateway.PreparedAttachment) (messaging_api.MessageInterface, bool) {
...
func (a *Adapter) lineImageURLs(cfg gateway.ChannelConfig, att gateway.PreparedAttachment) (string, string, bool, string) {
	if att.Logical.Type != "" && att.Logical.Type != gateway.AttachmentImage {
		return "", "", false, "unsupported_attachment_type"
	}
	if att.Kind == gateway.PreparedAttachmentPublicURL {
...
func (a *Adapter) publicMediaLineImageURLs(cfg gateway.ChannelConfig, att gateway.PreparedAttachment) (string, string, bool, string) {
	...
	if channelType == "" {
		channelType = gateway.ChannelTypeLine.String()
	}
...
func allowedLineImageURL(att gateway.PreparedAttachment, maxBytes int64) (string, bool, string) {
	if att.Kind != gateway.PreparedAttachmentPublicURL {
		return "", false, "attachment_not_public_url"
	}
	...
	if !gateway.IsPublicHost(parsed.Hostname()) {
...
func knownLineImageSize(att gateway.PreparedAttachment) int64 {
	if att.Size > 0 {
		return att.Size
	}
...
func isSupportedLineImageMime(att gateway.PreparedAttachment) bool {
	mimeType := attachment.NormalizeMime(att.Mime)
	if mimeType == "" {
		mimeType = attachment.NormalizeMime(att.Logical.Mime)
	}
...
func lineImageFilename(att gateway.PreparedAttachment) string {
...
func lineLogicalAttachments(attachments []gateway.PreparedAttachment) []gateway.Attachment {
	if len(attachments) == 0 {
		return nil
	}
	items := make([]gateway.Attachment, 0, len(attachments))

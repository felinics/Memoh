...
const Type gateway.ChannelType = "line"
...
func parseConfig(raw map[string]any) (Config, error) {
	secret := strings.TrimSpace(gateway.ReadString(raw, configKeyChannelSecret, "channelSecret"))
	token := strings.TrimSpace(gateway.ReadString(raw, configKeyChannelAccessToken, "channelAccessToken"))
	if secret == "" {
...
func parseUserConfig(raw map[string]any) (UserConfig, error) {
	userID := normalizeTarget(gateway.ReadString(raw, "userId", userConfigKeyUserID))
	if userID == "" {

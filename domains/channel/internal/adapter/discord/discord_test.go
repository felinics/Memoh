...
func TestDiscordSendUsesPartsRenderer(t *testing.T) {
	...
	err = sendDiscordMessage(context.Background(), session, "ch-1", gateway.PreparedOutboundMessage{
...
func TestDiscordAllowedMentionsFiltersAndDedupesUserIDs(t *testing.T) {
	...
	allowed := discordAllowedMentionsForMessage(gateway.Message{Parts: []gateway.MessagePart{
		{Type: gateway.MessagePartMention, Text: "@alice", ChannelIdentityID: "1234567890"},
		{Type: gateway.MessagePartMention, Text: "@alice again", ChannelIdentityID: "1234567890"},
		{Type: gateway.MessagePartMention, Text: "@role", ChannelIdentityID: "&1234567890"},
		{Type: gateway.MessagePartMention, Text: "@bad", ChannelIdentityID: "1234)>"},
		{Type: gateway.MessagePartText, Text: "@everyone"},
	}})
	if allowed == nil {
...
func TestDiscordAllowedMentionsCapsUserWhitelist(t *testing.T) {
	...
	parts := make([]gateway.MessagePart, 101)
	for i := range parts {
		parts[i] = gateway.MessagePart{
	...
	allowed := discordAllowedMentionsForMessage(gateway.Message{Parts: parts})
...
func TestDiscordPreparedOutboundValidationRejectsTooManyURLActions(t *testing.T) {
	...
	err := (&DiscordAdapter{}).ValidatePreparedOutbound(context.Background(), gateway.ChannelConfig{}, "C123", gateway.PreparedOutboundMessage{
		Target: "C123",
...
func TestDiscordURLActionComponentsEnforcesFieldLimits(t *testing.T) {
	...
	rows, err := discordURLActionComponents([]gateway.Action{{
	...
	_, err = discordURLActionComponents([]gateway.Action{{
		Label: "Open",
...
func TestDiscordPreparedAttachmentToFile(t *testing.T) {
	file, err := discordPreparedAttachmentToFile(context.Background(), gateway.PreparedAttachment{
		Logical: gateway.Attachment{Type: gateway.AttachmentFile},
	...
	_, err = discordPreparedAttachmentToFile(context.Background(), gateway.PreparedAttachment{
		Logical: gateway.Attachment{Type: gateway.AttachmentFile},

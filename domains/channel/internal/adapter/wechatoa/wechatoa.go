...
const Type gateway.ChannelType = "wechatoa"
...
func (*WeChatOAAdapter) Type() gateway.ChannelType { return Type }

func (*WeChatOAAdapter) Descriptor() gateway.Descriptor {
	return gateway.Descriptor{
		...
		ConfigSchema: gateway.ConfigSchema{
			...
			Fields: map[string]gateway.FieldSchema{
				"appId":          {Type: gateway.FieldString, Required: true, Order: 0, Title: "App ID"},
				"appSecret":      {Type: gateway.FieldSecret, Required: true, Order: 10, Title: "App Secret"},
				"token":          {Type: gateway.FieldSecret, Required: true, Order: 20, Title: "Token"},
				"encodingAESKey": {Type: gateway.FieldSecret, Order: 30, Title: "Encoding AES Key"},
				"httpProxyUrl": {
		...
		UserConfigSchema: gateway.ConfigSchema{
			Version: 1,
			Fields: map[string]gateway.FieldSchema{
				"openid":  {Type: gateway.FieldString},
				"unionid": {Type: gateway.FieldString},
			},
		},
...
func (*WeChatOAAdapter) MatchBinding(config map[string]any, criteria gateway.BindingCriteria) bool {
	return matchBinding(config, criteria)
}

func (*WeChatOAAdapter) BuildUserConfig(identity gateway.Identity) map[string]any {
...
func (*WeChatOAAdapter) Connect(ctx context.Context, cfg gateway.ChannelConfig, handler gateway.InboundHandler) (gateway.Connection, error) {
	_ = ctx
	_ = handler
...
func (a *WeChatOAAdapter) HandleWebhook(ctx context.Context, cfg gateway.ChannelConfig, handler gateway.InboundHandler, r *http.Request, w http.ResponseWriter) error {
...
func (a *WeChatOAAdapter) Send(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.PreparedOutboundMessage) error {
...
func (a *WeChatOAAdapter) ResolveAttachment(ctx context.Context, cfg gateway.ChannelConfig, attachment gateway.Attachment) (gateway.AttachmentPayload, error) {
	client, err := a.clientForConfig(cfg.Credentials)
	if err != nil {
	...
	if mediaID == "" {
		return gateway.AttachmentPayload{}, errors.New("wechatoa platform_key(media_id) is required")
	}
	return client.downloadMedia(ctx, mediaID)
}

func (a *WeChatOAAdapter) OpenStream(_ context.Context, cfg gateway.ChannelConfig, target string, opts gateway.StreamOptions) (gateway.PreparedOutboundStream, error) {
...
func (s *outboundStream) Push(_ context.Context, event gateway.PreparedStreamEvent) error {
	if s.closed {
...
func logicalAttachments(attachments []gateway.PreparedAttachment) []gateway.Attachment {

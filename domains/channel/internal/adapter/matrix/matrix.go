...
const Type gateway.ChannelType = "matrix"
...
func (*MatrixAdapter) Type() gateway.ChannelType {
	return Type
}

func (*MatrixAdapter) Descriptor() gateway.Descriptor {
...
func (*MatrixAdapter) MatchBinding(config map[string]any, criteria gateway.BindingCriteria) bool {
	return matchBinding(config, criteria)
}

func (*MatrixAdapter) BuildUserConfig(identity gateway.Identity) map[string]any {
	return buildUserConfig(identity)
}

func (a *MatrixAdapter) Connect(ctx context.Context, cfg gateway.ChannelConfig, handler gateway.InboundHandler) (gateway.Connection, error) {
...
func (a *MatrixAdapter) Send(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.PreparedOutboundMessage) error {
...
func (a *MatrixAdapter) OpenStream(_ context.Context, cfg gateway.ChannelConfig, target string, opts gateway.StreamOptions) (gateway.PreparedOutboundStream, error) {
	...
	if reply == nil && strings.TrimSpace(opts.SourceMessageID) != "" {
		reply = &gateway.ReplyRef{Target: normalizeTarget(target), MessageID: strings.TrimSpace(opts.SourceMessageID)}
	}
...
func (a *MatrixAdapter) Update(ctx context.Context, cfg gateway.ChannelConfig, target string, messageID string, msg gateway.PreparedMessage) error {
	parsed, err := parseConfig(cfg.Credentials)
	if err != nil {
		return err
	}
...
func (*MatrixAdapter) Unsend(context.Context, gateway.ChannelConfig, string, string) error {
	return errors.New("matrix unsend not supported")
}

func (a *MatrixAdapter) runSyncLoop(ctx context.Context, cfg gateway.ChannelConfig, parsed Config, handler gateway.InboundHandler) {
...
func (a *MatrixAdapter) bootstrapSinceToken(ctx context.Context, cfg gateway.ChannelConfig, parsed Config) (string, error) {
...
func (a *MatrixAdapter) syncOnce(ctx context.Context, cfg gateway.ChannelConfig, parsed Config, since string, handler gateway.InboundHandler) (string, bool, error) {
	query := url.Values{}
...
func (a *MatrixAdapter) handleInvites(ctx context.Context, cfg gateway.ChannelConfig, parsed Config, resp matrixSyncResponse) (bool, error) {
	joinedAny := false
...
func (a *MatrixAdapter) handleEvent(ctx context.Context, cfg gateway.ChannelConfig, parsed Config, evt matrixEvent, handler gateway.InboundHandler) (bool, error) {
	...
	var replyAttachments []gateway.Attachment
	...
	if replyTo != "" {
		...
		if err != nil {
			...
			if preview == "" {
				preview = strings.TrimSpace(gateway.ReadString(repliedEvent.Content, "body"))
			}
	...
	msg := gateway.InboundMessage{
		...
		Metadata: map[string]any{
			...
			"msgtype":         gateway.ReadString(evt.Content, "msgtype"),
			"raw_text":        rawText,
...
func buildMatrixMessageContent(msg gateway.Message, edit bool, originalEventID string) map[string]any {
...
func buildMatrixMediaContent(msg gateway.Message, att gateway.Attachment, contentURI string) map[string]any {
...
func isMatrixEditEvent(content map[string]any) bool {
	...
	return strings.EqualFold(strings.TrimSpace(gateway.ReadString(relatesTo, "rel_type")), "m.replace")
}
...
func readReplyToEventID(content map[string]any) string {
	...
	return strings.TrimSpace(gateway.ReadString(inReplyTo, "event_id"))
}

func extractMatrixInboundContent(content map[string]any) (string, []gateway.Attachment) {
	msgType := strings.TrimSpace(gateway.ReadString(content, "msgtype"))
	if !isMatrixAttachmentMsgType(msgType) {
		return strings.TrimSpace(gateway.ReadString(content, "body")), nil
	}
	att, ok := matrixAttachmentFromContent(content, msgType)
	if !ok {
		return strings.TrimSpace(gateway.ReadString(content, "body")), nil
	}
	return strings.TrimSpace(att.Caption), []gateway.Attachment{att}
}

func matrixAttachmentFromContent(content map[string]any, msgType string) (gateway.Attachment, bool) {
	contentURI := strings.TrimSpace(gateway.ReadString(content, "url"))
	if contentURI == "" {
		return gateway.Attachment{}, false
	}
	info, _ := content["info"].(map[string]any)
	body := strings.TrimSpace(gateway.ReadString(content, "body"))
	name := strings.TrimSpace(gateway.ReadString(content, "filename"))
	caption := ""
	if name == "" {
		name = body
...
func matrixAttachmentType(msgType string) gateway.AttachmentType {
	switch strings.TrimSpace(msgType) {
	case "m.image":
...
func matrixQuotedAttachments(replyTo matrixEvent) []gateway.Attachment {
...
func matrixSinceTokenFromRouting(routing map[string]any) string {
	...
	if !ok || state == nil {
		return strings.TrimSpace(gateway.ReadString(routing, "matrix_since_token", "since_token"))
	}
	switch value := state.(type) {
	case map[string]any:
		return strings.TrimSpace(gateway.ReadString(value, "since_token", "sinceToken"))
...
func isMatrixBotMentioned(botUserID string, content map[string]any) bool {
	...
	formatted := strings.TrimSpace(gateway.ReadString(content, "formatted_body", "formattedBody"))
	if formatted != "" {
...
func matrixAttachmentMsgType(attType gateway.AttachmentType) string {
	switch attType {
	case gateway.AttachmentImage, gateway.AttachmentGIF:
		return "m.image"
	case gateway.AttachmentVideo:
		return "m.video"
	case gateway.AttachmentAudio, gateway.AttachmentVoice:
		return "m.audio"
	default:
		return "m.file"
	}
...
func matrixAttachmentBody(att gateway.Attachment) string {
...
func matrixAttachmentInfo(att gateway.Attachment) map[string]any {
	info := map[string]any{}
...
func (a *MatrixAdapter) sendMediaAttachment(ctx context.Context, cfg Config, roomID string, msg gateway.Message, att gateway.PreparedAttachment) error {
...
func (a *MatrixAdapter) resolveMatrixContentURI(ctx context.Context, cfg Config, att gateway.PreparedAttachment) (string, gateway.Attachment, error) {
	if ref := strings.TrimSpace(att.NativeRef); att.Kind == gateway.PreparedAttachmentNativeRef && isMatrixContentURI(ref) {
...
func prepareMatrixUpload(ctx context.Context, att gateway.PreparedAttachment) (matrixUploadPayload, gateway.Attachment, error) {
	if att.Kind != gateway.PreparedAttachmentUpload {
		return matrixUploadPayload{}, gateway.Attachment{}, fmt.Errorf("matrix attachment requires upload source, got %s", att.Kind)
	}
	if att.Open == nil {
		return matrixUploadPayload{}, gateway.Attachment{}, errors.New("matrix attachment upload is not openable")
	}
	reader, err := att.Open(ctx)
	if err != nil {
		return matrixUploadPayload{}, gateway.Attachment{}, err
	}
...
func deriveMatrixUploadName(att gateway.Attachment, mime, fallbackName string) string {
	if name := strings.TrimSpace(att.Name); name != "" {
		return name
	}
	...
	return matrixAttachmentBody(gateway.Attachment{Type: att.Type, Mime: mime, Caption: att.Caption})
}
...
func (a *MatrixAdapter) ResolveAttachment(ctx context.Context, cfg gateway.ChannelConfig, attachment gateway.Attachment) (gateway.AttachmentPayload, error) {
	contentURI := strings.TrimSpace(attachment.PlatformKey)
	if contentURI == "" {
		contentURI = strings.TrimSpace(attachment.URL)
	}
	if contentURI == "" {
		return gateway.AttachmentPayload{}, errors.New("matrix attachment requires platform_key or url")
	}
	if !isMatrixContentURI(contentURI) {
		return gateway.AttachmentPayload{}, errors.New("matrix attachment reference must be mxc://")
	}
	parsed, err := parseConfig(cfg.Credentials)
	if err != nil {
	...
	if !ok {
		return gateway.AttachmentPayload{}, errors.New("invalid matrix content uri")
	}
...
func matrixDisplayName(evt matrixEvent) string {
	...
	if displayName := strings.TrimSpace(gateway.ReadString(evt.Unsigned, "displayname", "sender_display_name")); displayName != "" {
		return displayName
	}

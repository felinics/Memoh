...
func parseConfig(raw map[string]any) (Config, error) {
	homeserverURL := normalizeHomeserverURL(gateway.ReadString(raw, "homeserverUrl", "homeserver_url", "homeserver"))
	accessToken := strings.TrimSpace(gateway.ReadString(raw, "accessToken", "access_token"))
...
func parseUserConfig(raw map[string]any) (UserConfig, error) {
	roomID := normalizeTarget(gateway.ReadString(raw, "roomId", "room_id"))
	userID := normalizeTarget(gateway.ReadString(raw, "userId", "user_id"))
	if roomID == "" && userID == "" {

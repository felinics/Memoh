...
func TestBootstrapSinceTokenPersistsLatestCursor(t *testing.T) {
	...
	since, err := adapter.bootstrapSinceToken(context.Background(), gateway.ChannelConfig{ID: "cfg-1"}, Config{
...
func TestHandleInvitesSkipsWhenAutoJoinDisabled(t *testing.T) {
	...
	joined, err := adapter.handleInvites(
		context.Background(),
		gateway.ChannelConfig{ID: "cfg-1"},
...
func TestBuildMatrixMessageContentIncludesFormattedHTMLForMarkdown(t *testing.T) {
	content := buildMatrixMessageContent(gateway.Message{
		Text:   "**bold**\n\n- item",
...
func TestBuildMatrixMessageContentRendersPartsDirectly(t *testing.T) {
	content := buildMatrixMessageContent(gateway.Message{
		...
		Parts: []gateway.MessagePart{
			{Type: gateway.MessagePartText, Text: "ping", Styles: []gateway.MessageTextStyle{gateway.MessageStyleBold}},
			{Type: gateway.MessagePartMention, Text: "Alice", ChannelIdentityID: "@alice:example.com"},
			{Type: gateway.MessagePartLink, Text: "docs", URL: "https://example.test/a?b=1&c=2"},
			{Type: gateway.MessagePartText, Text: `<script>alert(1)</script>`},
		},
	}, false, "")
...
func TestMatrixHandleEventExpandsRepliedImageContext(t *testing.T) {
	...
	delivered, err := adapter.handleEvent(
		context.Background(),
		gateway.ChannelConfig{ID: "cfg-1", BotID: "bot-1"},
		...
		func(_ context.Context, _ gateway.ChannelConfig, msg gateway.InboundMessage) error {
			captured = msg
			return nil
		},
	)
...
func TestMatrixSendUploadsBase64AttachmentAndSendsMediaEvent(t *testing.T) {
	...
	logicalAttachment := gateway.Attachment{
	...
	err := adapter.Send(context.Background(), gateway.ChannelConfig{
		...
		Message: gateway.PreparedMessage{
			Message: gateway.Message{
				Attachments: []gateway.Attachment{logicalAttachment},
			},
		...
		Message: gateway.PreparedMessage{
			Message: gateway.Message{
				...
				Parts: []gateway.MessagePart{
					...
					{Type: gateway.MessagePartMention, Text: "Alice", ChannelIdentityID: "@alice:example.com"},
					{Type: gateway.MessagePartLink, Text: "docs", URL: "https://example.test/docs"},
				},
			},
		},
	})
...
func TestMatrixResolveAttachmentDownloadsMXC(t *testing.T) {
	...
	payload, err := adapter.ResolveAttachment(context.Background(), gateway.ChannelConfig{
		Credentials: map[string]any{
			"homeserverUrl": "https://matrix.example.com",
			"userId":        "@memoh:example.com",
			"accessToken":   "tok",

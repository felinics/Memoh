...
func formatMatrixMessage(msg gateway.Message) matrixFormattedMessage {
	if len(msg.Parts) > 0 {
		body := strings.TrimSpace(gateway.RenderPartsAsPlain(msg.Parts))
	...
	formatted := matrixFormattedMessage{Body: body}
	if msg.Format != gateway.MessageFormatMarkdown || body == "" {
		return formatted
	}
...
func matrixMessageBody(msg gateway.Message) string {
	return strings.TrimSpace(formatMatrixMessage(msg).Body)
}

func renderMatrixMessagePartsHTML(parts []gateway.MessagePart) string {
...
func writeMatrixRichInlinePart(b *strings.Builder, text string, styles []gateway.MessageTextStyle) {
...
func writeMatrixRichLinkPart(b *strings.Builder, part gateway.MessagePart) {
	...
	if rawURL == "" || !gateway.IsHTTPURL(rawURL) {
...
func writeMatrixRichMentionPart(b *strings.Builder, part gateway.MessagePart) {
...
func writeMatrixRichCodeBlockPart(b *strings.Builder, part gateway.MessagePart) {
...
func writeMatrixRichHeadingPart(b *strings.Builder, part gateway.MessagePart) {
...
func writeMatrixRichBlockquotePart(b *strings.Builder, part gateway.MessagePart) {
...
func writeMatrixRichListItemPart(b *strings.Builder, part gateway.MessagePart) {
	lines := gateway.SplitMessagePartTextLines(part.Text)
	if len(lines) == 0 {
		return
	}
...
func renderMatrixRichStyledInline(text string, styles []gateway.MessageTextStyle) string {
	escaped := stdhtml.EscapeString(text)
	if hasMatrixRichTextStyle(styles, gateway.MessageStyleCode) {
		return "<code>" + escaped + "</code>"
	}
	if hasMatrixRichTextStyle(styles, gateway.MessageStyleSpoiler) {
		escaped = "<span data-mx-spoiler>" + escaped + "</span>"
	}
	if hasMatrixRichTextStyle(styles, gateway.MessageStyleStrikethrough) {
		escaped = "<del>" + escaped + "</del>"
	}
	if hasMatrixRichTextStyle(styles, gateway.MessageStyleUnderline) {
		escaped = "<u>" + escaped + "</u>"
	}
	if hasMatrixRichTextStyle(styles, gateway.MessageStyleItalic) {
		escaped = "<em>" + escaped + "</em>"
	}
	if hasMatrixRichTextStyle(styles, gateway.MessageStyleBold) {
		escaped = "<strong>" + escaped + "</strong>"
	}
	return escaped
}

func hasMatrixRichTextStyle(styles []gateway.MessageTextStyle, want gateway.MessageTextStyle) bool {

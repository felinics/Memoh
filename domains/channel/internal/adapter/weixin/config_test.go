...
func TestMatchBinding(t *testing.T) {
	...
	if !matchBinding(config, gateway.BindingCriteria{SubjectID: "u1"}) {
		t.Error("should match by subject_id")
	}
	if matchBinding(config, gateway.BindingCriteria{SubjectID: "u2"}) {
		t.Error("should not match different subject_id")
	}
}

func TestBuildUserConfig(t *testing.T) {
	id := gateway.Identity{SubjectID: "u1"}

...
// FeishuAdapter implements the gateway.Adapter, gateway.Sender, and gateway.Receiver interfaces for Feishu.
...
func (*FeishuAdapter) Type() gateway.ChannelType {
	return Type
}

// Descriptor returns the Feishu channel metadata.
func (*FeishuAdapter) Descriptor() gateway.Descriptor {
	return gateway.Descriptor{
		...
		ConfigSchema: gateway.ConfigSchema{
			...
			Fields: map[string]gateway.FieldSchema{
				"appId":     {Type: gateway.FieldString, Required: true, Title: "App ID"},
				"appSecret": {Type: gateway.FieldSecret, Required: true, Title: "App Secret"},
				"encryptKey": {
		...
		UserConfigSchema: gateway.ConfigSchema{
			Version: 1,
			Fields: map[string]gateway.FieldSchema{
				"open_id": {Type: gateway.FieldString},
...
func (a *FeishuAdapter) ProcessingStarted(ctx context.Context, cfg gateway.ChannelConfig, _ gateway.InboundMessage, info gateway.ProcessingStatusInfo) (gateway.ProcessingStatusHandle, error) {
	messageID := strings.TrimSpace(info.SourceMessageID)
	if messageID == "" {
		return gateway.ProcessingStatusHandle{}, nil
	}
	reactionGW, err := a.processingReactionGateway(cfg)
	if err != nil {
		return gateway.ProcessingStatusHandle{}, err
	}
	token, err := addProcessingReaction(ctx, reactionGW, messageID, processingBusyReactionType)
	if err != nil {
		return gateway.ProcessingStatusHandle{}, err
	}
	return gateway.ProcessingStatusHandle{Token: token}, nil
}

// ProcessingCompleted removes the transient processing reaction before output is sent.
func (a *FeishuAdapter) ProcessingCompleted(ctx context.Context, cfg gateway.ChannelConfig, _ gateway.InboundMessage, info gateway.ProcessingStatusInfo, handle gateway.ProcessingStatusHandle) error {
	messageID := strings.TrimSpace(info.SourceMessageID)
	...
	return removeProcessingReaction(ctx, reactionGW, messageID, reactionID)
}

// ProcessingFailed removes the transient processing reaction when chat processing fails.
func (a *FeishuAdapter) ProcessingFailed(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.InboundMessage, info gateway.ProcessingStatusInfo, handle gateway.ProcessingStatusHandle, _ error) error {
	return a.ProcessingCompleted(ctx, cfg, msg, info, handle)
}

func (*FeishuAdapter) processingReactionGateway(cfg gateway.ChannelConfig) (processingReactionGateway, error) {
	feishuCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
		return nil, err
	}
	client := feishuCfg.newClient()
	reactionGW := &larkProcessingReactionGateway{api: client.Im.MessageReaction}
	return reactionGW, nil
}
...
func (a *FeishuAdapter) React(ctx context.Context, cfg gateway.ChannelConfig, _ string, messageID string, emoji string) error {
	reactionGW, err := a.processingReactionGateway(cfg)
	if err != nil {
		return err
	}
	_, err = reactionGW.Add(ctx, messageID, emoji)
	return err
}
...
func (a *FeishuAdapter) Unreact(ctx context.Context, cfg gateway.ChannelConfig, _ string, messageID string, reactionID string) error {
	...
	return reactionGW.Remove(ctx, messageID, reactionID)
}
...
// DiscoverSelf retrieves the bot's own identity from the Feishu iam.
...
func (*FeishuAdapter) MatchBinding(config map[string]any, criteria gateway.BindingCriteria) bool {
	return matchBinding(config, criteria)
}

// BuildUserConfig constructs a Feishu user-binding config from an Identity.
func (*FeishuAdapter) BuildUserConfig(identity gateway.Identity) map[string]any {
	return buildUserConfig(identity)
}

// Connect establishes a WebSocket connection to Feishu and forwards inbound messages to the handler.
func (a *FeishuAdapter) Connect(ctx context.Context, cfg gateway.ChannelConfig, handler gateway.InboundHandler) (gateway.Connection, error) {
	if a.logger != nil {
	...
	// happens after ws.Run tears down its websocket. Stop
	...
	conn := gateway.NewConnection(cfg, stop)
	go func() {
		defer close(done)
...
func (a *FeishuAdapter) Send(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.PreparedOutboundMessage) error {
	feishuCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
		if a.logger != nil {
			a.logger.ErrorContext(ctx, "decode config failed", slog.String("config_id", cfg.ID), slog.Any("error", err))
...
func buildFeishuOutboundContent(msg gateway.Message) (string, string, error) {
	if len(msg.Parts) > 0 {
		if body := renderFeishuMessagePartsLarkMD(msg); body != "" {
			if len([]rune(body)) > feishuStreamMaxRunes {
				content, err := buildFeishuTextContent(normalizeFeishuStreamText(gateway.RenderPartsAsPlain(msg.Parts)))
				if err != nil {
					return "", "", err
				}
	...
	var content string
	var err error
	if msg.Format == gateway.MessageFormatPlain {
...
func (a *FeishuAdapter) OpenStream(ctx context.Context, cfg gateway.ChannelConfig, target string, opts gateway.StreamOptions) (gateway.PreparedOutboundStream, error) {
...
func (a *FeishuAdapter) sendAttachment(ctx context.Context, client *lark.Client, receiveID, receiveType string, att gateway.PreparedAttachment) error {
	var msgType string
	var contentMap map[string]string
	if att.Kind == gateway.PreparedAttachmentNativeRef && strings.TrimSpace(att.NativeRef) != "" {
		if strings.HasPrefix(att.Mime, "image/") || att.Logical.Type == gateway.AttachmentImage {
...
func resolveAttachmentUploadReader(ctx context.Context, att gateway.PreparedAttachment) (io.ReadCloser, string, string, error) {
...
func (*FeishuAdapter) ResolveAttachment(ctx context.Context, cfg gateway.ChannelConfig, attachment gateway.Attachment) (gateway.AttachmentPayload, error) {
	platformKey := strings.TrimSpace(attachment.PlatformKey)
	if platformKey == "" {
		return gateway.AttachmentPayload{}, errors.New("feishu attachment platform_key is required")
	}
	messageID := ""
	...
	if messageID == "" {
		return gateway.AttachmentPayload{}, errors.New("feishu attachment metadata.message_id is required")
	}
	feishuCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
	...
	if err != nil {
		return gateway.AttachmentPayload{}, fmt.Errorf("download feishu resource: %w", err)
	}
	if !resp.Success() {
		return gateway.AttachmentPayload{}, fmt.Errorf("download feishu resource: %s (code: %d)", resp.Msg, resp.Code)
	}
	if resp.File == nil {
		return gateway.AttachmentPayload{}, errors.New("download feishu resource: empty payload")
	}
...
func isFeishuImageAttachment(att gateway.Attachment) bool {
	if att.Type == gateway.AttachmentImage || att.Type == gateway.AttachmentGIF {
		return true
	}

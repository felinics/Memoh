...
func (*FeishuAdapter) ListPeers(ctx context.Context, cfg gateway.ChannelConfig, query gateway.DirectoryQuery) ([]gateway.DirectoryEntry, error) {
	...
	entries := make([]gateway.DirectoryEntry, 0, len(resp.Data.Items))
...
func (*FeishuAdapter) ListGroups(ctx context.Context, cfg gateway.ChannelConfig, query gateway.DirectoryQuery) ([]gateway.DirectoryEntry, error) {
	feishuCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
		return nil, err
	}
	...
	entries := make([]gateway.DirectoryEntry, 0, len(items))
	for _, c := range items {
...
func (*FeishuAdapter) ListGroupMembers(ctx context.Context, cfg gateway.ChannelConfig, groupID string, query gateway.DirectoryQuery) ([]gateway.DirectoryEntry, error) {
	feishuCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
		return nil, err
	}
...
func (a *FeishuAdapter) ResolveEntry(ctx context.Context, cfg gateway.ChannelConfig, input string, kind gateway.DirectoryEntryKind) (gateway.DirectoryEntry, error) {
	feishuCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
		return gateway.DirectoryEntry{}, err
	}
	client := feishuCfg.newClient()
	input = strings.TrimSpace(input)
	switch kind {
	case gateway.DirectoryEntryUser:
		return a.resolveUser(ctx, client, input)
	case gateway.DirectoryEntryGroup:
		return a.resolveGroup(ctx, client, input)
	default:
		return gateway.DirectoryEntry{}, fmt.Errorf("feishu resolve entry: unsupported kind %q", kind)
	}
}

func (*FeishuAdapter) resolveUser(ctx context.Context, client *lark.Client, input string) (gateway.DirectoryEntry, error) {
	userID, userIDType := parseFeishuUserInput(input)
	if userID == "" {
		return gateway.DirectoryEntry{}, fmt.Errorf("feishu resolve entry user: invalid input %q", input)
	}
	...
	if err != nil {
		return gateway.DirectoryEntry{}, fmt.Errorf("feishu get user: %w", err)
	}
	if !resp.Success() {
		return gateway.DirectoryEntry{}, fmt.Errorf("feishu get user: code=%d msg=%s", resp.Code, resp.Msg)
	}
	if resp.Data == nil || resp.Data.User == nil {
		return gateway.DirectoryEntry{}, errors.New("feishu get user: empty response")
	}
	return feishuUserToEntry(resp.Data.User), nil
}

func (*FeishuAdapter) resolveGroup(ctx context.Context, client *lark.Client, input string) (gateway.DirectoryEntry, error) {
	chatID := strings.TrimSpace(input)
	chatID = strings.TrimPrefix(chatID, "chat_id:")
	if chatID == "" {
		return gateway.DirectoryEntry{}, fmt.Errorf("feishu resolve entry group: invalid input %q", input)
	}
	...
	if err != nil {
		return gateway.DirectoryEntry{}, fmt.Errorf("feishu get chat: %w", err)
	}
	if !resp.Success() {
		return gateway.DirectoryEntry{}, fmt.Errorf("feishu get chat: code=%d msg=%s", resp.Code, resp.Msg)
	}
	return gateway.DirectoryEntry{
		Kind:      gateway.DirectoryEntryGroup,
		ID:        "chat_id:" + chatID,
...
func feishuUserToEntry(u *larkcontact.User) gateway.DirectoryEntry {
...
func feishuChatToEntry(c *larkim.ListChat) gateway.DirectoryEntry {
...
func feishuMemberToEntry(m *larkim.ListMember) gateway.DirectoryEntry {

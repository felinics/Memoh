...
func TestExtractFeishuInboundFileAttachmentInfersVideoType(t *testing.T) {
	...
	att := got.Message.Attachments[0]
	if att.Type != gateway.AttachmentVideo {
...
func TestFeishuDescriptorIncludesStreamingAndMedia(t *testing.T) {
	...
	policy := gateway.NormalizeOutboundPolicy(desc.OutboundPolicy)
...
func TestIsFeishuImageAttachment(t *testing.T) {
	...
	if !isFeishuImageAttachment(gateway.Attachment{Type: gateway.AttachmentImage}) {
		t.Fatal("expected image type to be identified as image")
	}
	if !isFeishuImageAttachment(gateway.Attachment{Type: gateway.AttachmentGIF}) {
		t.Fatal("expected gif type to be identified as image")
	}
	if !isFeishuImageAttachment(gateway.Attachment{Mime: "image/jpeg"}) {
		t.Fatal("expected image/ mime to be identified as image")
	}
	if isFeishuImageAttachment(gateway.Attachment{Type: gateway.AttachmentFile}) {
		t.Fatal("expected file type to not be identified as image")
	}
	if isFeishuImageAttachment(gateway.Attachment{Type: gateway.AttachmentAudio}) {
...
func TestBuildFeishuOutboundContentPlainTextKeepsInteractiveCard(t *testing.T) {
	...
	msgType, content, err := buildFeishuOutboundContent(gateway.Message{Text: "你好 **bold**"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestBuildFeishuOutboundContentPlainFormatUsesPlainTextCard(t *testing.T) {
	...
	msgType, content, err := buildFeishuOutboundContent(gateway.Message{
		Text:   raw,
...
func TestResolveConfiguredBotOpenIDFromExternalIdentity(t *testing.T) {
	...
	cfg := gateway.ChannelConfig{ExternalIdentity: "open_id:ou_external_2"}
...
func TestResolveConfiguredBotOpenIDIgnoresNonOpenIDExternalIdentity(t *testing.T) {
	...
	cfg := gateway.ChannelConfig{ExternalIdentity: "chat_id:oc_group_1"}
...
func TestAddProcessingReactionFirstSuccess(t *testing.T) {
	...
	reactionGW := &fakeProcessingReactionGateway{
		addResponse: []struct {
			reactionID string
			err        error
		}{
	...
	token, err := addProcessingReaction(context.Background(), reactionGW, "om_1", "Typing")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if token != "reaction-1" {
	...
	if len(reactionGW.addCalls) != 1 {
		t.Fatalf("expected one add call, got %d", len(reactionGW.addCalls))
	}
	if reactionGW.addCalls[0].messageID != "om_1" || reactionGW.addCalls[0].reactionType != "Typing" {
		t.Fatalf("unexpected add params: %+v", reactionGW.addCalls[0])
	}
}
...
func TestAddProcessingReactionReturnsError(t *testing.T) {
	...
	token, err := addProcessingReaction(context.Background(), reactionGW, "om_2", "INVALID")
	if err == nil {
	...
	if reactionGW.addCalls[0].reactionType != "INVALID" {
		t.Fatalf("unexpected add call sequence: %+v", reactionGW.addCalls)
	}
}
...
func TestAddProcessingReactionNoMessageID(t *testing.T) {
	...
	reactionGW := &fakeProcessingReactionGateway{}
	token, err := addProcessingReaction(context.Background(), reactionGW, "", "Typing")
	if err != nil {
	...
	if len(reactionGW.addCalls) != 0 {
		t.Fatalf("expected no add calls, got %+v", reactionGW.addCalls)
	}
}
...
func TestRemoveProcessingReaction(t *testing.T) {
	...
	reactionGW := &fakeProcessingReactionGateway{}
	if err := removeProcessingReaction(context.Background(), reactionGW, "om_3", "reaction-3"); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(reactionGW.removeCalls) != 1 {
		t.Fatalf("expected one remove call, got %d", len(reactionGW.removeCalls))
	}
	if reactionGW.removeCalls[0].messageID != "om_3" || reactionGW.removeCalls[0].reactionID != "reaction-3" {
		t.Fatalf("unexpected remove params: %+v", reactionGW.removeCalls[0])
	}
}
...
func TestRemoveProcessingReactionNoopForEmptyToken(t *testing.T) {
	...
	reactionGW := &fakeProcessingReactionGateway{}
	if err := removeProcessingReaction(context.Background(), reactionGW, "om_4", ""); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(reactionGW.removeCalls) != 0 {
		t.Fatalf("expected no remove calls, got %+v", reactionGW.removeCalls)
	}
}
...
func TestFeishuProcessingStartedNoSourceMessageID(t *testing.T) {
	...
	handle, err := adapter.ProcessingStarted(
		context.Background(),
		gateway.ChannelConfig{},
		gateway.InboundMessage{},
		gateway.ProcessingStatusInfo{},
	)
	if err != nil {
...
func TestFeishuProcessingStartedRequiresConfigWhenSourceMessageExists(t *testing.T) {
	...
	_, err := adapter.ProcessingStarted(
		...
		gateway.ChannelConfig{},
		gateway.InboundMessage{},
		gateway.ProcessingStatusInfo{SourceMessageID: "om_5"},
	)
	if err == nil {
...
func TestFeishuProcessingCompletedNoopWithoutToken(t *testing.T) {
	...
	err := adapter.ProcessingCompleted(
		...
		gateway.ChannelConfig{},
		gateway.InboundMessage{},
		gateway.ProcessingStatusInfo{SourceMessageID: "om_6"},
		gateway.ProcessingStatusHandle{},
	)
	if err != nil {

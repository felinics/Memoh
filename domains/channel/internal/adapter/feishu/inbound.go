...
// extractFeishuInbound converts a Feishu P2MessageReceiveV1 event into a gateway.InboundMessage.
// botOpenID is the bot's own open_id used to filter mentions; if empty, any mention is treated as bot mention.
func extractFeishuInbound(event *larkim.P2MessageReceiveV1, botOpenID string, loggers ...*slog.Logger) gateway.InboundMessage {
	var log *slog.Logger
	if len(loggers) > 0 {
		log = loggers[0]
	}
	if event == nil || event.Event == nil || event.Event.Message == nil {
		return gateway.InboundMessage{Channel: Type}
	}
	...
	if message.MessageType != nil {
		...
		case larkim.MsgTypeImage:
			if key, ok := contentMap["image_key"].(string); ok {
				msg.Attachments = append(msg.Attachments, gateway.NormalizeInboundChannelAttachment(gateway.Attachment{
		...
		case larkim.MsgTypeFile, larkim.MsgTypeAudio, larkim.MsgTypeMedia:
			if key, ok := contentMap["file_key"].(string); ok {
				...
				attType := gateway.AttachmentFile
				switch *message.MessageType {
				case larkim.MsgTypeAudio:
					attType = gateway.AttachmentAudio
				case larkim.MsgTypeMedia:
					attType = gateway.AttachmentVideo
				}
				msg.Attachments = append(msg.Attachments, gateway.NormalizeInboundChannelAttachment(gateway.Attachment{
					Type:           attType,
					PlatformKey:    key,
	...
	if chatID != "" && chatType != gateway.ConversationTypePrivate {

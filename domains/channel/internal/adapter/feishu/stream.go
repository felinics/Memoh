...
func (s *feishuOutboundStream) Push(ctx context.Context, event gateway.PreparedStreamEvent) error {
	...
	case gateway.StreamEventToolCallStart:
		...
		s.textBuffer.Reset()
		return s.renderToolCallCard(ctx, event.ToolCall, gateway.BuildToolCallStart(event.ToolCall))
	case gateway.StreamEventToolCallEnd:
		s.cardMessageID = ""
		s.lastPatched = ""
		s.lastPatchedAt = time.Time{}
		s.textBuffer.Reset()
		return s.renderToolCallCard(ctx, event.ToolCall, gateway.BuildToolCallEnd(event.ToolCall))
	...
	case gateway.StreamEventPhaseStart, gateway.StreamEventPhaseEnd:
		return nil
	case gateway.StreamEventAgentStart, gateway.StreamEventAgentEnd, gateway.StreamEventProcessingStarted, gateway.StreamEventProcessingCompleted, gateway.StreamEventProcessingFailed:
		return nil
...
func renderFeishuStreamFinalText(msg gateway.Message, buffered string) string {
	if len(msg.Parts) > 0 {
		if body := renderFeishuMessagePartsLarkMD(msg); body != "" {
			if len([]rune(body)) > feishuStreamMaxRunes {
				return gateway.RenderPartsAsPlain(msg.Parts)
			}
			return body
		}
	}
...
func feishuLogicalAttachments(attachments []gateway.PreparedAttachment) []gateway.Attachment {
...
func (s *feishuOutboundStream) renderToolCallCard(
	...
	if p.Status == gateway.ToolCallStatusRunning && callID != "" && msgID != "" {

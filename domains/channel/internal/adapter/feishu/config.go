...
func parseConfig(raw map[string]any) (Config, error) {
	...
	appSecret := strings.TrimSpace(gateway.ReadString(raw, "appSecret", "app_secret"))
	encryptKey := strings.TrimSpace(gateway.ReadString(raw, "encryptKey", "encrypt_key"))
	verificationToken := strings.TrimSpace(gateway.ReadString(raw, "verificationToken", "verification_token"))
	region, err := normalizeRegion(gateway.ReadString(raw, "region"))
	if err != nil {
		return Config{}, err
	}
	inboundMode, err := normalizeInboundMode(gateway.ReadString(raw, "inboundMode", "inbound_mode"))
	if err != nil {
		return Config{}, err
	}
	if appID == "" || appSecret == "" {
...
func parseUserConfig(raw map[string]any) (UserConfig, error) {
	openID := strings.TrimSpace(gateway.ReadString(raw, "openId", "open_id"))
	userID := strings.TrimSpace(gateway.ReadString(raw, "userId", "user_id"))
	if openID == "" && userID == "" {

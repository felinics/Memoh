...
func parseConfig(raw map[string]any) (Config, error) {
	botToken := strings.TrimSpace(gateway.ReadString(raw, "botToken", "bot_token"))
	if botToken == "" {
		return Config{}, errors.New("slack botToken is required")
	}
	appToken := strings.TrimSpace(gateway.ReadString(raw, "appToken", "app_token"))
	if appToken == "" {
...
func parseUserConfig(raw map[string]any) (UserConfig, error) {
	...
	channelID := strings.TrimSpace(gateway.ReadString(raw, "channelId", "channel_id"))
	username := strings.TrimSpace(gateway.ReadString(raw, "username"))

...
var (
	_ gateway.Sender                   = (*SlackAdapter)(nil)
	_ gateway.StreamSender             = (*SlackAdapter)(nil)
	_ gateway.Reactor                  = (*SlackAdapter)(nil)
	_ gateway.Receiver                 = (*SlackAdapter)(nil)
	_ gateway.AttachmentResolver       = (*SlackAdapter)(nil)
	_ gateway.SelfDiscoverer           = (*SlackAdapter)(nil)
	_ gateway.ConfigNormalizer         = (*SlackAdapter)(nil)
	_ gateway.TargetResolver           = (*SlackAdapter)(nil)
	_ gateway.BindingMatcher           = (*SlackAdapter)(nil)
	_ gateway.ProcessingStatusNotifier = (*SlackAdapter)(nil)
)
...
func (*SlackAdapter) Type() gateway.ChannelType {
	return Type
}

func (*SlackAdapter) Descriptor() gateway.Descriptor {
...
func (a *SlackAdapter) getOrCreateConnection(channelCfg gateway.ChannelConfig, cfg Config) (*slackConnection, error) {
	a.mu.RLock()
...
func (a *SlackAdapter) Connect(ctx context.Context, cfg gateway.ChannelConfig, handler gateway.InboundHandler) (gateway.Connection, error) {
	if a.logger != nil {
...
func (a *SlackAdapter) Send(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.PreparedOutboundMessage) error {
	slackCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
		return err
	}
...
func (*SlackAdapter) ValidatePreparedOutbound(_ context.Context, _ gateway.ChannelConfig, _ string, msg gateway.PreparedOutboundMessage) error {
	return validateSlackPreparedOutbound(msg)
}

func (a *SlackAdapter) sendSlackMessage(ctx context.Context, api *slack.Client, channelID string, msg gateway.PreparedOutboundMessage) error {
...
func validateSlackPreparedOutbound(msg gateway.PreparedOutboundMessage) error {
	body := renderSlackOutboundBody(msg.Message.Message)
	displayText := truncateSlackText(body.Text)
	blockText := truncateSlackText(body.BlockText)
	text := displayText
...
func renderSlackOutboundBody(msg gateway.Message) slackOutboundBody {
	if rich := renderSlackMessagePartsMrkdwn(msg); rich != "" {
		...
		if plain := strings.TrimSpace(gateway.RenderPartsAsPlain(msg.Parts)); plain != "" {
...
func slackURLActionFallbackText(actions []gateway.Action) string {
...
func slackURLActionBlocks(text string, actions []gateway.Action) ([]slack.Block, error) {
	if len(actions) == 0 {
		return nil, nil
	}
...
func slackSectionTextBlocks(text string) []slack.Block {
	chunks := gateway.ChunkMarkdownText(text, slackMaxSectionText)
...
func (*SlackAdapter) uploadPreparedAttachment(ctx context.Context, api *slack.Client, channelID string, threadTS string, att gateway.PreparedAttachment) error {
...
func (a *SlackAdapter) ResolveAttachment(ctx context.Context, cfg gateway.ChannelConfig, attachment gateway.Attachment) (gateway.AttachmentPayload, error) {
	slackCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
...
func (*SlackAdapter) resolveAttachmentWithClient(ctx context.Context, api *slack.Client, attachment gateway.Attachment) (gateway.AttachmentPayload, error) {
	downloadURL := strings.TrimSpace(attachment.URL)
	if attachment.Size > media.MaxAssetBytes {
		return gateway.AttachmentPayload{}, fmt.Errorf("%w: max %d bytes", media.ErrAssetTooLarge, media.MaxAssetBytes)
	}
	if downloadURL == "" {
		fileID := strings.TrimSpace(attachment.PlatformKey)
		if fileID == "" {
			return gateway.AttachmentPayload{}, errors.New("slack attachment requires url or platform_key")
		}
		file, _, _, err := api.GetFileInfoContext(ctx, fileID, 0, 0)
		if err != nil {
			return gateway.AttachmentPayload{}, fmt.Errorf("slack get file info: %w", err)
		}
		if file == nil {
			return gateway.AttachmentPayload{}, errors.New("slack file info response is empty")
		}
	...
	if downloadURL == "" {
		return gateway.AttachmentPayload{}, errors.New("slack attachment download URL is empty")
	}
...
func (a *SlackAdapter) OpenStream(ctx context.Context, cfg gateway.ChannelConfig, target string, opts gateway.StreamOptions) (gateway.PreparedOutboundStream, error) {
...
func (*SlackAdapter) ProcessingStarted(_ context.Context, _ gateway.ChannelConfig, _ gateway.InboundMessage, _ gateway.ProcessingStatusInfo) (gateway.ProcessingStatusHandle, error) {
	// Slack does not have a public typing indicator API for bots
	return gateway.ProcessingStatusHandle{}, nil
}

func (*SlackAdapter) ProcessingCompleted(_ context.Context, _ gateway.ChannelConfig, _ gateway.InboundMessage, _ gateway.ProcessingStatusInfo, _ gateway.ProcessingStatusHandle) error {
	return nil
}

func (*SlackAdapter) ProcessingFailed(_ context.Context, _ gateway.ChannelConfig, _ gateway.InboundMessage, _ gateway.ProcessingStatusInfo, _ gateway.ProcessingStatusHandle, _ error) error {
	return nil
}

func (a *SlackAdapter) React(ctx context.Context, cfg gateway.ChannelConfig, target string, messageID string, emoji string) error {
	slackCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
		return err
	}
...
func (a *SlackAdapter) Unreact(ctx context.Context, cfg gateway.ChannelConfig, target string, messageID string, emoji string) error {
	slackCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
		return err
	}
...
func (*SlackAdapter) MatchBinding(config map[string]any, criteria gateway.BindingCriteria) bool {
	return matchBinding(config, criteria)
}

func (*SlackAdapter) BuildUserConfig(identity gateway.Identity) map[string]any {
...
func (*SlackAdapter) collectAttachments(msg *slack.Msg) []gateway.Attachment {
	if msg == nil || len(msg.Files) == 0 {
		return nil
	}

	attachments := make([]gateway.Attachment, 0, len(msg.Files))
...
func (a *SlackAdapter) fetchMessageAttachments(ctx context.Context, api *slack.Client, channelID string, ts string) []gateway.Attachment {
...
func buildSlackReplyRef(channelID, timestamp, threadTimestamp, parentUserID string) *gateway.ReplyRef {

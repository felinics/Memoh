...
func TestSlackRegistryExposesSupportedInterfaces(t *testing.T) {
	...
	if sender, ok := reg.GetSender(Type); !ok || sender == nil {
		t.Fatal("expected Slack adapter to implement gateway.Sender")
	}
	if streamSender, ok := reg.GetStreamSender(Type); !ok || streamSender == nil {
		t.Fatal("expected Slack adapter to implement gateway.StreamSender")
	}
	if editor, ok := reg.GetMessageEditor(Type); ok || editor != nil {
		t.Fatal("did not expect Slack adapter to implement gateway.MessageEditor")
	}
}
...
func TestSlackDescriptorUsesRichTextChunkLimit(t *testing.T) {
	...
	policy := gateway.NormalizeOutboundPolicy(NewSlackAdapter(nil).Descriptor().OutboundPolicy)
...
func TestRenderSlackOutboundBodyFallsBackWhenEscapedRichOverflows(t *testing.T) {
	...
	got := renderSlackOutboundBody(gateway.Message{
...
func TestSlackCollectAttachmentsOmitsPrivateURLForInboundFiles(t *testing.T) {
	...
	if attachments[0].Type != gateway.AttachmentImage {
...
func TestSlackConnectClearsCachedClientWhenAuthFails(t *testing.T) {
	...
	if _, err := adapter.Connect(context.Background(), cfg, func(context.Context, gateway.ChannelConfig, gateway.InboundMessage) error {
		return nil
	}); err == nil {
	...
	conn, err := adapter.Connect(context.Background(), cfg, func(context.Context, gateway.ChannelConfig, gateway.InboundMessage) error {
		return nil
	})
	if err != nil {
...
func TestSlackResolveAttachmentDownloadsPrivateURLWithBearerToken(t *testing.T) {
	...
	payload, err := adapter.resolveAttachmentWithClient(context.Background(), client, gateway.Attachment{
...
func TestSlackResolveAttachmentRejectsOversizedKnownSlackFile(t *testing.T) {
	...
	_, err := adapter.resolveAttachmentWithClient(context.Background(), client, gateway.Attachment{
		PlatformKey: "F999",
		Type:        gateway.AttachmentFile,
	})
	if err == nil {
...
func TestSlackHandleMessageEventStoresDMChannelID(t *testing.T) {
	...
	cfg := gateway.ChannelConfig{ID: "cfg-1", BotID: "bot-1"}
	msgCh := make(chan gateway.InboundMessage, 1)
	...
	}, cfg, func(_ context.Context, _ gateway.ChannelConfig, msg gateway.InboundMessage) error {
		msgCh <- msg
		return nil
	}, "UBOT")
...
func TestSlackHandleMessageEventSkipsChannelIDOutsideDM(t *testing.T) {
	...
	adapter := NewSlackAdapter(nil)
	conn := &slackConnection{}
	cfg := gateway.ChannelConfig{ID: "cfg-2", BotID: "bot-1"}
	msgCh := make(chan gateway.InboundMessage, 1)
...
func TestSlackHandleMessageEventResolvesConversationName(t *testing.T) {
	...
	cfg := gateway.ChannelConfig{ID: "cfg-name", BotID: "bot-1"}
	msgCh := make(chan gateway.InboundMessage, 1)
...
func TestSlackHandleMessageEventKeepsFlowWhenConversationNameLookupFails(t *testing.T) {
	...
	cfg := gateway.ChannelConfig{ID: "cfg-name-fail", BotID: "bot-1"}
	msgCh := make(chan gateway.InboundMessage, 1)
...
func TestSlackHandleAppMentionEventPreservesAttachments(t *testing.T) {
	...
	cfg := gateway.ChannelConfig{ID: "cfg-mention", BotID: "bot-1"}
	msgCh := make(chan gateway.InboundMessage, 1)
...
func TestSlackHandleAppMentionEventPreservesPrivateChannelType(t *testing.T) {
	...
	conn := &slackConnection{api: slack.New(testBotToken)}
	cfg := gateway.ChannelConfig{ID: "cfg-private", BotID: "bot-1"}
	msgCh := make(chan gateway.InboundMessage, 1)
	...
	select {
	case msg := <-msgCh:
		if msg.Conversation.Type != gateway.ConversationTypeGroup {
...
func TestSlackSendReturnsAttachmentUploadFailures(t *testing.T) {
	...
	err := adapter.sendSlackMessage(context.Background(), api, "C123", gateway.PreparedOutboundMessage{
		Message: gateway.PreparedMessage{
			Message: gateway.Message{
				Text: "hello",
			},
			Attachments: []gateway.PreparedAttachment{preparedSlackUploadAttachment("hello.txt", "text/plain", "hello")},
		},
	})
	if err == nil {
...
func TestSlackStreamAttachmentOnlyClearsPlaceholder(t *testing.T) {
	...
	stream := &slackOutboundStream{
		adapter: NewSlackAdapter(nil),
		cfg:     gateway.ChannelConfig{ID: "cfg-stream"},
		target:  "C123",
		api:     client,
	}
...
func TestSlackStreamFinalFallbackDeletesOldPlaceholder(t *testing.T) {
	...
	stream := &slackOutboundStream{
		adapter: NewSlackAdapter(nil),
		cfg:     gateway.ChannelConfig{ID: "cfg-stream-fallback"},
		target:  "C123",
		api:     client,
	}
	if err := stream.Push(context.Background(), gateway.PreparedStreamEvent{
	...
	if err := stream.Push(context.Background(), gateway.PreparedStreamEvent{
		...
		Final: &gateway.PreparedStreamFinalizePayload{
			Message: gateway.PreparedMessage{
				Message: gateway.Message{Text: "final answer"},
			},
		},
	}); err != nil {
...
func TestSlackStreamFinalUsesPartsRenderer(t *testing.T) {
	...
	stream := &slackOutboundStream{
		adapter: NewSlackAdapter(nil),
		cfg:     gateway.ChannelConfig{ID: "cfg-stream-rich"},
		target:  "C123",
		api:     client,
	}
...
func TestSlackStreamFinalPreservesURLActionsOnUpdate(t *testing.T) {
	...
	stream := &slackOutboundStream{
		adapter: NewSlackAdapter(nil),
		cfg:     gateway.ChannelConfig{ID: "cfg-stream-actions"},
		target:  "C123",
		api:     client,
	}
...
func preparedSlackUploadAttachment(name string, mime string, content string) gateway.PreparedAttachment {
	return gateway.PreparedAttachment{
		Logical: gateway.Attachment{
			Type: gateway.AttachmentFile,
			Name: name,
			Mime: mime,
...
func TestSlackSendResolvesUserTargetToDMChannel(t *testing.T) {
	...
	err := adapter.Send(context.Background(), gateway.ChannelConfig{
		Credentials: map[string]any{
			"botToken": testShortBotToken,
			"appToken": testAppToken,
		},
	}, gateway.PreparedOutboundMessage{
		Target: "U123",
		Message: gateway.PreparedMessage{
...
func TestSlackURLActionBlocksSplitAtPlatformElementLimit(t *testing.T) {
	...
	actions := make([]gateway.Action, 26)
	for i := range actions {
		actions[i] = gateway.Action{
			Label: "Open docs",
...
func TestSlackURLActionBlocksRejectsPlatformBlockOverflow(t *testing.T) {
	...
	actions := make([]gateway.Action, slackMaxActionBlockElements+1)
	for i := range actions {
		actions[i] = gateway.Action{
			Label: "Open docs",
			URL:   "https://example.test/docs",
...
func TestSlackPreparedOutboundValidationRejectsPlatformBlockOverflow(t *testing.T) {
	...
	actions := make([]gateway.Action, slackMaxActionBlockElements*slackMaxMessageBlocks+1)
	for i := range actions {
		actions[i] = gateway.Action{
			Label: "Open docs",
			URL:   "https://example.test/docs",
	...
	err := NewSlackAdapter(nil).ValidatePreparedOutbound(context.Background(), gateway.ChannelConfig{}, "C123", gateway.PreparedOutboundMessage{
		Target: "C123",
...
func TestSlackURLActionBlocksEnforcesFieldLimits(t *testing.T) {
	...
	blocks, err := slackURLActionBlocks(strings.Repeat("b", maxSlackSectionTextRunes+1), []gateway.Action{{
	...
	_, err = slackURLActionBlocks("", []gateway.Action{{
		Label: "Open",
...
func TestSlackURLActionBlocksSplitsLongTextIntoVisibleSections(t *testing.T) {
	...
	blocks, err := slackURLActionBlocks(strings.Repeat("b", slackMaxSectionText+1), []gateway.Action{{
		Label: "Open",
		URL:   "https://example.test/docs",
	}})
	if err != nil {
...
func TestSlackOpenStreamResolvesUserTargetToDMChannel(t *testing.T) {
	...
	stream, err := adapter.OpenStream(context.Background(), gateway.ChannelConfig{
		Credentials: map[string]any{
			"botToken": testShortBotToken,
			"appToken": testAppToken,
		},
	}, "U123", gateway.StreamOptions{})
	if err != nil {
...
func TestSlackReactConvertsSkinToneEmojiToSlackName(t *testing.T) {
	...
	err := adapter.React(context.Background(), gateway.ChannelConfig{
		Credentials: map[string]any{
			"botToken": testShortBotToken,
			"appToken": testAppToken,
		},
...
func TestSlackUnreactConvertsSkinToneEmojiToSlackName(t *testing.T) {
	...
	err := adapter.Unreact(context.Background(), gateway.ChannelConfig{
		Credentials: map[string]any{
			"botToken": testShortBotToken,
			"appToken": testAppToken,
		},

...
var _ gateway.PreparedOutboundStream = (*slackOutboundStream)(nil)

func (s *slackOutboundStream) Push(ctx context.Context, event gateway.PreparedStreamEvent) error {
	...
	case gateway.StreamEventAgentStart, gateway.StreamEventAgentEnd,
		gateway.StreamEventPhaseStart, gateway.StreamEventPhaseEnd,
		gateway.StreamEventProcessingStarted, gateway.StreamEventProcessingCompleted,
		gateway.StreamEventProcessingFailed,
		gateway.StreamEventReaction, gateway.StreamEventSpeech:
		return nil
...
func renderSlackStreamFinalBody(msg gateway.Message, buffered string) slackOutboundBody {
...
func (s *slackOutboundStream) finalizeMessage(ctx context.Context, text string, actions []gateway.Action) error {
	return s.finalizeMessageBody(ctx, slackDefaultBody(text), actions)
}

func (s *slackOutboundStream) finalizeMessageBody(ctx context.Context, body slackOutboundBody, actions []gateway.Action) error {
	s.mu.Lock()
...
func (s *slackOutboundStream) sendAttachment(ctx context.Context, att gateway.PreparedAttachment) error {
	threadTS := ""
...
func (s *slackOutboundStream) sendToolCallMessage(
	...
	text := truncateSlackText(strings.TrimSpace(gateway.RenderToolCallMessageMarkdown(p)))
	if text == "" {
		return nil
	}
	callID := ""
	...
	if p.Status == gateway.ToolCallStatusRunning && callID != "" && ts != "" {
...
func (s *slackOutboundStream) postMessageWithRetry(ctx context.Context, body slackOutboundBody, actions []gateway.Action) (string, error) {
	text := body.Text
...
func (s *slackOutboundStream) updateMessageText(ctx context.Context, msgTS string, body slackOutboundBody, actions []gateway.Action) error {
	text := body.Text
	blockText := body.BlockText
	if blockText == "" {
		blockText = text
...
func (s *slackOutboundStream) updateMessageTextWithRetry(ctx context.Context, msgTS string, body slackOutboundBody, actions []gateway.Action) error {
	var lastErr error
	for attempt := 0; attempt < slackStreamFinalMaxRetries; attempt++ {

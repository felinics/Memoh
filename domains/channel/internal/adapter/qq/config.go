...
func matchBinding(raw map[string]any, criteria gateway.BindingCriteria) bool {
	cfg, err := parseUserConfig(raw)
	if err != nil {
		return false
	}
...
func buildUserConfig(identity gateway.Identity) map[string]any {
...
func parseConfig(raw map[string]any) (Config, error) {
	appID := strings.TrimSpace(gateway.ReadString(raw, "appId", "app_id"))
	clientSecret := strings.TrimSpace(gateway.ReadString(raw, "clientSecret", "client_secret"))
	if appID == "" {
...
func parseUserConfig(raw map[string]any) (UserConfig, error) {
	targetType := strings.ToLower(strings.TrimSpace(gateway.ReadString(raw, "targetType", "target_type")))
	targetID := strings.TrimSpace(gateway.ReadString(raw, "targetId", "target_id"))
	if targetType == "" || targetID == "" {
		switch {
		case strings.TrimSpace(gateway.ReadString(raw, "userOpenid", "user_openid")) != "":
			targetType = "c2c"
			targetID = strings.TrimSpace(gateway.ReadString(raw, "userOpenid", "user_openid"))
		case strings.TrimSpace(gateway.ReadString(raw, "groupOpenid", "group_openid")) != "":
			targetType = "group"
			targetID = strings.TrimSpace(gateway.ReadString(raw, "groupOpenid", "group_openid"))
		case strings.TrimSpace(gateway.ReadString(raw, "channelId", "channel_id")) != "":
			targetType = "channel"
			targetID = strings.TrimSpace(gateway.ReadString(raw, "channelId", "channel_id"))
		}
	}
	if targetType == "" || targetID == "" {

...
import (
	...
	channeltest "github.com/memohai/memoh/domains/channel/internal/test/store"
	"github.com/memohai/memoh/domains/media"
)

func sendPreparedQQ(t *testing.T, adapter *QQAdapter, cfg gateway.ChannelConfig, store gateway.OutboundAttachmentStore, msg gateway.OutboundMessage) error {
	t.Helper()
	if cfg.ChannelType == "" {
	...
	prepared, err := gateway.PrepareOutboundMessage(context.Background(), store, cfg, msg)
	if err != nil {
		return err
	}
...
func TestQQSendTextReply(t *testing.T) {
	...
	err := sendPreparedQQ(t, adapter, gateway.ChannelConfig{
		ID:    "cfg-1",
		BotID: "bot-1",
...
func TestQQOutboundCapabilitiesReflectRuntimeMarkdownGate(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			caps := adapter.ResolveOutboundCapabilities(gateway.ChannelConfig{Credentials: tc.credentials}, tc.target, base)
...
func TestQQProcessingStartedSendsInputHintForDirectMessages(t *testing.T) {
	...
	adapter := newTestQQAdapter(server)
	_, err := adapter.ProcessingStarted(context.Background(), gateway.ChannelConfig{
		ID: "cfg-3",
		Credentials: map[string]any{
			"appId":           "4096",
			"clientSecret":    "secret",
	...
	}, gateway.InboundMessage{}, gateway.ProcessingStatusInfo{

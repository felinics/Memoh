...
func (a *QQAdapter) Send(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.PreparedOutboundMessage) error {
	...
	if text != "" {
		useMarkdown := parsed.MarkdownSupport && msg.Message.Message.Format == gateway.MessageFormatMarkdown && target.Kind != qqTargetChannel
		if msg.Message.Message.Format == gateway.MessageFormatMarkdown && !useMarkdown {
			text = gateway.StripInlineMarkup(text)
		}
...
func sendAttachment(ctx context.Context, client *qqClient, target qqTarget, replyTo string, att gateway.PreparedAttachment) error {
...
func prepareAttachmentUpload(ctx context.Context, att gateway.PreparedAttachment) (attachmentUpload, error) {
...
func deriveAttachmentName(att gateway.PreparedAttachment) string {
...
func deriveFileNameFromMime(mimeType string, attType gateway.AttachmentType) string {
...
func supportsQQVoiceUpload(att gateway.PreparedAttachment, fileName string, resolvedMime string) bool {

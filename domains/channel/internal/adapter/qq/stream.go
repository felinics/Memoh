...
type qqOutboundStream struct {
	...
	send   func(context.Context, gateway.PreparedOutboundMessage) error
...
func (a *QQAdapter) OpenStream(_ context.Context, cfg gateway.ChannelConfig, target string, opts gateway.StreamOptions) (gateway.PreparedOutboundStream, error) {
	...
	return &qqOutboundStream{
		target: target,
		reply:  opts.Reply,
		send: func(ctx context.Context, msg gateway.PreparedOutboundMessage) error {
			if msg.Target == "" {
				msg.Target = target
			}
...
func (s *qqOutboundStream) Push(ctx context.Context, event gateway.PreparedStreamEvent) error {
	if s == nil || s.send == nil {
	...
	case gateway.StreamEventToolCallEnd:
		text := strings.TrimSpace(gateway.RenderToolCallMessage(gateway.BuildToolCallEnd(event.ToolCall)))
		if text == "" {
			return nil
		}
		return s.send(ctx, gateway.PreparedOutboundMessage{
			Target: s.target,
			Message: gateway.PreparedMessage{
				Message: gateway.Message{Format: gateway.MessageFormatPlain, Text: text, Reply: s.reply},
			},
		})
	...
	case gateway.StreamEventError:
		...
		return s.flush(ctx, gateway.PreparedMessage{
...
func (s *qqOutboundStream) flush(ctx context.Context, msg gateway.PreparedMessage) error {
	s.mu.Lock()
	bufferedText := strings.TrimSpace(s.buffer.String())
	bufferedAttachments := append([]gateway.PreparedAttachment(nil), s.attachments...)
	...
	if bufferedText != "" {
		...
		if logicalMsg.Format == "" {
			logicalMsg.Format = gateway.MessageFormatPlain
		}
	} else if alreadySentText && len(bufferedAttachments) == 0 && len(msg.Attachments) == 0 && strings.TrimSpace(logicalMsg.PlainText()) != "" {
		return nil
	}
	preparedAttachments := append([]gateway.PreparedAttachment(nil), bufferedAttachments...)
	...
	if err := s.send(ctx, gateway.PreparedOutboundMessage{
		Target: s.target,
		Message: gateway.PreparedMessage{
			Message:     logicalMsg,
...
func preparedAttachmentLogicals(attachments []gateway.PreparedAttachment) []gateway.Attachment {

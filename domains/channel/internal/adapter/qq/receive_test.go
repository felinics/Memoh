...
func TestEventToInboundMessageChannelAt(t *testing.T) {
	...
	if msg.Conversation.Type != gateway.ConversationTypeThread {
		t.Fatalf("unexpected conversation type: %s", msg.Conversation.Type)
	}
...
func TestEventToInboundMessagePreservesGIFType(t *testing.T) {
	...
	if msg.Message.Attachments[0].Type != gateway.AttachmentGIF {
...
func TestHandleDispatchMarksHealthySessionForReadyAndResumed(t *testing.T) {
	...
	adapter := NewQQAdapter(nil)
	cfg := gateway.ChannelConfig{ID: "cfg-healthy", BotID: "bot-healthy"}

	session := sessionState{}
	healthy, err := adapter.handleDispatch(context.Background(), cfg, func(context.Context, gateway.ChannelConfig, gateway.InboundMessage) error {
		return nil

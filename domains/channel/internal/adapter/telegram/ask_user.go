...
func parseAskUserToolCall(tc *gateway.StreamToolCall) (requestID string, payload userinput.UIPayload, ok bool) {
...
func askUserAnswered(state userinput.TextInteractionState, questionID string) (agentdomain.QuestionAnswer, bool) {
	answer, ok := state.Answer(questionID)
	if !ok || answer.Skipped {
		return agentdomain.QuestionAnswer{}, false
	}
	return answer, true
}
...
func renderAskUserPage(requestID string, loc *i18n.Localizer, payload userinput.UIPayload, state userinput.TextInteractionState) (text string, actions []gateway.Action) {
...
func askUserOptionSelected(kind string, answer agentdomain.QuestionAnswer, answered bool, optionID string) bool {
	if !answered {
		return false
	}
...
func askUserBodyAnswer(q userinput.UIQuestion, answer agentdomain.QuestionAnswer, answered bool, loc *i18n.Localizer) string {
	if !answered {
		return ""
	}
...
func prepareTelegramAskUser(tc *gateway.StreamToolCall) (text string, actions []gateway.Action, requestID string, ok bool) {

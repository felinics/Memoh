...
// TelegramAdapter implements the gateway.Adapter, gateway.Sender, and gateway.Receiver interfaces for Telegram.
...
// pagination/selection (gateway.MessageEditor).
var _ gateway.MessageEditor = (*TelegramAdapter)(nil)
...
func (*TelegramAdapter) Type() gateway.ChannelType {
	return Type
}

// Descriptor returns the Telegram channel metadata.
func (*TelegramAdapter) Descriptor() gateway.Descriptor {
...
func (*TelegramAdapter) MatchBinding(config map[string]any, criteria gateway.BindingCriteria) bool {
	return matchBinding(config, criteria)
}

// BuildUserConfig constructs a Telegram user-binding config from an Identity.
func (*TelegramAdapter) BuildUserConfig(identity gateway.Identity) map[string]any {
...
func (a *TelegramAdapter) Connect(ctx context.Context, cfg gateway.ChannelConfig, handler gateway.InboundHandler) (gateway.Connection, error) {
	if a.logger != nil {
...
func (a *TelegramAdapter) dispatchInbound(ctx context.Context, cfg gateway.ChannelConfig, handler gateway.InboundHandler, msg gateway.InboundMessage) {
...
func (a *TelegramAdapter) buildTelegramInboundMessage(bot *tele.Bot, cfg gateway.ChannelConfig, update *tele.Update) (gateway.InboundMessage, bool) {
	raw := update.Message
	if raw == nil {
...
func (a *TelegramAdapter) handleTelegramCallback(ctx context.Context, cfg gateway.ChannelConfig, handler gateway.InboundHandler, bot *tele.Bot, update *tele.Update) {
	cb := update.Callback
	if cb == nil {
		return
	}
...
func (a *TelegramAdapter) buildTelegramCallbackInboundMessage(cfg gateway.ChannelConfig, update *tele.Update) (gateway.InboundMessage, bool) {
	cb := update.Callback
	...
	msg.Message.Reply = &gateway.ReplyRef{
		MessageID: replyID,
...
func (a *TelegramAdapter) handleAskUserWizardCallback(ctx context.Context, cfg gateway.ChannelConfig, handler gateway.InboundHandler, bot *tele.Bot, update *tele.Update) bool {
	cb := update.Callback
	if cb == nil {
		return false
	}
...
func (a *TelegramAdapter) tryHandleAskUserTextReply(ctx context.Context, cfg gateway.ChannelConfig, handler gateway.InboundHandler, bot *tele.Bot, update *tele.Update) bool {
	raw := update.Message
	...
	if result.Reject != userinput.RejectNone {
		...
		if a.logger != nil {
			a.logger.DebugContext(ctx, "telegram: ask_user text answer rejected", slog.String("reject", string(result.Reject)))
		}
		return true
	}
	req := result.Request
...
func (a *TelegramAdapter) submitAskUser(ctx context.Context, cfg gateway.ChannelConfig, handler gateway.InboundHandler, bot *tele.Bot, update *tele.Update, loc *i18n.Localizer, req userinput.Request, cardChatID int64, cardMsgID int) {
...
func (a *TelegramAdapter) buildAskUserSubmitInbound(cfg gateway.ChannelConfig, update *tele.Update, req userinput.Request, cardMsgID int) (gateway.InboundMessage, bool) {
	...
	if update != nil && update.Callback != nil && update.Callback.Message != nil {
		...
		msg.Message.Reply = &gateway.ReplyRef{MessageID: strconv.Itoa(raw.ID), AttachmentsKnown: true}
		return msg, true
	}
	if update != nil && update.Message != nil {
		...
		if cardMsgID > 0 {
			msg.Message.Reply = &gateway.ReplyRef{MessageID: strconv.Itoa(cardMsgID), AttachmentsKnown: true}
		}
		return msg, true
	}
	return gateway.InboundMessage{}, false
...
func (a *TelegramAdapter) logTelegramInbound(configID string, msg gateway.InboundMessage) {
	if a.logger == nil {
		return
	}
	a.logger.Info(
...
func (a *TelegramAdapter) Send(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.PreparedOutboundMessage) error {
	telegramCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
		if a.logger != nil {
			a.logger.ErrorContext(ctx, "decode config failed", slog.String("config_id", cfg.ID), slog.Any("error", err))
...
func (*TelegramAdapter) ValidatePreparedOutbound(_ context.Context, _ gateway.ChannelConfig, _ string, msg gateway.PreparedOutboundMessage) error {
	return validateTelegramPreparedOutbound(msg)
}

func validateTelegramPreparedOutbound(msg gateway.PreparedOutboundMessage) error {
	if msg.Message.Message.IsEmpty() {
		return errors.New("message is required")
	}
...
func renderTelegramOutboundBody(msg gateway.Message) (telegramInputRichMessage, string, string) {
	...
	if len(msg.Parts) > 0 {
		return telegramInputRichMessage{}, gateway.RenderPartsAsPlain(msg.Parts), ""
	}
	return telegramInputRichMessage{}, text, parseMode
}

func shouldSplitTelegramPlainFallback(msg gateway.Message, rich telegramInputRichMessage, text string, parseMode string) bool {
...
// satisfying gateway.MessageEditor. It powers interactive pagination/selection:
// passing empty Actions removes the keyboard. Channel-username targets are not
// supported (edits require a numeric chat ID).
func (a *TelegramAdapter) Update(ctx context.Context, cfg gateway.ChannelConfig, target string, messageID string, msg gateway.PreparedMessage) error {
	telegramCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
		return err
	}
...
// Unsend deletes a previously-sent message, satisfying gateway.MessageEditor.
func (a *TelegramAdapter) Unsend(_ context.Context, cfg gateway.ChannelConfig, target string, messageID string) error {
	telegramCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
		return err
	}
...
func (a *TelegramAdapter) OpenStream(ctx context.Context, cfg gateway.ChannelConfig, target string, opts gateway.StreamOptions) (gateway.PreparedOutboundStream, error) {
...
func parseReplyToMessageID(reply *gateway.ReplyRef) int {
	if reply == nil {
		return 0
	}
...
func normalizeTelegramConversationType(chatType string) string {
	...
	case "private":
		return gateway.ConversationTypePrivate
	case "group", "supergroup", "channel":
		return gateway.ConversationTypeGroup
	default:
		return gateway.ConversationTypeGroup
	}
}
...
func sendTelegramTextWithActions(bot *tele.Bot, target string, text string, replyTo int, parseMode string, actions []gateway.Action) error {
	_, _, err := sendTelegramTextWithActionsReturnMessage(bot, target, text, replyTo, parseMode, actions)
	return err
}

func sendTelegramTextChunksWithActions(bot *tele.Bot, target string, text string, replyTo int, parseMode string, actions []gateway.Action) error {
	chunks := gateway.ChunkText(text, telegramMaxMessageLength)
	if len(chunks) == 0 {
...
func sendTelegramTextChunkListWithActions(bot *tele.Bot, target string, chunks []string, replyTo int, parseMode string, actions []gateway.Action) error {
...
func sendTelegramTextWithActionsReturnMessage(bot *tele.Bot, target string, text string, replyTo int, parseMode string, actions []gateway.Action) (chatID int64, messageID int, err error) {
	recipient, parsedChatID, parseErr := telegramRecipient(target)
	if parseErr != nil {
		return 0, 0, parseErr
	}
...
func editTelegramMessageTextWithActions(bot *tele.Bot, chatID int64, messageID int, text string, parseMode string, actions []gateway.Action) error {
...
func rawEditTelegramMessageTextWithActions(bot *tele.Bot, chatID int64, messageID int, text string, parseMode string, actions []gateway.Action) error {
	text, markup, err := telegramTextWithActionMarkup(text, actions)
	if err != nil {
		return err
	}
...
func telegramTextWithActionMarkup(text string, actions []gateway.Action) (string, *tele.ReplyMarkup, error) {
...
func telegramInlineKeyboard(actions []gateway.Action) *tele.ReplyMarkup {
	markup, _ := telegramInlineKeyboardWithFallback(actions)
	return markup
}

func telegramInlineKeyboardWithFallback(actions []gateway.Action) (*tele.ReplyMarkup, string) {
...
func validateTelegramActions(actions []gateway.Action) error {
...
func telegramActionButton(label string, action gateway.Action) (tele.InlineButton, bool) {
	if value := strings.TrimSpace(action.Value); value != "" {
		if len([]byte(value)) > telegramMaxCallbackDataBytes {
...
func sendTelegramAttachmentWithAssets(ctx context.Context, bot *tele.Bot, target string, att gateway.PreparedAttachment, caption string, replyTo int, parseMode string, actions []gateway.Action) error {
	return sendTelegramAttachmentImpl(ctx, bot, target, att, caption, replyTo, parseMode, actions)
}

func sendTelegramAttachmentImpl(ctx context.Context, bot *tele.Bot, target string, att gateway.PreparedAttachment, caption string, replyTo int, parseMode string, actions []gateway.Action) error {
	if err := validateTelegramActions(actions); err != nil {
		return err
	}
...
func telegramAttachmentSendOptions(parseMode string, replyTo int, actions []gateway.Action) *tele.SendOptions {
	opts := &tele.SendOptions{ParseMode: parseMode}
	if replyTo > 0 {
		opts.ReplyTo = &tele.Message{ID: replyTo}
	}
...
func resolveTelegramFile(ctx context.Context, att gateway.PreparedAttachment) (tele.File, error) {
	switch att.Kind {
	case gateway.PreparedAttachmentNativeRef:
	...
	case gateway.PreparedAttachmentPublicURL:
	...
	case gateway.PreparedAttachmentUpload:
		if att.Open == nil {
...
func buildTelegramReplyRef(msg *tele.Message, chatID string) *gateway.ReplyRef {
	if msg == nil || msg.ReplyTo == nil {
		return nil
	}
	replyTo := msg.ReplyTo
	ref := &gateway.ReplyRef{
...
func buildTelegramForwardRef(msg *tele.Message) *gateway.ForwardRef {
	if msg == nil {
		return nil
	}
	ref := &gateway.ForwardRef{}
...
func (a *TelegramAdapter) collectTelegramAttachments(bot *tele.Bot, msg *tele.Message) []gateway.Attachment {
	if msg == nil {
		return nil
	}
	attachments := make([]gateway.Attachment, 0, 1)
	if msg.Photo != nil {
		att := a.buildTelegramAttachment(bot, gateway.AttachmentImage, msg.Photo.FileID, "", "", msg.Photo.FileSize)
		att.Width = msg.Photo.Width
		att.Height = msg.Photo.Height
		attachments = append(attachments, att)
	}
	if msg.Document != nil {
		att := a.buildTelegramAttachment(bot, gateway.AttachmentFile, msg.Document.FileID, msg.Document.FileName, msg.Document.MIME, msg.Document.FileSize)
		attachments = append(attachments, att)
	}
	if msg.Audio != nil {
		att := a.buildTelegramAttachment(bot, gateway.AttachmentAudio, msg.Audio.FileID, msg.Audio.FileName, msg.Audio.MIME, msg.Audio.FileSize)
		att.DurationMs = int64(msg.Audio.Duration) * 1000
		attachments = append(attachments, att)
	}
	if msg.Voice != nil {
		att := a.buildTelegramAttachment(bot, gateway.AttachmentVoice, msg.Voice.FileID, "", msg.Voice.MIME, msg.Voice.FileSize)
		att.DurationMs = int64(msg.Voice.Duration) * 1000
		attachments = append(attachments, att)
	}
	if msg.Video != nil {
		att := a.buildTelegramAttachment(bot, gateway.AttachmentVideo, msg.Video.FileID, msg.Video.FileName, msg.Video.MIME, msg.Video.FileSize)
	...
	if msg.Animation != nil {
		att := a.buildTelegramAttachment(bot, gateway.AttachmentGIF, msg.Animation.FileID, msg.Animation.FileName, msg.Animation.MIME, msg.Animation.FileSize)
	...
	if msg.Sticker != nil {
		att := a.buildTelegramAttachment(bot, gateway.AttachmentImage, msg.Sticker.FileID, "", "", msg.Sticker.FileSize)
...
func (a *TelegramAdapter) buildTelegramAttachment(bot *tele.Bot, attType gateway.AttachmentType, fileID, name, mime string, size int64) gateway.Attachment {
	url := ""
	...
	return gateway.NormalizeInboundChannelAttachment(att)
}
...
func (a *TelegramAdapter) ResolveAttachment(ctx context.Context, cfg gateway.ChannelConfig, attachment gateway.Attachment) (gateway.AttachmentPayload, error) {
	fileID := strings.TrimSpace(attachment.PlatformKey)
	if fileID == "" && strings.TrimSpace(attachment.URL) == "" {
		return gateway.AttachmentPayload{}, errors.New("telegram attachment requires platform_key or url")
	}
	telegramCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
	...
	if downloadURL == "" {
		...
		if err != nil {
			return gateway.AttachmentPayload{}, fmt.Errorf("resolve telegram file url: %w", err)
		}
	}
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, downloadURL, nil)
	if err != nil {
		return gateway.AttachmentPayload{}, fmt.Errorf("build download request: %w", err)
	}
	client := &http.Client{Timeout: 60 * time.Second}
	resp, err := client.Do(req) //nolint:gosec // G704: URL is a Telegram file download URL from the Telegram Bot API
	if err != nil {
		return gateway.AttachmentPayload{}, fmt.Errorf("download attachment: %w", err)
	}
	if resp.StatusCode != http.StatusOK {
		...
		return gateway.AttachmentPayload{}, fmt.Errorf("download attachment status: %d", resp.StatusCode)
	}
	...
	if resp.ContentLength > maxBytes {
		...
		_, _ = io.Copy(io.Discard, resp.Body)
		return gateway.AttachmentPayload{}, fmt.Errorf("%w: max %d bytes", media.ErrAssetTooLarge, maxBytes)
	}
...
// DiscoverSelf retrieves the bot's own identity from the Telegram iam.
...
func (a *TelegramAdapter) ProcessingStarted(ctx context.Context, cfg gateway.ChannelConfig, _ gateway.InboundMessage, info gateway.ProcessingStatusInfo) (gateway.ProcessingStatusHandle, error) {
...
func (*TelegramAdapter) ProcessingCompleted(_ context.Context, _ gateway.ChannelConfig, _ gateway.InboundMessage, _ gateway.ProcessingStatusInfo, _ gateway.ProcessingStatusHandle) error {
	return nil
}

// ProcessingFailed is a no-op for Telegram (typing indicator clears automatically).
func (*TelegramAdapter) ProcessingFailed(_ context.Context, _ gateway.ChannelConfig, _ gateway.InboundMessage, _ gateway.ProcessingStatusInfo, _ gateway.ProcessingStatusHandle, _ error) error {
	return nil
}
...
// React adds an emoji reaction to a message (implements gateway.Reactor).
func (a *TelegramAdapter) React(_ context.Context, cfg gateway.ChannelConfig, target string, messageID string, emoji string) error {
	telegramCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
		return err
	}
...
// Unreact removes the bot's reaction from a message (implements gateway.Reactor).
// The emoji parameter is ignored; Telegram clears all bot reactions at once.
func (a *TelegramAdapter) Unreact(_ context.Context, cfg gateway.ChannelConfig, target string, messageID string, _ string) error {
	telegramCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
		return err
	}

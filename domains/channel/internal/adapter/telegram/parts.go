...
// extractTelegramMessageParts converts Telegram entities into gateway.MessagePart
...
func extractTelegramMessageParts(msg *tele.Message) []gateway.MessagePart {
	if msg == nil {
		return nil
	}
	text := msg.Text
	...
	appendPlain := func(s string) {
		...
		parts = append(parts, gateway.MessagePart{Type: gateway.MessagePartText, Text: s})
	}
...
func telegramEntityToPart(ent tele.MessageEntity, slice string) (gateway.MessagePart, bool) {
	switch ent.Type {
	case tele.EntityBold:
		return styledText(slice, gateway.MessageStyleBold), true
	case tele.EntityItalic:
		return styledText(slice, gateway.MessageStyleItalic), true
	case tele.EntityStrikethrough:
		return styledText(slice, gateway.MessageStyleStrikethrough), true
	case tele.EntityCode:
		return styledText(slice, gateway.MessageStyleCode), true
	...
	case tele.EntityMention:
		return gateway.MessagePart{Type: gateway.MessagePartMention, Text: slice}, true
	case tele.EntityTMention:
		if ent.User == nil {
			return gateway.MessagePart{}, false
		}
...
func styledText(text string, style gateway.MessageTextStyle) gateway.MessagePart {
	return gateway.MessagePart{
		Type:   gateway.MessagePartText,
		Text:   text,
		Styles: []gateway.MessageTextStyle{style},
	}
}

func onlyPlainText(parts []gateway.MessagePart) bool {
	for _, p := range parts {
		if p.Type != gateway.MessagePartText || len(p.Styles) > 0 {
			return false
		}
	}
	return true

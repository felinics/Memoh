...
func (*TelegramAdapter) ListPeers(_ context.Context, _ gateway.ChannelConfig, _ gateway.DirectoryQuery) ([]gateway.DirectoryEntry, error) {
	return nil, nil
}

// ListGroups returns chats the bot is in. Telegram Bot API does not provide a list of chats; returns empty.
func (*TelegramAdapter) ListGroups(_ context.Context, _ gateway.ChannelConfig, _ gateway.DirectoryQuery) ([]gateway.DirectoryEntry, error) {
	return nil, nil
}

// ListGroupMembers returns group managers for the given group (Telegram only exposes this list, not all members).
func (a *TelegramAdapter) ListGroupMembers(_ context.Context, cfg gateway.ChannelConfig, groupID string, query gateway.DirectoryQuery) ([]gateway.DirectoryEntry, error) {
	...
	entries := make([]gateway.DirectoryEntry, 0, limit)
...
func (a *TelegramAdapter) ResolveEntry(ctx context.Context, cfg gateway.ChannelConfig, input string, kind gateway.DirectoryEntryKind) (gateway.DirectoryEntry, error) {
	telegramCfg, err := parseConfig(cfg.Credentials)
	if err != nil {
	...
	default:
		return gateway.DirectoryEntry{}, fmt.Errorf("telegram resolve entry: unsupported kind %q", kind)
	}
}

func (a *TelegramAdapter) resolveTelegramUser(_ context.Context, bot *tele.Bot, input string) (gateway.DirectoryEntry, error) {
	chatID, userID := parseTelegramUserInput(input)
	if chatID == 0 && userID == 0 {
		return gateway.DirectoryEntry{}, fmt.Errorf("telegram resolve entry user: invalid input %q", input)
	}
	if userID != 0 {
		member, err := bot.ChatMemberOf(tele.ChatID(chatID), tele.ChatID(userID))
		if err != nil {
			return gateway.DirectoryEntry{}, fmt.Errorf("telegram get chat member: %w", err)
		}
		if member == nil || member.User == nil {
			return gateway.DirectoryEntry{}, errors.New("telegram get chat member: empty user")
		}
		return a.telegramUserToEntryWithAvatar(bot, member.User), nil
	}
	chat, err := bot.ChatByID(chatID)
	if err != nil {
		return gateway.DirectoryEntry{}, fmt.Errorf("telegram get chat: %w", err)
	}
	if chat.Type != tele.ChatPrivate {
		return gateway.DirectoryEntry{}, fmt.Errorf("telegram resolve entry user: chat %d is not private", chatID)
	}
...
func (a *TelegramAdapter) resolveTelegramGroup(_ context.Context, bot *tele.Bot, input string) (gateway.DirectoryEntry, error) {
	chat, err := resolveTelegramChat(bot, input)
	if err != nil {
		return gateway.DirectoryEntry{}, fmt.Errorf("telegram resolve entry group: %w", err)
	}
	idStr := strconv.FormatInt(chat.ID, 10)
	name := strings.TrimSpace(chat.Title)
	if name == "" {
...
func (a *TelegramAdapter) telegramUserToEntryWithAvatar(bot *tele.Bot, u *tele.User) gateway.DirectoryEntry {
...
func telegramUserToEntry(u *tele.User) gateway.DirectoryEntry {
	if u == nil {
		return gateway.DirectoryEntry{Kind: gateway.DirectoryEntryUser}
	}

...
func formatTelegramOutput(text string, format gateway.MessageFormat) (string, string) {
	if format == gateway.MessageFormatMarkdown && strings.TrimSpace(text) != "" {
		return markdownToTelegramHTML(text), tele.ModeHTML
	}
	return text, ""
}

func renderTelegramMarkdownMathRichMessage(msg gateway.Message) telegramInputRichMessage {
	if len(msg.Parts) > 0 {
		return telegramInputRichMessage{}
	}
	switch msg.Format {
	case "", gateway.MessageFormatPlain, gateway.MessageFormatMarkdown:
	default:
		return telegramInputRichMessage{}
	}

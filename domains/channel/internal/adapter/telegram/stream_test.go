...
func mustPreparedTelegramEvent(t *testing.T, event gateway.StreamEvent) gateway.PreparedStreamEvent {
	t.Helper()
	prepared, err := gateway.PrepareStreamEvent(context.Background(), nil, gateway.ChannelConfig{
		BotID:       "bot-test",
		ChannelType: Type,
	}, event)
	if err != nil {
...
func TestTelegramOutboundStream_PushClosed(t *testing.T) {
	...
	ctx := context.Background()
	err := s.Push(ctx, mustPreparedTelegramEvent(t, gateway.StreamEvent{Type: gateway.StreamEventDelta, Delta: "x"}))
	if err == nil {
...
func TestTelegramOutboundStream_PushStatusNoOp(t *testing.T) {
	...
	ctx := context.Background()
	err := s.Push(ctx, mustPreparedTelegramEvent(t, gateway.StreamEvent{Type: gateway.StreamEventStatus}))
	if err != nil {
...
func TestTelegramOutboundStream_PushUnknownEventTypeSkipped(t *testing.T) {
	...
	err := s.Push(ctx, mustPreparedTelegramEvent(t, gateway.StreamEvent{Type: gateway.StreamEventType("unknown")}))
	if err != nil {
...
func TestTelegramOutboundStream_PushEmptyDeltaNoOp(t *testing.T) {
	...
	err := s.Push(ctx, mustPreparedTelegramEvent(t, gateway.StreamEvent{Type: gateway.StreamEventDelta, Delta: ""}))
	if err != nil {
...
func TestTelegramOutboundStream_PushErrorEventEmptyNoOp(t *testing.T) {
	...
	err := s.Push(ctx, mustPreparedTelegramEvent(t, gateway.StreamEvent{Type: gateway.StreamEventError, Error: ""}))
	if err != nil {
...
func TestTelegramOutboundStream_PushErrorEventRedactsRegisteredTokenFragments(t *testing.T) {
	...
	}, "12345", gateway.StreamOptions{Metadata: map[string]any{"conversation_type": "private"}})
	if err != nil {
	...
	err = s.Push(context.Background(), mustPreparedTelegramEvent(t, gateway.StreamEvent{Type: gateway.StreamEventError, Error: "request failed: " + prefixHalf}))
	if err != nil {
...
func TestEditStreamMessage_429SetsBackoffAndReturnsNil(t *testing.T) {
	...
	s := &telegramOutboundStream{
		adapter:      adapter,
		cfg:          gateway.ChannelConfig{ID: "test", Credentials: map[string]any{"bot_token": "fake"}},
		streamChatID: 1,
		streamMsgID:  1,
		lastEdited:   "a",
		lastEditedAt: before,
...
func TestDraftMode_DeltaUsesSendDraft(t *testing.T) {
	...
	err := s.Push(ctx, mustPreparedTelegramEvent(t, gateway.StreamEvent{Type: gateway.StreamEventDelta, Delta: "Hello "}))
	if err != nil {
...
func TestDraftMode_PhaseEndTextIsNoOp(t *testing.T) {
	...
	err := s.Push(ctx, mustPreparedTelegramEvent(t, gateway.StreamEvent{
		Type:  gateway.StreamEventPhaseEnd,
...
func TestToolCallFlow_FlushPreTextAndEditRunning(t *testing.T) {
	...
	tcStart := &gateway.StreamToolCall{Name: "read", CallID: "call_1", Input: map[string]any{"path": "/tmp/a"}}
	tcEnd := &gateway.StreamToolCall{Name: "read", CallID: "call_1", Input: map[string]any{"path": "/tmp/a"}, Result: map[string]any{"ok": true}}

	if err := s.Push(ctx, mustPreparedTelegramEvent(t, gateway.StreamEvent{Type: gateway.StreamEventToolCallStart, ToolCall: tcStart})); err != nil {
	...
	if err := s.Push(ctx, mustPreparedTelegramEvent(t, gateway.StreamEvent{Type: gateway.StreamEventToolCallEnd, ToolCall: tcEnd})); err != nil {
...
func TestToolCallFlow_NoPreTextEditsRunningInPlace(t *testing.T) {
	...
	tcStart := &gateway.StreamToolCall{Name: "read", CallID: "call_1", Input: map[string]any{"path": "/tmp/a"}}
	tcEnd := &gateway.StreamToolCall{Name: "read", CallID: "call_1", Result: map[string]any{"ok": true}}
...
func TestDraftMode_ToolCallStartSendsPermanentMessage(t *testing.T) {
	...
	err := s.Push(ctx, mustPreparedTelegramEvent(t, gateway.StreamEvent{Type: gateway.StreamEventToolCallStart}))
	if err != nil {
...
func TestDraftMode_FinalEmptyBufferSkipsDuplicate(t *testing.T) {
	...
	err := s.Push(ctx, mustPreparedTelegramEvent(t, gateway.StreamEvent{
		...
		Final: &gateway.StreamFinalizePayload{
			Message: gateway.Message{Text: "already sent text"},
		},
	}))
	if err != nil {
...
func TestDraftMode_MultipleFinalEventsOnlyOneSend(t *testing.T) {
	...
	for i, text := range []string{"intermediate 1", "intermediate 2", "final summary"} {
		err := s.Push(ctx, mustPreparedTelegramEvent(t, gateway.StreamEvent{
			Type: gateway.StreamEventFinal,
			Final: &gateway.StreamFinalizePayload{
				Message: gateway.Message{Text: text},
			},
		}))
		if err != nil {
...
func TestStreamFinal_RichHTMLOverflowRejectsInvalidActionsBeforeFallbackChunks(t *testing.T) {
	...
	err := s.Push(context.Background(), mustPreparedTelegramEvent(t, gateway.StreamEvent{
		Type: gateway.StreamEventFinal,
		Final: &gateway.StreamFinalizePayload{
			Message: gateway.Message{
				Format: gateway.MessageFormatRich,

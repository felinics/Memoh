...
func TestFormatAskUserSubmittedSummary(t *testing.T) {
	...
	state := userinput.TextInteractionState{Completed: true, Answers: []agentdomain.QuestionAnswer{
	...
	skipped := userinput.TextInteractionState{Completed: true, Answers: []agentdomain.QuestionAnswer{
...
func TestRenderAskUserChoiceShowsOnlyQuestion(t *testing.T) {
	...
	text, _ := renderToolCallPresentation(tc, gateway.BuildToolCallStart(tc))
	if text != "你更喜欢哪种编程语言？" {
...
func TestAskUserNavChromeLayout(t *testing.T) {
	...
	middle := userinput.TextInteractionState{QuestionIndex: 1, Answers: []agentdomain.QuestionAnswer{{QuestionID: "q1", Text: "a"}}}
	...
	last := userinput.TextInteractionState{QuestionIndex: 2, Answers: []agentdomain.QuestionAnswer{
...
func actionLabelsContain(actions []gateway.Action, want string) bool {
	for _, a := range actions {
...
func actionLabels(actions []gateway.Action) []string {
...
func navRowEquals(actions []gateway.Action, want ...string) bool {

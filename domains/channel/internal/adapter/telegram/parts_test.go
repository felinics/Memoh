...
func TestExtractTelegramMessageParts_BoldSpanInMiddle(t *testing.T) {
	...
	if parts[0].Type != gateway.MessagePartText || parts[0].Text != "hi " {
		t.Fatalf("part 0 wrong: %+v", parts[0])
	}
	if parts[1].Type != gateway.MessagePartText || parts[1].Text != "shout" || len(parts[1].Styles) != 1 || parts[1].Styles[0] != gateway.MessageStyleBold {
		t.Fatalf("part 1 wrong: %+v", parts[1])
	}
	if parts[2].Type != gateway.MessagePartText || parts[2].Text != " bye" {
...
func TestExtractTelegramMessageParts_ItalicStrikeCode(t *testing.T) {
	...
	cases := []struct {
		...
		style  gateway.MessageTextStyle
	}{
		{"italic", tele.EntityItalic, gateway.MessageStyleItalic},
		{"strikethrough", tele.EntityStrikethrough, gateway.MessageStyleStrikethrough},
		{"inline_code", tele.EntityCode, gateway.MessageStyleCode},
	}
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			parts := extractTelegramMessageParts(msg)
			if len(parts) != 1 || parts[0].Type != gateway.MessagePartText {
...
func TestExtractTelegramMessageParts_CodeBlockWithLanguage(t *testing.T) {
	...
	if parts[0].Type != gateway.MessagePartCodeBlock || parts[0].Language != "rust" || parts[0].Text != "fn main(){}" {
...
func TestExtractTelegramMessageParts_TextLinkWithURL(t *testing.T) {
	...
	if parts[1].Type != gateway.MessagePartLink || parts[1].URL != "https://example.com" || parts[1].Text != "Memoh" {
...
func TestExtractTelegramMessageParts_BareURLEntity(t *testing.T) {
	...
	if parts[1].Type != gateway.MessagePartLink || parts[1].URL != "https://example.com" || parts[1].Text != "https://example.com" {
...
func TestExtractTelegramMessageParts_MentionPreserved(t *testing.T) {
	...
	if parts[1].Type != gateway.MessagePartMention || parts[1].Text != "@bot" {
...
func TestExtractTelegramMessageParts_TextMentionCarriesUserMetadata(t *testing.T) {
	...
	if mention.Type != gateway.MessagePartMention {
...
func TestExtractTelegramMessageParts_TextMentionPreservesSenderLabel(t *testing.T) {
	...
	parts := extractTelegramMessageParts(msg)
	var mention *gateway.MessagePart
	for i := range parts {
		if parts[i].Type == gateway.MessagePartMention {
			mention = &parts[i]
			break
		}
	}
...
func TestExtractTelegramMessageParts_OverlappingEntityDropped(t *testing.T) {
	...
	if parts[0].Type != gateway.MessagePartText || parts[0].Text != "bold italic" || len(parts[0].Styles) != 1 || parts[0].Styles[0] != gateway.MessageStyleBold {
...
func TestExtractTelegramMessageParts_UsesCaptionAndCaptionEntities(t *testing.T) {
	...
	if parts[0].Type != gateway.MessagePartText || parts[0].Text != "x" || len(parts[0].Styles) != 1 {
		t.Fatalf("part 0 wrong: %+v", parts[0])
	}
}
...
func TestExtractTelegramMessageParts_HandlesSupplementaryPlaneEmoji(t *testing.T) {
	...
	parts := extractTelegramMessageParts(msg)
	var bold *gateway.MessagePart
	for i := range parts {
		if len(parts[i].Styles) == 1 && parts[i].Styles[0] == gateway.MessageStyleBold {
			bold = &parts[i]
			break
		}
	}
...
func TestExtractTelegramMessageParts_NestedLinkInsideBold(t *testing.T) {
	...
	if parts[0].Type != gateway.MessagePartText || parts[0].Text != "Check " || len(parts[0].Styles) != 1 || parts[0].Styles[0] != gateway.MessageStyleBold {
		t.Fatalf("part 0 wrong: %+v", parts[0])
	}
	if parts[1].Type != gateway.MessagePartLink || parts[1].URL != "https://example.com" || parts[1].Text != "this" {
		t.Fatalf("part 1 wrong: %+v", parts[1])
	}
	if parts[2].Type != gateway.MessagePartText || parts[2].Text != " out" || len(parts[2].Styles) != 1 || parts[2].Styles[0] != gateway.MessageStyleBold {
		t.Fatalf("part 2 wrong: %+v", parts[2])
	}
}
...
func TestExtractTelegramMessageParts_LinkCoextensiveWithStyle(t *testing.T) {
	...
	parts := extractTelegramMessageParts(msg)
	var link *gateway.MessagePart
	for i := range parts {
		if parts[i].Type == gateway.MessagePartLink {
			link = &parts[i]
			break
		}
	}
...
func TestExtractTelegramMessageParts_NestedMentionInsideStyle(t *testing.T) {
	...
	if parts[1].Type != gateway.MessagePartMention || parts[1].Text != "Alice" || parts[1].ChannelIdentityID != "7" {

...
func TestHandleTelegramUserInputCallbackBareRespondDoesNotDispatch(t *testing.T) {
	...
	adapter.handleTelegramCallback(context.Background(), gateway.ChannelConfig{}, func(context.Context, gateway.ChannelConfig, gateway.InboundMessage) error {
		dispatched = true
		return nil
	}, bot, update)
	if dispatched {
...
func TestBuildTelegramRespondCallbackInboundMessageMarksDirected(t *testing.T) {
	...
	msg, ok := adapter.buildTelegramCallbackInboundMessage(gateway.ChannelConfig{}, update)
	if !ok {
	...
	if _, ok := adapter.buildTelegramCallbackInboundMessage(gateway.ChannelConfig{}, update); ok {
...
func TestBuildTelegramAttachmentIncludesPlatformReference(t *testing.T) {
	...
	adapter := NewTelegramAdapter(nil)
	att := adapter.buildTelegramAttachment(nil, gateway.AttachmentFile, "file_1", "doc.txt", "text/plain", 10)
...
func TestBuildTelegramAttachmentInfersTypeFromMime(t *testing.T) {
	...
	adapter := NewTelegramAdapter(nil)
	att := adapter.buildTelegramAttachment(nil, gateway.AttachmentFile, "file_2", "photo.jpg", "IMAGE/JPEG; charset=utf-8", 10)
...
func TestTelegramResolveAttachmentRequiresReference(t *testing.T) {
	...
	adapter := NewTelegramAdapter(nil)
	_, err := adapter.ResolveAttachment(context.Background(), gateway.ChannelConfig{}, gateway.Attachment{})
	if err == nil {
...
func TestParseReplyToMessageID(t *testing.T) {
	...
	if got := parseReplyToMessageID(&gateway.ReplyRef{}); got != 0 {
		t.Fatalf("empty MessageID should return 0: %d", got)
	}
	if got := parseReplyToMessageID(&gateway.ReplyRef{MessageID: "  123  "}); got != 123 {
		t.Fatalf("expected 123: %d", got)
	}
	if got := parseReplyToMessageID(&gateway.ReplyRef{MessageID: "abc"}); got != 0 {
...
func TestTelegramInboundKeepsForwardOutOfText(t *testing.T) {
	...
	adapter := NewTelegramAdapter(nil)
	inbound, ok := adapter.toInboundTelegramMessage(nil, gateway.ChannelConfig{}, &tele.Message{
		ID:                10,
		Unixtime:          1710000000,
		Chat:              &tele.Chat{ID: -10001, Type: tele.ChatGroup, Title: "Test Group"},
...
func TestBuildTelegramInboundMessageIncludesUpdateIDMetadata(t *testing.T) {
	...
	inbound, ok := adapter.buildTelegramInboundMessage(bot, gateway.ChannelConfig{}, update)
	if !ok {
		t.Fatal("expected inbound message")
	}
...
func TestBuildTelegramInboundMessage_PlainFormatWhenNoEntities(t *testing.T) {
	...
	if inbound.Message.Format != gateway.MessageFormatPlain {
...
func TestBuildTelegramInboundMessage_RichFormatWhenEntitiesPopulateParts(t *testing.T) {
	...
	if inbound.Message.Format != gateway.MessageFormatRich {
...
func TestTelegramAdapter_OpenStreamEmptyTarget(t *testing.T) {
	...
	cfg := gateway.ChannelConfig{}
	_, err := adapter.OpenStream(ctx, cfg, "", gateway.StreamOptions{})
	if err == nil {
...
func TestTelegramAdapter_SendRichPartsUsesRichMessage(t *testing.T) {
	...
	msg := gateway.PreparedOutboundMessage{
		Target: "123",
		Message: gateway.PreparedMessage{Message: gateway.Message{
			Format: gateway.MessageFormatRich,
			Parts: []gateway.MessagePart{
				{Type: gateway.MessagePartText, Text: "hello <world>", Styles: []gateway.MessageTextStyle{gateway.MessageStyleBold}},
				{Type: gateway.MessagePartLink, Text: "docs & more", URL: "https://example.test/path?a=1&b=2"},
				{Type: gateway.MessagePartCodeBlock, Text: "fmt.Println(\"<ok>\")", Language: "go"},
			},
			Reply: &gateway.ReplyRef{MessageID: "42"},
	...
	if err := adapter.Send(context.Background(), gateway.ChannelConfig{ID: "test", Credentials: map[string]any{"bot_token": "fake"}}, msg); err != nil {
...
func TestTelegramAdapter_MediumRichPartsUsesRichMessage(t *testing.T) {
	...
	msg := gateway.PreparedOutboundMessage{
		...
		Message: gateway.PreparedMessage{Message: gateway.Message{
			Format: gateway.MessageFormatRich,
			Parts: []gateway.MessagePart{
				{Type: gateway.MessagePartText, Text: strings.Repeat("你", telegramMaxMessageLength+100), Styles: []gateway.MessageTextStyle{gateway.MessageStyleBold}},
			},
		}},
	}
...
func TestTelegramAdapter_LongRichPartsFallsBackToPlainText(t *testing.T) {
	...
	msg := gateway.PreparedOutboundMessage{
		...
		Message: gateway.PreparedMessage{Message: gateway.Message{
			Format: gateway.MessageFormatRich,
			Parts: []gateway.MessagePart{
				{Type: gateway.MessagePartText, Text: strings.Repeat("你", telegramMaxMessageLength*9), Styles: []gateway.MessageTextStyle{gateway.MessageStyleBold}},
			},
		}},
	}
...
func TestTelegramAdapter_RichHTMLOverflowFallbackSplitsPlainText(t *testing.T) {
	...
	plain := gateway.RenderPartsAsPlain(parts)
	rich := renderTelegramMessagePartsRichMessage(gateway.Message{Parts: parts})
...
func TestTelegramDescriptorUsesRichTextChunkLimit(t *testing.T) {
	...
	policy := gateway.NormalizeOutboundPolicy(NewTelegramAdapter(nil).Descriptor().OutboundPolicy)
...
func TestTelegramAttachmentSendOptionsCarriesActions(t *testing.T) {
	...
	opts := telegramAttachmentSendOptions("", 0, []gateway.Action{{
		Label: "Open",
...
func TestSendTelegramTextWithActionsUsesLabelFallbackForActionOnlyMessage(t *testing.T) {
	...
	_, _, err := sendTelegramTextWithActionsReturnMessage(bot, "123", "", 0, "", []gateway.Action{{
		Label: "Open",
		URL:   "https://example.com",
	}})
	if err != nil {
...
func TestSendTelegramTextWithActionsRejectsTextInvalidButtons(t *testing.T) {
	...
	_, _, err := sendTelegramTextWithActionsReturnMessage(bot, "123", "Choose", 0, "", []gateway.Action{{
		Label: "Unsafe",
...
func TestSendTelegramRichMessageRejectsInvalidActionsBeforeRequest(t *testing.T) {
	...
	_, _, err := sendTelegramRichMessageReturnMessage(bot, "123", telegramInputRichMessage{HTML: "<p>Choose</p>"}, 0, []gateway.Action{{
		Label: "Too large",
		Value: strings.Repeat("x", telegramMaxCallbackDataBytes+1),
	}})
	if err == nil || !strings.Contains(err.Error(), "callback data") {
...
func TestTelegramPreparedOutboundValidationRejectsInvalidActions(t *testing.T) {
	...
	err := NewTelegramAdapter(nil).ValidatePreparedOutbound(context.Background(), gateway.ChannelConfig{}, "123", gateway.PreparedOutboundMessage{
		Target: "123",
		Message: gateway.PreparedMessage{Message: gateway.Message{
			Text: "Choose",
			Actions: []gateway.Action{{
...
func telegramHTMLOverflowParts() []gateway.MessagePart {
	parts := make([]gateway.MessagePart, 0, 3000)
	for range 3000 {
		parts = append(parts, gateway.MessagePart{
			Type:   gateway.MessagePartText,
			Text:   "x",
			Styles: []gateway.MessageTextStyle{gateway.MessageStyleBold},
		})
	}
	return parts
}
...
func TestTelegramAdapter_SendRichPartsFallbackEscapesLiteralMarkdown(t *testing.T) {
	...
	msg := gateway.PreparedOutboundMessage{
		...
		Message: gateway.PreparedMessage{Message: gateway.Message{
			Format: gateway.MessageFormatRich,
			Parts: []gateway.MessagePart{
				{Type: gateway.MessagePartText, Text: `literal [evil](https://evil.test) <tag>`},
				{Type: gateway.MessagePartLink, Text: `docs "quoted"`, URL: `https://example.test/?q="x"&ok=1`},
			},
		}},
	}
...
func TestTelegramAdapter_UpdateRichPartsUsesRichMessage(t *testing.T) {
	...
	err := adapter.Update(context.Background(),
		gateway.ChannelConfig{ID: "test", Credentials: map[string]any{"bot_token": "fake"}},
		"123",
		"88",
		gateway.PreparedMessage{Message: gateway.Message{
			Format: gateway.MessageFormatRich,
			Parts: []gateway.MessagePart{
				{Type: gateway.MessagePartText, Text: "done", Styles: []gateway.MessageTextStyle{gateway.MessageStyleItalic}},
			},
		}},
	)
	if err != nil {
...
func TestProcessingStarted_EmptyParams(t *testing.T) {
	...
	handle, err := adapter.ProcessingStarted(ctx, cfg, msg, gateway.ProcessingStatusInfo{})
	if err != nil {
...
func TestProcessingCompleted_EmptyHandle(t *testing.T) {
	...
	err := adapter.ProcessingCompleted(ctx, gateway.ChannelConfig{}, gateway.InboundMessage{}, gateway.ProcessingStatusInfo{}, gateway.ProcessingStatusHandle{})
	if err != nil {
...
func TestProcessingFailed_DelegatesToCompleted(t *testing.T) {
	...
	err := adapter.ProcessingFailed(ctx, gateway.ChannelConfig{}, gateway.InboundMessage{}, gateway.ProcessingStatusInfo{}, gateway.ProcessingStatusHandle{}, errors.New("test"))
	if err != nil {
		t.Fatalf("empty handle should be no-op: %v", err)
	}
}
...
func TestResolveTelegramFile_PlatformKey(t *testing.T) {
	...
	file, err := resolveTelegramFile(context.Background(), gateway.PreparedAttachment{
		Logical:   gateway.Attachment{Type: gateway.AttachmentImage},
		Kind:      gateway.PreparedAttachmentNativeRef,
...
func TestResolveTelegramFile_PublicURL(t *testing.T) {
	...
	file, err := resolveTelegramFile(context.Background(), gateway.PreparedAttachment{
		Logical:   gateway.Attachment{Type: gateway.AttachmentImage},
		Kind:      gateway.PreparedAttachmentPublicURL,
...
func TestResolveTelegramFile_Upload(t *testing.T) {
	...
	file, err := resolveTelegramFile(context.Background(), gateway.PreparedAttachment{
		Logical: gateway.Attachment{Type: gateway.AttachmentImage},
		Kind:    gateway.PreparedAttachmentUpload,
		Mime:    "image/png",
		Name:    "test.png",
...
func TestResolveTelegramFile_NoReference(t *testing.T) {
	...
	_, err := resolveTelegramFile(context.Background(), gateway.PreparedAttachment{
		Logical: gateway.Attachment{Type: gateway.AttachmentImage},
	})
	if err == nil {

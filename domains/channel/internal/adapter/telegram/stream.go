...
func (s *telegramOutboundStream) deliverFinalTextWithActions(ctx context.Context, text, parseMode string, actions []gateway.Action) error {
	if len(actions) == 0 {
...
func (s *telegramOutboundStream) pushToolCallStart(ctx context.Context, tc *gateway.StreamToolCall) error {
	s.mu.Lock()
	...
	s.resetStreamState()
	return s.sendToolCallMessage(ctx, tc, gateway.BuildToolCallStart(tc))
}

func (s *telegramOutboundStream) pushToolCallEnd(ctx context.Context, tc *gateway.StreamToolCall) error {
	s.resetStreamState()
	return s.sendToolCallMessage(ctx, tc, gateway.BuildToolCallEnd(tc))
}
...
func renderToolCallPresentation(tc *gateway.StreamToolCall, p gateway.ToolCallPresentation) (string, string) {
	if isTelegramAskUser(tc) {
		p.Body = nil
		p.Footer = ""
	}
	rendered := strings.TrimSpace(gateway.RenderToolCallMessageMarkdown(p))
	if rendered == "" {
		return "", ""
	}
	text, parseMode := formatTelegramOutput(rendered, gateway.MessageFormatMarkdown)
	if parseMode == "" {
...
func isTelegramAskUser(tc *gateway.StreamToolCall) bool {
...
func (s *telegramOutboundStream) sendToolCallMessage(
	...
	if isTelegramToolCallEndStatus(p.Status) && callID != "" {
		if existing, ok := s.lookupToolCallMessage(callID); ok {
			...
			editErr := error(nil)
			switch {
			case p.Status == gateway.ToolCallStatusApprovalRequired && len(actions) > 0 && testEditFunc == nil:
				editErr = editTelegramMessageTextWithActions(bot, existing.chatID, existing.msgID, text, parseMode, actions)
			case (p.Status == gateway.ToolCallStatusCompleted || p.Status == gateway.ToolCallStatusFailed) && existing.hasActions && testEditFunc == nil:
			...
			if editErr == nil {
				if p.Status != gateway.ToolCallStatusApprovalRequired {
...
func isTelegramAskUserStart(p gateway.ToolCallPresentation) bool {
	return p.Status == gateway.ToolCallStatusRunning || p.Status == gateway.ToolCallStatusWaiting
}

func isTelegramToolCallStartStatus(status gateway.ToolCallStatus) bool {
	return status == gateway.ToolCallStatusRunning || status == gateway.ToolCallStatusWaiting
}

func isTelegramToolCallEndStatus(status gateway.ToolCallStatus) bool {
	switch status {
	case gateway.ToolCallStatusCompleted, gateway.ToolCallStatusFailed, gateway.ToolCallStatusApprovalRequired:
		return true
	default:
		return false
	}
...
func tcActions(tc *gateway.StreamToolCall) []gateway.Action {
	if tc == nil {
		return nil
	}
	return tc.Actions
...
func (s *telegramOutboundStream) pushAttachment(ctx context.Context, event gateway.PreparedStreamEvent) error {
...
func (s *telegramOutboundStream) pushPhaseEnd(ctx context.Context, event gateway.PreparedStreamEvent) error {
...
func (s *telegramOutboundStream) pushDelta(ctx context.Context, event gateway.PreparedStreamEvent) error {
...
func (s *telegramOutboundStream) pushFinal(ctx context.Context, event gateway.PreparedStreamEvent) error {
...
// message.
func (s *telegramOutboundStream) pushFinalRich(ctx context.Context, msg gateway.PreparedMessage) error {
...
func shouldUseTelegramFinalRichMessage(msg gateway.Message) bool {
	return len(msg.Parts) > 0 || renderTelegramMarkdownMathRichMessage(msg).hasContent()
}

func (s *telegramOutboundStream) deliverLongPlainFallback(ctx context.Context, text string, actions []gateway.Action) error {
...
// final path.
func (s *telegramOutboundStream) pushFinalAttachments(ctx context.Context, msg gateway.PreparedMessage) error {
	bot, err := s.getBot(ctx)
	if err != nil {
		return err
	}
...
func (s *telegramOutboundStream) pushError(ctx context.Context, event gateway.PreparedStreamEvent) error {
...
func (s *telegramOutboundStream) Push(ctx context.Context, event gateway.PreparedStreamEvent) error {
...
func (s *telegramOutboundStream) formatStreamContent(text string) string {
	if gateway.ContainsMarkdown(text) {
		formatted, pm := formatTelegramOutput(text, gateway.MessageFormatMarkdown)
		if pm != "" {
			s.mu.Lock()
			s.parseMode = pm
			s.mu.Unlock()

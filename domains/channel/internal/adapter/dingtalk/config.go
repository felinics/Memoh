...
func parseConfig(raw map[string]any) (adapterConfig, error) {
	cfg := adapterConfig{
		AppKey:    strings.TrimSpace(gateway.ReadString(raw, "appKey", "app_key")),
...
func parseUserConfig(raw map[string]any) (UserConfig, error) {
	cfg := UserConfig{
		...
		OpenConversationID: strings.TrimSpace(gateway.ReadString(raw, "openConversationId", "open_conversation_id")),
		DisplayName:        strings.TrimSpace(gateway.ReadString(raw, "displayName", "display_name")),
	}

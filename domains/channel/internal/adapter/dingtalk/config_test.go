...
func TestMatchBindingAndBuildUserConfig(t *testing.T) {
	...
	if !matchBinding(binding, gateway.BindingCriteria{Attributes: map[string]string{"staff_id": "staff-1"}}) {
		t.Fatal("expected staff_id to match user binding")
	}
	if !matchBinding(binding, gateway.BindingCriteria{SubjectID: "group-1"}) {
		t.Fatal("expected subject id to match group binding")
	}
	if matchBinding(binding, gateway.BindingCriteria{SubjectID: "other"}) {
		t.Fatal("did not expect unrelated subject to match")
	}

	got := buildUserConfig(gateway.Identity{
		DisplayName: "Alice",

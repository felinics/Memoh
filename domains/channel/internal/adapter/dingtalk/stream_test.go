...
func TestOutboundStreamSnapshotBuffersVisibleTextAndAttachments(t *testing.T) {
	reply := &gateway.ReplyRef{MessageID: "msg-1", Target: "user:alice"}
	...
	if err := stream.Push(context.Background(), gateway.PreparedStreamEvent{
		...
		Attachments: []gateway.PreparedAttachment{{
			...
			Logical: gateway.Attachment{Type: gateway.AttachmentImage, URL: "https://example.com/a.png"},
		}},
	}); err != nil {
...
func TestOutboundStreamSnapshotFinalMessageWinsOverBufferedText(t *testing.T) {
	...
	stream.final = &gateway.PreparedMessage{
		Message: gateway.Message{Text: "final"},
	}
...
func TestOutboundStreamRejectsPushAfterClose(t *testing.T) {
	...
	if err := stream.Push(context.Background(), gateway.PreparedStreamEvent{Type: gateway.StreamEventDelta, Delta: "late"}); err == nil {
...
func TestOpenStreamUsesSourceMessageIDAsReply(t *testing.T) {
	...
	stream, err := adapter.OpenStream(context.Background(), gateway.ChannelConfig{}, "user:alice", gateway.StreamOptions{

...
func TestBuildAPIPayloadTextAndMarkdown(t *testing.T) {
	msgKey, msgParam, err := buildAPIPayload(gateway.Message{
...
func TestBuildAPIPayloadRejectsEmptyMessage(t *testing.T) {
	if _, _, err := buildAPIPayload(gateway.Message{}, nil); err == nil {
		t.Fatal("expected empty message error")
	}
}

func TestBuildAttachmentPayloadImageURLAndFileRef(t *testing.T) {
	msgKey, msgParam, err := buildAttachmentPayload(gateway.PreparedAttachment{
		Kind:      gateway.PreparedAttachmentPublicURL,
		PublicURL: "https://example.com/image.png",
		Logical:   gateway.Attachment{Type: gateway.AttachmentImage, Name: "image.png"},
	})
	if err != nil {
...
func TestBuildAttachmentPayloadRequiresImageReference(t *testing.T) {
	if _, _, err := buildAttachmentPayload(gateway.PreparedAttachment{
...
func TestBuildWebhookBody(t *testing.T) {
	body, err := buildWebhookBody(gateway.Message{
		Format: gateway.MessageFormatMarkdown,
		Text:   "## Status\nAll green",
	})
	if err != nil {
	...
	body, err = buildWebhookBody(gateway.Message{
		Attachments: []gateway.Attachment{{Type: gateway.AttachmentFile, Name: "report.pdf"}},
	})
	if err != nil {
...
func TestExtractMarkdownTitleAndFileExt(t *testing.T) {
	...
	if got := resolveFileType(gateway.Attachment{Name: "archive.tar.gz"}); got != "gz" {

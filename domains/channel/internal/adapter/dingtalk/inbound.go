...
// buildInboundMessage converts a DingTalk BotCallbackDataModel to a gateway.InboundMessage.
// Returns false when the message should be silently ignored (e.g. empty content).
func buildInboundMessage(data *chatbot.BotCallbackDataModel) (gateway.InboundMessage, bool) {
...
func extractContent(data *chatbot.BotCallbackDataModel) (string, gateway.MessageFormat, []gateway.Attachment) {
	msgtype := strings.ToLower(strings.TrimSpace(data.Msgtype))
	switch msgtype {
	case "text":
		return strings.TrimSpace(data.Text.Content), gateway.MessageFormatPlain, nil
	...
	case "markdown":
		...
		return content, gateway.MessageFormatMarkdown, nil
	...
	case "picture", "image":
		...
		if err := json.Unmarshal(raw, &pic); err != nil || strings.TrimSpace(pic.DownloadCode) == "" {
			return "", gateway.MessageFormatPlain, nil
		}
		...
		return "", gateway.MessageFormatPlain, []gateway.Attachment{gateway.NormalizeInboundChannelAttachment(att)}
	...
	case "audio", "voice":
		...
		if rec := strings.TrimSpace(a.Recognition); rec != "" {
			return rec, gateway.MessageFormatPlain, nil
		}
	...
	default:
		...
		if text := extractStringField(data.Content, "content", "text"); text != "" {
			return text, gateway.MessageFormatPlain, nil
		}
		return "", gateway.MessageFormatPlain, nil
	}
}
...
func extractRichText(raw any) (string, gateway.MessageFormat, []gateway.Attachment) {
	...
	for _, item := range payload.RichText {
		...
		case "picture", "image":
			if code := strings.TrimSpace(item.DownloadCode); code != "" {
				...
				attachments = append(attachments, gateway.NormalizeInboundChannelAttachment(att))
			}
		}
	}
	return strings.Join(textParts, "\n"), gateway.MessageFormatPlain, attachments
}
...
func buildReplyTarget(convType, conversationID, senderID, staffID string) string {
	if gateway.IsPrivateConversationType(convType) {

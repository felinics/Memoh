...
func TestBuildInboundMessageTextGroup(t *testing.T) {
	...
	if msg.Message.ID != "msg-1" || msg.Message.Text != "hello" || msg.Message.Format != gateway.MessageFormatPlain {
	...
	if msg.Conversation.ID != "cid-1" || msg.Conversation.Type != gateway.ConversationTypeGroup || msg.Conversation.Name != "Project Room" {
...
func TestBuildInboundMessagePrivatePrefersStaffIDReplyTarget(t *testing.T) {
	...
	if msg.Conversation.Type != gateway.ConversationTypePrivate {
...
func TestExtractContentAttachments(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantType: gateway.AttachmentImage,
			wantKey:  "dl-img",
		},
		{
			name: "file",
			...
			wantType: gateway.AttachmentFile,
			wantKey:  "dl-file",
		},
		{
			name: "video",
			...
			wantType: gateway.AttachmentVideo,
			wantKey:  "video-1",
		},
	}
...
func TestExtractContentAudioRecognitionAndRichText(t *testing.T) {
	...
	if text != "voice transcript" || format != gateway.MessageFormatPlain || len(attachments) != 0 {
	...
	if text != "hello" || format != gateway.MessageFormatPlain || len(attachments) != 1 {
		t.Fatalf("unexpected richtext result: text=%q format=%q attachments=%#v", text, format, attachments)
	}
	if attachments[0].Type != gateway.AttachmentImage || attachments[0].PlatformKey != "pic-1" {

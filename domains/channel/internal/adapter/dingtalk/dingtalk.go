...
func (*DingTalkAdapter) Type() gateway.ChannelType { return Type }

func (*DingTalkAdapter) Descriptor() gateway.Descriptor {
	return gateway.Descriptor{
		...
		ConfigSchema: gateway.ConfigSchema{
			...
			Fields: map[string]gateway.FieldSchema{
				"appKey":    {Type: gateway.FieldString, Required: true, Title: "App Key (Client ID)"},
				"appSecret": {Type: gateway.FieldSecret, Required: true, Title: "App Secret (Client Secret)"},
			},
		},
		UserConfigSchema: gateway.ConfigSchema{
			Version: 1,
			Fields: map[string]gateway.FieldSchema{
				"user_id":              {Type: gateway.FieldString, Title: "User ID (single chat)"},
				"open_conversation_id": {Type: gateway.FieldString, Title: "Open Conversation ID (group chat)"},
				"display_name":         {Type: gateway.FieldString, Title: "Display Name"},
			},
		},
...
func (*DingTalkAdapter) MatchBinding(config map[string]any, criteria gateway.BindingCriteria) bool {
	return matchBinding(config, criteria)
}

// BuildUserConfig constructs a user config map from a channel Identity.
func (*DingTalkAdapter) BuildUserConfig(identity gateway.Identity) map[string]any {
...
func (a *DingTalkAdapter) Connect(ctx context.Context, cfg gateway.ChannelConfig, handler gateway.InboundHandler) (gateway.Connection, error) {
...
func (a *DingTalkAdapter) Send(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.PreparedOutboundMessage) error {
...
func (a *DingTalkAdapter) sendViaAPI(
	...
	target string,
	logicalMsg gateway.Message,
	prepared []gateway.PreparedAttachment,
) error {
	parsed, err := parseConfig(cfg.Credentials)
	if err != nil {
		return err
...
func resolveUploadAttachments(
	...
	apiCli *apiClient,
	prepared []gateway.PreparedAttachment,
) ([]gateway.PreparedAttachment, error) {
	if len(prepared) == 0 {
		return prepared, nil
	}
	result := make([]gateway.PreparedAttachment, 0, len(prepared))
	for _, att := range prepared {
		if att.Kind != gateway.PreparedAttachmentUpload || att.Open == nil {
		...
		uploaded := att
		uploaded.Kind = gateway.PreparedAttachmentNativeRef
...
func dingtalkMediaType(att gateway.PreparedAttachment) string {
...
// ResolveAttachment implements gateway.AttachmentResolver. It downloads a file
...
func (a *DingTalkAdapter) ResolveAttachment(ctx context.Context, cfg gateway.ChannelConfig, att gateway.Attachment) (gateway.AttachmentPayload, error) {
	downloadCode := strings.TrimSpace(att.PlatformKey)
	if downloadCode == "" {
		return gateway.AttachmentPayload{}, errors.New("dingtalk: attachment has no downloadCode (platform_key)")
	}
	parsed, err := parseConfig(cfg.Credentials)
	if err != nil {
		return gateway.AttachmentPayload{}, fmt.Errorf("dingtalk: resolve attachment config: %w", err)
	}
	apiCli := a.getOrNewAPIClient(cfg)
...
func (a *DingTalkAdapter) OpenStream(ctx context.Context, cfg gateway.ChannelConfig, target string, opts gateway.StreamOptions) (gateway.PreparedOutboundStream, error) {
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	default:
...
func (a *DingTalkAdapter) newChatBotHandler(cfg gateway.ChannelConfig, handler gateway.InboundHandler) chatbot.IChatBotMessageHandler {
	return func(ctx context.Context, data *chatbot.BotCallbackDataModel) ([]byte, error) {
		...
		a.logger.DebugContext(ctx, "dingtalk: inbound callback",
		...
		if err := handler(ctx, cfg, msg); err != nil {
			a.logger.ErrorContext(ctx, "dingtalk: inbound handler error",
...
func (a *DingTalkAdapter) getOrNewAPIClient(cfg gateway.ChannelConfig) *apiClient {
	a.mu.RLock()
...
func (a *DingTalkAdapter) lookupWebhook(configID string, reply *gateway.ReplyRef) (sessionWebhookContext, bool) {
	if reply == nil {

...
// buildAPIPayload converts a gateway.Message and its prepared attachments to a DingTalk
...
func buildAPIPayload(msg gateway.Message, prepared []gateway.PreparedAttachment) (msgKey, msgParam string, err error) {
...
func buildAttachmentPayload(att gateway.PreparedAttachment) (string, string, error) {
	logical := att.Logical
...
// buildWebhookBody converts a gateway.Message to a DingTalk session webhook payload.
// session webhook uses msgtype/text style instead of msgKey/msgParam.
func buildWebhookBody(msg gateway.Message) (map[string]any, error) {
...
func resolveFileType(att gateway.Attachment) string {

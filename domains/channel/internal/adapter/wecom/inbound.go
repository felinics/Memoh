...
func (a *WeComAdapter) handleFrame(ctx context.Context, cfg gateway.ChannelConfig, frame WSFrame, handler gateway.InboundHandler) error {
	switch frame.Cmd {
...
func buildInboundMessage(body MessageCallbackBody, reqID string) (gateway.InboundMessage, bool) {
...
func messageReplyRef(quote *MessageQuote, target string) *gateway.ReplyRef {
	if quote == nil {
		return nil
	}
...
func buildInboundEventMessage(body EventCallbackBody, reqID string) (gateway.InboundMessage, bool) {
...
func extractBodyContent(body MessageCallbackBody) (string, []gateway.Attachment) {
	...
	case "video":
		...
		return "", []gateway.Attachment{gateway.NormalizeInboundChannelAttachment(att)}
	case "image":
		if body.Image == nil {
			return "", nil
		}

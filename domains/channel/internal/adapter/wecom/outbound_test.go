...
func TestBuildSendPayload_Text(t *testing.T) {
	payload, cmd, reqID, err := buildSendPayload(gateway.Message{
...
func TestBuildSendPayload_AttachmentNotSupported(t *testing.T) {
	_, _, _, err := buildSendPayload(gateway.Message{
		Attachments: []gateway.Attachment{
			{Type: gateway.AttachmentImage, Base64: "aGVsbG8="},
		},
	}, "chat_1")
	if err == nil {
...
func TestBuildRespondPayload_Stream(t *testing.T) {
	payload, cmd, reqID, err := buildRespondPayload(gateway.Message{
...
func TestBuildRespondPayload_StreamWithFeedback(t *testing.T) {
	payload, cmd, _, err := buildRespondPayload(gateway.Message{
		Text: "world",
...
func TestBuildRespondPayloadWithStream_Delta(t *testing.T) {
	payload, cmd, reqID, err := buildRespondPayloadWithStream(gateway.Message{
		Text: "delta",
...
func TestBuildRespondPayloadWithStream_EmptyDeltaRejected(t *testing.T) {
	_, _, _, err := buildRespondPayloadWithStream(gateway.Message{}, "req_abc", "stream_1", false)
	if err == nil {
...
func TestBuildSendPayload_TemplateCard(t *testing.T) {
	payload, cmd, _, err := buildSendPayload(gateway.Message{
		Metadata: map[string]any{
...
func TestBuildPreparedRespondPayloadWithStream_ImageAttachment(t *testing.T) {
	...
	payload, cmd, reqID, err := buildPreparedRespondPayloadWithStream(context.Background(), gateway.PreparedMessage{
		Message: gateway.Message{
			Reply: &gateway.ReplyRef{MessageID: "msg_1"},
		},
...
func TestBuildPreparedRespondPayloadWithStream_RejectsNonImageAttachment(t *testing.T) {
	_, _, _, err := buildPreparedRespondPayloadWithStream(context.Background(), gateway.PreparedMessage{
		Attachments: []gateway.PreparedAttachment{
			{

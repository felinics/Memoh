...
func parseConfig(raw map[string]any) (adapterConfig, error) {
	cfg := adapterConfig{
		BotID:      strings.TrimSpace(gateway.ReadString(raw, "botId", "bot_id")),
		Credential: strings.TrimSpace(gateway.ReadString(raw, "secret")),
		WSURL:      strings.TrimSpace(gateway.ReadString(raw, "wsUrl", "ws_url")),
	}
...
func parseUserConfig(raw map[string]any) (UserConfig, error) {
	cfg := UserConfig{
		ChatID: strings.TrimSpace(gateway.ReadString(raw, "chatId", "chat_id")),

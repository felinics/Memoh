...
func (a *WeComAdapter) ResolveAttachment(ctx context.Context, cfg gateway.ChannelConfig, attachment gateway.Attachment) (gateway.AttachmentPayload, error) {
	_ = cfg
	if a.http == nil {
		return gateway.AttachmentPayload{}, errors.New("wecom http client not configured")
	}
	url := strings.TrimSpace(attachment.URL)
	if url == "" {
		return gateway.AttachmentPayload{}, errors.New("wecom attachment url is required")
	}
	aesKey := ""
...
func buildSendPayload(msg gateway.Message, targetID string) (any, string, string, error) {
...
func buildRespondPayload(msg gateway.Message, replyReqID string) (any, string, string, error) {
	return buildRespondPayloadWithStream(msg, replyReqID, "", true)
}

func buildPreparedRespondPayload(ctx context.Context, msg gateway.PreparedMessage, replyReqID string) (any, string, string, error) {
	return buildPreparedRespondPayloadWithStream(ctx, msg, replyReqID, "", true)
}

func buildRespondPayloadWithStream(msg gateway.Message, replyReqID string, streamID string, finish bool) (any, string, string, error) {
...
func buildPreparedStreamReplyItems(ctx context.Context, msg gateway.PreparedMessage) ([]StreamReplyItem, error) {
	if len(msg.Attachments) == 0 {
...
func buildPreparedStreamReplyItem(ctx context.Context, att gateway.PreparedAttachment) (StreamReplyItem, error) {
...
func buildLegacyStreamReplyItems(msg gateway.Message) []StreamReplyItem {
	if len(msg.Attachments) == 0 {
		return nil
	}
...
func isWeComReplyImageAttachment(att gateway.PreparedAttachment) bool {
...
func buildWelcomePayload(msg gateway.Message) (any, bool) {
...
func (a *WeComAdapter) lookupCallbackContext(configID string, reply *gateway.ReplyRef) (callbackContext, bool) {

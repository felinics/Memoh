...
var (
	_ gateway.Sender       = (*MisskeyAdapter)(nil)
	_ gateway.StreamSender = (*MisskeyAdapter)(nil)
	_ gateway.Reactor      = (*MisskeyAdapter)(nil)
	_ gateway.Receiver     = (*MisskeyAdapter)(nil)
)

func withMisskeyHTTPStub(t *testing.T, handler http.HandlerFunc) (gateway.ChannelConfig, *httptest.Server) {
	t.Helper()
...
func TestSendDeliversPreparedOutboundMessage(t *testing.T) {
	...
	err := adapter.Send(context.Background(), cfg, gateway.PreparedOutboundMessage{
		Target: "note-source",
...
func TestOpenStreamReturnsPreparedOutboundStream(t *testing.T) {
	...
	adapter := NewMisskeyAdapter(nil)
	stream, err := adapter.OpenStream(context.Background(), cfg, "note-source", gateway.StreamOptions{})
	if err != nil {

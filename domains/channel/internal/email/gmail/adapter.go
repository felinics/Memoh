...
type Adapter struct {
	logger       *slog.Logger
	tokenStore   emailport.OAuthTokenStore
	oauthClients emailport.OAuthClientResolver
}

func New(log *slog.Logger, tokenStore emailport.OAuthTokenStore, oauthClients emailport.OAuthClientResolver) *Adapter {
	return &Adapter{
...
func (a *Adapter) ExchangeCode(ctx context.Context, config map[string]any, providerID, code, redirectURI string) error {
	...
	return a.tokenStore.Save(ctx, emailport.OAuthToken{
...
func (a *Adapter) Send(ctx context.Context, config map[string]any, msg emailport.OutboundEmail) (string, error) {
...
func (a *Adapter) ListMailbox(ctx context.Context, config map[string]any, page, pageSize int) ([]emailport.InboundEmail, int, error) {
	...
	var results []emailport.InboundEmail
	for {
		msgData := fetchCmd.Next()
		if msgData == nil {
			break
		...
		results = append(results, emailport.InboundEmail{
...
func (a *Adapter) ReadMailbox(ctx context.Context, config map[string]any, uid uint32) (*emailport.InboundEmail, error) {
	client, err := a.dialIMAP(ctx, config)
	if err != nil {
...
func (a *Adapter) validToken(ctx context.Context, config map[string]any, providerID string) (accessToken, emailAddr string, err error) {
	...
	if stored.AccessToken == "" || (!stored.ExpiresAt.IsZero() && time.Until(stored.ExpiresAt) < 2*time.Minute) {
		...
		_ = a.tokenStore.Save(ctx, emailport.OAuthToken{
			ProviderID:   providerID,
...
func (a *Adapter) oauthClient() (emailport.OAuthClient, bool) {
...
func bufToInbound(buf *imapclient.FetchMessageBuffer) *emailport.InboundEmail {
	env := buf.Envelope
	if env == nil {
		return nil
	}
...
var (
	...
	_ emailport.MailboxReader = (*Adapter)(nil)
)

...
func (*imapConn) bufToInbound(buf *imapclient.FetchMessageBuffer) *emailport.InboundEmail {
	env := buf.Envelope
	if env == nil {
		return nil
	}
...
func (a *Adapter) ListMailbox(_ context.Context, config map[string]any, page, pageSize int) ([]emailport.InboundEmail, int, error) {
...
func (a *Adapter) ReadMailbox(_ context.Context, config map[string]any, uid uint32) (*emailport.InboundEmail, error) {
	client, err := a.dialIMAP(config)
	if err != nil {

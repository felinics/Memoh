...
func TestHandleReplyWithTurn_PassesContextAndImageRefs(t *testing.T) {
	...
	cmd := svc.lastCmd
	if cmd.Mode != agentdomain.ModeDiscuss || cmd.TeamID != "team-1" || cmd.BotID != "bot-1" {
		t.Fatalf("cmd = %+v", cmd)
	}
...
func TestHandleReplyWithTurnReadsConfigUnderDriverLock(t *testing.T) {
	...
	svc := &fakeTurnService{onStart: func(cmd agentdomain.StartTurnCommand) {
		calls <- cmd.BotID
	}}
	driver := NewDiscussDriver(DiscussDriverDeps{})
	sess := &discussSession{
...
func TestAgentEventToChannelEventMapsACPDecisionRequests(t *testing.T) {
	approval, ok := agentEventToChannelEvent(agentdomain.StreamEvent{
	...
	if !ok || approval.Type != gateway.StreamEventToolCallStart || approval.ToolCall == nil {
	...
	userInput, ok := agentEventToChannelEvent(agentdomain.StreamEvent{
	...
	if !ok || userInput.Type != gateway.StreamEventToolCallStart || userInput.ToolCall == nil {
...
type fakeTurnService struct {
	...
	startErr    error
	streamErr   error
	onStart     func(agentdomain.StartTurnCommand)
	calls       int
...
func (f *fakeTurnService) StartTurn(_ context.Context, cmd agentdomain.StartTurnCommand) (agentdomain.RunHandle, error) {
	...
	h := &fakeRunHandle{events: make(chan agentdomain.Event, 8), errs: make(chan error, 1)}
	go func() {
		...
		emit := func(kind string, payload []byte) {
			seq++
			h.events <- agentdomain.Event{RunID: "run-1", Seq: seq, Kind: kind, Payload: payload}
		}
		resolved, _ := json.Marshal(agentdomain.DiscussRunResolvedPayload{RuntimeType: runtimeType})
		emit(agentdomain.DiscussEventRunResolved, resolved)
		if runtimeType == sessionpkg.RuntimeACPAgent && !cmd.DiscussAddressed {
			emit(agentdomain.DiscussEventSkipped, nil)
			return
		}
		if f.streamErr != nil {
			h.errs <- f.streamErr
		...
		end, _ := json.Marshal(agentdomain.StreamEvent{Type: agentdomain.AgentEnd})
		emit(string(agentdomain.AgentEnd), end)
	}()
	return h, nil
}

func (*fakeTurnService) RespondToolApproval(context.Context, agentdomain.ToolApprovalResponse, chan<- json.RawMessage) error {
	return nil
}

func (*fakeTurnService) RespondUserInput(context.Context, agentdomain.UserInputResponse, chan<- json.RawMessage) error {
	return nil
}

func (*fakeTurnService) AdvancePlainTextUserInput(context.Context, agentdomain.AdvanceTextInput) (agentdomain.AdvanceTextResult, error) {
...
func (h *fakeRunHandle) Events() <-chan agentdomain.Event                      { return h.events }
func (h *fakeRunHandle) Errs() <-chan error                                    { return h.errs }
func (*fakeRunHandle) Inject(context.Context, agentdomain.InjectMessage) error { return nil }
func (*fakeRunHandle) AddOutboundAssets([]agentdomain.OutboundAssetRef)        {}
...
func (f *fakeDiscussCursorStore) UpsertDiscussCursor(_ context.Context, botID, _, scopeKey, routeID, _ string, position timeline.DiscussCursorPosition) error {
	f.upsertBotID = botID

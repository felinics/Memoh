...
type fakeConnectionObserver struct {
	items []gateway.ConnectionStatus
}

func (f *fakeConnectionObserver) ConnectionStatusesByBot(_ string) []gateway.ConnectionStatus {
	return f.items
}
		...
		items: []gateway.ConnectionStatus{
			{
				ConfigID:    "cfg-1",
				BotID:       "bot-1",
				ChannelType: gateway.ChannelType("telegram"),
				Running:     true,
				UpdatedAt:   now,
			},
			{

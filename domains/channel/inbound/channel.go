...
type mediaIngestor interface {
	gateway.OutboundAttachmentStore
	gateway.ContainerAttachmentIngester
}
...
type UserInputRunner interface {
	RespondUserInput(ctx context.Context, input agentdomain.UserInputResponse, eventCh chan<- json.RawMessage) error
}

type PlainTextUserInputRunner interface {
	AdvancePlainTextUserInput(ctx context.Context, input agentdomain.AdvanceTextInput) (agentdomain.AdvanceTextResult, error)
}
...
type ChannelInboundProcessor struct {
	...
	acl                 chatACL
	observer            gateway.StreamObserver
	...
	acpProfiles         agentdomain.ACPProfileResolver
...
func (p *ChannelInboundProcessor) IdentityMiddleware() gateway.Middleware {
...
func (p *ChannelInboundProcessor) SetStreamObserver(observer gateway.StreamObserver) {
	if p == nil {
		return
	}
	p.observer = observer
...
func (p *ChannelInboundProcessor) SetACPProfileResolver(resolver agentdomain.ACPProfileResolver) {
	if p == nil {
		return
	}
...
func (p *ChannelInboundProcessor) HandleInbound(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.InboundMessage, sender gateway.StreamReplySender) (retErr error) {
	if p.turnSvc == nil {
	...
	if err := gateway.RejectReservedSkillMetadata(msg.Message); err != nil {
	...
	if state.Decision != nil && state.Decision.Stop {
		if !state.Decision.Reply.IsEmpty() {
			return sender.Send(ctx, gateway.OutboundMessage{
	...
	if pendingSkillIntent == nil && slashDecision.Kind == slash.DecisionCommandAction && p.commandHandler != nil && !isModeCommand && !isToolApprovalCommand && !isUserInputResponseCommand && invocation != nil && (isDirectedAtBot(msg) || slashDirected) {
		...
		var caps gateway.ChannelCapabilities
		if p.registry != nil {
			caps, _ = p.registry.GetCapabilities(msg.Channel)
		}
		var outMsg gateway.Message
		if err != nil {
			if p.logger != nil {
		...
		if editID, ok := msg.Metadata["edit_message_id"].(string); ok && strings.TrimSpace(editID) != "" && caps.Edit {
			...
			outMsg.Reply = &gateway.ReplyRef{MessageID: mid}
		}
		return sender.Send(ctx, gateway.OutboundMessage{
			Target:  strings.TrimSpace(msg.ReplyTarget),
			Message: outMsg,
	...
	if !aclAllowed {
		...
		if isDirectedAtBot(msg) {
			...
			out := applyMessageFormat(gateway.Message{Text: command.AccessDeniedMessage(loc, role)}, p.channelCaps(msg.Channel))
			if mid := strings.TrimSpace(msg.Message.ID); mid != "" {
				out.Reply = &gateway.ReplyRef{MessageID: mid}
			}
			if sendErr := sender.Send(ctx, gateway.OutboundMessage{
				Target:  strings.TrimSpace(msg.ReplyTarget),
				Message: out,
	...
	var requestedSkillContexts []agentdomain.RequestedSkillContext
	var skillActivation *agentdomain.SkillActivation
	userMessageKind := ""
	userVisibleText := ""
	modelText := text
	...
	if pendingSkillIntent != nil {
		...
		requestedSkillContexts = make([]agentdomain.RequestedSkillContext, len(resolvedSkillContexts))
		for i := range resolvedSkillContexts {
			requestedSkillContexts[i] = agentdomain.RequestedSkillContext{
		...
		skillActivation = agentdomain.NewSkillActivation(requestedSkillContexts, pendingSkillIntent.Prompt)
		text = strings.TrimSpace(pendingSkillIntent.Prompt)
		modelText = strings.TrimSpace(agentdomain.SkillActivationModelQuery(skillActivation))
	...
	if p.pipeline != nil && sessionID != "" && pendingSkillIntent == nil {
		...
		if p.eventStore != nil {
			...
			if persistErr != nil {
				if p.logger != nil {
					p.logger.WarnContext(ctx, "persist pipeline event failed", slog.Any("error", persistErr))
				}
			} else {
				eventID = eid
			}
	...
	if p.dispatcher != nil && !isLocalChannelType(msg.Channel) && inboundMode != ModeParallel {
		if p.dispatcher.IsActive(routeID) {
			...
			headerifiedText := agentdomain.FormatUserHeader(agentdomain.UserMessageHeaderInput{
			...
			case ModeInject:
				...
				injected := p.dispatcher.Inject(routeID, agentdomain.InjectMessage{
					Text:            text,
				...
				if injected {
					...
					if p.logger != nil {
						p.logger.WarnContext(ctx, "inject failed (channel full), falling through to new stream",
...
startStream:
	...
	statusHandle := gateway.ProcessingStatusHandle{}
	...
	sourceMessageID := strings.TrimSpace(msg.Message.ID)
	replyRef := &gateway.ReplyRef{Target: target}
	if sourceMessageID != "" {
		replyRef.MessageID = sourceMessageID
	}
	stream, err := sender.OpenStream(ctx, target, gateway.StreamOptions{
		Reply:           replyRef,
	...
	if !isLocalChannelType(msg.Channel) && !p.shouldShowToolCallsInIM(ctx, identity.BotID) {
		stream = gateway.NewToolCallDroppingStream(stream)
	}
	...
	if p.observer != nil && !isLocalChannelType(msg.Channel) {
		stream = gateway.NewTeeStream(stream, p.observer, strings.TrimSpace(identity.BotID), msg.Channel)
	...
	if err := stream.Push(ctx, gateway.StreamEvent{
		Type:   gateway.StreamEventStatus,
		Status: gateway.StreamStatusStarted,
	}); err != nil {
		if statusNotifier != nil {
			if notifyErr := p.notifyProcessingFailed(ctx, statusNotifier, cfg, msg, statusInfo, statusHandle, err); notifyErr != nil {
				p.logProcessingStatusError("processing_failed", msg, identity, notifyErr)
	...
	var injectCh <-chan agentdomain.InjectMessage
	if p.dispatcher != nil && !isLocalChannelType(msg.Channel) && inboundMode != ModeParallel {
	...
	cmd := agentdomain.StartTurnCommand{
		SchemaVersion:             1,
		TeamID:                    cfg.TeamID,
	...
	if startErr != nil {
		if errors.Is(startErr, agentdomain.ErrDuplicateTurn) {
		...
		_ = stream.Push(ctx, gateway.StreamEvent{
			Type:  gateway.StreamEventError,
	...
	var (
		finalMessages []agentdomain.ModelMessage
		streamErr     error
		pushBroken    bool
	)
	for chunkCh != nil || streamErrCh != nil {
		...
		case turnEvent, ok := <-chunkCh:
			...
			for i, event := range events {
				...
				if event.Type == gateway.StreamEventAttachment && len(event.Attachments) > 0 {
				...
				if event.Type == gateway.StreamEventReaction && len(event.Reactions) > 0 {
					p.dispatchReactions(ctx, identity.BotID, msg.Channel, target, sourceMessageID, event.Reactions)
					continue
				}
				if event.Type == gateway.StreamEventSpeech && len(event.Speeches) > 0 {
	...
	if suppressReplies {
		if err := stream.Push(ctx, gateway.StreamEvent{
			Type:   gateway.StreamEventStatus,
			Status: gateway.StreamStatusCompleted,
		}); err != nil {
			return err
		}
	...
	outputs := agentdomain.ExtractAssistantOutputs(finalMessages)
	for _, output := range outputs {
		...
		if outMessage.Reply == nil && sourceMessageID != "" {
			outMessage.Reply = &gateway.ReplyRef{
				Target:    target,
				MessageID: sourceMessageID,
			}
		}
		if err := stream.Push(ctx, gateway.StreamEvent{
			Type: gateway.StreamEventFinal,
			Final: &gateway.StreamFinalizePayload{
				Message: outMessage,
			},
		}); err != nil {
			return err
...
func turnIdempotencyKey(channelType gateway.ChannelType, routeID, externalMessageID string) string {
...
func (p *ChannelInboundProcessor) sendModeConfirmation(
	...
	if p.reactor != nil {
		...
		_ = p.reactor.React(ctx, strings.TrimSpace(identity.BotID), msg.Channel, gateway.ReactRequest{
			Target:    target,
			MessageID: sourceMessageID,
			Emoji:     emoji,
		})
...
func (p *ChannelInboundProcessor) drainQueue(ctx context.Context, routeID string) {
	...
	for _, task := range result.QueuedTasks {
		if p.logger != nil {
			p.logger.InfoContext(ctx, "processing queued task",
				slog.String("route_id", routeID),
		...
		if err := p.HandleInbound(ctx, task.Cfg, task.Msg, task.Sender); err != nil { //nolint:contextcheck // ctx is already WithoutCancel from the defer caller
			if p.logger != nil {
				p.logger.ErrorContext(ctx, "queued task processing failed",
					slog.String("route_id", routeID),
					slog.Any("error", err),
				)
			}
...
func collectAttachmentPaths(attachments []agentdomain.Attachment) []string {
...
func shouldTriggerAssistantResponse(msg gateway.InboundMessage) bool {
...
func isDirectedAtBot(msg gateway.InboundMessage) bool {
	if isDirectConversationType(msg.Conversation.Type) {
		return true
	}
...
func (p *ChannelInboundProcessor) classifyChannelSlash(text string, msg gateway.InboundMessage, identity InboundIdentity) slash.Decision {
	return slash.Classify(slash.ClassifyInput{
		Text:           text,
		HasAttachments: hasSlashControlAttachments(msg),
		Surface:        slash.SurfaceChannel,
		IsGroup:        !gateway.IsPrivateConversationType(msg.Conversation.Type),
...
func hasSlashControlAttachments(msg gateway.InboundMessage) bool {
...
func replyHasOrMayHaveAttachments(reply *gateway.ReplyRef) bool {
	if reply == nil {
		return false
	}
...
func forwardHasOrMayHaveAttachments(forward *gateway.ForwardRef) bool {
	if forward == nil {
		return false
	}
...
func channelSlashAliases(msg gateway.InboundMessage, identity InboundIdentity) []string {
	aliases := []string{
		identity.BotID,
		msg.BotID,
	}
...
func (p *ChannelInboundProcessor) sendSlashError(ctx context.Context, sender gateway.StreamReplySender, msg gateway.InboundMessage, code string) error {
	if code == "" {
		code = slash.CodeUnknownSlash
	}
	out := applyMessageFormat(gateway.Message{Text: slashChannelMessage(p.localizer(ctx, msg.BotID), code)}, p.channelCaps(msg.Channel))
	if mid := strings.TrimSpace(msg.Message.ID); mid != "" {
		out.Reply = &gateway.ReplyRef{MessageID: mid}
	}
	return sender.Send(ctx, gateway.OutboundMessage{
...
func (p *ChannelInboundProcessor) channelCaps(channelType gateway.ChannelType) gateway.ChannelCapabilities {
	if p.registry == nil {
		return gateway.ChannelCapabilities{}
	}
...
func rawTextForCommand(msg gateway.InboundMessage, fallback string) string {
...
func inboundReplyMessageID(reply *gateway.ReplyRef) string {
	if reply == nil {
		return ""
	}
	return strings.TrimSpace(reply.MessageID)
...
func inboundReplySender(reply *gateway.ReplyRef) string {
	if reply == nil {
		return ""
	}
...
func inboundReplyPreview(reply *gateway.ReplyRef) string {
	if reply == nil {
		return ""
	}
...
func replyAttachmentsFromMessage(reply *gateway.ReplyRef) []gateway.Attachment {
	if reply == nil {
		return nil
	}
...
func inboundForwardMessageID(forward *gateway.ForwardRef) string {
	if forward == nil {
		return ""
	}
	return strings.TrimSpace(forward.MessageID)
...
func inboundForwardFromUserID(forward *gateway.ForwardRef) string {
	if forward == nil {
		return ""
	}
...
func inboundForwardFromConversationID(forward *gateway.ForwardRef) string {
	if forward == nil {
		return ""
	}
...
func inboundForwardSender(forward *gateway.ForwardRef) string {
	if forward == nil {
		return ""
	}
...
func inboundForwardDate(forward *gateway.ForwardRef) int64 {
	if forward == nil {
		return 0
	}
	return forward.Date
...
func messageReplyMetadata(reply *gateway.ReplyRef) map[string]any {
	if reply == nil {
		return nil
	}
...
func channelAttachmentMetadata(attachments []gateway.Attachment) []map[string]any {
	if len(attachments) == 0 {
		return nil
	}
	result := make([]map[string]any, 0, len(attachments))
	for _, att := range attachments {
		item := gateway.BundleFromAttachment(att).ToMap()
		if len(item) > 0 {
...
func messageForwardMetadata(forward *gateway.ForwardRef) map[string]any {
	if forward == nil {
		return nil
	}
	result := map[string]any{}
...
func isDirectConversationType(conversationType string) bool {
	return gateway.IsPrivateConversationType(conversationType)
}
...
func (p *ChannelInboundProcessor) persistPassiveMessage(
	...
	msg gateway.InboundMessage,
	text string,
	attachments []agentdomain.Attachment,
	...
	modelMsg := agentdomain.ModelMessage{Role: "user", Content: agentdomain.NewTextContent(headerifiedText)}
	...
	if _, err := p.message.Persist(ctx, messagepkg.PersistInput{
		...
		SenderDisplayName:       strings.TrimSpace(ident.DisplayName),
		SenderAvatarURL:         strings.TrimSpace(ident.AvatarURL),
...
func buildChannelMessage(output agentdomain.AssistantOutput, capabilities gateway.ChannelCapabilities) gateway.Message {
	msg := gateway.Message{}
	if strings.TrimSpace(output.Content) != "" {
		msg.Text = strings.TrimSpace(output.Content)
		if gateway.ContainsMarkdown(msg.Text) && (capabilities.Markdown || capabilities.RichText) {
			msg.Format = gateway.MessageFormatMarkdown
		}
	}
	...
	if capabilities.RichText && hasNonTrivialContentPart(output.Parts) {
		parts := make([]gateway.MessagePart, 0, len(output.Parts))
		for _, part := range output.Parts {
			if !contentPartHasValue(part) {
				continue
			}
			partType := normalizeContentPartType(part.Type)
			parts = append(parts, gateway.MessagePart{
				Type:              partType,
				Text:              part.Text,
				URL:               part.URL,
		...
		if len(parts) > 0 {
			msg.Text = ""
			msg.Parts = parts
			msg.Format = gateway.MessageFormatRich
		}
		return msg
	}
	...
	if len(textParts) > 0 {
		...
		if msg.Format == "" && gateway.ContainsMarkdown(msg.Text) && (capabilities.Markdown || capabilities.RichText) {
			msg.Format = gateway.MessageFormatMarkdown
		}
	}
	return msg
...
func hasNonTrivialContentPart(parts []agentdomain.ContentPart) bool {
	for _, part := range parts {
		if !contentPartHasValue(part) {
			continue
		}
		if normalizeContentPartType(part.Type) != gateway.MessagePartText {
			return true
		}
...
func contentPartHasValue(part agentdomain.ContentPart) bool {
...
func contentPartText(part agentdomain.ContentPart) string {
	if strings.TrimSpace(part.Text) != "" {
		return part.Text
	}
	if strings.TrimSpace(part.URL) != "" {
...
func mapStreamChunkToChannelEvents(chunk json.RawMessage) ([]gateway.StreamEvent, []agentdomain.ModelMessage, error) {
	if len(chunk) == 0 {
		return nil, nil, nil
	}
	...
	finalMessages := make([]agentdomain.ModelMessage, 0, len(envelope.Messages))
		...
		return []gateway.StreamEvent{
			{
				Type:  gateway.StreamEventDelta,
				Delta: envelope.Delta,
				Phase: gateway.StreamPhaseReasoning,
			},
		}, finalMessages, nil
	...
	case "reasoning_start":
		return []gateway.StreamEvent{
			{Type: gateway.StreamEventPhaseStart, Phase: gateway.StreamPhaseReasoning},
		}, finalMessages, nil
	case "reasoning_end":
		return []gateway.StreamEvent{
			{Type: gateway.StreamEventPhaseEnd, Phase: gateway.StreamPhaseReasoning},
		}, finalMessages, nil
	case "text_start":
		return []gateway.StreamEvent{
			{Type: gateway.StreamEventPhaseStart, Phase: gateway.StreamPhaseText},
		}, finalMessages, nil
	case "text_end":
		return []gateway.StreamEvent{
			{Type: gateway.StreamEventPhaseEnd, Phase: gateway.StreamPhaseText},
		}, finalMessages, nil
	case "attachment_delta":
		...
		return []gateway.StreamEvent{
			{Type: gateway.StreamEventAttachment, Attachments: attachments},
		}, finalMessages, nil
	case "reaction_delta":
		...
		return []gateway.StreamEvent{
			{Type: gateway.StreamEventReaction, Reactions: reactions},
		}, finalMessages, nil
	case "speech_delta":
		...
		return []gateway.StreamEvent{
			{Type: gateway.StreamEventSpeech, Speeches: speeches},
		}, finalMessages, nil
	case "agent_start":
		return []gateway.StreamEvent{
			{
				Type: gateway.StreamEventAgentStart,
				Metadata: map[string]any{
					"input": parseRawJSON(envelope.Input),
	...
	case "agent_end":
		return []gateway.StreamEvent{
			{
				Type: gateway.StreamEventAgentEnd,
				Metadata: map[string]any{
					"result": parseRawJSON(envelope.Result),
					"data":   parseRawJSON(envelope.Data),
				},
	...
	case "processing_started":
		return []gateway.StreamEvent{
			{Type: gateway.StreamEventProcessingStarted},
		}, finalMessages, nil
	case "processing_completed":
		return []gateway.StreamEvent{
			{Type: gateway.StreamEventProcessingCompleted},
		}, finalMessages, nil
	case "processing_failed":
		...
		return []gateway.StreamEvent{
			{
				Type:  gateway.StreamEventProcessingFailed,
				Error: streamError,
			},
		}, finalMessages, nil
	case "error":
...
func userInputActions(userInputID string) []gateway.Action {
	userInputID = strings.TrimSpace(userInputID)
	if userInputID == "" {
		return nil
	}
	return []gateway.Action{{Type: "user_input", Label: "Reply", Value: "respond:" + userInputID}}
}

func normalizeContentPartType(raw string) gateway.MessagePartType {
	switch strings.TrimSpace(strings.ToLower(raw)) {
	case "link":
		return gateway.MessagePartLink
	case "code_block":
		return gateway.MessagePartCodeBlock
	case "mention":
		return gateway.MessagePartMention
	case "emoji":
		return gateway.MessagePartEmoji
	default:
		return gateway.MessagePartText
	}
}

func normalizeContentPartStyles(styles []string) []gateway.MessageTextStyle {
	if len(styles) == 0 {
		return nil
	}
	result := make([]gateway.MessageTextStyle, 0, len(styles))
	for _, style := range styles {
		switch strings.TrimSpace(strings.ToLower(style)) {
		case "bold":
			result = append(result, gateway.MessageStyleBold)
		case "italic":
			result = append(result, gateway.MessageStyleItalic)
		case "strikethrough", "lineThrough":
			result = append(result, gateway.MessageStyleStrikethrough)
		case "code":
			result = append(result, gateway.MessageStyleCode)
		default:
			continue
		}
	}
...
func collectMessageToolContext(registry *gateway.Registry, messages []agentdomain.ModelMessage, channelType gateway.ChannelType, replyTarget string) ([]string, bool) {
	if len(messages) == 0 {
		return nil, false
	}
...
func shouldSuppressForToolCall(registry *gateway.Registry, args sendMessageToolArgs, channelType gateway.ChannelType, replyTarget string) bool {
...
func normalizeReplyTarget(registry *gateway.Registry, channelType gateway.ChannelType, target string) string {
	if registry == nil {
...
func (p *ChannelInboundProcessor) requireIdentity(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.InboundMessage) (IdentityState, error) {
	if p.identity == nil {
...
func (p *ChannelInboundProcessor) resolveProcessingStatusNotifier(channelType gateway.ChannelType) gateway.ProcessingStatusNotifier {
...
func (*ChannelInboundProcessor) notifyProcessingStarted(
	ctx context.Context,
	notifier gateway.ProcessingStatusNotifier,
	cfg gateway.ChannelConfig,
	msg gateway.InboundMessage,
	info gateway.ProcessingStatusInfo,
) (gateway.ProcessingStatusHandle, error) {
	if notifier == nil {
...
func (*ChannelInboundProcessor) notifyProcessingCompleted(
	...
	msg gateway.InboundMessage,
	info gateway.ProcessingStatusInfo,
	handle gateway.ProcessingStatusHandle,
) error {
	if notifier == nil {
		return nil
	}
...
func (p *ChannelInboundProcessor) ingestInboundAttachments(
	...
	result := make([]gateway.Attachment, 0, len(attachments))
	for _, att := range attachments {
		item := att
		...
		item = gateway.AttachmentFromBundle(gateway.BundleFromAttachment(item).WithAssetAccess(
			botID,
			asset,
...
func (p *ChannelInboundProcessor) transcribeInboundAttachments(ctx context.Context, botID string, attachments []gateway.Attachment) string {
	...
	for _, att := range attachments {
		if att.Type != gateway.AttachmentAudio && att.Type != gateway.AttachmentVoice {
			continue
		}
...
func containsVoiceAttachment(attachments []gateway.Attachment) bool {
	for _, att := range attachments {
		if att.Type == gateway.AttachmentAudio || att.Type == gateway.AttachmentVoice {
			return true
		}
...
func formatVoiceTranscriptionUnavailableNotice(attachments []gateway.Attachment) string {
	paths := make([]string, 0, len(attachments))
	for _, att := range attachments {
		if att.Type != gateway.AttachmentAudio && att.Type != gateway.AttachmentVoice {
			continue
...
func (p *ChannelInboundProcessor) resolveAttachmentResolver(channelType gateway.ChannelType) gateway.AttachmentResolver {
	if p == nil || p.registry == nil {
		return nil
	}
...
func (p *ChannelInboundProcessor) ingestOutboundAttachments(ctx context.Context, botID string, channelType gateway.ChannelType, attachments []gateway.Attachment) []gateway.Attachment {
	if len(attachments) == 0 || p.mediaService == nil || strings.TrimSpace(botID) == "" {
		return attachments
	}
	prepared, err := gateway.PrepareStreamEvent(ctx, p.mediaService, gateway.ChannelConfig{
		BotID:       botID,
		ChannelType: channelType,
	}, gateway.StreamEvent{
		Type:        gateway.StreamEventAttachment,
		Attachments: attachments,
	})
	if err != nil {
		if p.logger != nil {
...
func isDataURL(raw string) bool {
	return gateway.IsDataURL(raw)
}

func isHTTPURL(raw string) bool {
	return gateway.IsHTTPURL(raw)
}
...
func extractStorageKey(accessPath string, _ string) string {
	return media.StorageKeyFromAccessPath(runtimedomain.DefaultDataMount, accessPath)
}
...
func isLocalChannelType(ct gateway.ChannelType) bool {
...
func (p *ChannelInboundProcessor) broadcastInboundMessage(
	...
	text string,
	identity InboundIdentity,
	resolvedAttachments []gateway.Attachment,
) {
	if p.observer == nil || strings.TrimSpace(botID) == "" {
		return
	}
	inboundMsg := gateway.Message{
		Text:        text,
	...
	p.observer.OnStreamEvent(ctx, botID, msg.Channel, gateway.StreamEvent{
		Type: gateway.StreamEventFinal,
		Final: &gateway.StreamFinalizePayload{
			Message: inboundMsg,
		},
...
func channelAttachmentsToAssetRefs(attachments []gateway.Attachment, role string) []messagepkg.AssetRef {
	if len(attachments) == 0 {
		return nil
	}
...
func mapChannelToChatAttachments(attachments []gateway.Attachment) []agentdomain.Attachment {
	if len(attachments) == 0 {
		return nil
	}
	result := make([]agentdomain.Attachment, 0, len(attachments))
	for _, att := range attachments {
		if att.Type == gateway.AttachmentAudio || att.Type == gateway.AttachmentVoice {
			continue
		}
		bundle := gateway.BundleFromAttachment(att)
		ca := turnAttachmentFromBundle(bundle)
		switch {
...
func parseAttachmentDelta(raw json.RawMessage) []gateway.Attachment {
	if len(raw) == 0 {
		return nil
	}
	...
	attachments := make([]gateway.Attachment, 0, len(items))
	for _, item := range items {
		bundle := attachment.BundleFromMap(item)
		attachments = append(attachments, gateway.AttachmentFromBundle(bundle))
	}
	return attachments
}
...
type assetTracker struct {
	mu   sync.Mutex
	refs []agentdomain.OutboundAssetRef
	run  agentdomain.RunHandle
}

func (t *assetTracker) add(attachments []gateway.Attachment) {
	t.mu.Lock()
...
func (p *ChannelInboundProcessor) synthesizeAndPushVoice(
	...
	speeches []gateway.SpeechRequest,
	stream gateway.OutboundStream,
	assets *assetTracker,
) {
	...
	for _, speech := range speeches {
		...
		voiceEvent := gateway.StreamEvent{
			Type: gateway.StreamEventAttachment,
			Attachments: []gateway.Attachment{
				{
...
func parseSpeechDelta(raw json.RawMessage) []gateway.SpeechRequest {
	if len(raw) == 0 {
		return nil
	}
	var items []struct {
	...
	speeches := make([]gateway.SpeechRequest, 0, len(items))
	for _, item := range items {
		text := strings.TrimSpace(item.Text)
		if text == "" {
			continue
		}
		speeches = append(speeches, gateway.SpeechRequest{Text: text})
	}
	return speeches
}

func buildAssetRefs(attachments []gateway.Attachment, startOrdinal int) []agentdomain.OutboundAssetRef {
	var refs []agentdomain.OutboundAssetRef
	for _, att := range attachments {
		contentHash := strings.TrimSpace(att.ContentHash)
		if contentHash == "" {
			continue
		}
		ref := agentdomain.OutboundAssetRef{
			ContentHash: contentHash,
			Role:        "attachment",
...
func parseReactionDelta(raw json.RawMessage) []gateway.ReactRequest {
	if len(raw) == 0 {
		return nil
	}
	var items []struct {
	...
	reactions := make([]gateway.ReactRequest, 0, len(items))
	for _, item := range items {
		emoji := strings.TrimSpace(item.Emoji)
		if emoji == "" {
			continue
		}
		reactions = append(reactions, gateway.ReactRequest{
			Emoji: emoji,
		})
	}
	return reactions
...
func (p *ChannelInboundProcessor) dispatchReactions(
	...
	reactions []gateway.ReactRequest,
) {
	if p.reactor == nil {
		return
	}
...
func buildRouteMetadata(msg gateway.InboundMessage, identity InboundIdentity) map[string]any {
...
func (p *ChannelInboundProcessor) enrichConversationAvatar(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.InboundMessage, meta map[string]any) {
	...
	defer cancel()
	entry, err := directoryAdapter.ResolveEntry(lookupCtx, cfg, convID, gateway.DirectoryEntryGroup)
	if err != nil {
		if p.logger != nil {
...
func (p *ChannelInboundProcessor) handleStartCommand(
	...
	out := applyMessageFormat(gateway.Message{Text: formatStartWelcomeMessage(loc, botUsername)}, caps)
	if mid := strings.TrimSpace(msg.Message.ID); mid != "" {
		out.Reply = &gateway.ReplyRef{MessageID: mid}
	}
	return sender.Send(ctx, gateway.OutboundMessage{Target: target, Message: out})
}

func (p *ChannelInboundProcessor) handleToolApprovalCommand(ctx context.Context, msg gateway.InboundMessage, sender gateway.StreamReplySender, identity InboundIdentity, routeID, sessionID string, invocation command.Invocation) error {
	loc := p.localizer(ctx, identity.BotID)
	caps := p.channelCaps(msg.Channel)
	if p.turnSvc == nil {
		return sender.Send(ctx, gateway.OutboundMessage{
			Target:  strings.TrimSpace(msg.ReplyTarget),
			Message: applyMessageFormat(gateway.Message{Text: loc.T("cmd.toolApproval.unavailable")}, caps),
		})
	}
	...
	return p.streamToolApprovalCommand(ctx, msg, sender, identity, routeID, approvalRunner, agentdomain.ToolApprovalResponse{
		BotID:                  strings.TrimSpace(identity.BotID),
...
func (p *ChannelInboundProcessor) handleUserInputResponseCommand(ctx context.Context, msg gateway.InboundMessage, sender gateway.StreamReplySender, identity InboundIdentity, routeID, sessionID string, invocation command.Invocation) error {
	loc := p.localizer(ctx, identity.BotID)
	caps := p.channelCaps(msg.Channel)
	if p.turnSvc == nil {
		return sender.Send(ctx, gateway.OutboundMessage{
			Target:  strings.TrimSpace(msg.ReplyTarget),
			Message: applyMessageFormat(gateway.Message{Text: loc.T("cmd.userInput.unavailable")}, caps),
		})
	}
	...
	if err != nil {
		return sender.Send(ctx, gateway.OutboundMessage{
			Target:  strings.TrimSpace(msg.ReplyTarget),
			Message: applyMessageFormat(gateway.Message{Text: loc.T("cmd.userInput.parseFailed")}, caps),
		})
	}
	...
	return p.streamUserInputResponseCommand(ctx, msg, sender, identity, routeID, userInputRunner, agentdomain.UserInputResponse{
		BotID:                  strings.TrimSpace(identity.BotID),
		ThreadID:               strings.TrimSpace(sessionID),
		ActorChannelIdentityID: strings.TrimSpace(identity.ChannelIdentityID),
		ActorUserID:            strings.TrimSpace(identity.UserID),
...
func userInputAnswersFromMetadata(meta map[string]any) []agentdomain.QuestionAnswer {
	if len(meta) == 0 {
		return nil
	}
	...
	out := make([]agentdomain.QuestionAnswer, 0, len(entries))
	for _, entry := range entries {
		...
		answer := agentdomain.QuestionAnswer{QuestionID: qID}
...
func (p *ChannelInboundProcessor) streamToolApprovalCommand(ctx context.Context, msg gateway.InboundMessage, sender gateway.StreamReplySender, identity InboundIdentity, routeID string, approvalRunner ToolApprovalRunner, input agentdomain.ToolApprovalResponse) error {
...
func (p *ChannelInboundProcessor) streamUserInputResponseCommand(ctx context.Context, msg gateway.InboundMessage, sender gateway.StreamReplySender, identity InboundIdentity, routeID string, userInputRunner UserInputRunner, input agentdomain.UserInputResponse) error {
	return p.streamContinuationCommand(ctx, msg, sender, identity, routeID, func(runCtx context.Context, eventCh chan<- json.RawMessage) error {
...
func (p *ChannelInboundProcessor) streamContinuationCommand(ctx context.Context, msg gateway.InboundMessage, sender gateway.StreamReplySender, identity InboundIdentity, routeID string, run streamContinuationFunc) error {
	target := strings.TrimSpace(msg.ReplyTarget)
	if target == "" {
	...
	if err := stream.Push(ctx, gateway.StreamEvent{Type: gateway.StreamEventStatus, Status: gateway.StreamStatusStarted}); err != nil {
		return err
	}
	...
	var finalMessages []agentdomain.ModelMessage
	for eventCh != nil || errCh != nil {
		...
		case chunk, ok := <-eventCh:
			...
			for _, event := range events {
				...
				if event.Type == gateway.StreamEventToolCallStart &&
		...
		case runErr, ok := <-errCh:
			...
			if runErr != nil {
				_ = stream.Push(ctx, gateway.StreamEvent{Type: gateway.StreamEventError, Error: runErr.Error()})
				return runErr
			}
		}
	}
	...
	if !suppressReplies {
		outputs := agentdomain.ExtractAssistantOutputs(finalMessages)
		for _, output := range outputs {
			outMessage := buildChannelMessage(output, gateway.ChannelCapabilities{Text: true, Markdown: true, Reply: true})
			if outMessage.IsEmpty() {
				continue
			}
			plainText := strings.TrimSpace(outMessage.PlainText())
			...
			if outMessage.Reply == nil && sourceMessageID != "" {
				outMessage.Reply = &gateway.ReplyRef{Target: target, MessageID: sourceMessageID}
			}
			if err := stream.Push(ctx, gateway.StreamEvent{
				Type:  gateway.StreamEventFinal,
				Final: &gateway.StreamFinalizePayload{Message: outMessage},
			}); err != nil {
				return err
			}
		}
	}
	if err := stream.Push(ctx, gateway.StreamEvent{Type: gateway.StreamEventStatus, Status: gateway.StreamStatusCompleted}); err != nil {
		return err
	}
	return closeStream()
}

func hasUserInputAction(actions []gateway.Action) bool {
...
func (p *ChannelInboundProcessor) issueChatToken(identity InboundIdentity, routeID string, msg gateway.InboundMessage) string {
	if p.jwtSecret == "" || strings.TrimSpace(msg.ReplyTarget) == "" {
		return ""
	}
	signed, _, err := auth.GenerateChatToken(auth.ChatToken{
...
func resolveNewSessionSpecParsed(parsed command.ParsedCommand, msg gateway.InboundMessage, profiles agentdomain.ACPProfileResolver) (NewSessionSpec, error) {
	...
	case "":
		...
		switch {
		case isLocalChannelType(msg.Channel), gateway.IsPrivateConversationType(msg.Conversation.Type):
			mode = sessionpkg.TypeChat
		default:
			mode = sessionpkg.TypeDiscuss
		}
...
func (p *ChannelInboundProcessor) handleNewSessionCommand(
	...
	out := applyMessageFormat(gateway.Message{Text: text}, p.channelCaps(msg.Channel))
...
func newSessionDisplayModeLabel(loc *i18n.Localizer, spec NewSessionSpec, profiles agentdomain.ACPProfileResolver) string {
...
func (p *ChannelInboundProcessor) defaultSessionSpecForInbound(ctx context.Context, identity InboundIdentity, msg gateway.InboundMessage) (NewSessionSpec, bool, error) {
	mode := sessionpkg.TypeChat
...
func (p *ChannelInboundProcessor) applyDefaultChatRuntimeToNewSessionSpec(ctx context.Context, identity InboundIdentity, msg gateway.InboundMessage, spec NewSessionSpec) (NewSessionSpec, error) {
	if spec.Runtime == sessionpkg.RuntimeACPAgent {
...
func (p *ChannelInboundProcessor) requireWorkspaceExecForACPPrincipal(ctx context.Context, botID, accountUserID string) error {
	...
	allowed, err := p.permissionChecker.HasBotPermission(ctx, strings.TrimSpace(botID), accountUserID, bot.PermissionWorkspaceExec)
	if err != nil {
		return err
	}
	if !allowed {
...
func isGroupConversation(msg gateway.InboundMessage) bool {
	return !isLocalChannelType(msg.Channel) && !gateway.IsPrivateConversationType(msg.Conversation.Type)
}
...
func (p *ChannelInboundProcessor) sendACPFeedbackError(ctx context.Context, sender gateway.StreamReplySender, msg gateway.InboundMessage, identity InboundIdentity, err error) error {
...
func currentContextForNewSessionSpec(cc command.CurrentContext, spec NewSessionSpec, profiles agentdomain.ACPProfileResolver) command.CurrentContext {
	if spec.Runtime != sessionpkg.RuntimeACPAgent {
		return cc
	}
...
func newSessionACPRuntimeLabel(spec NewSessionSpec, profiles agentdomain.ACPProfileResolver) string {
	agentID := acpNewSessionAgentID(spec)
	if agentID == "" {
		return ""
	}
...
func resolveACPProfile(profiles agentdomain.ACPProfileResolver, agentID string) agentdomain.ACPAgentProfile {
	agentID = normalizeACPAgentID(agentID)
	if profiles == nil {
		return agentdomain.ACPAgentProfile{ID: agentID}
	}
...
func (*ChannelInboundProcessor) sendNewConfirmation(
	...
	out := applyMessageFormat(gateway.Message{Text: text}, caps)
	out.Actions = []gateway.Action{
	...
	return sender.Send(ctx, gateway.OutboundMessage{Target: strings.TrimSpace(msg.ReplyTarget), Message: out})
}
...
func (p *ChannelInboundProcessor) handleStatusCommand(
	...
	statusOut := applyMessageFormat(gateway.Message{Text: reply}, p.channelCaps(msg.Channel))
	if mid := strings.TrimSpace(msg.Message.ID); mid != "" {
		statusOut.Reply = &gateway.ReplyRef{MessageID: mid}
	}
	return sender.Send(ctx, gateway.OutboundMessage{
		Target:  target,
		Message: statusOut,

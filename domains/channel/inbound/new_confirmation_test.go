...
func (testACPProfiles) ResolveACPProfile(agentID string) agentdomain.ACPAgentProfile {
...
func (testACPProfiles) ResolveACPSetupPreflight(agentID string, metadata map[string]any) agentdomain.ACPSetupPreflight {
	...
	result := agentdomain.ACPSetupPreflight{Enabled: enabled}
	...
	if value, _ := managed[requiredID].(string); strings.TrimSpace(value) == "" {
		result.MissingManagedField = &agentdomain.ACPManagedField{ID: requiredID, Label: requiredLabel}
	}
	return result
}

func resolveNewSessionTypeForTest(t *testing.T, text string, msg gateway.InboundMessage) (string, error) {
	t.Helper()
...
func TestResolveNewSessionType_BareConfirmFlag(t *testing.T) {
	msg := gateway.InboundMessage{Channel: gateway.ChannelTypeTelegram}
...
func TestResolveNewSessionSpec_ACPAgent(t *testing.T) {
	dm := gateway.InboundMessage{Channel: gateway.ChannelTypeTelegram, Conversation: gateway.Conversation{Type: "private"}}
	group := gateway.InboundMessage{Channel: gateway.ChannelTypeTelegram, Conversation: gateway.Conversation{Type: "group"}}
...
func TestResolveNewSessionSpecCanonicalBotMentionIsNotAnAgent(t *testing.T) {
	t.Parallel()
	group := gateway.InboundMessage{Channel: gateway.ChannelTypeTelegram, Conversation: gateway.Conversation{Type: gateway.ConversationTypeGroup}}
...
func TestChannelSlashAliasesExcludeSenderAndReplyTarget(t *testing.T) {
	t.Parallel()
	aliases := channelSlashAliases(gateway.InboundMessage{
		BotID:       "bot-1",
		ReplyTarget: "group-1",
...
func TestClassifySlackAppMentionCommandUsesAdapterBotAlias(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		BotID:   "bot-1",
		Channel: gateway.ChannelTypeSlack,
...
func TestClassifyMatrixLocalpartMentionUsesAdapterAliases(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		BotID:   "bot-1",
		Channel: gateway.ChannelTypeMatrix,
		Conversation: gateway.Conversation{
			ID:   "!room:example.com",
			Type: gateway.ConversationTypeGroup,
		},
...
func TestHandleInboundNewCommandIgnoresCurrentBotMentionArguments(t *testing.T) {
	for _, text := range []string{"/new @memoh1bot", "/new discuss @memoh1bot", "/new discuss@memoh1bot"} {
		t.Run(text, func(t *testing.T) {
			channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channel-identity-1"}}
			...
			msg := gateway.InboundMessage{
				...
				Message:     gateway.Message{ID: "msg-1", Text: text},
				ReplyTarget: "group-1",
				Sender:      gateway.Identity{SubjectID: "user-1", DisplayName: "Alice"},
				Conversation: gateway.Conversation{
					ID:   "group-1",
					Type: gateway.ConversationTypeGroup,
				},
			...
			cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-1", BotID: "bot-1", ChannelType: gateway.ChannelTypeTelegram}
...
func TestHandleNewSessionCommandCreatesACPChatSpec(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		Channel:     gateway.ChannelTypeTelegram,
		Message:     gateway.Message{ID: "msg-1", Text: "/new chat codex"},
	...
	err := p.handleNewSessionCommand(context.Background(), gateway.ChannelConfig{TeamID: "team-test"}, msg, sender, InboundIdentity{
		BotID:             "bot-1",
...
func TestHandleNewSessionCommandCreatesNativeSessionWithCreator(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		Channel:     gateway.ChannelTypeTelegram,
		Message:     gateway.Message{ID: "msg-1", Text: "/new chat"},
		ReplyTarget: "target-1",
		Conversation: gateway.Conversation{
			ID:   "dm-1",
			Type: gateway.ConversationTypePrivate,
...
func TestHandleNewSessionCommandCancelsActiveStream(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		Channel:     gateway.ChannelTypeTelegram,
		Message:     gateway.Message{ID: "msg-1", Text: "/new --confirm"},
		ReplyTarget: "target-1",
		Conversation: gateway.Conversation{
			ID:   "dm-1",
			Type: gateway.ConversationTypePrivate,
...
func TestHandleNewSessionCommandBareNewInheritsDefaultACP(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		Channel:     gateway.ChannelTypeTelegram,
		Message:     gateway.Message{ID: "msg-1", Text: "/new"},
		ReplyTarget: "target-1",
		Conversation: gateway.Conversation{
			ID:   "dm-1",
			Type: gateway.ConversationTypePrivate,
...
func TestHandleNewSessionCommandExplicitACPInheritsDefaultProject(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		Channel:     gateway.ChannelTypeTelegram,
		Message:     gateway.Message{ID: "msg-1", Text: "/new codex"},
		ReplyTarget: "target-1",
		Conversation: gateway.Conversation{
			ID:   "dm-1",
			Type: gateway.ConversationTypePrivate,
...
func TestHandleNewSessionCommandPreflightsACPSetup(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		Channel:     gateway.ChannelTypeTelegram,
		Message:     gateway.Message{ID: "msg-1", Text: "/new claude-code"},
		ReplyTarget: "target-1",
		Conversation: gateway.Conversation{
			ID:   "dm-1",
			Type: gateway.ConversationTypePrivate,
...
func TestSendACPFeedbackErrorUsesI18nKey(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		Channel:     gateway.ChannelTypeTelegram,
		Message:     gateway.Message{ID: "msg-1"},
		ReplyTarget: "target-1",
	}
	...
	for _, tc := range cases {
		t.Run(tc.locale, func(t *testing.T) {
			...
			err := p.sendNewConfirmation(
				context.Background(),
				gateway.InboundMessage{ReplyTarget: "test-target"},
				s,
				i18n.New(tc.locale),
				"chat",
				i18n.New(tc.locale).T("newSession.modeChat"),
				gateway.ChannelCapabilities{Buttons: true, Markdown: true, Text: true},
			)
			if err != nil {
			...
			var confirm, cancel gateway.Action

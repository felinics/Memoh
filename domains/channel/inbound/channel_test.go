...
type fakeChatGateway struct {
	...
	err              error
	gotReq           agentdomain.StartTurnCommand
	onChat           func(agentdomain.StartTurnCommand)
	userInputCalls   int
	userInputInput   agentdomain.UserInputResponse
	userInputErr     error
	userInputStarted chan struct{}
	userInputRelease chan struct{}
	advanceCalls     int
	advanceInput     agentdomain.AdvanceTextInput
	advanceResult    agentdomain.AdvanceTextResult
	advanceErr       error
}
...
func (h *fakeTurnRun) Events() <-chan agentdomain.Event                      { return h.events }
func (h *fakeTurnRun) Errs() <-chan error                                    { return h.errs }
func (*fakeTurnRun) Inject(context.Context, agentdomain.InjectMessage) error { return nil }
func (*fakeTurnRun) AddOutboundAssets([]agentdomain.OutboundAssetRef)        {}
func (*fakeTurnRun) Cancel()                                                 {}

func (f *fakeChatGateway) AdvancePlainTextUserInput(_ context.Context, input agentdomain.AdvanceTextInput) (agentdomain.AdvanceTextResult, error) {
	f.advanceCalls++
	f.advanceInput = input
	return f.advanceResult, f.advanceErr
}

type nativeUserInputTestAdapter struct{ typ gateway.ChannelType }

func (a *nativeUserInputTestAdapter) Type() gateway.ChannelType { return a.typ }

func (a *nativeUserInputTestAdapter) Descriptor() gateway.Descriptor {
	return gateway.Descriptor{Type: a.typ, Capabilities: gateway.ChannelCapabilities{Text: true, NativeUserInput: true}}
}
...
func TestRejectReservedSkillMetadataInInboundMessage(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			msg: gateway.InboundMessage{Message: gateway.Message{Metadata: map[string]any{
				"model_requested_skills": []string{"alpha"},
			}}},
		},
		{
			name: "part metadata",
			msg: gateway.InboundMessage{Message: gateway.Message{Parts: []gateway.MessagePart{{
		...
		},
		{
			...
			msg: gateway.InboundMessage{Message: gateway.Message{Attachments: []gateway.Attachment{{
		...
		},
		{
			...
			msg: gateway.InboundMessage{Message: gateway.Message{Reply: &gateway.ReplyRef{Attachments: []gateway.Attachment{{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := gateway.RejectReservedSkillMetadata(tt.msg.Message)
...
func (f *fakeChatGateway) StartTurn(_ context.Context, cmd agentdomain.StartTurnCommand) (agentdomain.RunHandle, error) {
	f.gotReq = cmd
	if f.onChat != nil {
		f.onChat(cmd)
	}
	events := make(chan agentdomain.Event, 1)
...
func (*fakeChatGateway) RespondToolApproval(context.Context, agentdomain.ToolApprovalResponse, chan<- json.RawMessage) error {
	return nil
}

func (f *fakeChatGateway) RespondUserInput(_ context.Context, input agentdomain.UserInputResponse, eventCh chan<- json.RawMessage) error {
	f.userInputCalls++
...
func (s *fakeReplySender) Send(_ context.Context, msg gateway.OutboundMessage) error {
	s.sent = append(s.sent, msg)
	return nil
}

func (s *fakeReplySender) OpenStream(_ context.Context, target string, _ gateway.StreamOptions) (gateway.OutboundStream, error) {
...
func (s *fakeOutboundStream) Push(_ context.Context, event gateway.StreamEvent) error {
	if s == nil || s.sender == nil {
		return nil
	}
	s.sender.events = append(s.sender.events, event)
	if event.Type == gateway.StreamEventFinal && event.Final != nil && !event.Final.Message.IsEmpty() {
		s.sender.sent = append(s.sender.sent, gateway.OutboundMessage{
			Target:  s.target,
...
type fakeProcessingStatusNotifier struct {
	startedHandle gateway.ProcessingStatusHandle
	startedErr    error
	completedErr  error
	failedErr     error
	events        []string
	info          []gateway.ProcessingStatusInfo
	completedSeen gateway.ProcessingStatusHandle
	failedSeen    gateway.ProcessingStatusHandle
	failedCause   error
}

func (n *fakeProcessingStatusNotifier) ProcessingStarted(_ context.Context, _ gateway.ChannelConfig, _ gateway.InboundMessage, info gateway.ProcessingStatusInfo) (gateway.ProcessingStatusHandle, error) {
	n.events = append(n.events, "started")
	n.info = append(n.info, info)
	return n.startedHandle, n.startedErr
}

func (n *fakeProcessingStatusNotifier) ProcessingCompleted(_ context.Context, _ gateway.ChannelConfig, _ gateway.InboundMessage, info gateway.ProcessingStatusInfo, handle gateway.ProcessingStatusHandle) error {
...
func (n *fakeProcessingStatusNotifier) ProcessingFailed(_ context.Context, _ gateway.ChannelConfig, _ gateway.InboundMessage, info gateway.ProcessingStatusInfo, handle gateway.ProcessingStatusHandle, cause error) error {
...
func (*fakeProcessingStatusAdapter) Type() gateway.ChannelType {
	return gateway.ChannelType("feishu")
}

func (*fakeProcessingStatusAdapter) Descriptor() gateway.Descriptor {
	return gateway.Descriptor{
		Type: gateway.ChannelType("feishu"),
...
func (a *fakeProcessingStatusAdapter) ProcessingStarted(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.InboundMessage, info gateway.ProcessingStatusInfo) (gateway.ProcessingStatusHandle, error) {
	return a.notifier.ProcessingStarted(ctx, cfg, msg, info)
}

func (a *fakeProcessingStatusAdapter) ProcessingCompleted(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.InboundMessage, info gateway.ProcessingStatusInfo, handle gateway.ProcessingStatusHandle) error {
	return a.notifier.ProcessingCompleted(ctx, cfg, msg, info, handle)
}

func (a *fakeProcessingStatusAdapter) ProcessingFailed(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.InboundMessage, info gateway.ProcessingStatusInfo, handle gateway.ProcessingStatusHandle, cause error) error {
...
type fakeCommandQueries struct {
	messageCount int64
	usage        int64
	cacheRow     command.CacheStats
	skills       []string

	gotCountSession string
}

func (*fakeCommandQueries) GetLatestSessionIDByBot(_ context.Context, _ string) (string, error) {
	return "", errors.New("unexpected latest session lookup")
}

func (f *fakeCommandQueries) CountMessagesBySession(_ context.Context, sessionID string) (int64, error) {
	f.gotCountSession = sessionID
	return f.messageCount, nil
}

func (f *fakeCommandQueries) GetLatestAssistantUsage(_ context.Context, _ string) (int64, error) {
	return f.usage, nil
}

func (f *fakeCommandQueries) GetSessionCacheStats(_ context.Context, _ string) (command.CacheStats, error) {
	return f.cacheRow, nil
}

func (f *fakeCommandQueries) GetSessionUsedSkills(_ context.Context, _ string) ([]string, error) {
	return f.skills, nil
}

func (*fakeCommandQueries) GetTokenUsageByDayAndType(_ context.Context, _ string, _, _ time.Time) ([]command.UsageByDay, error) {
	return nil, nil
}

func (*fakeCommandQueries) GetTokenUsageByModel(_ context.Context, _ string, _, _ time.Time) ([]command.UsageByModel, error) {
	return nil, nil
}
...
type fakeAttachmentResolverAdapter struct {
	typ     gateway.ChannelType
	payload gateway.AttachmentPayload
}

func (f *fakeAttachmentResolverAdapter) Type() gateway.ChannelType {
	if f != nil && strings.TrimSpace(f.typ.String()) != "" {
		return f.typ
	}
	return gateway.ChannelType("resolver-test")
}

func (f *fakeAttachmentResolverAdapter) Descriptor() gateway.Descriptor {
	return gateway.Descriptor{
		Type:        f.Type(),
...
func (f *fakeAttachmentResolverAdapter) ResolveAttachment(_ context.Context, _ gateway.ChannelConfig, _ gateway.Attachment) (gateway.AttachmentPayload, error) {
...
func TestPersistPassiveMessageCopiesSenderSnapshot(t *testing.T) {
	chatSvc := &fakeChatService{}
	processor := &ChannelInboundProcessor{logger: slog.Default(), message: chatSvc}
	processor.persistPassiveMessage(
		...
		gateway.InboundMessage{
			...
			Message: gateway.Message{ID: "message-1"},
		},
		"hello",
		nil,
		"route-1",
	...
	if len(chatSvc.persistedIn) != 1 {
		t.Fatalf("persisted messages = %d, want 1", len(chatSvc.persistedIn))
	}
	got := chatSvc.persistedIn[0]
	if got.SenderDisplayName != "Alice" || got.SenderAvatarURL != "https://example.com/alice.png" {
		t.Fatalf("sender snapshot = %q/%q", got.SenderDisplayName, got.SenderAvatarURL)
	}
}
...
func TestChannelInboundProcessorWithIdentity(t *testing.T) {
	...
	reactionGW := &fakeChatGateway{
		resp: fakeChatResponse{
			Messages: []agentdomain.ModelMessage{
				{Role: "assistant", Content: agentdomain.NewTextContent("AI reply")},
			},
		},
	}
	processor := NewChannelInboundProcessor(slog.Default(), nil, chatSvc, chatSvc, reactionGW, channelIdentitySvc, policySvc, "", 0)
	sender := &fakeReplySender{}

	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-1", BotID: "bot-1", ChannelType: gateway.ChannelType("feishu")}
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("feishu"),
		Message:     gateway.Message{Text: "hello"},
		ReplyTarget: "target-id",
		Sender:      gateway.Identity{SubjectID: "ext-1", DisplayName: "User1"},
	...
	if reactionGW.gotReq.Query != "hello" {
		t.Errorf("expected query 'hello', got: %s", reactionGW.gotReq.Query)
	}
	if reactionGW.gotReq.UserID != "" {
		t.Errorf("expected empty user_id, got: %s", reactionGW.gotReq.UserID)
	}
	if reactionGW.gotReq.SourceChannelIdentityID != "channelIdentity-1" {
		t.Errorf("expected source_channel_identity_id 'channelIdentity-1', got: %s", reactionGW.gotReq.SourceChannelIdentityID)
	}
	if reactionGW.gotReq.ChatID != "bot-1" {
		t.Errorf("expected bot-scoped chat id 'bot-1', got: %s", reactionGW.gotReq.ChatID)
	}
...
func TestTurnIdempotencyKeyScopesExternalMessageIDByRoute(t *testing.T) {
	first := turnIdempotencyKey(gateway.ChannelType("telegram"), "route-1", "42")
	retry := turnIdempotencyKey(gateway.ChannelType("telegram"), "route-1", "42")
	otherChat := turnIdempotencyKey(gateway.ChannelType("telegram"), "route-2", "42")
	otherPlatform := turnIdempotencyKey(gateway.ChannelType("discord"), "route-1", "42")
	...
	if got := turnIdempotencyKey(gateway.ChannelType("telegram"), "route-1", "  "); got != "" {
...
func TestChannelInboundProcessorRespondCommandRoutesUserInput(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{
		channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-1"},
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("feishu"),
		Message:     gateway.Message{ID: "msg-1", Text: "/respond input-1 Plan B"},
		ReplyTarget: "target-id",
		Sender:      gateway.Identity{SubjectID: "ext-1", DisplayName: "User1"},
		Conversation: gateway.Conversation{
			ID:   "chat-1",
	...
	if reactionGW.userInputCalls != 1 {
		t.Fatalf("RespondUserInput calls = %d, want 1", reactionGW.userInputCalls)
	}
	got := reactionGW.userInputInput
	...
	if reactionGW.gotReq.BotID != "" {
		t.Fatalf("ordinary chat should not run, got request %#v", reactionGW.gotReq)
	}
}
...
func TestChannelInboundProcessorPlainTextUserInputShowsOnlyNextQuestion(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-1"}}
	chatSvc := &fakeChatService{resolveResult: route.ResolveConversationResult{BotID: "chat-1", RouteID: "route-1"}}
	reactionGW := &fakeChatGateway{advanceResult: agentdomain.AdvanceTextResult{
		Handled: true,
		Request: agentdomain.AdvanceTextRequest{
			ID: "input-1",
			UIPayload: agentdomain.AdvanceTextUIPayload{Questions: []agentdomain.AdvanceTextUIQuestion{
				{ID: "q1", Text: "Previous question", Kind: userinput.QuestionKindText},
				{ID: "q2", Text: "Choose topics", Kind: userinput.QuestionKindMultiSelect, Options: []agentdomain.AdvanceTextUIOption{
					{ID: "q2.o1", Label: "Go"}, {ID: "q2.o2", Label: "Rust"},
				}},
			}},
			Interaction: agentdomain.AdvanceTextInteraction{
				QuestionIndex: 1,
				Answers:       []agentdomain.QuestionAnswer{{QuestionID: "q1", Text: "hidden answer"}},
			},
		},
	}}
	processor := NewChannelInboundProcessor(slog.Default(), nil, chatSvc, chatSvc, reactionGW, channelIdentitySvc, &fakePolicyService{}, "", 0)
	processor.SetACLService(&fakeChatACL{allowed: true})
	processor.SetSessionEnsurer(&fakeSessionEnsurer{activeSession: SessionResult{ID: "session-1"}})
	sender := &fakeReplySender{}
	msg := gateway.InboundMessage{
		BotID: "bot-1", Channel: gateway.ChannelType("weixin"), ReplyTarget: "target-id",
		Message:      gateway.Message{ID: "msg-2", Text: "first answer"},
		Sender:       gateway.Identity{SubjectID: "ext-1"},
		Conversation: gateway.Conversation{ID: "chat-1", Type: gateway.ConversationTypePrivate},
	}
	if err := processor.HandleInbound(context.Background(), gateway.ChannelConfig{TeamID: "team-test", BotID: "bot-1", ChannelType: msg.Channel}, msg, sender); err != nil {
		t.Fatalf("HandleInbound() error = %v", err)
	}
	if reactionGW.advanceCalls != 1 || reactionGW.userInputCalls != 0 || reactionGW.gotReq.BotID != "" {
		t.Fatalf("unexpected routing: advance=%d respond=%d chat=%#v", reactionGW.advanceCalls, reactionGW.userInputCalls, reactionGW.gotReq)
	}
...
func TestChannelInboundProcessorPlainTextUserInputCompletesWithFullSummary(t *testing.T) {
	...
	request := agentdomain.AdvanceTextRequest{
		ID: "input-1",
		UIPayload: agentdomain.AdvanceTextUIPayload{Questions: []agentdomain.AdvanceTextUIQuestion{
			{ID: "q1", Text: "Plan?", Kind: userinput.QuestionKindSingleSelect, Options: []agentdomain.AdvanceTextUIOption{{ID: "q1.o1", Label: "Alpha"}, {ID: "q1.o2", Label: "Beta"}}},
			{ID: "q2", Text: "Notes?", Kind: userinput.QuestionKindText},
		}},
		Interaction: agentdomain.AdvanceTextInteraction{QuestionIndex: 1, Completed: true, Answers: []agentdomain.QuestionAnswer{
			{QuestionID: "q1", OptionIDs: []string{"q1.o2"}}, {QuestionID: "q2", Text: "Ship today"},
		}},
	}
	reactionGW := &fakeChatGateway{advanceResult: agentdomain.AdvanceTextResult{Handled: true, Request: request}}
	processor := NewChannelInboundProcessor(slog.Default(), nil, chatSvc, chatSvc, reactionGW, channelIdentitySvc, &fakePolicyService{}, "", 0)
	processor.SetACLService(&fakeChatACL{allowed: true})
	processor.SetSessionEnsurer(&fakeSessionEnsurer{activeSession: SessionResult{ID: "session-1"}})
	sender := &fakeReplySender{}
	msg := gateway.InboundMessage{
		BotID: "bot-1", Channel: gateway.ChannelType("weixin"), ReplyTarget: "target-id",
		Message: gateway.Message{ID: "msg-2", Text: "Ship today"}, Sender: gateway.Identity{SubjectID: "ext-1"},
		Conversation: gateway.Conversation{ID: "chat-1", Type: gateway.ConversationTypePrivate},
	}
	if err := processor.HandleInbound(context.Background(), gateway.ChannelConfig{TeamID: "team-test", BotID: "bot-1", ChannelType: msg.Channel}, msg, sender); err != nil {
		t.Fatalf("HandleInbound() error = %v", err)
	}
	if reactionGW.userInputCalls != 1 || reactionGW.userInputInput.ExplicitID != "input-1" || len(reactionGW.userInputInput.Answers) != 2 {
		t.Fatalf("response routing = calls:%d input:%#v", reactionGW.userInputCalls, reactionGW.userInputInput)
	}
...
func TestChannelInboundProcessorNativeUserInputBypassesTextFallback(t *testing.T) {
	...
	registry.MustRegister(&nativeUserInputTestAdapter{typ: gateway.ChannelType("native-test")})
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-1"}}
	chatSvc := &fakeChatService{resolveResult: route.ResolveConversationResult{BotID: "chat-1", RouteID: "route-1"}}
	reactionGW := &fakeChatGateway{resp: fakeChatResponse{Messages: []agentdomain.ModelMessage{{Role: "assistant", Content: agentdomain.NewTextContent("normal reply")}}}}
	processor := NewChannelInboundProcessor(slog.Default(), registry, chatSvc, chatSvc, reactionGW, channelIdentitySvc, &fakePolicyService{}, "", 0)
	processor.SetACLService(&fakeChatACL{allowed: true})
	processor.SetSessionEnsurer(&fakeSessionEnsurer{activeSession: SessionResult{ID: "session-1"}})
	sender := &fakeReplySender{}
	msg := gateway.InboundMessage{
		BotID: "bot-1", Channel: gateway.ChannelType("native-test"), ReplyTarget: "target-id",
		Message: gateway.Message{Text: "normal chat"}, Sender: gateway.Identity{SubjectID: "ext-1"},
		Conversation: gateway.Conversation{ID: "chat-1", Type: gateway.ConversationTypePrivate},
	}
	if err := processor.HandleInbound(context.Background(), gateway.ChannelConfig{TeamID: "team-test", BotID: "bot-1", ChannelType: msg.Channel}, msg, sender); err != nil {
		t.Fatalf("HandleInbound() error = %v", err)
	}
	if reactionGW.advanceCalls != 0 || reactionGW.gotReq.Query != "normal chat" {
		t.Fatalf("native channel routing: advance=%d query=%q", reactionGW.advanceCalls, reactionGW.gotReq.Query)
	}
}
...
func TestChannelInboundProcessorModeCommandBypassesTextFallback(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		BotID: "bot-1", Channel: gateway.ChannelType("weixin"), ReplyTarget: "target-id",
		Message: gateway.Message{Text: "/btw side question"}, Sender: gateway.Identity{SubjectID: "ext-1"},
		Conversation: gateway.Conversation{ID: "chat-1", Type: gateway.ConversationTypePrivate},
	}
	if err := processor.HandleInbound(context.Background(), gateway.ChannelConfig{TeamID: "team-test", BotID: "bot-1", ChannelType: msg.Channel}, msg, &fakeReplySender{}); err != nil {
		t.Fatalf("HandleInbound() error = %v", err)
	}
	if reactionGW.advanceCalls != 0 {
		t.Fatalf("mode command advanced user input %d times", reactionGW.advanceCalls)
	}
}
...
func TestChannelInboundProcessorPlainTextUserInputIgnoresUndirectedGroupMessage(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-1"}}
	chatSvc := &fakeChatService{resolveResult: route.ResolveConversationResult{BotID: "chat-1", RouteID: "route-1"}}
	reactionGW := &fakeChatGateway{advanceResult: agentdomain.AdvanceTextResult{Handled: true}}
	processor := NewChannelInboundProcessor(slog.Default(), nil, chatSvc, chatSvc, reactionGW, channelIdentitySvc, &fakePolicyService{}, "", 0)
	processor.SetACLService(&fakeChatACL{allowed: true})
	processor.SetSessionEnsurer(&fakeSessionEnsurer{activeSession: SessionResult{ID: "session-1"}})
	msg := gateway.InboundMessage{
		BotID: "bot-1", Channel: gateway.ChannelType("wecom"), ReplyTarget: "group-id",
		Message: gateway.Message{Text: "1"}, Sender: gateway.Identity{SubjectID: "ext-1"},
		Conversation: gateway.Conversation{ID: "group-1", Type: gateway.ConversationTypeGroup},
	}
	if err := processor.HandleInbound(context.Background(), gateway.ChannelConfig{TeamID: "team-test", BotID: "bot-1", ChannelType: msg.Channel}, msg, &fakeReplySender{}); err != nil {
		t.Fatalf("HandleInbound() error = %v", err)
	}
	if reactionGW.advanceCalls != 0 {
		t.Fatalf("undirected group message advanced user input %d times", reactionGW.advanceCalls)
	}
}
...
func TestChannelInboundProcessorPlainTextUserInputHandlesDirectedGroupMessage(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		BotID: "bot-1", Channel: gateway.ChannelType("wecom"), ReplyTarget: "group-id",
		Message:      gateway.Message{Text: "1"},
		Sender:       gateway.Identity{SubjectID: "ext-1"},
		Conversation: gateway.Conversation{ID: "group-1", Type: gateway.ConversationTypeGroup},
	...
	if reactionGW.advanceCalls != 1 {
		t.Fatalf("directed group message advanced user input %d times, want 1", reactionGW.advanceCalls)
	}
}
...
func TestChannelInboundProcessorRespondReplyUsesReplyTargetAndPreservesAnswer(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("feishu"),
		Message:     gateway.Message{ID: "msg-1", Text: "/respond Plan B", Reply: &gateway.ReplyRef{MessageID: "ask-msg-1"}},
		ReplyTarget: "target-id",
		Sender:      gateway.Identity{SubjectID: "ext-1", DisplayName: "User1"},
		Conversation: gateway.Conversation{
			ID:   "chat-1",
...
func TestChannelInboundProcessorRespondCallbackUsesBoundRequest(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		...
		Message:     gateway.Message{ID: "ask-msg-1", Text: "/respond q1.o2", Reply: &gateway.ReplyRef{MessageID: "ask-msg-1"}},
		ReplyTarget: "target-id",
		Sender:      gateway.Identity{SubjectID: "ext-1", DisplayName: "User1"},
		Conversation: gateway.Conversation{
			ID:   "chat-1",
	...
	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-1", BotID: "bot-1", ChannelType: gateway.ChannelType("telegram")}
...
func TestChannelInboundProcessorRespondStructuredAnswersFromWizard(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("telegram"),
		Message:     gateway.Message{ID: "ask-msg-1", Text: "/respond", Reply: &gateway.ReplyRef{MessageID: "ask-msg-1"}},
		ReplyTarget: "target-id",
		Sender:      gateway.Identity{SubjectID: "ext-1", DisplayName: "User1"},
		Conversation: gateway.Conversation{
			ID:   "chat-1",
...
func TestChannelInboundProcessorAutoCreatesDefaultACPSession(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{
		channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-acp"},
	...
	reactionGW := &fakeChatGateway{
		resp: fakeChatResponse{
			Messages: []agentdomain.ModelMessage{
				{Role: "assistant", Content: agentdomain.NewTextContent("ACP reply")},
			},
		},
	}
	...
	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-1", BotID: "bot-1", ChannelType: gateway.ChannelType("web")}
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("web"),
		Message:     gateway.Message{Text: "hello"},
		ReplyTarget: "target-id",
		Sender:      gateway.Identity{SubjectID: "ext-1", DisplayName: "User1"},
		Conversation: gateway.Conversation{
	...
	if reactionGW.gotReq.ThreadID != "created-session" {
		t.Fatalf("StreamChat session = %q, want created-session", reactionGW.gotReq.ThreadID)
	}
}

func TestChannelInboundProcessorDefaultACPRequiresWorkspaceExec(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{
		channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-no-exec"},
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("web"),
		Message:     gateway.Message{ID: "msg-1", Text: "hello"},
		ReplyTarget: "target-id",
		Sender:      gateway.Identity{SubjectID: "ext-1", DisplayName: "User1"},
		Conversation: gateway.Conversation{
			ID:   "chat-acp",
	...
	if reactionGW.gotReq.Query != "" {
		t.Fatalf("chat should not run, got query %q", reactionGW.gotReq.Query)
	}
...
func TestChannelInboundProcessorActiveACPRequiresRuntimeOwner(t *testing.T) {
	...
	if reactionGW.gotReq.Query != "" {
		t.Fatalf("chat should not run without runtime owner, got query %q", reactionGW.gotReq.Query)
	}
...
func TestChannelInboundProcessorActiveACPRequiresCurrentActorOwnerOrManage(t *testing.T) {
	...
	t.Run("workspace exec non-owner cannot drive runtime", func(t *testing.T) {
		channelIdentitySvc := &fakeChannelIdentityService{
			channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-other"},
		...
		processor.SetBotPermissionChecker(&fakeBotPermissionChecker{values: map[string]bool{
			"bot-1:" + ownerID + ":" + bot.PermissionWorkspaceExec: true,
		...
		if reactionGW.gotReq.Query != "" {
			t.Fatalf("chat should not run for non-owner actor, got query %q", reactionGW.gotReq.Query)
		}
		if len(sender.sent) != 1 {
	...
	t.Run("manager cannot drive another user's runtime", func(t *testing.T) {
		channelIdentitySvc := &fakeChannelIdentityService{
			channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-manager"},
			linkedUserIDs:   map[string][]string{"channelIdentity-manager": {managerID}},
		}
		chatSvc := &fakeChatService{resolveResult: route.ResolveConversationResult{BotID: "chat-acp", RouteID: "route-acp"}}
		reactionGW := &fakeChatGateway{
			resp: fakeChatResponse{Messages: []agentdomain.ModelMessage{
				{Role: "assistant", Content: agentdomain.NewTextContent("ok")},
			}},
		}
		ensurer := &fakeSessionEnsurer{activeSession: SessionResult{
			ID:                    "active-acp-session",
		...
		processor.SetBotPermissionChecker(&fakeBotPermissionChecker{values: map[string]bool{
			"bot-1:" + ownerID + ":" + bot.PermissionWorkspaceExec: true,
			"bot-1:" + managerID + ":" + bot.PermissionManage:      true,
		}})
		sender := &fakeReplySender{}
		...
		msg := gateway.InboundMessage{
			...
			Message:     gateway.Message{ID: "msg-1", Text: "hello"},
			ReplyTarget: "target-id",
			Sender:      gateway.Identity{SubjectID: "ext-1", DisplayName: "Manager"},
			Conversation: gateway.Conversation{
				ID:   "chat-acp",
				Type: gateway.ConversationTypePrivate,
			},
		...
		if reactionGW.gotReq.Query != "" {
			t.Fatalf("chat should not run for manager on another user's runtime, got query %q", reactionGW.gotReq.Query)
		}
		if len(sender.sent) != 1 {
			t.Fatalf("expected ACP feedback, got %+v", sender.sent)
		}
...
func TestChannelInboundProcessorDiscussACPAllowsNonOwnerMemberThroughACL(t *testing.T) {
	...
	channelIdentitySvc := &fakeChannelIdentityService{
		channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-member"},
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("feishu"),
		Message:     gateway.Message{ID: "msg-1", Text: "@bot hello"},
		ReplyTarget: "group-target",
		Sender:      gateway.Identity{SubjectID: "ext-member", DisplayName: "Member"},
		Conversation: gateway.Conversation{
			ID:   "group-discuss",
...
func TestChannelInboundProcessorACLDeniedManagerMessageDoesNotSuggestLink(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-manager"}}
	policySvc := &fakePolicyService{}
	chatSvc := &fakeChatService{resolveResult: route.ResolveConversationResult{BotID: "chat-denied", RouteID: "route-denied"}}
	reactionGW := &fakeChatGateway{}
	processor := NewChannelInboundProcessor(slog.Default(), nil, chatSvc, chatSvc, reactionGW, channelIdentitySvc, policySvc, "", 0)
	...
	msg := gateway.InboundMessage{
		...
		Message:     gateway.Message{Text: "hello"},
		ReplyTarget: "target-id",
		Sender:      gateway.Identity{SubjectID: "manager-1"},
		Conversation: gateway.Conversation{
			ID:   "chat-1",
			Type: gateway.ConversationTypePrivate,
		},
...
func TestChannelInboundProcessorACLGuestDeniedDowngradesToNotify(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-acl-deny"}}
	policySvc := &fakePolicyService{}
	...
	msg := gateway.InboundMessage{
		...
		Message:     gateway.Message{Text: "hello"},
		ReplyTarget: "target-id",
		Sender:      gateway.Identity{SubjectID: "guest-1"},
		Conversation: gateway.Conversation{
			ID:   "chat-1",
			Type: gateway.ConversationTypePrivate,
		},
	...
	if aclSvc.lastReq.ChannelType != "feishu" ||
		aclSvc.lastReq.SourceScope.ConversationType != gateway.ConversationTypePrivate ||
...
func TestChannelInboundProcessorACLReceivesThreadScope(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			msg: gateway.InboundMessage{
				BotID:       "bot-1",
				Channel:     gateway.ChannelType("discord"),
				Message:     gateway.Message{Text: "hello"},
				ReplyTarget: "discord:thread-1",
				Sender:      gateway.Identity{SubjectID: "guest-thread"},
				Conversation: gateway.Conversation{
					ID:       "guild-chat-1",
					Type:     gateway.ConversationTypeThread,
					ThreadID: "thread-1",
				},
				Metadata: map[string]any{
					"is_mentioned": true,
		...
		},
		{
			...
			msg: gateway.InboundMessage{
				BotID:       "bot-1",
				Channel:     gateway.ChannelType("discord"),
				Message:     gateway.Message{Text: "hello", Thread: &gateway.ThreadRef{ID: "thread-1"}},
				ReplyTarget: "discord:thread-1",
				Sender:      gateway.Identity{SubjectID: "guest-thread"},
				Conversation: gateway.Conversation{
					ID:   "guild-chat-1",
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-thread-scope"}}
			policySvc := &fakePolicyService{}
			...
			if err := processor.HandleInbound(context.Background(), gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-1", BotID: "bot-1"}, tc.msg, sender); err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
			if aclSvc.calls != 1 {
				t.Fatalf("expected acl to be checked once, got %d", aclSvc.calls)
			...
			if aclSvc.lastReq.ChannelType != "discord" ||
				aclSvc.lastReq.SourceScope.ConversationType != gateway.ConversationTypeThread ||
...
func TestChannelInboundProcessorQQAndWeixinWriteCommandsNeedLinkedManager(t *testing.T) {
	for _, channelType := range []gateway.ChannelType{gateway.ChannelType("qq"), gateway.ChannelType("weixin")} {
		t.Run(channelType.String(), func(t *testing.T) {
			channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-im-command"}}
			policySvc := &fakePolicyService{}
			...
			msg := gateway.InboundMessage{
				BotID:       "bot-1",
				Channel:     channelType,
				Message:     gateway.Message{Text: "/model set"},
				ReplyTarget: "target-id",
				Sender:      gateway.Identity{SubjectID: "im-user-1"},
				Conversation: gateway.Conversation{
					ID:   "im-user-1",
					Type: gateway.ConversationTypePrivate,
				},
			...
			if err := processor.HandleInbound(context.Background(), gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-1", BotID: "bot-1", ChannelType: channelType}, msg, sender); err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
			if reactionGW.gotReq.Query != "" {
				t.Fatalf("slash command should not trigger chat call, got query %q", reactionGW.gotReq.Query)
			}
			if len(sender.sent) != 1 {
...
func TestChannelInboundProcessorRejectsDirectSkillBeforeAutoDiscussSession(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-skill-use-group"}}
	policySvc := &fakePolicyService{}
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("telegram"),
		Message:     gateway.Message{ID: "msg-1", Text: "@bot /alpha hello"},
		ReplyTarget: "telegram:group-1",
		Sender:      gateway.Identity{SubjectID: "user-1"},
		Conversation: gateway.Conversation{
			ID:   "group-1",
			Type: gateway.ConversationTypeGroup,
		},
	...
	if err := processor.HandleInbound(context.Background(), gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-1", BotID: "bot-1", ChannelType: gateway.ChannelType("telegram")}, msg, sender); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if reactionGW.gotReq.Query != "" {
		t.Fatalf("skill slash should not trigger chat call, got query %q", reactionGW.gotReq.Query)
	}
...
func TestChannelInboundProcessorRejectsDirectSkillBeforeActiveStreamInjection(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-skill-use-active"}}
	policySvc := &fakePolicyService{}
...
func TestChannelInboundProcessorRejectsDirectSkillDuringContinuationStream(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{
		channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-skill-use-continuation"},
	...
	sender := &fakeReplySender{}
	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-1", BotID: "bot-1", ChannelType: gateway.ChannelType("telegram")}
	baseMsg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("telegram"),
		ReplyTarget: "telegram:group-1",
		Sender:      gateway.Identity{SubjectID: "user-1"},
	...
	go func() {
		msg := baseMsg
		msg.Message = gateway.Message{ID: "msg-respond", Text: "/respond input-1 Plan B"}
		done <- processor.HandleInbound(context.Background(), cfg, msg, sender)
	}()

	<-reactionGW.userInputStarted
	msg := baseMsg
	msg.Message = gateway.Message{ID: "msg-skill", Text: "/alpha hello"}
	if err := processor.HandleInbound(context.Background(), cfg, msg, sender); err != nil {
		t.Fatalf("skill slash HandleInbound() error = %v", err)
	}
	close(reactionGW.userInputRelease)
	if err := <-done; err != nil {
	...
	if reactionGW.gotReq.BotID != "" {
		t.Fatalf("ordinary chat should not run during active continuation, got request %#v", reactionGW.gotReq)
	}
...
func TestChannelInboundProcessorDirectSkillStartsStreamWithDispatcherInjectCh(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-skill-use-dispatch"}}
	policySvc := &fakePolicyService{}
	...
	if reactionGW.gotReq.Query != "hello" {
		t.Fatalf("StreamChat query = %q, want hello", reactionGW.gotReq.Query)
	}
	if reactionGW.gotReq.UserMessageKind != agentdomain.UserMessageKindSkillActivation {
		t.Fatalf("UserMessageKind = %q, want skill_activation", reactionGW.gotReq.UserMessageKind)
	}
	if reactionGW.gotReq.SkillActivation == nil || len(reactionGW.gotReq.SkillActivation.Skills) != 1 || reactionGW.gotReq.SkillActivation.Skills[0].Name != "alpha" {
		t.Fatalf("SkillActivation = %#v, want alpha", reactionGW.gotReq.SkillActivation)
	}
	if len(reactionGW.gotReq.RequestedSkills) != 1 || reactionGW.gotReq.RequestedSkills[0].Name != "alpha" || reactionGW.gotReq.RequestedSkills[0].Content != "alpha skill content" {
		t.Fatalf("StreamChat requested skills = %#v, want resolved alpha", reactionGW.gotReq.RequestedSkills)
	}
	if len(sender.sent) != 0 {
...
func TestChannelInboundProcessorDirectSkillAllowsEmptyPrompt(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-skill-empty-prompt"}}
	policySvc := &fakePolicyService{}
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("telegram"),
		Message:     gateway.Message{ID: "msg-1", Text: "/alpha"},
	...
	if reactionGW.gotReq.Query != "" {
		t.Fatalf("StreamChat query = %q, want empty visible prompt", reactionGW.gotReq.Query)
	}
	if !strings.Contains(reactionGW.gotReq.ModelQuery, "activated the following skill") {
		t.Fatalf("StreamChat model query = %q, want activation marker", reactionGW.gotReq.ModelQuery)
	}
	if !reactionGW.gotReq.SkipMemoryExtraction || !reactionGW.gotReq.SkipTitleGeneration {
		t.Fatalf("empty-prompt activation should skip memory/title, got memory=%v title=%v", reactionGW.gotReq.SkipMemoryExtraction, reactionGW.gotReq.SkipTitleGeneration)
	}
	if len(sender.sent) != 0 {
		t.Fatalf("skill slash stream should not send slash error reply, got %+v", sender.sent)
	}
...
func TestChannelInboundProcessorDirectSkillRejectsReplyWithUnknownAttachmentState(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-skill-reply-unknown"}}
	policySvc := &fakePolicyService{}
	...
	msg := gateway.InboundMessage{
		...
		Message: gateway.Message{
			ID:    "msg-1",
			Text:  "/alpha",
			Reply: &gateway.ReplyRef{MessageID: "source-msg"},
		},
		ReplyTarget: "telegram:dm-1",
		Sender:      gateway.Identity{SubjectID: "user-1"},
		Conversation: gateway.Conversation{
	...
	if reactionGW.gotReq.BotID != "" {
		t.Fatalf("StreamChat should not run for slash reply with unknown attachment state, got %#v", reactionGW.gotReq)
	}
...
func TestChannelInboundProcessorDirectSkillAllowsReplyKnownWithoutAttachments(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-skill-reply-known"}}
	policySvc := &fakePolicyService{}
	...
	msg := gateway.InboundMessage{
		...
		Message: gateway.Message{
			ID:    "msg-1",
			Text:  "/alpha",
			Reply: &gateway.ReplyRef{MessageID: "source-msg", AttachmentsKnown: true},
		},
		ReplyTarget: "telegram:dm-1",
		Sender:      gateway.Identity{SubjectID: "user-1"},
		Conversation: gateway.Conversation{
	...
	if reactionGW.gotReq.SourceReplyToMessageID != "source-msg" {
		t.Fatalf("SourceReplyToMessageID = %q, want source-msg", reactionGW.gotReq.SourceReplyToMessageID)
	}
	if len(sender.sent) != 0 {
		t.Fatalf("skill slash stream should not send slash error reply, got %+v", sender.sent)
	}
...
func TestChannelInboundProcessorDirectSkillRejectsForwardWithUnknownAttachmentState(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-skill-forward-unknown"}}
	policySvc := &fakePolicyService{}
	...
	msg := gateway.InboundMessage{
		BotID:   "bot-1",
		Channel: gateway.ChannelType("misskey"),
		Message: gateway.Message{
			ID:      "msg-1",
			Text:    "/alpha",
			Forward: &gateway.ForwardRef{MessageID: "forward-msg"},
		},
	...
	if err := processor.HandleInbound(context.Background(), gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-1", BotID: "bot-1", ChannelType: gateway.ChannelType("misskey")}, msg, sender); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if skillResolver.calls != 0 {
	...
	if reactionGW.gotReq.BotID != "" {
		t.Fatalf("StreamChat should not run for slash forward with unknown attachment state, got %#v", reactionGW.gotReq)
	}
	if len(sender.sent) != 1 || !strings.Contains(sender.sent[0].Message.PlainText(), "attachment") {
		t.Fatalf("expected attachment slash error reply, got %+v", sender.sent)
	}
...
func TestChannelInboundProcessorIgnoreEmpty(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-3"}}
	policySvc := &fakePolicyService{}
	chatSvc := &fakeChatService{}
	reactionGW := &fakeChatGateway{}
	processor := NewChannelInboundProcessor(slog.Default(), nil, chatSvc, chatSvc, reactionGW, channelIdentitySvc, policySvc, "", 0)
	...
	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-1"}
	msg := gateway.InboundMessage{Message: gateway.Message{Text: "  "}}
...
func TestChannelInboundProcessorStatusUsesRouteSession(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-status"}}
	policySvc := &fakePolicyService{}
	...
	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-1", BotID: "bot-1", ChannelType: gateway.ChannelType("discord")}
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("discord"),
		Message:     gateway.Message{Text: "/status"},
	...
	if got := cmdQueries.gotCountSession; got != "11111111-1111-1111-1111-111111111111" {
...
func TestChannelInboundProcessorDirectedModeCommandPermissionDeniedReplies(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-denied-status"}}
	policySvc := &fakePolicyService{}
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("discord"),
		Message:     gateway.Message{ID: "msg-denied-status", Text: "/status"},
	...
	if reactionGW.gotReq.Query != "" {
		t.Fatalf("denied command should not trigger chat, got query %q", reactionGW.gotReq.Query)
	}
...
func TestChannelInboundProcessorAttachmentOnlyUsesFallbackQuery(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-fallback"}}
	policySvc := &fakePolicyService{}
	...
	msg := gateway.InboundMessage{
		...
		Message: gateway.Message{
			Attachments: []gateway.Attachment{
				{Type: gateway.AttachmentImage, URL: "https://example.com/a.png"},
				{Type: gateway.AttachmentImage, URL: "https://example.com/b.png"},
			},
		},
	...
	if reactionGW.gotReq.Query != "" {
		t.Fatalf("expected empty query for attachment-only message, got %q", reactionGW.gotReq.Query)
	}
	if len(reactionGW.gotReq.Attachments) != 2 {
		t.Fatalf("expected attachments to pass through, got %d", len(reactionGW.gotReq.Attachments))
	}
}

func TestChannelInboundProcessorSilentReply(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-4"}}
	policySvc := &fakePolicyService{}
	chatSvc := &fakeChatService{resolveResult: route.ResolveConversationResult{BotID: "chat-4", RouteID: "route-4"}}
	reactionGW := &fakeChatGateway{
		resp: fakeChatResponse{
			Messages: []agentdomain.ModelMessage{
				{Role: "assistant", Content: agentdomain.NewTextContent("NO_REPLY")},
			},
		},
	}
	processor := NewChannelInboundProcessor(slog.Default(), nil, chatSvc, chatSvc, reactionGW, channelIdentitySvc, policySvc, "", 0)
	...
	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-1", BotID: "bot-1"}
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("telegram"),
		Message:     gateway.Message{Text: "test"},
		ReplyTarget: "chat-123",
		Sender:      gateway.Identity{SubjectID: "user-1"},
		Conversation: gateway.Conversation{
			ID:   "conv-1",
...
func TestBuildChannelMessageKeepsReasoningTextMarkdownOnRichChannels(t *testing.T) {
	msg := buildChannelMessage(agentdomain.AssistantOutput{
		Content: "**bold**",
		Parts: []agentdomain.ContentPart{
			{Type: "text", Text: "**bold**"},
		},
	}, gateway.ChannelCapabilities{Text: true, Markdown: true, RichText: true})
	...
	if msg.Format != gateway.MessageFormatMarkdown {
...
func TestBuildChannelMessagePromotesStyledPartWithoutDuplicateText(t *testing.T) {
	...
	if msg.Format != gateway.MessageFormatRich {
		t.Fatalf("Format = %q, want rich", msg.Format)
	}
	if len(msg.Parts) != 1 {
		t.Fatalf("Parts len = %d, want 1", len(msg.Parts))
	}
	if got := msg.Parts[0]; got.Type != gateway.MessagePartText || got.Text != "bold" || len(got.Styles) != 1 || got.Styles[0] != gateway.MessageStyleBold {
...
func TestChannelInboundProcessorGroupPassiveSync(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("feishu"),
		Message:     gateway.Message{ID: "msg-1", Text: "hello everyone"},
...
func TestChannelInboundProcessorGroupMentionTriggersReply(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-6"}}
	policySvc := &fakePolicyService{}
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("feishu"),
		Message:     gateway.Message{ID: "msg-2", Text: "@bot ping"},
		ReplyTarget: "chat_id:oc_123",
		Sender:      gateway.Identity{SubjectID: "user-1"},
		Conversation: gateway.Conversation{
			ID:   "oc_123",
...
func (*failingOpenStreamSender) Send(_ context.Context, _ gateway.OutboundMessage) error {
	return nil
}

func (s *failingOpenStreamSender) OpenStream(_ context.Context, _ string, _ gateway.StreamOptions) (gateway.OutboundStream, error) {
	if s != nil && s.err != nil {
		return nil, s.err
	}
...
func (*failingCloseSender) Send(_ context.Context, _ gateway.OutboundMessage) error {
	return nil
}

func (s *failingCloseSender) OpenStream(_ context.Context, target string, _ gateway.StreamOptions) (gateway.OutboundStream, error) {
...
func (*failingCloseStream) Push(_ context.Context, _ gateway.StreamEvent) error {
	return nil
}
...
func TestChannelInboundProcessorDoesNotPersistBeforeOpenStream(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-openstream"}}
	policySvc := &fakePolicyService{}
	...
	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-openstream", BotID: "bot-1", ChannelType: gateway.ChannelType("qq")}
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("qq"),
		Message:     gateway.Message{ID: "msg-openstream-1", Text: "hello"},
...
func TestChannelInboundProcessorReturnsCloseStreamError(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-closestream"}}
	policySvc := &fakePolicyService{}
	...
	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-closestream", BotID: "bot-1", ChannelType: gateway.ChannelType("wechatoa")}
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("wechatoa"),
		Message:     gateway.Message{ID: "msg-closestream-1", Text: "hello"},
...
func TestChannelInboundProcessorPersistsAttachmentAssetRefs(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-asset"}}
	policySvc := &fakePolicyService{}
	...
	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-asset", BotID: "bot-1"}
	msg := gateway.InboundMessage{
		BotID:   "bot-1",
		Channel: gateway.ChannelType("feishu"),
		Message: gateway.Message{
		...
		Sender:      gateway.Identity{SubjectID: "ext-asset"},
		Conversation: gateway.Conversation{
			ID:   "oc_asset",
			Type: gateway.ConversationTypePrivate,
		},
	...
	if len(reactionGW.gotReq.Attachments) != 1 {
		t.Fatalf("expected one gateway attachment, got %d", len(reactionGW.gotReq.Attachments))
	}
	if got := reactionGW.gotReq.Attachments[0].ContentHash; got != "asset-1" {
...
func TestChannelInboundProcessorIngestsPlatformKeyWithResolver(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-resolver"}}
	policySvc := &fakePolicyService{}
	...
	processor := NewChannelInboundProcessor(slog.Default(), registry, chatSvc, chatSvc, reactionGW, channelIdentitySvc, policySvc, "", 0)
	mediaSvc := &fakeMediaIngestor{nextID: "asset-resolved-1", nextMime: "application/octet-stream"}
	processor.SetMediaService(mediaSvc)
	sender := &fakeReplySender{}

	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-resolver", BotID: "bot-1", ChannelType: gateway.ChannelType("resolver-test")}
	msg := gateway.InboundMessage{
		BotID:   "bot-1",
		Channel: gateway.ChannelType("resolver-test"),
		Message: gateway.Message{
			ID:   "msg-resolver-1",
		...
		Sender:      gateway.Identity{SubjectID: "resolver-user"},
		Conversation: gateway.Conversation{
			ID:   "resolver-conv",
			Type: gateway.ConversationTypePrivate,
		},
	...
	if got := reactionGW.gotReq.Attachments[0].ContentHash; got != "asset-resolved-1" {
...
func TestChannelInboundProcessorIngestsBase64Attachment(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-base64"}}
	policySvc := &fakePolicyService{}
	...
	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-base64", BotID: "bot-1", ChannelType: gateway.ChannelType("local")}
	msg := gateway.InboundMessage{
		BotID:   "bot-1",
		Channel: gateway.ChannelType("local"),
		Message: gateway.Message{
			ID:   "msg-base64-1",
	...
	gotAttachment := reactionGW.gotReq.Attachments[0]
...
func TestChannelInboundProcessorIngestsQQFileAttachmentKeepsOriginalExtWhenMimeGeneric(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-qq-file"}}
	policySvc := &fakePolicyService{}
	...
	mediaSvc := mediasvc.NewService(slog.Default(), storage)
	processor.SetMediaService(mediaSvc)
	sender := &fakeReplySender{}

	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-qq-file", BotID: "bot-1", ChannelType: gateway.ChannelType("qq")}
	msg := gateway.InboundMessage{
		BotID:   "bot-1",
		Channel: gateway.ChannelType("qq"),
		Message: gateway.Message{
		...
		ReplyTarget: "c2c:user-openid",
		Sender:      gateway.Identity{SubjectID: "qq-user"},
		Conversation: gateway.Conversation{
			ID:   "qq-user",
			Type: gateway.ConversationTypePrivate,
		},
	...
	if len(reactionGW.gotReq.Attachments) != 1 {
		t.Fatalf("expected one attachment in gateway request, got %d", len(reactionGW.gotReq.Attachments))
	}
	storageKey, _ := reactionGW.gotReq.Attachments[0].Metadata["storage_key"].(string)
...
func TestChannelInboundProcessorPipelineUsesResolvedAttachments(t *testing.T) {
	...
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-pipeline-asset"}}
	policySvc := &fakePolicyService{}
	...
	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-pipeline-asset", BotID: "bot-1", ChannelType: gateway.ChannelTypeTelegram}
	msg := gateway.InboundMessage{
		BotID:   "bot-1",
		Channel: gateway.ChannelTypeTelegram,
		Message: gateway.Message{
		...
		ReplyTarget: "12345",
		Sender:      gateway.Identity{SubjectID: "telegram-user"},
		Conversation: gateway.Conversation{
			ID:   "12345",
			Type: gateway.ConversationTypePrivate,
		},
...
func TestChannelInboundProcessorPersonalGroupNonOwnerIgnored(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-member"}}
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("feishu"),
		Message:     gateway.Message{ID: "msg-personal-1", Text: "hello"},
		ReplyTarget: "chat_id:oc_personal",
		Sender:      gateway.Identity{SubjectID: "ext-member-1"},
		Conversation: gateway.Conversation{
			ID:   "oc_personal",
			Type: "group",
		},
...
func TestChannelInboundProcessorPersonalGroupOwnerWithoutMentionUsesPassivePersistence(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("feishu"),
		Message:     gateway.Message{ID: "msg-personal-2", Text: "owner says hi"},
		ReplyTarget: "chat_id:oc_personal",
...
func TestChannelInboundProcessorProcessingStatusSuccessLifecycle(t *testing.T) {
	notifier := &fakeProcessingStatusNotifier{
		startedHandle: gateway.ProcessingStatusHandle{Token: "reaction-1"},
	}
	registry := gateway.NewRegistry()
	...
	reactionGW := &fakeChatGateway{
		...
		onChat: func(_ agentdomain.StartTurnCommand) {
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("feishu"),
		Message:     gateway.Message{ID: "om_123", Text: "hello"},
		ReplyTarget: "chat_id:oc_123",
		Sender:      gateway.Identity{SubjectID: "ext-1"},
		Conversation: gateway.Conversation{
			ID:   "oc_123",
...
func TestChannelInboundProcessorProcessingStatusFailureLifecycle(t *testing.T) {
	notifier := &fakeProcessingStatusNotifier{
		startedHandle: gateway.ProcessingStatusHandle{Token: "reaction-2"},
	}
	registry := gateway.NewRegistry()
	registry.MustRegister(&fakeProcessingStatusAdapter{notifier: notifier})
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-2"}}
	...
	reactionGW := &fakeChatGateway{err: chatErr}
	processor := NewChannelInboundProcessor(slog.Default(), registry, chatSvc, chatSvc, reactionGW, channelIdentitySvc, policySvc, "", 0)
	sender := &fakeReplySender{}
	cfg := gateway.ChannelConfig{TeamID: "team-test", ID: "cfg-1", BotID: "bot-1", ChannelType: gateway.ChannelType("feishu")}
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("feishu"),
		Message:     gateway.Message{ID: "om_456", Text: "hello"},
		ReplyTarget: "chat_id:oc_456",
		Sender:      gateway.Identity{SubjectID: "ext-2"},
		Conversation: gateway.Conversation{
			ID:   "oc_456",
			Type: gateway.ConversationTypePrivate,
		},
...
func TestChannelInboundProcessorProcessingStatusErrorsAreBestEffort(t *testing.T) {
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("feishu"),
		Message:     gateway.Message{ID: "om_789", Text: "hello"},
		ReplyTarget: "chat_id:oc_789",
		Sender:      gateway.Identity{SubjectID: "ext-3"},
		Conversation: gateway.Conversation{
			ID:   "oc_789",
			Type: gateway.ConversationTypePrivate,
		},
...
func TestChannelInboundProcessorProcessingFailedNotifyErrorDoesNotOverrideChatError(t *testing.T) {
	notifier := &fakeProcessingStatusNotifier{
		startedHandle: gateway.ProcessingStatusHandle{Token: "reaction-4"},
	...
	msg := gateway.InboundMessage{
		BotID:       "bot-1",
		Channel:     gateway.ChannelType("feishu"),
		Message:     gateway.Message{ID: "om_999", Text: "hello"},
		ReplyTarget: "chat_id:oc_999",
		Sender:      gateway.Identity{SubjectID: "ext-4"},
		Conversation: gateway.Conversation{
			ID:   "oc_999",
			Type: gateway.ConversationTypePrivate,
		},
...
func TestMapStreamChunkToChannelEvents(t *testing.T) {
	...
	tests := []struct {
		name          string
		chunk         string
		wantType      gateway.StreamEventType
		wantDelta     string
		wantPhase     gateway.StreamPhase
		wantToolName  string
		wantAttCount  int
		wantError     string
		wantNilEvents bool
	}{
		{
			...
			wantType:  gateway.StreamEventDelta,
			wantDelta: "hello",
			wantPhase: gateway.StreamPhaseText,
		},
		{
		...
		},
		{
			...
			wantType:  gateway.StreamEventDelta,
			wantDelta: "thinking",
			wantPhase: gateway.StreamPhaseReasoning,
		},
		{
		...
		},
		{
			...
			wantType:  gateway.StreamEventPhaseStart,
			wantPhase: gateway.StreamPhaseReasoning,
		},
		{
			name:      "reasoning_end",
			chunk:     `{"type":"reasoning_end"}`,
			wantType:  gateway.StreamEventPhaseEnd,
			wantPhase: gateway.StreamPhaseReasoning,
		},
		{
			name:      "text_start",
		...
		},
		{
			...
			wantType:  gateway.StreamEventError,
		...
		},
		{
			...
			wantType: gateway.StreamEventAgentStart,
		},
		{
			name:     "agent_end",
			chunk:    `{"type":"agent_end","result":{"ok":true}}`,
			wantType: gateway.StreamEventAgentEnd,
		},
		{
			name:     "processing_started",
			chunk:    `{"type":"processing_started"}`,
			wantType: gateway.StreamEventProcessingStarted,
		},
		{
			name:     "processing_completed",
			chunk:    `{"type":"processing_completed"}`,
			wantType: gateway.StreamEventProcessingCompleted,
		},
		{
			name:      "processing_failed",
			chunk:     `{"type":"processing_failed","error":"failed"}`,
			wantType:  gateway.StreamEventProcessingFailed,
			wantError: "failed",
		},
		{
			name:          "empty chunk",
...
func TestMapStreamChunkToChannelEvents_UserInputRequest(t *testing.T) {
	...
	tc := events[0].ToolCall
	if events[0].Type != gateway.StreamEventToolCallStart || tc == nil {
...
func TestMapStreamChunkToChannelEvents_UserInputMultiQuestionKeepsActions(t *testing.T) {
	...
	stream := gateway.NewToolCallDroppingStream(sink)
...
func (r *recordingOutboundForUserInput) Push(_ context.Context, event gateway.StreamEvent) error {
...
func TestMapStreamChunkToChannelEvents_FinalMessages(t *testing.T) {
	...
	tests := []struct {
		name      string
		chunk     string
		eventType gateway.StreamEventType
		wantEvent bool
	}{
		{
			name:      "agent_end",
			chunk:     `{"type":"agent_end","messages":[{"role":"assistant","content":"done"}]}`,
			eventType: gateway.StreamEventAgentEnd,
			wantEvent: true,
		},
		{
			name:  "agent_abort",
...
func TestIngestOutboundAttachments_DataURL(t *testing.T) {
	...
	attachments := []gateway.Attachment{
		{Type: gateway.AttachmentImage, URL: "data:image/png;base64,iVBORw0KGgo=", Mime: "image/png"},
	}
	// Without media service, attachments pass through unchanged.
	result := p.ingestOutboundAttachments(context.Background(), "bot-1", gateway.ChannelType("telegram"), attachments)
	if len(result) != 1 {
...
func TestIngestOutboundAttachments_NonDataURL(t *testing.T) {
	...
	p := &ChannelInboundProcessor{}
	attachments := []gateway.Attachment{
		{Type: gateway.AttachmentImage, URL: "https://example.com/img.png"},
		{Type: gateway.AttachmentImage, ContentHash: "existing-asset", URL: "/data/media/img.png"},
	}
	result := p.ingestOutboundAttachments(context.Background(), "bot-1", gateway.ChannelType("telegram"), attachments)
	if len(result) != 2 {
...
func TestChannelAttachmentsToAssetRefs(t *testing.T) {
	...
	attachments := []gateway.Attachment{
		{ContentHash: "a1", Type: gateway.AttachmentImage},
		{Type: gateway.AttachmentFile},
		{ContentHash: "a2", Type: gateway.AttachmentAudio},
	}
...
func TestIngestOutboundAttachments_ContainerPath(t *testing.T) {
	...
	attachments := []gateway.Attachment{
		{Type: gateway.AttachmentImage, Path: "/data/media/26da/26da0cc7.jpg"},
	}
	result := p.ingestOutboundAttachments(context.Background(), "bot-1", gateway.ChannelType("telegram"), attachments)
	if len(result) != 1 {
		t.Fatalf("expected 1 attachment, got %d", len(result))
...
func TestIngestOutboundAttachments_ContainerPathNotFound(t *testing.T) {
	...
	p := &ChannelInboundProcessor{mediaService: ms}
	attachments := []gateway.Attachment{
		{Type: gateway.AttachmentImage, Path: "/data/media/26da/missing.jpg"},
	}
	result := p.ingestOutboundAttachments(context.Background(), "bot-1", gateway.ChannelType("telegram"), attachments)
	if len(result) != 1 {
...
func TestChannelInboundProcessorCommandExecutesWithUnprovenReplyAttachments(t *testing.T) {
	channelIdentitySvc := &fakeChannelIdentityService{channelIdentity: identity.ChannelIdentity{ID: "channelIdentity-cmd-reply"}}
	policySvc := &fakePolicyService{}
...
func (f *tailThenErrGateway) StartTurn(_ context.Context, cmd agentdomain.StartTurnCommand) (agentdomain.RunHandle, error) {
	f.gotReq = cmd
	events := make(chan agentdomain.Event, len(f.deltas))
	errs := make(chan error, 1)
...
// error is surfaced — the buffered agentdomain.Service pair lets the error
...
func TestChannelInboundProcessorDeliversTailEventsBeforeError(t *testing.T) {
	...
	policySvc := &fakePolicyService{}
	chatSvc := &fakeChatService{resolveResult: route.ResolveConversationResult{BotID: "chat-1", RouteID: "route-1"}}
	reactionGW := &tailThenErrGateway{deltas: []string{"partial ", "answer"}}
	processor := NewChannelInboundProcessor(slog.Default(), nil, chatSvc, chatSvc, reactionGW, channelIdentitySvc, policySvc, "", 0)
	sender := &fakeReplySender{}

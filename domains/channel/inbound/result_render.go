...
// command.Result into a gateway.Message: the target channel's capability matrix
...
func telegramGroupBotUsername(msg gateway.InboundMessage) string {
	if msg.Channel != gateway.ChannelTypeTelegram || msg.Conversation.Type != gateway.ConversationTypeGroup {
		return ""
	}
...
// renderResult converts a neutral command.Result into a gateway.Message,
...
func renderResult(result *command.Result, rc RenderContext) gateway.Message {
	if result == nil {
		return gateway.Message{}
	}
	t := rc.localizerFor(result)
	if result.FeedbackError != nil {
		return applyMessageFormat(gateway.Message{Text: renderACPFeedbackText(result.FeedbackError, t)}, rc.Caps)
	}
	var msg gateway.Message
	if result.Interactive == nil || !rc.Caps.Buttons {
		msg = gateway.Message{Text: appendFallbackTrailer(result.Text, result.Interactive, rc.Caps, t)}
	} else {
		...
		default:
			msg = gateway.Message{Text: result.Text}
		}
	}
...
func appendFallbackTrailer(text string, iv *command.Interactive, caps gateway.ChannelCapabilities, t *i18n.Localizer) string {
...
func applyMessageFormat(msg gateway.Message, caps gateway.ChannelCapabilities) gateway.Message {
	if caps.Markdown || caps.RichText {
		msg.Format = gateway.MessageFormatMarkdown
	} else {
		msg.Text = gateway.StripInlineMarkup(msg.Text)
		msg.Format = gateway.MessageFormatPlain
	}
	return msg
}

// plainTextMessage builds a fully-formed gateway.Message from `text`, routing
...
func plainTextMessage(text string, caps gateway.ChannelCapabilities) gateway.Message {
	return applyMessageFormat(gateway.Message{Text: text}, caps)
}
...
// prior behavior.
func renderListView(text string, lv *command.ListView, t *i18n.Localizer) gateway.Message {
	if lv != nil && strings.TrimSpace(lv.ButtonText) != "" {
		text = lv.ButtonText
	}
	msg := gateway.Message{Text: text}
	if lv == nil {
		return msg
	}
	...
	for _, item := range lv.Items {
		...
		actions = append(actions, gateway.Action{
...
func renderModelPicker(fallbackText string, p *command.ModelPickerView, t *i18n.Localizer) gateway.Message {
	if p == nil {
		return gateway.Message{Text: fallbackText}
	}
	pageSize := p.PageSize
	if pageSize <= 0 {
		pageSize = 8
	...
	msg := gateway.Message{Text: modelPickerHeader(p, start, end, t)}
...
func renderChoicesView(fallbackText string, cv *command.ChoicesView, t *i18n.Localizer) gateway.Message {
	if cv == nil {
		return gateway.Message{Text: fallbackText}
	}
	msg := gateway.Message{Text: cv.Title}
	var actions []gateway.Action
...
func renderRangeView(text string, rv *command.RangeView, t *i18n.Localizer) gateway.Message {
	msg := gateway.Message{Text: text}
	if rv == nil {
		return msg
	}

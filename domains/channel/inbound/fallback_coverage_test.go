...
func TestNoButtonFallbackCoverage(t *testing.T) {
	noButton := gateway.ChannelCapabilities{Text: true}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !tc.expectTrailer {
				...
				expected := gateway.StripInlineMarkup(tc.result.Text)
			...
			if !strings.Contains(msg.Text, gateway.StripInlineMarkup(tc.result.Text)) {
...
func TestTelegramPathSkipsTrailer(t *testing.T) {
	withButtons := gateway.ChannelCapabilities{Text: true, Buttons: true, Markdown: true}

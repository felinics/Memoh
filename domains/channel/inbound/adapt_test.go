...
func TestAdaptInbound_FallsBackToText_WhenPartsEmpty(t *testing.T) {
	msg := gateway.InboundMessage{
		...
		Message:    gateway.Message{ID: "m1", Text: "plain hello"},
...
func TestAdaptAttachments_ContentHash(t *testing.T) {
	atts := []gateway.Attachment{
		{Type: gateway.AttachmentImage, ContentHash: "abc123", URL: "/data/media/bot/ab/abc123.jpg", Mime: "image/jpeg"},
		{Type: gateway.AttachmentFile, URL: "https://example.com/doc.pdf", Mime: "application/pdf"},
	}
	...
	msg := gateway.InboundMessage{
		...
		Message: gateway.Message{
			...
			Parts: []gateway.MessagePart{
				...
				{Type: gateway.MessagePartMention, Text: "@bot", ChannelIdentityID: "ci-bot"},
			},
		},
		ReceivedAt: time.Unix(1700000000, 0).UTC(),
	}
...
func TestAdaptInbound_PreservesAddressingMetadata(t *testing.T) {
	msg := gateway.InboundMessage{
		Channel: gateway.ChannelTypeTelegram,
		Message: gateway.Message{ID: "m1", Text: "@akazwz_bot hi"},
...
func TestAdaptInbound_PreservesLink_FromParts(t *testing.T) {
	msg := gateway.InboundMessage{
		Message: gateway.Message{
			ID: "m1",
			Parts: []gateway.MessagePart{
				{Type: gateway.MessagePartLink, Text: "Memoh", URL: "https://example.com"},
			},
		},
	}
...
func TestAdaptInbound_CodeBlockIsLeafWithLanguage(t *testing.T) {
	msg := gateway.InboundMessage{
		Message: gateway.Message{
			ID: "m1",
			Parts: []gateway.MessagePart{
				{Type: gateway.MessagePartCodeBlock, Text: "fn main(){}", Language: "rust"},
			},
		},
	}
	me := AdaptInbound(msg, "sess", "", "").(timeline.MessageEvent)
...
func TestAdaptInbound_InlineCodeStyleIsLeaf(t *testing.T) {
	msg := gateway.InboundMessage{
		Message: gateway.Message{
			ID: "m1",
			Parts: []gateway.MessagePart{
				{Type: gateway.MessagePartText, Text: "x", Styles: []gateway.MessageTextStyle{gateway.MessageStyleCode}},
			},
		},
	}
	me := AdaptInbound(msg, "sess", "", "").(timeline.MessageEvent)
			...
			Parts: []gateway.MessagePart{
				{
					...
					Text:   "shout",
					Styles: []gateway.MessageTextStyle{gateway.MessageStyleBold, gateway.MessageStyleItalic},
				},
			},
		},
	}
...
func TestAdaptInbound_StrikethroughStyleWraps(t *testing.T) {
	msg := gateway.InboundMessage{
		Message: gateway.Message{
			ID: "m1",
			Parts: []gateway.MessagePart{
				{Type: gateway.MessagePartText, Text: "gone", Styles: []gateway.MessageTextStyle{gateway.MessageStyleStrikethrough}},
			},
		},
	}
	me := AdaptInbound(msg, "sess", "", "").(timeline.MessageEvent)
	...
	msg := gateway.InboundMessage{
		Message: gateway.Message{
			...
			Parts: []gateway.MessagePart{
				...
				{Type: gateway.MessagePartText, Text: "\n"},
				{Type: gateway.MessagePartText, Text: "next"},
			},
		},
	}
	me := AdaptInbound(msg, "sess", "", "").(timeline.MessageEvent)
...
func TestAdaptInbound_RoundTripsThroughRenderer(t *testing.T) {
	msg := gateway.InboundMessage{
		...
		Message: gateway.Message{
			...
			Format: gateway.MessageFormatRich,
			Parts: []gateway.MessagePart{
				{Type: gateway.MessagePartText, Text: "hi "},
				{Type: gateway.MessagePartMention, Text: "@bot", ChannelIdentityID: "ci-bot"},
				{Type: gateway.MessagePartText, Text: " see "},
				{Type: gateway.MessagePartLink, Text: "Memoh", URL: "https://example.com"},
			},
		},
		ReceivedAt: time.Unix(1700000000, 0).UTC(),

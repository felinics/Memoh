...
func TestRenderResultTextFallbackWhenNoButtons(t *testing.T) {
	...
	msg := renderResult(res, RenderContext{Caps: gateway.ChannelCapabilities{Buttons: false}, T: i18n.New("en")})
...
func TestRenderListUsesButtonTextWhenButtonsAreAvailable(t *testing.T) {
	...
	msg := renderResult(res, RenderContext{Caps: gateway.ChannelCapabilities{Buttons: true}, T: i18n.New("en")})
...
func TestRenderResultFormatGate(t *testing.T) {
	...
	md := renderResult(res, RenderContext{Caps: gateway.ChannelCapabilities{Markdown: true}, T: i18n.New("en")})
	if md.Format != gateway.MessageFormatMarkdown {
	...
	rich := renderResult(res, RenderContext{Caps: gateway.ChannelCapabilities{RichText: true}, T: i18n.New("en")})
	if rich.Format != gateway.MessageFormatMarkdown {
		t.Errorf("richtext channel: Format = %q, want markdown", rich.Format)
	}
	// Text-only channel: markers stripped, format stays plain.
	plain := renderResult(res, RenderContext{Caps: gateway.ChannelCapabilities{}, T: i18n.New("en")})
	if plain.Format == gateway.MessageFormatMarkdown {
		t.Error("text-only channel should not be markdown")
	}
	if plain.Format != gateway.MessageFormatPlain {
...
func TestRenderResultTextFallbackWhenNoInteractive(t *testing.T) {
	msg := renderResult(&command.Result{Text: "plain"}, RenderContext{Caps: gateway.ChannelCapabilities{Buttons: true}, T: i18n.New("en")})
...
func TestRenderListSinglePageNoButtons(t *testing.T) {
	msg := renderResult(listResult(5, 0, 12), RenderContext{Caps: gateway.ChannelCapabilities{Buttons: true}, T: i18n.New("en")})
	if len(msg.Actions) != 0 {
...
func TestRenderListMultiPageNavButtons(t *testing.T) {
	...
	msg := renderResult(listResult(50, 0, 12), RenderContext{Caps: gateway.ChannelCapabilities{Buttons: true}, T: i18n.New("en")})
	if len(msg.Actions) == 0 {
...
func TestRenderListLastPageHasPrevNotNext(t *testing.T) {
	...
	msg := renderResult(listResult(50, 4, 12), RenderContext{Caps: gateway.ChannelCapabilities{Buttons: true}, T: i18n.New("en")})
...
func TestRenderRangeView(t *testing.T) {
	...
	msg := renderResult(res, RenderContext{Caps: gateway.ChannelCapabilities{Buttons: true, Markdown: true}, T: i18n.New("en")})
...
func TestFormatNewSessionMessage(t *testing.T) {
	...
	plain := gateway.StripInlineMarkup(got)
	if strings.Contains(plain, "**") || strings.Contains(plain, "`") {
		t.Errorf("gateway.StripInlineMarkup left markers: %q", plain)
	}
...
func TestTelegramGroupBotUsername(t *testing.T) {
	group := gateway.InboundMessage{
		Channel:      gateway.ChannelTypeTelegram,
		Conversation: gateway.Conversation{Type: gateway.ConversationTypeGroup},
	...
	group.Conversation.Type = gateway.ConversationTypePrivate
	if got := telegramGroupBotUsername(group); got != "" {
		t.Fatalf("telegramGroupBotUsername(private) = %q, want empty", got)
	}
	group.Conversation.Type = gateway.ConversationTypeGroup
	group.Channel = gateway.ChannelTypeDiscord
	if got := telegramGroupBotUsername(group); got != "" {
...
func TestTelegramCallbackTapRoundTrip(t *testing.T) {
	...
	tgCaps := gateway.ChannelCapabilities{Text: true, Markdown: true, Buttons: true}
	loc := i18n.New("en")

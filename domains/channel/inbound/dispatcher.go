...
func (d *RouteDispatcher) getOrCreate(routeID string) *routeState {
	...
	rs = &routeState{
		injectCh: make(chan agentdomain.InjectMessage, injectChBuffer),
		lastUsed: time.Now(),
	}
	d.routes[routeID] = rs
	return rs
...
func (d *RouteDispatcher) MarkActive(routeID string) <-chan agentdomain.InjectMessage {
	routeID = strings.TrimSpace(routeID)
	if routeID == "" {
		return nil
	}
...
func (d *RouteDispatcher) Inject(routeID string, msg agentdomain.InjectMessage) bool {
	routeID = strings.TrimSpace(routeID)
	if routeID == "" {
		return false
	}
...
func drainInjectCh(ch chan agentdomain.InjectMessage) {
	for {
		select {
		case <-ch:
		default:

...
// AdaptInbound converts a gateway.InboundMessage into a timeline.CanonicalEvent.
...
func AdaptInbound(msg gateway.InboundMessage, sessionID, channelIdentityID, displayName string) timeline.CanonicalEvent {
...
func adaptMessage(msg gateway.InboundMessage, sessionID, channelIdentityID, displayName string) timeline.MessageEvent {
	now := msg.ReceivedAt
	if now.IsZero() {
		now = time.Now()
	}
	...
	convType := gateway.NormalizeConversationType(msg.Conversation.Type)
...
func adaptBody(m gateway.Message) []timeline.ContentNode {
...
func adaptParts(parts []gateway.MessagePart) []timeline.ContentNode {
	if len(parts) == 0 {
		return nil
	}
...
func partToNode(p gateway.MessagePart) (timeline.ContentNode, bool) {
	switch p.Type {
...
func textPartToNode(p gateway.MessagePart) (timeline.ContentNode, bool) {
	if p.Text == "" {
		return timeline.ContentNode{}, false
	}

	hasInlineCode := false
	wrappers := make([]gateway.MessageTextStyle, 0, len(p.Styles))
	for _, s := range p.Styles {
		if s == gateway.MessageStyleCode {
			hasInlineCode = true
			continue
		}
...
func styleToNodeType(s gateway.MessageTextStyle) string {
	switch s {
	case gateway.MessageStyleBold:
		return "bold"
	case gateway.MessageStyleItalic:
		return "italic"
	case gateway.MessageStyleStrikethrough:
...
func adaptAttachments(atts []gateway.Attachment) []timeline.Attachment {
	if len(atts) == 0 {
		return nil
	}
	result := make([]timeline.Attachment, 0, len(atts))
	for _, a := range atts {
		bundle := gateway.BundleFromAttachment(a)
...
func adaptForward(ref *gateway.ForwardRef) *timeline.ForwardInfo {
	if ref == nil {
		return nil
	}
...
func adaptEdit(msg gateway.InboundMessage, sessionID, channelIdentityID, displayName string) timeline.EditEvent {
	now := msg.ReceivedAt
	if now.IsZero() {
		now = time.Now()
	}
...
func adaptService(msg gateway.InboundMessage, sessionID string) timeline.ServiceEvent {
	now := msg.ReceivedAt
	if now.IsZero() {
		now = time.Now()
	}

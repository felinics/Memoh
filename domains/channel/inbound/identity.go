...
type ChannelIdentityService interface {
	ResolveByChannelIdentity(ctx context.Context, channel, channelSubjectID, displayName string, meta map[string]any) (identity.ChannelIdentity, error)
...
func (r *IdentityResolver) Middleware() gateway.Middleware {
	return func(next gateway.InboundHandler) gateway.InboundHandler {
		return func(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.InboundMessage) error {
...
func (r *IdentityResolver) Resolve(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.InboundMessage) (IdentityState, error) {
...
func (r *IdentityResolver) resolveIdentity(ctx context.Context, msg gateway.InboundMessage, primarySubjectID, displayName, avatarURL string) (string, error) {
...
func extractSubjectIdentity(msg gateway.InboundMessage) string {
...
func identitySubjectCandidates(msg gateway.InboundMessage, primary string) []string {
...
func extractDisplayName(msg gateway.InboundMessage) string {
...
func (r *IdentityResolver) configlessUserID(msg gateway.InboundMessage) string {
	if r.registry == nil || !r.registry.IsConfigless(msg.Channel) {
		return ""
	}
...
func (r *IdentityResolver) resolveProfile(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.InboundMessage, subjectID string) (string, string) {
...
func (r *IdentityResolver) resolveProfileFromDirectory(ctx context.Context, cfg gateway.ChannelConfig, msg gateway.InboundMessage, subjectID string) (string, string) {
	if r.registry == nil {
		return "", ""
	}
	...
	defer cancel()
	entry, err := directoryAdapter.ResolveEntry(lookupCtx, cfg, subjectID, gateway.DirectoryEntryUser)
	if err != nil {
		if r.logger != nil {
...
func extractThreadID(msg gateway.InboundMessage) string {

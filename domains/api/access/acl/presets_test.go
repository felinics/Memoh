...
func TestResolvePreset(t *testing.T) {
	tests := []struct {
		...
		{name: "empty falls back to allow all", wantKey: PresetAllowAll, wantEffect: EffectAllow},
		{name: "private only", key: PresetPrivateOnly, wantKey: PresetPrivateOnly, wantEffect: EffectDeny, wantRuleCount: 1, wantFirstType: "private"},
		{name: "group and thread only", key: PresetGroupAndThreadOnly, wantKey: PresetGroupAndThreadOnly, wantEffect: EffectDeny, wantRuleCount: 2, wantFirstType: "group"},
...
func (f *presetTransactionFake) CreateRule(_ context.Context, input RuleWrite) (Rule, error) {
...
func TestApplyPresetUsesOneTransaction(t *testing.T) {
	...
	transactions := 0
	store := &storeFake{transactionFn: func(_ context.Context, fn func(PresetTransaction) error) error {
		transactions++
		return fn(tx)
	}}
	...
	if tx.defaultEffect != EffectDeny {
...
func TestApplyPresetPropagatesWriteError(t *testing.T) {
	...
	store := &storeFake{transactionFn: func(_ context.Context, fn func(PresetTransaction) error) error { return fn(tx) }}
	err := NewService(nil, store, nil, nil).ApplyPreset(t.Context(), "bot-id", "actor-id", PresetGroupAndThreadOnly)
...
func TestApplyPresetRejectsUnknownBeforeTransaction(t *testing.T) {
	transactions := 0
	store := &storeFake{transactionFn: func(context.Context, func(PresetTransaction) error) error { transactions++; return nil }}
...
func TestPresetSourceChannel(t *testing.T) {
	channel, err := presetSourceChannel(" slack ", SourceScope{ConversationType: "group", ConversationID: "C123"})
	if err != nil || channel != "slack" {
		t.Fatalf("presetSourceChannel() = (%q, %v)", channel, err)
	}
	_, err = presetSourceChannel("", SourceScope{ConversationType: "thread", ConversationID: "C123", ThreadID: "T1"})

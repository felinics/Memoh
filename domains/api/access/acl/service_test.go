...
type storeFake struct {
	evaluateFn     func(context.Context, Evaluation) (string, error)
	defaultFn      func(context.Context, string) (string, error)
	setDefaultFn   func(context.Context, string, string) error
	listRulesFn    func(context.Context, string) ([]Rule, error)
	createRuleFn   func(context.Context, RuleWrite) (Rule, error)
	updateRuleFn   func(context.Context, RuleWrite) (Rule, error)
	deleteRuleFn   func(context.Context, string) error
	getManageFn    func(context.Context, string, string) (ManageOverride, bool, error)
	listManageFn   func(context.Context, string) ([]ManageOverride, error)
	upsertManageFn func(context.Context, ManageOverrideWrite) (ManageOverride, error)
	deleteManageFn func(context.Context, string, string) error
	transactionFn  func(context.Context, func(PresetTransaction) error) error
}

type identityReaderFake struct {
	listFn func(context.Context, []string) ([]ChannelIdentity, error)
}

func (f *identityReaderFake) ListChannelIdentities(ctx context.Context, ids []string) ([]ChannelIdentity, error) {
...
type observedReaderFake struct {
	listFn func(context.Context, ObservedConversationQuery) ([]ObservedConversationCandidate, error)
}

func (f *observedReaderFake) ListObservedConversations(ctx context.Context, input ObservedConversationQuery) ([]ObservedConversationCandidate, error) {
	if f == nil || f.listFn == nil {
		return nil, nil
	}
...
func (f *storeFake) EvaluateRule(ctx context.Context, input Evaluation) (string, error) {
	if f.evaluateFn != nil {
...
func (f *storeFake) ListRules(ctx context.Context, botID string) ([]Rule, error) {
...
func (f *storeFake) CreateRule(ctx context.Context, input RuleWrite) (Rule, error) {
...
func (f *storeFake) UpdateRule(ctx context.Context, input RuleWrite) (Rule, error) {
...
func (f *storeFake) GetManageOverride(ctx context.Context, botID, identityID string) (ManageOverride, bool, error) {
	if f.getManageFn != nil {
		return f.getManageFn(ctx, botID, identityID)
	}
	return ManageOverride{}, false, nil
}

func (f *storeFake) ListManageOverrides(ctx context.Context, botID string) ([]ManageOverride, error) {
...
func (f *storeFake) UpsertManageOverride(ctx context.Context, input ManageOverrideWrite) (ManageOverride, error) {
	if f.upsertManageFn != nil {
		return f.upsertManageFn(ctx, input)
	}
	return ManageOverride{}, nil
}
...
func (f *storeFake) RunPresetTransaction(ctx context.Context, fn func(PresetTransaction) error) error {
...
func TestEvaluate(t *testing.T) {
	tests := []struct {
		...
		{name: "allow", effect: EffectAllow, allowed: true},
		{name: "deny", effect: EffectDeny, allowed: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var got Evaluation
			store := &storeFake{evaluateFn: func(_ context.Context, input Evaluation) (string, error) {
				got = input
				return tt.effect, nil
			}}
			allowed, err := NewService(nil, store, nil, nil).Evaluate(t.Context(), EvaluateRequest{
				BotID: " bot-id ", ChannelIdentityID: " identity-id ", ChannelType: " slack ",
				SourceScope: SourceScope{ConversationType: "group", ConversationID: " C123 "},
			})
			if err != nil {
			...
			if got.BotID != "bot-id" || got.Action != ActionChatTrigger || got.ChannelIdentityID != "identity-id" || got.ChannelType != "slack" {
...
func TestEvaluateRejectsInvalidScopeBeforeConfiguration(t *testing.T) {
	_, err := NewService(nil, nil, nil, nil).Evaluate(t.Context(), EvaluateRequest{
		SourceScope: SourceScope{ThreadID: "thread-1"},
	})
...
func TestValidateTarget(t *testing.T) {
	identities := &identityReaderFake{listFn: func(_ context.Context, ids []string) ([]ChannelIdentity, error) {
		if ids[0] == "missing" {
			return nil, nil
		}
		return []ChannelIdentity{{ID: ids[0], Channel: "telegram"}}, nil
	}}
...
func TestCreateRuleBuildsPersistenceCommand(t *testing.T) {
	var got RuleWrite
	store := &storeFake{createRuleFn: func(_ context.Context, input RuleWrite) (Rule, error) {
		got = input
		return Rule{ID: "rule-id", SubjectChannelType: input.SubjectChannelType, SourceScope: &input.SourceScope}, nil
	}}
	service := NewService(nil, store, nil, nil)
	rule, err := service.CreateRule(t.Context(), " bot-id ", " actor-id ", CreateRuleRequest{
		Enabled: true, Description: " group rule ", Effect: EffectDeny, SubjectChannelType: "slack",
		SourceScope: &SourceScope{ConversationType: "group", ConversationID: "C123"},
	})
	if err != nil {
...
func TestCreateRuleResolvesSourceChannelFromIdentity(t *testing.T) {
	...
	identities := &identityReaderFake{
		listFn: func(_ context.Context, ids []string) ([]ChannelIdentity, error) {
			findCalls++
			return []ChannelIdentity{{ID: ids[0], Channel: "telegram"}}, nil
		},
	}
	store := &storeFake{
		createRuleFn: func(_ context.Context, input RuleWrite) (Rule, error) { got = input; return Rule{}, nil },
	}
	_, err := NewService(nil, store, identities, nil).CreateRule(t.Context(), "bot", "actor", CreateRuleRequest{
		Enabled: true, Effect: EffectAllow, ChannelIdentityID: "identity",
		SourceScope: &SourceScope{ConversationType: "group", ConversationID: "group-1"},
	})
	if err != nil {
		t.Fatalf("CreateRule() error = %v", err)
	}
...
func TestRuleCRUDAndDefaultEffectDelegate(t *testing.T) {
	store := &storeFake{
		...
		setDefaultFn: func(_ context.Context, botID, effect string) error {
			if botID != "bot" || effect != EffectAllow {
				t.Fatalf("SetDefaultEffect = (%q, %q)", botID, effect)
			}
			return nil
		},
		listRulesFn: func(_ context.Context, botID string) ([]Rule, error) { return []Rule{{ID: botID + "-rule"}}, nil },
		updateRuleFn: func(_ context.Context, input RuleWrite) (Rule, error) {
			return Rule{ID: input.ID, Effect: input.Effect}, nil
		},
	...
	updated, err := service.UpdateRule(t.Context(), " rule ", UpdateRuleRequest{Enabled: true, Effect: EffectAllow})
...
func TestManageOverrides(t *testing.T) {
	...
	store := &storeFake{
		getManageFn: func(context.Context, string, string) (ManageOverride, bool, error) {
			return ManageOverride{Granted: false}, true, nil
		},
		upsertManageFn: func(_ context.Context, input ManageOverrideWrite) (ManageOverride, error) {
			return ManageOverride{BotID: input.BotID, ChannelIdentityID: input.ChannelIdentityID, Granted: input.Granted, CreatedAt: now}, nil
		},
	}
	identities := &identityReaderFake{listFn: func(context.Context, []string) ([]ChannelIdentity, error) {
		return []ChannelIdentity{{ID: "identity"}}, nil
	}}
...
func TestListManageOverridesEnrichesInOneBatch(t *testing.T) {
	store := &storeFake{listManageFn: func(context.Context, string) ([]ManageOverride, error) {
...
func TestListObservedConversations(t *testing.T) {
	var got ObservedConversationQuery
	observed := &observedReaderFake{listFn: func(_ context.Context, input ObservedConversationQuery) ([]ObservedConversationCandidate, error) {
		got = input
		return []ObservedConversationCandidate{{RouteID: "route", ConversationID: "chat"}}, nil
	}}

...
func (h *ContainerdHandler) FSStat(c echo.Context) error {
	botID, err := h.requireBotAccessWithPermission(c, bot.PermissionWorkspaceRead)
	if err != nil {
		return err
	}
...
func (h *ContainerdHandler) FSDownload(c echo.Context) error {
	...
	requireAccess := func(c echo.Context) (string, error) {
		return h.requireBotAccessWithPermission(c, bot.PermissionWorkspaceRead)
	}
...
func (h *ContainerdHandler) FSWrite(c echo.Context) error {
	botID, err := h.requireBotAccessWithPermission(c, bot.PermissionWorkspaceWrite)
	if err != nil {
		return err
	}
...
func (h *ContainerdHandler) FSUpload(c echo.Context) error {
	...
	// Workspace uploads have no size ceiling, so reading the body must not race
	// the server ReadTimeout.

...
type ContainerdHandler struct {
	...
	displayService   runtimedisplay.Service
...
func NewWorkspaceSetupAppError(setupErr error, requestID string) (CreateContainerErrorEvent, bool) {
	...
	switch {
	case errors.Is(setupErr, runtimedomain.ErrWorkspaceImageIncompatible):
...
func NewContainerdHandler(log *slog.Logger, manager containerWorkspace, cfg config.WorkspaceConfig, containerBackend string, botService *bot.Service, accountService *account.Service, policyService *policy.Service, displayService runtimedisplay.Service) *ContainerdHandler {
...
func (h *ContainerdHandler) CreateContainer(c echo.Context) error {
	...
	send := func(payload any) {
		...
		_ = httpx.WriteSSEData(writer, flusher, string(data))
	}
	...
	if pullErr != nil {
		...
		sendError("workspace_image_prepare_failed", "bot.container.createFailed", "image preparation failed: "+pullErr.Error())
		return nil
	}
	...
	if err := h.manager.StartWithResolvedConfig(ctx, botID, image, gpu); err != nil {
		...
		sendError("workspace_start_failed", "bot.container.createFailed", "workspace failed to start")
		return nil
	}
	if err := h.manager.WaitForWorkspaceReady(ctx, botID); err != nil {
		h.logger.Error("container bridge not ready",
			slog.String("bot_id", botID), slog.Any("error", err))
		sendError("workspace_not_ready", "bot.container.createFailed", "workspace runtime is not ready")
		return nil
	}
	if err := h.manager.InitializeNativeWorkspace(ctx, botID); err != nil {
		...
		if event, ok := NewWorkspaceSetupAppError(err, httpx.RequestID(c)); ok {
			...
			sendError("workspace_setup_failed", "bot.container.createFailed", "workspace initialization failed")
		}
		return nil
	}
	...
	if err != nil {
		h.logger.Error("container ID resolution failed after start",
			slog.String("bot_id", botID), slog.Any("error", err))
		sendError("workspace_runtime_id_failed", "bot.container.createFailed", "workspace runtime ID could not be resolved")
		return nil
	}
	...
	if req.RestoreData && h.manager.HasPreservedData(botID) {
		if err := h.manager.RestorePreservedData(ctx, botID); err != nil {
			h.logger.Error("restore preserved data failed",
				slog.String("bot_id", botID), slog.Any("error", err))
			sendError("workspace_restore_failed", "bot.container.createFailed", "restore preserved data failed: "+err.Error())
			return nil
		}
		dataRestored = true
	}
...
func (h *ContainerdHandler) recordContainerSetupFailure(ctx context.Context, botID, phase string, err error) {
	...
	if recordErr := h.botService.RecordContainerSetupFailure(ctx, botID, phase, err); recordErr != nil {
		h.logger.WarnContext(ctx, "record bot container setup failure failed",
			slog.String("bot_id", botID),
...
func (h *ContainerdHandler) clearContainerSetupFailure(ctx context.Context, botID string) {
	...
	if err := h.botService.ClearContainerSetupFailure(ctx, botID); err != nil {
		h.logger.WarnContext(ctx, "clear bot container setup failure failed",
			slog.String("bot_id", botID),
			slog.Any("error", err),
		)
	}
...
func (h *ContainerdHandler) requireBotAccess(c echo.Context) (string, error) {
	return h.requireBotAccessWithPermission(c, bot.PermissionManage)
}
...
func (h *ContainerdHandler) authorizeBotAccessWithPermission(ctx context.Context, channelIdentityID, botID, permission string) (bot.Bot, error) {
	return httpx.AuthorizeBotAccessWithPermission(ctx, h.botService, h.accountService, channelIdentityID, botID, permission)
}

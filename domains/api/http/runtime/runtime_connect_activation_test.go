...
type runtimeConnectTestStore struct {
	userruntime.CredentialStore

	mu      sync.Mutex
	runtime userruntime.CredentialRecord
}
...
func (s *runtimeConnectTestStore) FindCredentialByAPIToken(context.Context, string) (userruntime.CredentialRecord, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.runtime, nil
}

func (*runtimeConnectTestStore) HasActiveTeamMembership(context.Context, string) (bool, error) {
	return true, nil
}
...
func (*runtimeConnectTestService) Stat(_ context.Context, req *bridgepb.StatRequest) (*bridgepb.StatResponse, error) {
	return &bridgepb.StatResponse{Entry: &bridgepb.FileEntry{Path: req.GetPath(), IsDir: true}}, nil
}
...
func TestRuntimeConnectActivatesReadyConnectionAndUnregistersDisconnect(t *testing.T) {
	...
	service := userruntime.NewService(store, store, hub)
	...
	bridgepb.RegisterContainerServiceServer(grpcServer, &runtimeConnectTestService{})

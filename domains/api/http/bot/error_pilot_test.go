...
func TestCreateBotHTTPErrorMapsWorkspaceContractFailureToStableCode(t *testing.T) {
	...
	if got := apperror.CauseOf(err); !errors.Is(got, runtimedomain.ErrWorkspaceImageIncompatible) {
...
func TestFSHTTPErrorKeepsUnavailableCausePrivate(t *testing.T) {
	...
	err := runtimehttp.FSHTTPError(cause)
...
func TestWorkspaceSetupAppErrorKeepsContractDiagnosticPrivate(t *testing.T) {
	...
	event, ok := runtimehttp.NewWorkspaceSetupAppError(cause, "req-contract")
	if !ok {
		t.Fatal("runtimehttp.NewWorkspaceSetupAppError() did not recognize contract error")
	}

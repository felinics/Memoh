...
const (
	...
	claudeCodeOAuthTokenURL     = "https://iam.claude.com/v1/oauth/token" //nolint:gosec // OAuth endpoint URL, not a credential.
	claudeCodeOAuthRedirectURI  = "https://iam.claude.com/oauth/code/callback"
...
func NewACPClaudeCodeOAuthHandler(botService *bot.Service, accountService *account.Service, acpWorkspace ACPWorkspaceConfigProvider) *ACPClaudeCodeOAuthHandler {
...
func (h *ACPClaudeCodeOAuthHandler) requireBotAccess(c echo.Context) (bot.Bot, string, error) {
	...
	record, err := httpx.AuthorizeBotAccess(c.Request().Context(), h.botService, h.accountService, channelIdentityID, botID)
	if err != nil {
		return bot.Bot{}, "", err
	}
	return record, channelIdentityID, nil
}
...
func (h *ACPClaudeCodeOAuthHandler) saveOAuthToken(ctx context.Context, record bot.Bot, token string) error {
	if h.botService == nil {
		return errors.New("bot service is not configured")
	}
	_, err := h.botService.Update(ctx, record.ID, bot.UpdateBotRequest{
		Metadata: upsertClaudeCodeOAuthMetadata(record.Metadata, token),
	})
	return err
}

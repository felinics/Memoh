...
func NewHooksHandler(log *slog.Logger, botService *bot.Service, accountService *account.Service, service *hooks.Service, agent *engine.Agent, provider bridge.Provider) *HooksHandler {
	if log == nil {
		log = slog.Default()
	}
	return &HooksHandler{
...
func (h *HooksHandler) Test(c echo.Context) error {
	...
	result, err := h.service.RunConfig(ctx, cfg, req, hookTestToolRunner{agent: h.agent, cfg: engine.RunConfig{

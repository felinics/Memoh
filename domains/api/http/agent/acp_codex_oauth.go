...
func NewACPCodexOAuthHandler(provider *providers.Service, botService *bot.Service, accountService *account.Service, acpWorkspace ACPWorkspaceConfigProvider, callbackURL string) *ACPCodexOAuthHandler {
...
<html>
  ...
  <body style="font-family: sans-serif; padding: 24px;">
	...
	return c.HTML(http.StatusOK, httpx.ExecuteHTMLTemplate(page, map[string]string{"BotID": oauthState.BotID}))
}
...
func (h *ACPCodexOAuthHandler) requireBotAccess(c echo.Context) (string, string, error) {
	...
	return record.ID, channelIdentityID, nil
}

...
func TestParseACPClaudeCodeOAuthCode(t *testing.T) {
	...
	code, state = parseACPClaudeCodeOAuthCode("https://iam.claude.com/oauth/code/callback?code=query-code&state=query-state")
	if code != "query-code" || state != "query-state" {
		t.Fatalf("parse callback URL = %q/%q", code, state)
	}
	code, state = parseACPClaudeCodeOAuthCode("https://iam.claude.com/oauth/code/callback#code=fragment-code&state=fragment-state")

...
type acpCodexDeviceHTTPTestEnv struct {
	...
	recorder *httpfixture.ACPConfigBridgeServer
	botID    string
	userID   string
}
...
func newACPCodexDeviceHTTPTestEnv(t *testing.T, provider *acpCodexDeviceIntegrationProvider) *acpCodexDeviceHTTPTestEnv {
	...
	handler := &ACPCodexOAuthHandler{
		provider:       provider,
		botService:     httpfixture.NewBotService(nil, httpfixture.NewBotStore(queries.bot)),
		accountService: httpfixture.NewAdminAccountService("member"),

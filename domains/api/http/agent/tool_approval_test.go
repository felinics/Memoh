...
type recordingToolApprovalResponder struct {
	input agentdomain.ToolApprovalResponse
}

func (r *recordingToolApprovalResponder) RespondToolApproval(_ context.Context, input agentdomain.ToolApprovalResponse, _ chan<- json.RawMessage) error {
	r.input = input
	return nil
}

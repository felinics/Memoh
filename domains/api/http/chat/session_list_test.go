...
type sessionListQueries struct {
	...
	userPagedCall      session.ListRecord
	userPagedCallCount int
	userPagedRows      []session.Thread
}
...
func (q *sessionListQueries) InTransaction(_ context.Context, fn func(session.Store) error) error {
	return fn(q)
}

func (q *sessionListQueries) GetBotMetadata(_ context.Context, _ string) (map[string]any, error) {
	return botMetadataMap(q.bot), nil
}

func (q *sessionListQueries) ListThreadsByBot(_ context.Context, record session.ListRecord) ([]session.Thread, error) {
	q.pagedCall = record
	q.pagedCallCount++
	rows := make([]session.Thread, 0, len(q.pagedRows))
	rows = append(rows, q.pagedRows...)
	return rows, nil
}

func (q *sessionListQueries) ListThreadsByUser(_ context.Context, record session.ListRecord) ([]session.Thread, error) {
	q.userPagedCall = record
	q.userPagedCallCount++
	rows := make([]session.Thread, 0, len(q.userPagedRows))
	rows = append(rows, q.userPagedRows...)
	return rows, nil
}
...
func TestListSessionsDefaultsToUserFacingTypes(t *testing.T) {
	...
	queries := &sessionListQueries{bot: httpfixture.BotRow(botID, nil)}
	...
	if queries.pagedCall.Limit != sessionListDefaultLimit+1 {
		t.Fatalf("default limit = %d, want %d (default+1 has-more probe)", queries.pagedCall.Limit, sessionListDefaultLimit+1)
	}
...
func TestListSessionsClampsLimit(t *testing.T) {
	...
	if queries.pagedCall.Limit != 11 {
		t.Fatalf("limit = %d, want 11 (request+1 has-more probe)", queries.pagedCall.Limit)
	}
}
...
func TestListSessionsCursorRoundTrip(t *testing.T) {
	...
	queries := &sessionListQueries{
		bot: httpfixture.BotRow(botID, nil),
		pagedRows: []session.Thread{
	...
	queries.pagedCall = session.ListRecord{}
	...
	if !queries.pagedCall.Cursor.UpdatedAt.Equal(rowUpdated) {
		t.Fatalf("CursorUpdatedAt = %v, want %v", queries.pagedCall.Cursor.UpdatedAt, rowUpdated)
	}
	if queries.pagedCall.Cursor.ID != rowID {
		t.Fatalf("CursorID = %s, want %s", queries.pagedCall.Cursor.ID, rowID)
	}
}
...
func TestListSessionsCursorNotTruncatedByPermissionFilter(t *testing.T) {
	...
	queries.userPagedRows = []session.Thread{
...
func userPagedRow(id, userID, typ string, updatedAt time.Time) session.Thread {
	th := testThreadFromLegacyFull(id, "11111111-1111-1111-1111-111111111111", typ, "", "", []byte(`{}`), nil, userID, "test session")
	th.CreatedAt = updatedAt
	th.UpdatedAt = updatedAt
	return th
}

func pagedRow(id string, updatedAt time.Time) session.Thread {
	th := testThreadFromLegacy(id, "11111111-1111-1111-1111-111111111111", session.TypeChat, []byte(`{}`))
	th.Title = "test session"
	th.CreatedAt = updatedAt
	th.UpdatedAt = updatedAt
	return th
}

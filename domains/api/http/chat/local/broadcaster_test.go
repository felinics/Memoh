...
func TestRouteHubBroadcaster_EmptyBotID(t *testing.T) {
	...
	broadcaster := NewRouteHubBroadcaster(hub)
	broadcaster.OnStreamEvent(context.Background(), "", "telegram", gateway.StreamEvent{
		Type:  gateway.StreamEventDelta,
...
func TestRouteHubBroadcaster_NilHub(_ *testing.T) {
	...
	// Must not panic.
	broadcaster.OnStreamEvent(context.Background(), "bot1", "telegram", gateway.StreamEvent{
		Type:  gateway.StreamEventDelta,
		Delta: "no-op",
	})
}

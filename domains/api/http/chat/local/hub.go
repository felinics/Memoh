...
type RouteHubEvent struct {
	...
	Event  gateway.StreamEvent `json:"event"`
}
...
func (h *RouteHub) Publish(routeKey string, msg gateway.OutboundMessage) {
	h.PublishEvent(routeKey, gateway.StreamEvent{
...
// Slow receivers are silently dropped.
func (h *RouteHub) PublishEvent(routeKey string, event gateway.StreamEvent) {
	h.mu.RLock()
	defer h.mu.RUnlock()
...
func newLocalOutboundStream(hub *RouteHub, target string) gateway.PreparedOutboundStream {
...
func (s *localOutboundStream) Push(ctx context.Context, event gateway.PreparedStreamEvent) error {
	if s == nil || s.hub == nil {

...
func buildSampleBundle(t *testing.T) []byte {
	...
	write("bot/profile.json", "profile", bot.Bot{ID: "src-bot", DisplayName: "Src Bot", Timezone: "UTC", IsActive: true})
...
func TestMCPRequestFromConnection(t *testing.T) {
	stdio := mcpRequestFromConnection(mcppersistence.Connection{
		Name:     "local",
		Type:     "stdio",
		Active:   true,
		AuthType: "none",
	...
	sse := mcpRequestFromConnection(mcppersistence.Connection{
		Name: "remote",
		Type: "sse",
		Config: map[string]any{

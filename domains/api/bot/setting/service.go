...
type Service struct {
	...
	models  settingpersistence.ModelReader
	acl     *acl.Service
...
func NewService(log *slog.Logger, store settingpersistence.Store, models settingpersistence.ModelReader, aclService *acl.Service, networkService *netctl.Service) *Service {
	if log == nil {
		log = slog.Default()
	}
	return &Service{
...
func (s *Service) UpsertBot(ctx context.Context, botID string, req UpsertRequest) (Settings, error) {
	...
	if settingsRow, settingsErr := s.store.Get(ctx, botID); settingsErr != nil {
		if !errors.Is(settingsErr, settingpersistence.ErrNotFound) {
	...
	if req.ToolApprovalConfig != nil {
		current.ToolApprovalConfig = settingpersistence.NormalizeToolApprovalConfig(*req.ToolApprovalConfig)
	}
	...
	updated, err := s.store.Upsert(ctx, settingpersistence.UpsertInput{
		BotID:                  botID,
...
func normalizeBotSetting(language string, commandUILanguage string, aclDefaultEffect string, reasoningEnabled bool, reasoningEffort string, heartbeatEnabled bool, heartbeatInterval int32, compactionEnabled bool, compactionThreshold int32, compactionRatio int32) Settings {
	settings := Settings{
		...
		ToolApprovalConfig:  settingpersistence.DefaultToolApprovalConfig(),
...
func normalizeBotSettingsRecord(row settingpersistence.Record) Settings {
...
func parseToolApprovalConfig(raw []byte) settingpersistence.ToolApprovalConfig {
	if len(raw) == 0 {
		return settingpersistence.DefaultToolApprovalConfig()
	}
	var cfg settingpersistence.ToolApprovalConfig
	if err := json.Unmarshal(raw, &cfg); err != nil {
		return settingpersistence.DefaultToolApprovalConfig()
	}
	return settingpersistence.NormalizeToolApprovalConfig(cfg)
}
...
func settingsOverlayConfigFromRow(row settingpersistence.OverlayRecord) netctl.BotOverlayConfig {

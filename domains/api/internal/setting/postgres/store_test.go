...
func TestRuntimeSettingsReaderReturnsTypedProjection(t *testing.T) {
	...
	if projection.ToolApprovalConfig.Read.Mode != setting.ToolApprovalDeny {
		t.Fatalf("read mode = %q, want %q", projection.ToolApprovalConfig.Read.Mode, setting.ToolApprovalDeny)
	}
}
...
func TestUpsertUsesOwnerGeneratedParamsAndReturnsPersistedReferences(t *testing.T) {
	...
	got, err := store.Upsert(t.Context(), setting.UpsertInput{
		BotID:              testBotID,
	...
	if uuidString(fake.upsertParams.ID) != testBotID {
		t.Fatalf("upsert bot ID = %q", uuidString(fake.upsertParams.ID))
	}
	if uuidString(fake.upsertParams.MemoryProviderID) != "55555555-5555-5555-5555-555555555555" {
		t.Fatalf("memory provider ID = %q", uuidString(fake.upsertParams.MemoryProviderID))
	}
	if !fake.upsertParams.FetchProviderIDSet || uuidString(fake.upsertParams.FetchProviderID) != "66666666-6666-6666-6666-666666666666" {
...
func TestMapQueryErrorPreservesPostgresCause(t *testing.T) {
	...
	if !errors.Is(err, setting.ErrNotFound) || !errors.Is(err, pgx.ErrNoRows) {

// Package bot implements Bots-owned PostgreSQL projections.
package bot
...
var (
	_ botpersistence.BotStore        = (*Store)(nil)
	_ botpersistence.ActivityWriter  = (*Store)(nil)
	_ botpersistence.HeartbeatReader = (*Store)(nil)
	_ botpersistence.GrantStore      = (*Store)(nil)
...
func (s *Store) ListHeartbeatEnabledBots(ctx context.Context) ([]botpersistence.HeartbeatRecord, error) {
	rows, err := s.queries.ListHeartbeatEnabledBots(ctx)
	if err != nil {
		return nil, err
	}
	items := make([]botpersistence.HeartbeatRecord, 0, len(rows))
	for _, row := range rows {
		items = append(items, botpersistence.HeartbeatRecord{
			ID:                row.ID.String(),
...
func (s *Store) CreateBot(ctx context.Context, input botpersistence.CreateInput) (botpersistence.Record, error) {
	ownerID, err := db.ParseUUID(input.OwnerUserID)
	if err != nil {
		return botpersistence.Record{}, err
	}
	...
	if err != nil {
		return botpersistence.Record{}, mapBotError(err)
	}
...
func (s *Store) GetBotByID(ctx context.Context, botID string) (botpersistence.Record, error) {
	id, err := db.ParseUUID(botID)
	if err != nil {
		return botpersistence.Record{}, err
	}
...
func (s *Store) GetBotByName(ctx context.Context, name string) (botpersistence.Record, error) {
...
func (s *Store) ListBotsByOwner(ctx context.Context, ownerUserID string) ([]botpersistence.Record, error) {
	...
	items := make([]botpersistence.Record, 0, len(rows))
	for _, row := range rows {
...
func (s *Store) ListAccessibleBots(ctx context.Context, userID string) ([]botpersistence.Record, error) {
...
func (s *Store) UpdateBot(ctx context.Context, input botpersistence.UpdateInput) (botpersistence.Record, error) {
...
func (s *Store) UpdateBotOwner(ctx context.Context, botID, ownerUserID string) (botpersistence.Record, error) {
	id, err := db.ParseUUID(botID)
	if err != nil {
		return botpersistence.Record{}, err
	}
...
func (s *Store) ListGrants(ctx context.Context, botID string) ([]botpersistence.GrantRecord, error) {
	id, err := db.ParseUUID(botID)
	if err != nil {
		return nil, err
	}
	...
	items := make([]botpersistence.GrantRecord, 0, len(rows))
	for _, row := range rows {
		items = append(items, botpersistence.GrantRecord{
			ID: row.ID.String(), BotID: row.BotID.String(), SubjectType: row.SubjectType,
			UserID: db.UUIDString(row.UserID), Permissions: cloneBytes(row.Permissions),
...
func (s *Store) ListGrantsForUser(ctx context.Context, botID, userID string) ([]botpersistence.GrantRecord, error) {
	id, err := db.ParseUUID(botID)
	if err != nil {
		return nil, err
	}
...
func (s *Store) GetGrant(ctx context.Context, grantID string) (botpersistence.GrantRecord, error) {
	id, err := db.ParseUUID(grantID)
	if err != nil {
		return botpersistence.GrantRecord{}, err
	}
	row, err := s.queries.GetBotUserGrantByID(ctx, id)
	if err != nil {
		return botpersistence.GrantRecord{}, mapGrantError(err)
	}
	return grantRecord(row), nil
}

func (s *Store) CreateGrant(ctx context.Context, input botpersistence.CreateGrantInput) (botpersistence.GrantRecord, error) {
...
func (s *Store) UpdateGrantPermissions(ctx context.Context, grantID string, permissions []byte) (botpersistence.GrantRecord, error) {
	id, err := db.ParseUUID(grantID)
	if err != nil {
		return botpersistence.GrantRecord{}, err
	}
...
) botpersistence.Record {
	return botpersistence.Record{
		ID:          id.String(),
		OwnerUserID: ownerUserID.String(),
		Name:        name,
		DisplayName: db.TextToString(displayName),
		AvatarURL:   db.TextToString(avatarURL),
...
func grantRecord(row apisqlc.ApiBotUserGrant) botpersistence.GrantRecord {
...
func mapBotError(err error) error {
	if errors.Is(err, pgx.ErrNoRows) {
		return botpersistence.ErrBotNotFound
	}
	if db.IsUniqueViolation(err) {
		return botpersistence.ErrBotNameTaken
	}
	return err
}
...
func mapGrantError(err error) error {
	if errors.Is(err, pgx.ErrNoRows) {
		return botpersistence.ErrGrantNotFound
	}
	if db.IsUniqueViolation(err) {
		return botpersistence.ErrGrantExists
	}
	return err
}

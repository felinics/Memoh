...
func TestOwnerErrorMappingHidesDatabaseCauses(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "bot", mapf: mapBotError, want: botpersistence.ErrBotNotFound},
		{name: "grant", mapf: mapGrantError, want: botpersistence.ErrGrantNotFound},
	}
	...
	if !errors.Is(mapped, botpersistence.ErrGrantExists) {

...
type queryFake struct {
	...
	getByUserID   func(context.Context, pgtype.UUID) (dbsqlc.PlatformTeamAccount, error)
...
func (f queryFake) GetAccountByUserID(ctx context.Context, id pgtype.UUID) (dbsqlc.PlatformTeamAccount, error) {
...
func TestStoreGetByUserIDMapsPrincipalAndMembershipState(t *testing.T) {
	...
	store := NewStore(queryFake{getByUserID: func(_ context.Context, got pgtype.UUID) (dbsqlc.PlatformTeamAccount, error) {
		if got != userID {
			t.Fatalf("GetAccountByUserID() id = %v, want %v", got, userID)
		}
		return dbsqlc.PlatformTeamAccount{
			ID:                 userID,
...
func TestStoreCreateAccountMapsPersistenceNeutralInput(t *testing.T) {
	...
	store := NewStore(queryFake{createAccount: func(_ context.Context, got dbsqlc.CreateAccountParams) (dbsqlc.CreateAccountRow, error) {
		...
		if got.Email.Valid || got.AvatarUrl.Valid || got.DataRoot.Valid {
	...
	record, err := store.CreateAccount(context.Background(), account.CreateInput{
...
func TestStoreUpdateProfileMapsNullableTitleModel(t *testing.T) {
	...
	record, err := store.UpdateProfile(context.Background(), account.ProfileUpdate{
...
func TestMapQueryErr(t *testing.T) {
	...
	if err := mapQueryErr(pgx.ErrNoRows); !errors.Is(err, account.ErrAccountNotFound) {
	...
	if err := mapQueryErr(lastAdmin); !errors.Is(err, account.ErrLastActiveAdmin) {

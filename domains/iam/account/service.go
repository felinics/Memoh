...
// HasActiveTeamMembership reports whether the user and their membership in the
// current team are both active. Missing users are reported as inactive.
func (s *Service) HasActiveTeamMembership(ctx context.Context, userID string) (bool, error) {
	err := s.ValidateSession(ctx, userID)
...
func (s *Service) UpdateProfile(ctx context.Context, userID string, req UpdateProfileRequest) (Account, error) {
	...
	if req.TitleModelID != nil {
		...
		if titleModelID != "" {
			if s.titleModels == nil {
				return Account{}, errors.New("title model validator not configured")
			}
			valid, err := s.titleModels.IsValidTitleModel(ctx, titleModelID)
			if err != nil {
				return Account{}, err
			}
			if !valid {

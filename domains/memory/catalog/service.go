...
type Service struct {
	store    memport.ProviderStore
	registry *registry.Registry
	logger   *slog.Logger
}

func NewService(log *slog.Logger, store memport.ProviderStore) *Service {
	return &Service{
		store:  store,
...
func (s *Service) Create(ctx context.Context, req ProviderCreateRequest) (ProviderGetResponse, error) {
	...
	row, err := s.store.CreateProvider(ctx, memport.ProviderCreate{
...
func (s *Service) Update(ctx context.Context, id string, req ProviderUpdateRequest) (ProviderGetResponse, error) {
	...
	updated, err := s.store.UpdateProvider(ctx, memport.ProviderUpdate{
		ID:     id,
		Name:   name,
		Config: config,
	})
...
func (s *Service) EnsureDefault(ctx context.Context) (ProviderGetResponse, error) {
	row, err := s.store.FindDefaultProvider(ctx)
	if err == nil {
		return s.toGetResponse(row), nil
	}
	configJSON, _ := json.Marshal(map[string]any{})
	created, err := s.store.CreateProvider(ctx, memport.ProviderCreate{
...
func (s *Service) toGetResponse(row memport.ProviderRecord) ProviderGetResponse {
	var cfg map[string]any
	if len(row.Config) > 0 {
		if err := json.Unmarshal(row.Config, &cfg); err != nil {
			s.logger.Warn("memory provider config unmarshal failed", slog.String("id", row.ID), slog.Any("error", err))
		}
	}

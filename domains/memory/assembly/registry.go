...
type Deps struct {
	...
	TeamIDResolver  registry.TeamIDResolver
}
...
func NewRegistry(deps Deps) (*registry.Registry, func(), error) {
	log := deps.Log
	if log == nil {
		log = slog.Default()
	}
	var resolvers []registry.TeamIDResolver
	if deps.TeamIDResolver != nil {
		resolvers = append(resolvers, deps.TeamIDResolver)
	}
	reg := registry.NewRegistry(log, resolvers...)
	...
	reg.RegisterFactory(registry.ProviderBuiltin, func(ctx context.Context, teamID, _ string, providerConfig map[string]any) (registry.Instance, error) {
		if wikiStore == nil {
			return nil, errors.New("graph runtime: wiki store not configured")
		}
		runtime, err := builtin.NewBuiltinRuntimeFromConfigContext(ctx, log, providerConfig, fileStore, embeddingModels, vectorStore, wikiStore, registry.FixedTeamIDResolver(teamID))
		if err != nil {
			return nil, err
		}
	...
	reg.RegisterFactory(registry.ProviderMem0, func(_ context.Context, _, _ string, providerConfig map[string]any) (registry.Instance, error) {
		return mem0.NewMem0Provider(log, providerConfig, fileStore)
	})
	reg.RegisterFactory(registry.ProviderOpenViking, func(_ context.Context, _, _ string, providerConfig map[string]any) (registry.Instance, error) {
	...
	reg.Register(registry.DefaultBuiltinProviderID, defaultProvider)

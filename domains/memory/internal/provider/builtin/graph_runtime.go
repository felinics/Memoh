...
func (r *graphRuntime) semanticUpsertBestEffort(botID string, n memorydomain.NodeSpec) {
	if r.semantic == nil {
		return
	}
...
func (r *graphRuntime) searchGraph(ctx context.Context, botID, query string, limit int) (memprovider.SearchResponse, error) {
	...
	results := make([]memorydomain.Item, 0, len(ids))
	for _, id := range ids {
		n, ok := graph.nodes[id]
		if !ok {
			continue
...
func (r *graphRuntime) GetAll(ctx context.Context, req memprovider.GetAllRequest) (memprovider.SearchResponse, error) {
	...
	out := make([]memorydomain.Item, 0, len(nodes))
	for _, n := range nodes {
...
func (r *graphRuntime) Update(ctx context.Context, req memprovider.UpdateRequest) (memorydomain.Item, error) {
	...
	if memoryID == "" {
		return memorydomain.Item{}, errors.New("graph runtime: memory_id is required")
	}
	text := strings.TrimSpace(req.Memory)
	if text == "" {
		return memorydomain.Item{}, errors.New("graph runtime: memory is required")
	}
	botID := runtimeBotIDFromMemoryID(memoryID)
	if botID == "" {
		return memorydomain.Item{}, errors.New("graph runtime: invalid memory_id")
	}
	existing, storedID, err := r.resolveNodeByMemoryID(ctx, botID, memoryID)
	if err != nil {
		return memorydomain.Item{}, fmt.Errorf("graph runtime: get node: %w", err)
	}
	...
	if err != nil {
		return memorydomain.Item{}, fmt.Errorf("graph runtime: update node: %w", err)
	}
	if storedID != memoryID {
		if err := r.store.DeleteNode(ctx, botID, storedID); err != nil {
			return memorydomain.Item{}, fmt.Errorf("graph runtime: delete legacy node: %w", err)
		}
		if r.semantic != nil {
...
func (r *graphRuntime) resolveNodeByMemoryID(ctx context.Context, botID, memoryID string) (memorydomain.NodeSpec, string, error) {
	...
	if !errors.Is(err, wikistore.ErrNodeNotFound) {
		return memorydomain.NodeSpec{}, "", err
	}
	...
	if legacyErr != nil {
		...
		return memorydomain.NodeSpec{}, "", legacyErr
	}
...
// memoryItemToNodeSpec derives a NodeSpec from a memorydomain.Item, classifying the
// layer from metadata (defaulting to note) and pulling profile_ref/topic out.
func memoryItemToNodeSpec(item memorydomain.Item, botID string) memorydomain.NodeSpec {
	body := strings.TrimSpace(item.Memory)
	layer := memorydomain.LayerNote
	if raw, ok := item.Metadata["layer"].(string); ok && raw != "" {
		switch memorydomain.Layer(strings.ToLower(strings.TrimSpace(raw))) {
		case memorydomain.LayerPreference, memorydomain.LayerIdentity, memorydomain.LayerContext,
			memorydomain.LayerExperience, memorydomain.LayerActivity, memorydomain.LayerPersona, memorydomain.LayerNote:
			layer = memorydomain.Layer(raw)
		}
	}

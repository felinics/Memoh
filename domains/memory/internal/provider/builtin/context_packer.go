...
func packContext(items []memorydomain.Item, cfg contextPackerConfig) contextPackResult {
	if len(items) == 0 {
	...
	for _, c := range reordered {
		snippet := memport.TruncateSnippet(c.text, c.charLen)
...
func deduplicateAndSort(items []memorydomain.Item) []memorydomain.Item {
	deduped := memport.DeduplicateItems(items)

...
func (r *graphRuntime) normalizeCompactNodeIDs(ctx context.Context, botID string, nodes []memorydomain.NodeSpec) ([]memorydomain.NodeSpec, bool, error) {
	type bucket struct {
		canonicalID string
	...
	changed := false
	normalized := make([]memorydomain.NodeSpec, 0, len(keys))
...
func (r *graphRuntime) mergeCompactConcept(ctx context.Context, botID string, nodes []memorydomain.NodeSpec, ratio float64, decayDays int, llm memreg.LLM, now time.Time) (memorydomain.NodeSpec, []string, error) {
	...
	if llm != nil {
		...
		if len(candidates) > 0 {
			...
			if err != nil {
				return memorydomain.NodeSpec{}, nil, fmt.Errorf("graph runtime: llm compact concept: %w", err)
			}
			if len(resp.Facts) > 0 {
	...
	if err != nil {
		return memorydomain.NodeSpec{}, nil, fmt.Errorf("graph runtime: compact upsert merged concept: %w", err)
	}
	r.semanticUpsertBestEffort(botID, saved) //nolint:contextcheck // async semantic upsert uses its own bounded context
	return saved, uniqueCompactStrings(superseded), nil
}

func groupCompactConcepts(botID string, nodes []memorydomain.NodeSpec) [][]memorydomain.NodeSpec {
	groups := make(map[string][]memorydomain.NodeSpec, len(nodes))
	for _, node := range nodes {
	...
	concepts := make([][]memorydomain.NodeSpec, 0, len(keys))
	for _, key := range keys {
		nodes := groups[key]
		sortCompactNodes(nodes)
...
func compactConceptKey(botID string, node memorydomain.NodeSpec) string {
	...
	subjectSlug := memorydomain.NodeSlug("", node.Subject, "")
	if subjectSlug == "" {
		subjectSlug = memorydomain.NodeSlug("", metadataStringVal(node.Metadata, "subject"), "")
	}
	topicSlug := memorydomain.NodeSlug("", "", node.Topic)
	if topicSlug == "" {
		topicSlug = memorydomain.NodeSlug("", "", metadataStringVal(node.Metadata, "topic"))
	}
	if subjectSlug != "" {
...
func splitCompactProtectedNodes(nodes []memorydomain.NodeSpec) ([]memorydomain.NodeSpec, []memorydomain.NodeSpec) {
	protected := make([]memorydomain.NodeSpec, 0)
	compactable := make([]memorydomain.NodeSpec, 0, len(nodes))
	for _, node := range nodes {
...
func compactProtectedNode(node memorydomain.NodeSpec) bool {
...
func compactSameBody(nodes []memorydomain.NodeSpec) bool {
	if len(nodes) < 2 {
		return true
	}
...
func compactMetadata(metadata map[string]any, nodes []memorydomain.NodeSpec, representativeID string, ratio float64, compactedAt time.Time) map[string]any {
...
func compactSourceIDs(nodes []memorydomain.NodeSpec) []string {
...
func completeCompactNode(node memorydomain.NodeSpec, sources []memorydomain.NodeSpec) memorydomain.NodeSpec {
	if node.Layer == "" {
		node.Layer = memorydomain.LayerNote
	}
...
func mergeCompactSourceMessageIDs(nodes []memorydomain.NodeSpec) []string {
...
func sortCompactNodes(nodes []memorydomain.NodeSpec) {
...
func compactNodeRank(node memorydomain.NodeSpec) int {

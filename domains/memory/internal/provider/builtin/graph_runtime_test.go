...
type fakeWikiStore struct {
	nodes map[string]memorydomain.NodeSpec // keyed by node ID
	edges map[string]memorydomain.EdgeSpec // keyed by src\0dst\0rel
}

func newFakeWikiStore() *fakeWikiStore {
	return &fakeWikiStore{nodes: map[string]memorydomain.NodeSpec{}, edges: map[string]memorydomain.EdgeSpec{}}
}

func (s *fakeWikiStore) UpsertNode(_ context.Context, node memorydomain.NodeSpec) (memorydomain.NodeSpec, error) {
	s.nodes[node.ID] = node
	return node, nil
}

func (s *fakeWikiStore) GetNode(_ context.Context, _, nodeID string) (memorydomain.NodeSpec, error) {
...
func (s *fakeWikiStore) ListNodes(_ context.Context, _ string) ([]memorydomain.NodeSpec, error) {
	out := make([]memorydomain.NodeSpec, 0, len(s.nodes))
...
func (s *fakeWikiStore) ListNodesByLayer(_ context.Context, _ string, layer memorydomain.Layer) ([]memorydomain.NodeSpec, error) {
	out := []memorydomain.NodeSpec{}
	for _, n := range s.nodes {
		if n.Layer == layer {
			out = append(out, n)
		}
...
func (s *fakeWikiStore) DeleteAllNodes(_ context.Context, _ string) error {
	s.nodes = map[string]memorydomain.NodeSpec{}
	s.edges = map[string]memorydomain.EdgeSpec{}
	return nil
}
...
func (s *fakeWikiStore) UpsertEdges(_ context.Context, edges []memorydomain.EdgeSpec) error {
	for _, e := range edges {
...
func (s *fakeWikiStore) ListEdges(_ context.Context, _ string) ([]memorydomain.EdgeSpec, error) {
	out := make([]memorydomain.EdgeSpec, 0, len(s.edges))
...
func (s *fakeWikiStore) RebuildDerivedEdges(_ context.Context, _ string) (int, error) {
	...
	derived := memorydomain.PlanFromNodes(nodes)
...
func TestGraphRuntimeSearchExpandsRefs(t *testing.T) {
	...
	for _, rel := range resp.Relations {
		if relation, ok := rel.(map[string]any); ok && relation["rel"] == string(memorydomain.EdgeRefs) {
			foundRefs = true
		}
	}
	if !foundBerlin {
...
func TestGraphRuntimeUpdateCanonicalIDMigratesLegacyBareNode(t *testing.T) {
	...
	ctx := context.Background()
	botID := "bot-1"
	store.nodes["mem_legacy"] = memorydomain.NodeSpec{
		ID:    "mem_legacy",
		BotID: botID,
		Body:  "old body",
...
func TestGraphRuntimeCompactCanonicalizesLegacyBareIDs(t *testing.T) {
	...
	store.nodes[botID+":mem_other"] = memorydomain.NodeSpec{
		ID:         botID + ":mem_other",
		BotID:      botID,
...
func TestGraphRuntimeCompactWithLLMMergesConceptNodes(t *testing.T) {
	...
	botID := "bot-1"
	now := time.Date(2026, 7, 7, 10, 0, 0, 0, time.UTC)
	store.nodes[botID+":mem_alice_1"] = memorydomain.NodeSpec{
	...
	store.nodes["mem_alice_2"] = memorydomain.NodeSpec{
		ID:         "mem_alice_2",
		BotID:      botID,
	...
	store.nodes[botID+":mem_bob"] = memorydomain.NodeSpec{
		ID:         botID + ":mem_bob",
		BotID:      botID,
		Body:       "Bob uses Vim.",
		Layer:      memorydomain.LayerNote,
...
func TestGraphRuntimeCompactWithLLMPreservesProtectedConceptNodes(t *testing.T) {
	...
	botID := "bot-1"
	now := time.Date(2026, 7, 7, 10, 0, 0, 0, time.UTC)
	store.nodes[botID+":mem_pinned"] = memorydomain.NodeSpec{
		ID:         botID + ":mem_pinned",
		BotID:      botID,
	...
	store.nodes[botID+":mem_a"] = memorydomain.NodeSpec{
		ID:         botID + ":mem_a",
		BotID:      botID,
		Body:       "Alice likes tea.",
		Layer:      memorydomain.LayerNote,
	...
	store.nodes[botID+":mem_b"] = memorydomain.NodeSpec{
		ID:         botID + ":mem_b",
		BotID:      botID,
...
func graphTestNodesBySubject(nodes map[string]memorydomain.NodeSpec, subject string) []memorydomain.NodeSpec {
	out := make([]memorydomain.NodeSpec, 0)
...
func TestGraphRuntimeFileFallback(t *testing.T) {
	...
	store := &errWikiStore{}
	fs := newFakeStore(
		memorydomain.Item{ID: "bot-x:m1", Memory: "fallback oolong memory", CreatedAt: time.Now().Format(time.RFC3339)},
	)
...
func (errWikiStore) UpsertNode(context.Context, memorydomain.NodeSpec) (memorydomain.NodeSpec, error) {
	return memorydomain.NodeSpec{}, errForced
}

func (errWikiStore) GetNode(context.Context, string, string) (memorydomain.NodeSpec, error) {
	return memorydomain.NodeSpec{}, errForced
}

func (errWikiStore) ListNodes(context.Context, string) ([]memorydomain.NodeSpec, error) {
	return nil, errForced
}

func (errWikiStore) ListNodesByLayer(context.Context, string, memorydomain.Layer) ([]memorydomain.NodeSpec, error) {
	return nil, errForced
}
...
func (errWikiStore) UpsertEdges(context.Context, []memorydomain.EdgeSpec) error { return nil }
func (errWikiStore) ListEdges(context.Context, string) ([]memorydomain.EdgeSpec, error) {
	return nil, errForced
}

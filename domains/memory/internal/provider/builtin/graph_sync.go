...
func (s *graphSync) syncMarkdownFromNodes(ctx context.Context, botID string, nodes []memorydomain.NodeSpec) error {
	if s.fs == nil {
	...
	items := make([]memorydomain.Item, 0, len(nodes))
	for _, n := range nodes {
...
func nodeSpecToStoreItem(n memorydomain.NodeSpec) memorydomain.Item {
...
func buildNodeMetadata(n memorydomain.NodeSpec) map[string]any {
	meta := map[string]any{}
...
// nodeSpecToMemoryItem converts a wiki node into the cross-package memorydomain.Item
// shape used by the Runtime interface and search responses.
func nodeSpecToMemoryItem(n memorydomain.NodeSpec) memorydomain.Item {
	return memorydomain.Item{
		ID:        n.ID,
		Memory:    n.Body,
		Hash:      n.Hash,

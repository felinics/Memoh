...
// domains/memory.PlanFromItems (so layer/subject/confidence/topic are derived
// consistently with the chat-extract path), then fills change-detection hash
// and capture time the way runtime.Add does, while preserving the item's
// (possibly synthesised) id so re-ingest is idempotent.
func nodeSpecFromIngestItem(botID string, now time.Time, item memorydomain.Item) memorydomain.NodeSpec {
	nodes, _ := memorydomain.PlanFromItems(botID, []memorydomain.Item{item})
	if len(nodes) == 0 {
		return memorydomain.NodeSpec{ID: item.ID, BotID: botID, Body: strings.TrimSpace(item.Memory), Layer: memorydomain.LayerNote, CapturedAt: now}
	}
	spec := nodes[0]

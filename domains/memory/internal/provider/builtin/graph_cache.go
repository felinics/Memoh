...
type neighbor struct {
	node   memorydomain.NodeSpec
	rel    memorydomain.EdgeRel
	weight float32
}
...
type botGraph struct {
	nodes   map[string]memorydomain.NodeSpec
...
func (g *botGraph) nodeSlice() []memorydomain.NodeSpec {
	out := make([]memorydomain.NodeSpec, 0, len(g.nodes))
...
func buildBotGraph(nodes []memorydomain.NodeSpec, edges []memorydomain.EdgeSpec) *botGraph {
	g := &botGraph{
		nodes: make(map[string]memorydomain.NodeSpec, len(nodes)),
...
func graphContentHash(nodes []memorydomain.NodeSpec, edges []memorydomain.EdgeSpec) string {
	h := sha256.New()
	ids := make([]string, 0, len(nodes))
	byID := make(map[string]memorydomain.NodeSpec, len(nodes))
	for _, n := range nodes {
		ids = append(ids, n.ID)
		byID[n.ID] = n
	}

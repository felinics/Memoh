...
SELECT id, bot_id, body, hash, layer, fact_type, subject, confidence, metadata, source_message_ids, profile_ref, topic, captured_at, expires_at, updated_at, created_at, team_id FROM memory.memory_nodes
WHERE team_id = iam.memoh_current_team_id() AND bot_id = $1 AND id = $2
`
...
func (q *Queries) GetMemoryNode(ctx context.Context, arg GetMemoryNodeParams) (MemoryMemoryNode, error) {
	row := q.db.QueryRow(ctx, getMemoryNode, arg.BotID, arg.ID)
	var i MemoryMemoryNode
	err := row.Scan(
		&i.ID,
		&i.BotID,
		&i.Body,
...
SELECT id, bot_id, src_node, dst_node, rel, weight, metadata, created_at, team_id FROM memory.memory_edges
WHERE team_id = iam.memoh_current_team_id() AND bot_id = $1
`

func (q *Queries) ListMemoryEdgesByBot(ctx context.Context, botID pgtype.UUID) ([]MemoryMemoryEdge, error) {
	rows, err := q.db.Query(ctx, listMemoryEdgesByBot, botID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []MemoryMemoryEdge
	for rows.Next() {
		var i MemoryMemoryEdge
		if err := rows.Scan(
			&i.ID,
			&i.BotID,
			&i.SrcNode,
...
func (q *Queries) ListMemoryEdgesByRel(ctx context.Context, arg ListMemoryEdgesByRelParams) ([]MemoryMemoryEdge, error) {
...
func (q *Queries) ListMemoryEdgesFromNode(ctx context.Context, arg ListMemoryEdgesFromNodeParams) ([]MemoryMemoryEdge, error) {
...
func (q *Queries) ListMemoryNodesByBot(ctx context.Context, botID pgtype.UUID) ([]MemoryMemoryNode, error) {
	rows, err := q.db.Query(ctx, listMemoryNodesByBot, botID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []MemoryMemoryNode
	for rows.Next() {
		var i MemoryMemoryNode
		if err := rows.Scan(
			&i.ID,
...
func (q *Queries) ListMemoryNodesByBotLayer(ctx context.Context, arg ListMemoryNodesByBotLayerParams) ([]MemoryMemoryNode, error) {
...
func (q *Queries) ListMemoryNodesByBotProfile(ctx context.Context, arg ListMemoryNodesByBotProfileParams) ([]MemoryMemoryNode, error) {
...
func (q *Queries) UpsertMemoryNode(ctx context.Context, arg UpsertMemoryNodeParams) (MemoryMemoryNode, error) {

...
import (
	...
	memslug "github.com/memohai/memoh/domains/memory/internal/slug"
...
func (s *Service) PersistMemories(ctx context.Context, botID string, items []memorydomain.Item, _ map[string]any) error {
	if s.provider == nil {
...
func (s *Service) RebuildFiles(ctx context.Context, botID string, items []memorydomain.Item, _ map[string]any) error {
	if s.provider == nil {
		return ErrNotConfigured
	}
...
// not ours to delete.
func isProjectionItems(botID string, items []memorydomain.Item) bool {
	if len(items) == 0 {
		return false
	}
	prefix := botID + ":"
...
func (s *Service) ReadAllMemoryFiles(ctx context.Context, botID string) ([]memorydomain.Item, error) {
	if s.provider == nil {
		return nil, ErrNotConfigured
	}
	c, err := s.client(ctx, botID)
	...
	items := make([]memorydomain.Item, 0, len(entries))
...
func (s *Service) ReadAllMemoryFilesForIngest(ctx context.Context, botID string) ([]memorydomain.Item, error) {
...
func (s *Service) listMemoryMarkdownEntries(ctx context.Context, botID string) ([]*bridgepb.FileEntry, error) {
	if s.provider == nil {
		return nil, ErrNotConfigured
	}
	c, err := s.client(ctx, botID)
...
func (s *Service) readMemoryDay(ctx context.Context, botID, filePath string) ([]memorydomain.Item, error) {
...
func (s *Service) writeMemoryDay(ctx context.Context, botID, filePath string, items []memorydomain.Item) error {
...
func (s *Service) removeIDsFromFiles(ctx context.Context, botID string, removals map[string]map[string]struct{}) error {
	for filePath, ids := range removals {
		...
		filtered := make([]memorydomain.Item, 0, len(items))
		for _, item := range items {
...
func memoryOverviewPath() string { return path.Join(runtimedomain.DefaultDataMount, "MEMORY.md") }
func memoryDirPath() string      { return path.Join(runtimedomain.DefaultDataMount, "memory") }
...
func memoryEntryPath(entryPath string) string {
	...
	if strings.HasPrefix(clean, runtimedomain.DefaultDataMount+"/") || clean == runtimedomain.DefaultDataMount {
		return clean
	}
...
func memoryRelPath(filePath string) string {
	...
	dataRoot := path.Clean(runtimedomain.DefaultDataMount)
	if clean == dataRoot {
		return ""
	}
...
func parseConceptMD(content string) (memorydomain.Item, error) {
	content = strings.TrimSpace(content)
	if content == "" {
		return memorydomain.Item{}, errors.New("empty concept file")
	}
	if !strings.HasPrefix(content, "---") {
		return memorydomain.Item{}, errors.New("missing frontmatter")
	}
	lines := strings.Split(content, "\n")
	if strings.TrimSpace(lines[0]) != "---" {
		return memorydomain.Item{}, errors.New("missing frontmatter delimiter")
	}
	end := -1
	...
	if end <= 1 {
		return memorydomain.Item{}, errors.New("missing frontmatter end")
	}
	...
	if strings.TrimSpace(fm.Type) == "" {
		return memorydomain.Item{}, errors.New("frontmatter type is required")
	}
	body := strings.TrimSpace(strings.Join(lines[end+1:], "\n"))
	if body == "" {
		return memorydomain.Item{}, errors.New("empty concept body")
	}
...
func parseMemoryFile(content string) ([]memorydomain.Item, error) {
	if item, err := parseConceptMD(content); err == nil {
		return []memorydomain.Item{item}, nil
	}
...
func formatMemoryDayMD(date string, items []memorydomain.Item) string {
	var b strings.Builder
...
func parseMemoryDayMD(content string) ([]memorydomain.Item, error) {
	content = strings.TrimSpace(content)
	if content == "" {
		return nil, errors.New("empty memory file")
	}
	lines := strings.Split(content, "\n")
	items := make([]memorydomain.Item, 0, 8)
...
func parseJSONMemoryItems(content string) ([]memorydomain.Item, error) {
	content = strings.TrimSpace(content)
	if content == "" {
		return nil, errors.New("empty memory file")
	}
...
func normalizeJSONMemoryItems(records []jsonMemoryRecord) []memorydomain.Item {
	now := time.Now().UTC()
	items := make([]memorydomain.Item, 0, len(records))
...
func formatMemoryOverviewMD(items []memorydomain.Item) string {
	return formatMemoryOverviewMDWithPaths(items, nil)
}

func formatMemoryOverviewMDWithPaths(items []memorydomain.Item, relPaths map[string]string) string {
	var b strings.Builder
	b.WriteString("# MEMORY\n\n")
	if len(items) == 0 {
...
func planConceptPersistence(index map[string]scanEntry, items []memorydomain.Item) ([]conceptPersistWrite, map[string]conceptLinkTarget, []memorydomain.Item, map[string]string) {
	existing := make(map[string]memorydomain.Item, len(index))
	...
	incoming := make(map[string]memorydomain.Item, len(items))
	for _, item := range items {
		item = normalizeMemoryItem(item)
		if item.ID == "" || item.Memory == "" {
			continue
	...
	merged := make(map[string]memorydomain.Item, len(existing)+len(incoming))
...
func planConceptFileWithReservedPath(item memorydomain.Item, reservedPaths map[string]string) conceptFilePlan {
	item = normalizeMemoryItem(item)
	layer := conceptLayer(item)
...
func planConceptFiles(items []memorydomain.Item) []conceptFilePlan {
	normalized := make([]memorydomain.Item, 0, len(items))
	for _, item := range items {
		item = normalizeMemoryItem(item)
		if item.ID == "" || item.Memory == "" {
			continue
...
func conceptLinkIndexForItems(items []memorydomain.Item, relPaths map[string]string) map[string]conceptLinkTarget {
	plans := planConceptFiles(items)
	index := make(map[string]conceptLinkTarget, len(plans)*2)
	for _, plan := range plans {
...
func sortedMemoryIDs(items map[string]memorydomain.Item) []string {
...
func normalizeMemoryItem(item memorydomain.Item) memorydomain.Item {
	item.ID = strings.TrimSpace(item.ID)
	item.Memory = strings.TrimSpace(item.Memory)
...
func conceptLayer(item memorydomain.Item) string {
...
func conceptTitle(item memorydomain.Item) string {
...
func mapToItems(m map[string]memorydomain.Item) []memorydomain.Item {
	items := make([]memorydomain.Item, 0, len(m))
	for _, item := range m {
		items = append(items, item)
	}
	return items
...
func memoryTime(item memorydomain.Item) time.Time {

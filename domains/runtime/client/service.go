...
var (
	...
	ErrRuntimeNameTaken          = errors.New("runtime name already exists")
...
func NewService(store CredentialStore, memberships MembershipReader, hub *Hub) *Service {
	return &Service{
		store:          store,
...
func (s *Service) CreateRuntime(ctx context.Context, userID string, req CreateRuntimeRequest) (Runtime, error) {
	...
	row, err := s.store.CreateCredential(ctx, CreateCredentialInput{
...
func (s *Service) ListRuntimes(ctx context.Context, userID string) ([]Runtime, error) {
	...
	rows, err := s.store.ListCredentials(ctx, userID)
	if err != nil {
		return nil, err
	}
...
func (s *Service) RevokeRuntime(ctx context.Context, userID, runtimeID string) error {
	...
	defer release()
	if err := s.store.RevokeCredential(ctx, runtimeID, userID); err != nil {
		return err
	}
	if s.hub != nil {
...
// FindCredentialByAPIToken resolves a live credential only while its owner has
// an active membership in the current team.
func (s *Service) FindCredentialByAPIToken(ctx context.Context, apiToken string) (CredentialRecord, error) {
	if s == nil || s.store == nil || s.memberships == nil {
		return CredentialRecord{}, errors.New("user runtime service not configured")
	}
	row, err := s.store.FindCredentialByAPIToken(ctx, apiToken)
	if err != nil {
		return CredentialRecord{}, err
	}
	active, err := s.memberships.HasActiveTeamMembership(ctx, row.UserID)
	if err != nil {
		return CredentialRecord{}, fmt.Errorf("check runtime owner membership: %w", err)
	}
	if !active {
...
func (s *Service) authenticateKeyRecord(ctx context.Context, key string) (CredentialRecord, error) {
	if s == nil || s.store == nil || s.memberships == nil {
		return CredentialRecord{}, errors.New("user runtime service not configured")
	}
	key = strings.TrimSpace(key)
	if err := secret.ValidateKeyFormat(key); err != nil {
		return CredentialRecord{}, ErrInvalidKey
	}
	row, err := s.FindCredentialByAPIToken(ctx, key)
	if err != nil {
		return CredentialRecord{}, err
	}
	if row.APIToken != key {
...
func runtimeFromRecord(row CredentialRecord, connection *Connection) Runtime {
	runtime := Runtime{

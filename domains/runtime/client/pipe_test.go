...
func (s *directPipeTestService) Stat(ctx context.Context, req *bridgepb.StatRequest) (*bridgepb.StatResponse, error) {
...
func TestDirectPipeWebSocketRoundTripAndDisconnect(t *testing.T) {
	...
	httpServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		client := bridgepb.NewContainerServiceClient(conn)
		response, err := client.Stat(r.Context(), &bridgepb.StatRequest{Path: "/workspace"})
		if err != nil {
			roundTrip <- result{err: err}
			return
		}
		roundTrip <- result{path: response.GetEntry().GetPath()}
		_, err = client.Stat(context.WithoutCancel(r.Context()), &bridgepb.StatRequest{Path: "/block"})
		disconnected <- err
	}))
	...
	bridgepb.RegisterContainerServiceServer(grpcServer, service)

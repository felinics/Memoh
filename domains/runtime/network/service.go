...
func NewServiceWithPersistence(log *slog.Logger, persistence Persistence, registry *Registry, runtime statusRuntime, runtimeKind, cniBinDir, cniConfDir, networkStateRoot string) *Service {
	return &Service{
		configReader:     persistence.Config,
		workspaceReader:  persistence.Workspaces,
		registry:         registry,
		runtime:          runtime,
...
// New constructs Service and Controller together. The Service is the
// BindingResolver for the Controller; both are wired in one step so callers
// never need a post-construction setter.
func New(log *slog.Logger, persistence Persistence, registry *Registry, overlayRuntime Runtime, status statusRuntime, runtimeKind, cniBinDir, cniConfDir, networkStateRoot string) (*Service, Controller) {
	svc := NewServiceWithPersistence(log, persistence, registry, status, runtimeKind, cniBinDir, cniConfDir, networkStateRoot)
	ctrl := NewController(overlayRuntime, svc, registry)
	svc.controller = ctrl
	return svc, ctrl
}

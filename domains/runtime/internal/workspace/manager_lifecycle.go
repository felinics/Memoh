...
func (m *Manager) ContainerID(ctx context.Context, botID string) (string, error) {
	...
	containers, err := m.service.ListContainersByLabel(ctx, runtimeworkspace.BotLabelKey, botID)
	if err != nil {
		return "", err
	}
	...
	for _, info := range containers {
		resolvedBotID, ok := workspacelifecycle.BotIDFromContainerInfo(info)
	...
	return "", runtimeworkspace.ErrContainerNotFound
}
...
func (m *Manager) EnsureNativeRunning(ctx context.Context, botID string) error {
	...
	if err != nil {
		if errors.Is(err, runtimeworkspace.ErrContainerNotFound) {
			m.logger.WarnContext(ctx, "container missing, rebuilding", slog.String("bot_id", botID))
	...
	if err != nil {
		...
		m.logger.WarnContext(ctx, "container missing in containerd, rebuilding",
...
func (m *Manager) GetContainerInfo(ctx context.Context, botID string) (*runtimeworkspace.ContainerStatus, error) {
	if m.containers != nil {
		if validateBotID(botID) == nil {
			row, dbErr := m.containers.FindContainer(ctx, botID)
			if dbErr == nil {
				...
				if liveInfo, liveErr := m.service.GetContainer(ctx, row.ContainerID); liveErr == nil {
					...
					return nil, runtimeworkspace.ErrContainerNotFound
				}
				...
				return &runtimeworkspace.ContainerStatus{
...
func (m *Manager) SetupBotContainerWithProgress(ctx context.Context, botID string, progress runtimeworkspace.ContainerSetupProgress) error {
	return m.setupBotContainer(ctx, botID, progress)
}

func (m *Manager) setupBotContainer(ctx context.Context, botID string, progress runtimeworkspace.ContainerSetupProgress) error {
	emit := func(event runtimeworkspace.ContainerSetupEvent) {
		if progress != nil {
			progress(event)
		}
	}
	...
	if err != nil {
		m.logger.ErrorContext(ctx, "setup bot container: resolve image failed",
			slog.String("bot_id", botID),
			slog.Any("error", err))
		return err
	}
	emit(runtimeworkspace.ContainerSetupEvent{Type: "pulling", Image: image})
	result, err := m.PrepareImageForCreate(ctx, image, &ctr.PullImageOptions{
		...
		OnProgress: func(p ctr.PullProgress) {
			emit(runtimeworkspace.ContainerSetupEvent{Type: "pull_progress", Layers: p.Layers})
		},
	})
	if err != nil {
		m.logger.ErrorContext(ctx, "setup bot container: prepare image failed",
			slog.String("bot_id", botID),
	...
	switch result.Mode {
	case runtimeworkspace.ImagePrepareSkipped:
		emit(runtimeworkspace.ContainerSetupEvent{Type: "pull_skipped", Image: image, Message: result.Message})
	case runtimeworkspace.ImagePrepareDelegated:
		emit(runtimeworkspace.ContainerSetupEvent{Type: "pull_delegated", Image: image, Message: result.Message})
	}
	...
	emit(runtimeworkspace.ContainerSetupEvent{Type: "creating"})
	hadPreservedData := m.HasPreservedData(botID)
	if hadPreservedData {
		emit(runtimeworkspace.ContainerSetupEvent{Type: "restoring"})
	}

	if err := m.StartWithResolvedConfig(ctx, botID, image, gpu); err != nil {
		m.logger.ErrorContext(ctx, "setup bot container: start failed",
			slog.String("bot_id", botID),
			slog.Any("error", err))
		return err
	}
	if err := m.WaitForWorkspaceReady(ctx, botID); err != nil {
		m.logger.ErrorContext(ctx, "setup bot container: bridge not ready",
			slog.String("bot_id", botID),
			slog.Any("error", err))
		return err
	}
	if err := m.InitializeNativeWorkspace(ctx, botID); err != nil {
		m.logger.ErrorContext(ctx, "setup bot container: workspace initialization failed",
			slog.String("bot_id", botID),
			slog.Any("error", err))
		return err
	}
	if err := m.RememberWorkspaceImage(ctx, botID, image); err != nil {
		m.logger.WarnContext(ctx, "setup bot container: remember workspace image failed",
			slog.String("bot_id", botID),
			slog.String("image", image),
			slog.Any("error", err))
	}
	...
	event := runtimeworkspace.ContainerSetupEvent{
		Type:             "complete",
		Image:            image,
		ContainerID:      containerID,
		WorkspaceBackend: workspaceBackendFromRecord(""),
...
func (m *Manager) ReconcileContainers(ctx context.Context) {
	...
	rows, err := m.containers.ListAutoStartContainers(ctx)
	if err != nil {
	...
	for _, row := range rows {
		...
		if err != nil {
			...
			m.logger.WarnContext(ctx, "reconcile: container missing, rebuilding",
				slog.String("bot_id", botID), slog.String("container_id", containerID))
			if setupErr := m.SetupBotContainer(ctx, botID); setupErr != nil {
				m.logger.ErrorContext(ctx, "reconcile: rebuild failed",
...
func (m *Manager) upsertContainerRecord(ctx context.Context, botID, containerID, status, image string) {
	...
	if dbErr := m.containers.UpsertContainer(ctx, runtimeworkspace.UpsertContainerCommand{
		BotID:            botID,
		ContainerID:      containerID,
		Image:            image,
		Status:           status,
		Namespace:        ns,
		AutoStart:        true,
		ContainerPath:    runtimedomain.DefaultDataMount,
...
func (m *Manager) deleteContainerRecord(ctx context.Context, botID string) {
	...
	if dbErr := m.containers.DeleteContainer(ctx, botID); dbErr != nil {
...
func (m *Manager) markContainerStarted(ctx context.Context, botID string) {
	...
	if dbErr := m.containers.MarkContainerStarted(ctx, botID); dbErr != nil {
...
func (m *Manager) markContainerStopped(ctx context.Context, botID string) {
	...
	if dbErr := m.containers.MarkContainerStopped(ctx, botID); dbErr != nil {
...
func (m *Manager) markContainerStatus(ctx context.Context, botID, status string) {
	...
	if dbErr := m.containers.MarkContainerStatus(ctx, botID, status); dbErr != nil {

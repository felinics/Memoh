...
func (m *Manager) GetContainerMetrics(ctx context.Context, botID string) (*runtimeworkspace.ContainerMetricsResult, error) {
	result := &runtimeworkspace.ContainerMetricsResult{
		Supported: true,
		Status: runtimeworkspace.ContainerMetricsStatus{
			Exists: false,
		},
	}
...
func (m *Manager) collectStorageMetrics(ctx context.Context, info ctr.ContainerInfo) (*runtimeworkspace.ContainerStorageMetrics, error) {
	...
	return &runtimeworkspace.ContainerStorageMetrics{
		Path:      "/",
		UsedBytes: usedBytes,
	}, nil
}

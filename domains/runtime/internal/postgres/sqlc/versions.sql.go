...
WHERE cv.team_id = iam.memoh_current_team_id() AND s.team_id = iam.memoh_current_team_id() AND cv.container_id = $1
  AND cv.version = $2
`
...
func (q *Queries) InsertVersion(ctx context.Context, arg InsertVersionParams) (RuntimeContainerVersion, error) {
	row := q.db.QueryRow(ctx, insertVersion, arg.ContainerID, arg.SnapshotID, arg.Version)
	var i RuntimeContainerVersion
	err := row.Scan(
		&i.ID,
		&i.ContainerID,
		&i.SnapshotID,
...
SELECT COALESCE(MAX(version), 0) + 1 FROM runtime.container_versions WHERE team_id = iam.memoh_current_team_id() AND container_id = $1
`

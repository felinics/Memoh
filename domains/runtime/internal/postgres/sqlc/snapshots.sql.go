// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.31.1
// source: snapshots.sql

package sqlc

import (
	"context"

	"github.com/jackc/pgx/v5/pgtype"
)

const getSnapshotByContainerAndRuntimeName = `-- name: GetSnapshotByContainerAndRuntimeName :one
SELECT
  id,
  container_id,
  runtime_snapshot_name,
  display_name,
  parent_runtime_snapshot_name,
  snapshotter,
  source,
  created_at,
  team_id
FROM runtime.snapshots
WHERE team_id = iam.memoh_current_team_id() AND container_id = $1
  AND runtime_snapshot_name = $2
LIMIT 1
`

type GetSnapshotByContainerAndRuntimeNameParams struct {
	ContainerID         string `json:"container_id"`
	RuntimeSnapshotName string `json:"runtime_snapshot_name"`
}

func (q *Queries) GetSnapshotByContainerAndRuntimeName(ctx context.Context, arg GetSnapshotByContainerAndRuntimeNameParams) (RuntimeSnapshot, error) {
	row := q.db.QueryRow(ctx, getSnapshotByContainerAndRuntimeName, arg.ContainerID, arg.RuntimeSnapshotName)
	var i RuntimeSnapshot
	err := row.Scan(
		&i.ID,
		&i.ContainerID,
		&i.RuntimeSnapshotName,
		&i.DisplayName,
		&i.ParentRuntimeSnapshotName,
		&i.Snapshotter,
		&i.Source,
		&i.CreatedAt,
		&i.TeamID,
	)
	return i, err
}

const listSnapshotsByContainerID = `-- name: ListSnapshotsByContainerID :many
SELECT
  id,
  container_id,
  runtime_snapshot_name,
  display_name,
  parent_runtime_snapshot_name,
  snapshotter,
  source,
  created_at,
  team_id
FROM runtime.snapshots
WHERE team_id = iam.memoh_current_team_id() AND container_id = $1
ORDER BY created_at DESC
`

func (q *Queries) ListSnapshotsByContainerID(ctx context.Context, containerID string) ([]RuntimeSnapshot, error) {
	rows, err := q.db.Query(ctx, listSnapshotsByContainerID, containerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []RuntimeSnapshot
	for rows.Next() {
		var i RuntimeSnapshot
		if err := rows.Scan(
			&i.ID,
			&i.ContainerID,
			&i.RuntimeSnapshotName,
			&i.DisplayName,
			&i.ParentRuntimeSnapshotName,
			&i.Snapshotter,
			&i.Source,
			&i.CreatedAt,
			&i.TeamID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listSnapshotsWithVersionByContainerID = `-- name: ListSnapshotsWithVersionByContainerID :many
SELECT
  s.id,
  s.container_id,
  s.runtime_snapshot_name,
  s.display_name,
  s.parent_runtime_snapshot_name,
  s.snapshotter,
  s.source,
  s.created_at,
  cv.version
FROM runtime.snapshots s
LEFT JOIN runtime.container_versions cv ON cv.snapshot_id = s.id AND cv.team_id = iam.memoh_current_team_id()
WHERE s.team_id = iam.memoh_current_team_id() AND s.container_id = $1
ORDER BY s.created_at DESC
`

type ListSnapshotsWithVersionByContainerIDRow struct {
	ID                        pgtype.UUID        `json:"id"`
	ContainerID               string             `json:"container_id"`
	RuntimeSnapshotName       string             `json:"runtime_snapshot_name"`
	DisplayName               pgtype.Text        `json:"display_name"`
	ParentRuntimeSnapshotName pgtype.Text        `json:"parent_runtime_snapshot_name"`
	Snapshotter               string             `json:"snapshotter"`
	Source                    string             `json:"source"`
	CreatedAt                 pgtype.Timestamptz `json:"created_at"`
	Version                   pgtype.Int4        `json:"version"`
}

func (q *Queries) ListSnapshotsWithVersionByContainerID(ctx context.Context, containerID string) ([]ListSnapshotsWithVersionByContainerIDRow, error) {
	rows, err := q.db.Query(ctx, listSnapshotsWithVersionByContainerID, containerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListSnapshotsWithVersionByContainerIDRow
	for rows.Next() {
		var i ListSnapshotsWithVersionByContainerIDRow
		if err := rows.Scan(
			&i.ID,
			&i.ContainerID,
			&i.RuntimeSnapshotName,
			&i.DisplayName,
			&i.ParentRuntimeSnapshotName,
			&i.Snapshotter,
			&i.Source,
			&i.CreatedAt,
			&i.Version,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const upsertSnapshot = `-- name: UpsertSnapshot :one
INSERT INTO runtime.snapshots (
  container_id,
  runtime_snapshot_name,
  display_name,
  parent_runtime_snapshot_name,
  snapshotter,
  source
)
VALUES (
  $1,
  $2,
  $3,
  $4,
  $5,
  $6
)
ON CONFLICT (team_id, container_id, runtime_snapshot_name) DO UPDATE
SET
  display_name = EXCLUDED.display_name,
  parent_runtime_snapshot_name = EXCLUDED.parent_runtime_snapshot_name,
  snapshotter = EXCLUDED.snapshotter,
  source = EXCLUDED.source
RETURNING id, container_id, runtime_snapshot_name, display_name, parent_runtime_snapshot_name, snapshotter, source, created_at, team_id
`

type UpsertSnapshotParams struct {
	ContainerID               string      `json:"container_id"`
	RuntimeSnapshotName       string      `json:"runtime_snapshot_name"`
	DisplayName               pgtype.Text `json:"display_name"`
	ParentRuntimeSnapshotName pgtype.Text `json:"parent_runtime_snapshot_name"`
	Snapshotter               string      `json:"snapshotter"`
	Source                    string      `json:"source"`
}

func (q *Queries) UpsertSnapshot(ctx context.Context, arg UpsertSnapshotParams) (RuntimeSnapshot, error) {
	row := q.db.QueryRow(ctx, upsertSnapshot,
		arg.ContainerID,
		arg.RuntimeSnapshotName,
		arg.DisplayName,
		arg.ParentRuntimeSnapshotName,
		arg.Snapshotter,
		arg.Source,
	)
	var i RuntimeSnapshot
	err := row.Scan(
		&i.ID,
		&i.ContainerID,
		&i.RuntimeSnapshotName,
		&i.DisplayName,
		&i.ParentRuntimeSnapshotName,
		&i.Snapshotter,
		&i.Source,
		&i.CreatedAt,
		&i.TeamID,
	)
	return i, err
}

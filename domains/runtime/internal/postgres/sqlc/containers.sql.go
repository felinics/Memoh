...
DELETE FROM runtime.containers WHERE team_id = iam.memoh_current_team_id() AND bot_id = $1
`
...
SELECT id, bot_id, container_id, container_name, image, status, namespace, auto_start, container_path, workspace_backend, created_at, updated_at, last_started_at, last_stopped_at, team_id FROM runtime.containers WHERE team_id = iam.memoh_current_team_id() AND bot_id = $1 ORDER BY updated_at DESC LIMIT 1
`

func (q *Queries) GetContainerByBotID(ctx context.Context, botID pgtype.UUID) (RuntimeContainer, error) {
	row := q.db.QueryRow(ctx, getContainerByBotID, botID)
	var i RuntimeContainer
	err := row.Scan(
		&i.ID,
		&i.BotID,
		&i.ContainerID,
...
SELECT id, bot_id, container_id, container_name, image, status, namespace, auto_start, container_path, workspace_backend, created_at, updated_at, last_started_at, last_stopped_at, team_id FROM runtime.containers WHERE team_id = iam.memoh_current_team_id() AND auto_start = true ORDER BY updated_at DESC
`

func (q *Queries) ListAutoStartContainers(ctx context.Context) ([]RuntimeContainer, error) {
	rows, err := q.db.Query(ctx, listAutoStartContainers)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []RuntimeContainer
	for rows.Next() {
		var i RuntimeContainer
		if err := rows.Scan(
			&i.ID,
...
WHERE team_id = iam.memoh_current_team_id() AND bot_id = $2
`

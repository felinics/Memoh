...
// Options configures the containerd runtime backend.
type Options struct {
	Namespace    string
	RuntimeType  string
	CNIBinaryDir string
...
func NewService(log *slog.Logger, client *containerd.Client, opts Options) *DefaultService {
	return NewDefaultService(log, client, opts)
}

func NewDefaultService(log *slog.Logger, client *containerd.Client, opts Options) *DefaultService {
	namespace := opts.Namespace
	if namespace == "" {
		namespace = containerapi.DefaultNamespace
	}
	cniBinDir := opts.CNIBinaryDir
	if cniBinDir == "" {
		cniBinDir = config.DefaultCNIBinaryDir
	}
	cniConfDir := opts.CNIConfigDir
	if cniConfDir == "" {
		cniConfDir = config.DefaultCNIConfigDir
	}
	runtimeType := strings.TrimSpace(opts.RuntimeType)
	if runtimeType == "" {
		runtimeType = config.DefaultContainerdRuntimeType
	}
...
func (s *DefaultService) PullImage(ctx context.Context, ref string, opts *containerapi.PullImageOptions) (containerapi.ImageInfo, error) {
	if ref == "" {
		...
		go func() {
			...
			for {
				...
				case <-ticker.C:
					...
					layers := make([]containerapi.LayerStatus, len(statuses))
					for i, st := range statuses {
						layers[i] = containerapi.LayerStatus{
							Ref:    st.Ref,
							Offset: st.Offset,
							Total:  st.Total,
						}
					}
					opts.OnProgress(containerapi.PullProgress{Layers: layers})
				}
			}
		}()
		defer close(stop)
	...
	if err != nil {
		return containerapi.ImageInfo{}, mapContainerdErr(err)
	}
	return toImageInfo(img), nil
}

func (s *DefaultService) GetImage(ctx context.Context, ref string) (containerapi.ImageInfo, error) {
	if ref == "" {
		return containerapi.ImageInfo{}, containerapi.ErrInvalidArgument
	}
	ctx = s.withNamespace(ctx)
...
func (s *DefaultService) ListImages(ctx context.Context) ([]containerapi.ImageInfo, error) {
	ctx = s.withNamespace(ctx)
	imgs, err := s.client.ListImages(ctx)
	if err != nil {
		return nil, mapContainerdErr(err)
	}
	result := make([]containerapi.ImageInfo, len(imgs))
...
func (s *DefaultService) DeleteImage(ctx context.Context, ref string, opts *containerapi.DeleteImageOptions) error {
	if ref == "" {
...
func specOptsFromSpec(spec containerapi.ContainerSpec) []oci.SpecOpts {
...
func specOptsFromResourceLimits(limits containerapi.ResourceLimits) []oci.SpecOpts {
	var opts []oci.SpecOpts
...
func networkJoinTargetValue(spec containerapi.ContainerSpec) string {
...
func (s *DefaultService) CreateContainer(ctx context.Context, req containerapi.CreateContainerRequest) (containerapi.ContainerInfo, error) {
	...
	if err != nil {
		return containerapi.ContainerInfo{}, mapContainerdErr(err)
	}
	defer func() { _ = done(ctx) }()
	image, err := s.getImageWithFallback(ctx, req.ImageRef)
	if err != nil {
		pullOpts := &containerapi.PullImageOptions{Unpack: true, StorageDriver: req.StorageRef.Driver}
	...
	if snapshotter != "" {
		...
		if !ok {
			return containerapi.ContainerInfo{}, fmt.Errorf("parent snapshot %s does not exist", parent)
		}
...
func (s *DefaultService) snapshotExists(ctx context.Context, snapshotter, key string) (bool, error) {
	if snapshotter == "" || key == "" {
		return false, containerapi.ErrInvalidArgument
	}
...
func (s *DefaultService) GetContainer(ctx context.Context, id string) (containerapi.ContainerInfo, error) {
	if id == "" {
		return containerapi.ContainerInfo{}, containerapi.ErrInvalidArgument
	}
	ctx = s.withNamespace(ctx)
...
func (s *DefaultService) ListContainers(ctx context.Context) ([]containerapi.ContainerInfo, error) {
	ctx = s.withNamespace(ctx)
	ctrs, err := s.client.Containers(ctx)
	if err != nil {
		return nil, mapContainerdErr(err)
	}
	result := make([]containerapi.ContainerInfo, 0, len(ctrs))
	for _, c := range ctrs {
...
func (s *DefaultService) DeleteContainer(ctx context.Context, id string, opts *containerapi.DeleteContainerOptions) error {
	if id == "" {
		return containerapi.ErrInvalidArgument
	}
...
func (s *DefaultService) StartContainer(ctx context.Context, containerID string, _ *containerapi.StartTaskOptions) error {
	if containerID == "" {
		return containerapi.ErrInvalidArgument
	}
...
func (s *DefaultService) getTask(ctx context.Context, containerID string) (containerd.Task, context.Context, error) {
	if containerID == "" {
		return nil, nil, containerapi.ErrInvalidArgument
	}
...
func (s *DefaultService) GetTaskInfo(ctx context.Context, containerID string) (containerapi.TaskInfo, error) {
	task, ctx, err := s.getTask(ctx, containerID)
	if err != nil {
		return containerapi.TaskInfo{}, mapContainerdErr(err)
	}
...
func (s *DefaultService) ListTasks(ctx context.Context, opts *containerapi.ListTasksOptions) ([]containerapi.TaskInfo, error) {
	ctx = s.withNamespace(ctx)
	...
	tasks := make([]containerapi.TaskInfo, 0, len(response.Tasks))
	for _, task := range response.Tasks {
		tasks = append(tasks, containerapi.TaskInfo{
...
func (s *DefaultService) StopContainer(ctx context.Context, containerID string, opts *containerapi.StopTaskOptions) error {
	if containerID == "" {
		return containerapi.ErrInvalidArgument
	}
...
func (s *DefaultService) DeleteTask(ctx context.Context, containerID string, opts *containerapi.DeleteTaskOptions) error {
	if containerID == "" {
		return containerapi.ErrInvalidArgument
	}
...
func (s *DefaultService) ListContainersByLabel(ctx context.Context, key, value string) ([]containerapi.ContainerInfo, error) {
	if key == "" {
	...
	filtered := make([]containerapi.ContainerInfo, 0, len(containers))
...
func (s *DefaultService) CommitSnapshot(ctx context.Context, req containerapi.CommitSnapshotRequest) error {
...
func (s *DefaultService) ListSnapshots(ctx context.Context, req containerapi.ListSnapshotsRequest) ([]containerapi.SnapshotInfo, error) {
	snapshotter := strings.TrimSpace(req.Driver)
	if snapshotter == "" {
		return nil, containerapi.ErrInvalidArgument
	}
	ctx = s.withNamespace(ctx)
	var infos []containerapi.SnapshotInfo
	if err := s.client.SnapshotService(snapshotter).Walk(ctx, func(_ context.Context, info snapshots.Info) error {
		infos = append(infos, containerapi.SnapshotInfo{
			Name:    info.Name,
			Parent:  info.Parent,
...
func (s *DefaultService) PrepareSnapshot(ctx context.Context, req containerapi.PrepareSnapshotRequest) error {
...
func (s *DefaultService) RestoreContainer(ctx context.Context, req containerapi.CreateContainerRequest) (containerapi.ContainerInfo, error) {
	snapshotID := strings.TrimSpace(req.StorageRef.Key)
	snapshotter := strings.TrimSpace(req.StorageRef.Driver)
	...
	if err != nil {
		_, pullErr := s.PullImage(ctx, imageRef, &containerapi.PullImageOptions{
			Unpack:        true,
			StorageDriver: snapshotter,
		})
		if pullErr != nil {
			return containerapi.ContainerInfo{}, mapContainerdErr(pullErr)
		}
		image, err = s.getImageWithFallback(ctx, imageRef)
		if err != nil {
			return containerapi.ContainerInfo{}, mapContainerdErr(err)
...
func (s *DefaultService) SnapshotMounts(ctx context.Context, snapshotter, key string) ([]containerapi.MountInfo, error) {
	if snapshotter == "" || key == "" {
		return nil, containerapi.ErrInvalidArgument
	}
	ctx = s.withNamespace(ctx)
	...
	result := make([]containerapi.MountInfo, len(mounts))
	for i, m := range mounts {
		result[i] = containerapi.MountInfo{
			Type:    m.Type,
			Source:  m.Source,
			Options: m.Options,
		}
...
func (s *DefaultService) SnapshotUsage(ctx context.Context, snapshotter, key string) (containerapi.SnapshotUsage, error) {
	if snapshotter == "" || key == "" {
		return containerapi.SnapshotUsage{}, containerapi.ErrInvalidArgument
	}
	ctx = s.withNamespace(ctx)
	usage, err := s.client.SnapshotService(snapshotter).Usage(ctx, key)
	if err != nil {
		return containerapi.SnapshotUsage{}, mapContainerdErr(err)
	}
	result := containerapi.SnapshotUsage{}
	if usage.Size > 0 {
...
func (s *DefaultService) SetupNetwork(ctx context.Context, req containerapi.NetworkRequest) (containerapi.NetworkResult, error) {
	if req.ContainerID == "" {
	...
	if req.JoinTarget.PID == 0 {
		...
		if err != nil {
			return containerapi.NetworkResult{}, mapContainerdErr(err)
		}
		ctx = taskCtx
	...
	return containerapi.NetworkResult{IP: ip}, nil
}

func (s *DefaultService) RemoveNetwork(ctx context.Context, req containerapi.NetworkRequest) error {
	if req.ContainerID == "" {
		return containerapi.ErrInvalidArgument
	}
	if req.JoinTarget.PID == 0 {
...
func (s *DefaultService) CheckNetwork(ctx context.Context, req containerapi.NetworkRequest) error {
	if req.ContainerID == "" {
		return containerapi.ErrInvalidArgument
	}
	if req.JoinTarget.PID == 0 {
...
func toImageInfo(img containerd.Image) containerapi.ImageInfo {
	return containerapi.ImageInfo{
		Name: img.Name(),
...
func toContainerInfo(ctx context.Context, c containerd.Container) (containerapi.ContainerInfo, error) {
	info, err := c.Info(ctx)
	if err != nil {
		return containerapi.ContainerInfo{}, err
	}
	return containerapi.ContainerInfo{
		...
		Image:      info.Image,
		Labels:     info.Labels,
		StorageRef: containerapi.StorageRef{Driver: info.Snapshotter, Key: info.SnapshotKey, Kind: "snapshot"},
		Runtime:    containerapi.RuntimeInfo{Name: info.Runtime.Name},
...
func convertTaskStatus(s containerd.ProcessStatus) containerapi.TaskStatus {
	switch s {
...
func convertContainerdTaskStatus(s tasktypes.Status) containerapi.TaskStatus {
	switch s {

...
func (d *nativeDriver) listNodes(ctx context.Context, botID string) ([]netctl.NodeOption, error) {
	...
	selectedValue := config.String(d.config.Config, "exit_node")
	...
	for peerKey, peer := range localStatus.Peer {
		...
		value := config.FirstNonEmpty(append(addresses, strings.TrimSuffix(strings.TrimSpace(peer.DNSName), "."), strings.TrimSpace(peer.HostName), strings.TrimSpace(peer.ID), strings.TrimSpace(peerKey))...)
		displayName := config.FirstNonEmpty(strings.TrimSpace(peer.HostName), strings.TrimSuffix(strings.TrimSpace(peer.DNSName), "."), value)
		items = append(items, netctl.NodeOption{
			ID:          config.FirstNonEmpty(strings.TrimSpace(peer.ID), strings.TrimSpace(peerKey), value),
			Value:       value,

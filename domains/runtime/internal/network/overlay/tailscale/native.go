...
import (
	...
	config "github.com/memohai/memoh/domains/runtime/internal/network/overlay/config"
...
func (d *nativeDriver) buildSpec(req netctl.AttachmentRequest) (sidecar.Spec, error) {
	cfg := d.config.Config
	authKey := config.String(cfg, "auth_key")
	userspace := config.Bool(cfg, "userspace")
	...
	if hostname := config.FirstNonEmpty(config.String(cfg, "hostname"), req.BotID); hostname != "" {
		env = append(env, "TS_HOSTNAME="+hostname)
	}
	if routes := config.String(cfg, "advertise_routes"); routes != "" {
		env = append(env, "TS_ROUTES="+routes)
	}
	if config.Bool(cfg, "accept_dns", true) {
		env = append(env, "TS_ACCEPT_DNS=true")
	}
	extraArgs := make([]string, 0, 4)
	if controlURL := config.String(cfg, "control_url"); controlURL != "" {
		extraArgs = append(extraArgs, "--login-server="+controlURL)
	}
	extraArgs = append(extraArgs, "--accept-routes="+strconv.FormatBool(config.Bool(cfg, "accept_routes")))
	exitNode := config.String(req.Overlay.Config, "exit_node")
	if exitNode != "" {
		extraArgs = append(extraArgs, "--exit-node="+exitNode)
	}
	if more := strings.TrimSpace(config.String(cfg, "extra_args")); more != "" {
	...
	if config.Bool(cfg, "socks5_enabled") {
		port := config.Int(cfg, "socks5_port", 1055)
		env = append(env, "TS_SOCKS5_SERVER=:"+strconv.Itoa(port))
		proxyAddress = "socks5://127.0.0.1:" + strconv.Itoa(port)
	}
	if config.Bool(cfg, "http_proxy_enabled") {
		port := config.Int(cfg, "http_proxy_port", 1056)

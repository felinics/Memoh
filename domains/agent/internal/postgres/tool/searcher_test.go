...
func TestSearcherMapsFilterAndRows(t *testing.T) {
	...
	results, err := searcher.SearchHistory(context.Background(), toolpersistence.HistorySearchFilter{
		BotID:     botID,
		SessionID: sessionID,
...
func TestSearcherRejectsInvalidBotID(t *testing.T) {
	...
	queries := &fakeQueries{}
	_, err := NewSearcher(queries).SearchHistory(context.Background(), toolpersistence.HistorySearchFilter{
		BotID: "not-a-uuid",
		Limit: 50,
	})

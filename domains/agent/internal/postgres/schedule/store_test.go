...
type botReaderStub struct {
	getBot func(context.Context, string) (schedulepersistence.BotRecord, error)
}

func (s botReaderStub) GetBot(ctx context.Context, id string) (schedulepersistence.BotRecord, error) {
...
func TestStoreCreateMapsCommandAndRecord(t *testing.T) {
	...
	record, err := store.Create(t.Context(), schedulepersistence.CreateCommand{
...
func TestStoreMapsNotFoundAndRejectsInvalidUUID(t *testing.T) {
	...
	if _, err := store.Get(t.Context(), testScheduleID); !errors.Is(err, schedulepersistence.ErrNotFound) {
...
func TestStoreMapsBotAndEnabledSchedules(t *testing.T) {
	...
	}, botReaderStub{getBot: func(_ context.Context, id string) (schedulepersistence.BotRecord, error) {
		botCalled = true
		if id != testBotID {
			t.Fatalf("bot id = %s", id)
		}
		return schedulepersistence.BotRecord{OwnerUserID: testOwnerID, Timezone: "Asia/Tokyo"}, nil
	}})
...
func TestStoreMapsLogCommandsAndRows(t *testing.T) {
	...
	logID, err := store.CreateLog(t.Context(), schedulepersistence.CreateLogCommand{ScheduleID: testScheduleID, BotID: testBotID, SessionID: testSessionID})
	if err != nil || logID != testLogID {
		t.Fatalf("CreateLog() = %q, %v", logID, err)
	}
	if err := store.CompleteLog(t.Context(), schedulepersistence.CompleteLogCommand{ID: testLogID, Status: "ok", Usage: []byte(`{"tokens":2}`), ModelID: testModelID}); err != nil {
		t.Fatalf("CompleteLog() error = %v", err)
	}
	rows, err := store.ListLogsBySchedule(t.Context(), schedulepersistence.LogPage{ID: testScheduleID, Limit: 20, Offset: 40})

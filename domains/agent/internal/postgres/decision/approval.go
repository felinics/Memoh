...
func (s *approvalStore) Create(ctx context.Context, in approval.CreateRecordInput) (approval.Record, error) {
	botID, err := db.ParseUUID(in.BotID)
	if err != nil {
		return approval.Record{}, err
	}
	sessionID, err := db.ParseUUID(in.SessionID)
	if err != nil {
		return approval.Record{}, err
	}
...
func (s *approvalStore) CancelPendingBySession(ctx context.Context, in approval.CancelSessionInput) ([]approval.Record, error) {
	bot, err := db.ParseUUID(in.BotID)
	if err != nil {
		return nil, err
	}
	session, err := db.ParseUUID(in.SessionID)
	if err != nil {
		return nil, err
	}
...
func approvalRecord(row agentsqlc.AgentToolApprovalRequest) approval.Record {
	return approval.Record{
		...
		RequestedByChannelIdentityID: db.UUIDString(row.RequestedByChannelIdentityID),
		DecidedByChannelIdentityID:   db.UUIDString(row.DecidedByChannelIdentityID),
		RequestedMessageID:           db.UUIDString(row.RequestedMessageID),
		PromptMessageID:              db.UUIDString(row.PromptMessageID),

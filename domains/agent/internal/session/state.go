...
func runtimeDeltaForAgentEvent(event agentdomain.StreamEvent, messages []chatview.UIMessage) (RuntimeDelta, bool) {
	switch event.Type {
	case agentdomain.AgentEnd, agentdomain.AgentAbort, agentdomain.Error:
		return RuntimeDelta{MessageUpserts: append([]chatview.UIMessage(nil), messages...)}, true
	case agentdomain.Retry:
		return RuntimeDelta{ResetMessages: true}, true
	case agentdomain.TextDelta, agentdomain.ReasoningDelta:
	...
	case agentdomain.ToolCallProgress:
		if len(messages) == 0 {
			return RuntimeDelta{}, false
		}
		message := messages[len(messages)-1]

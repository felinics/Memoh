...
func (m *Manager) applySteerCommand(ctx context.Context, cmd Command) {
	...
	if ctrl.injectCh != nil && strings.TrimSpace(cmd.Text) != "" {
		...
		if err != nil {
			m.logger.WarnContext(ctx, "acknowledge queued steer failed", slog.Any("error", err), slog.String("stream_id", cmd.StreamID))
			return
		}
		if !queued {
			return
		}
		sent, sendError := ctrl.sendInject(ctx, agentdomain.InjectMessage{
			Text: strings.TrimSpace(cmd.Text),
			Applied: func() {
				if err := m.updateSteerStatus(context.WithoutCancel(ctx), handle, cmd.SteerID, SteerStatusApplied, ""); err != nil {
					m.logger.WarnContext(ctx, "acknowledge applied steer failed", slog.Any("error", err), slog.String("stream_id", cmd.StreamID))
				}
			},
		})
		if sent {
	...
	if err := m.updateSteerStatus(context.WithoutCancel(ctx), handle, cmd.SteerID, SteerStatusRejected, errText); err != nil {
		m.logger.WarnContext(ctx, "update steer status failed", slog.Any("error", err), slog.String("stream_id", cmd.StreamID))
	}
}
...
func (m *Manager) steerCommandIsPending(ctx context.Context, cmd Command) bool {
	...
	if err != nil {
		m.logger.WarnContext(ctx, "load steer state failed", slog.Any("error", err), slog.String("stream_id", cmd.StreamID))
		return false
	}
...
func (m *Manager) rejectPendingSteerAfterTimeout(ctx context.Context, handle RunHandle, steerID string) {
	...
	time.AfterFunc(timeout, func() {
		...
		if err != nil {
			m.logger.WarnContext(ctx, "reject pending steer failed", slog.Any("error", err), slog.String("stream_id", handle.StreamID))
		}
	})
}

...
func runRedisDurableCommandResultContract(t *testing.T, redisURL string) {
	...
	if err := owner.StartRun(context.Background(), testBotID, sessionID, streamID, make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start run: %v", err)
	}
	if _, err := owner.HandleAgentEvent(context.Background(), requireRunHandle(t, owner, testBotID, sessionID, streamID), agentdomain.StreamEvent{
		Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-durable-result", ApprovalID: approvalID, Status: "pending",
	}); err != nil {
	...
	if _, err := owner.HandleAgentEvent(context.Background(), requireRunHandle(t, owner, testBotID, sessionID, streamID), agentdomain.StreamEvent{
		Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-durable-result-barrier", ApprovalID: barrierApprovalID, Status: "pending",
	}); err != nil {
	...
	if _, err := owner.HandleAgentEvent(context.Background(), requireRunHandle(t, owner, testBotID, sessionID, streamID), agentdomain.StreamEvent{
		Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-durable-result-local-owner", ApprovalID: localApprovalID, Status: "pending",
	}); err != nil {
	...
	if _, err := owner.HandleAgentEvent(context.Background(), requireRunHandle(t, owner, testBotID, sessionID, streamID), agentdomain.StreamEvent{
		Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-durable-result-local-barrier", ApprovalID: localBarrierApprovalID, Status: "pending",
	}); err != nil {
...
func runRedisBoundedCommandWorkersContract(t *testing.T, redisURL string) {
	...
	for i := range commandCount {
		...
		if _, err := owner.HandleAgentEvent(context.Background(), requireRunHandle(t, owner, testBotID, sessionID, streamID), agentdomain.StreamEvent{
			Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: fmt.Sprintf("call-bounded-worker-%d", i), ApprovalID: approvalID, Status: "pending",
		}); err != nil {
...
func runRedisExpiredActiveResponseTransportContract(t *testing.T, redisURL string) {
	...
	if err := owner.StartRun(context.Background(), testBotID, "session-expired-command", "stream-expired-command", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start run: %v", err)
	}
	for _, event := range []agentdomain.StreamEvent{
		{Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-expired-approval", ApprovalID: "approval-expired", Status: "pending"},
		{Type: agentdomain.UserInputRequest, ToolName: "ask_user", ToolCallID: "call-expired-input", UserInputID: "input-expired", Status: "pending"},
	} {
...
func runRedisSubscriptionReconnectContract(t *testing.T, redisURL string) {
	...
	ownerBackend.arm()
	if _, err := owner.HandleAgentEvent(context.Background(), requireRunHandle(t, owner, testBotID, "session-reconnect", "stream-reconnect"), agentdomain.StreamEvent{Type: agentdomain.TextDelta, Delta: "during disconnect"}); err != nil {
	...
	if _, err := owner.HandleAgentEvent(context.Background(), requireRunHandle(t, owner, testBotID, "session-reconnect", "stream-reconnect"), agentdomain.StreamEvent{Type: agentdomain.TextDelta, Delta: "after reconnect"}); err != nil {

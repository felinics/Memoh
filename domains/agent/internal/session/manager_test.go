...
type overflowObservingBackend struct {
	Backend
	overflow chan Event
	// gate blocks the forwarder before it drains the backend subscription, so a
	// test can fill the subscriber buffer past capacity and make the overflow
	// deterministic. A forwarder that keeps draining hands slots back to the
	// producer, which makes "did it overflow?" a scheduling race.
	gate chan struct{}
}
...
func richRuntimeAgentScript() []agentdomain.StreamEvent {
	return []agentdomain.StreamEvent{
		{Type: agentdomain.AgentStart},
		{Type: agentdomain.ReasoningDelta, Delta: "I need to inspect the workspace."},
		{Type: agentdomain.TextDelta, Delta: "I will check the current state."},
		{Type: agentdomain.ToolCallStart, ToolName: "exec", ToolCallID: "call-exec", Input: map[string]any{"command": "pwd"}},
		{Type: agentdomain.ToolCallProgress, ToolName: "exec", ToolCallID: "call-exec", Progress: "queued"},
		{Type: agentdomain.ToolCallEnd, ToolName: "exec", ToolCallID: "call-exec", Result: map[string]any{"stdout": "/workspace\n"}},
		{Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-approval", Input: map[string]any{"command": "rm -rf build"}, ApprovalID: "approval-1", ShortID: 7, Status: "pending"},
		{Type: agentdomain.UserInputRequest, ToolName: "ask_user", ToolCallID: "call-ask", Input: map[string]any{"questions": []any{map[string]any{"text": "Continue?", "kind": "single_select"}}}, UserInputID: "input-1", ShortID: 8, Status: "pending", Metadata: map[string]any{"ui_payload": map[string]any{"version": 2, "questions": []any{map[string]any{"id": "q1", "text": "Continue?", "kind": "single_select"}}}}},
	}
}
...
func TestFinishRunSerializesInjectSendAndClose(t *testing.T) {
	...
	for i := range 100 {
		...
		injectCh := make(chan agentdomain.InjectMessage, 1)
...
func waitInjectChannelClosed(t *testing.T, injectCh <-chan agentdomain.InjectMessage) {
	t.Helper()
...
func TestRuntimeManagerRejectsRunAfterClose(t *testing.T) {
	...
	err := manager.StartRun(
		...
		make(chan struct{}, 1),
		func() {},
		make(chan agentdomain.InjectMessage, 1),
	)
...
func TestRuntimeManagerDoesNotTerminalizeAdmissionRejectedByClose(t *testing.T) {
	...
	go func() {
		startDone <- manager.StartRun(
			...
			make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1),
		)
	}()
	<-backend.entered
...
func runManagerDoesNotRetryFinishAfterOwnershipLoss(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := manager.StartRun(context.Background(), testBotID, testSessionID, "stream-expired-finish", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
...
func TestLocalRuntimeDoesNotUseOwnerLeases(t *testing.T) {
	...
	if err := manager.StartRun(context.Background(), testBotID, "session-local-no-lease", "stream-local-no-lease", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
	...
	if _, err := manager.HandleAgentEvent(context.Background(), handle, agentdomain.StreamEvent{Type: agentdomain.TextDelta, Delta: "still running"}); err != nil {
...
func runRuntimeManagerScopesIdenticalStreamIDsContract(t *testing.T, suite runtimeBackendContractSuite) {
	...
	for _, sessionID := range []string{"session-scope-a", "session-scope-b"} {
		...
		if err := manager.StartRun(context.Background(), testBotID, sessionID, streamID, abortCh, func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
...
func runRuntimeManagerDetectsFirstDeltaGapContract(t *testing.T, suite runtimeBackendContractSuite) {
	...
	if err := manager.StartRun(context.Background(), testBotID, sessionID, "stream-first-gap", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start run: %v", err)
	}
...
func TestRuntimeManagerDropsGapWhenSnapshotIsBehind(t *testing.T) {
	...
	if err := manager.StartRun(context.Background(), testBotID, sessionID, "stream-uncovered-gap", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start run: %v", err)
	}
	sub, err := manager.Subscribe(context.Background(), testBotID, sessionID)
	if err != nil {
...
func runRuntimeManagerFencesDelayedOwnerMutationsContract(t *testing.T, suite runtimeBackendContractSuite) {
	...
	oldInject := make(chan agentdomain.InjectMessage, 1)
	...
	newInject := make(chan agentdomain.InjectMessage, 1)
	...
	if _, err := manager.HandleAgentEvent(context.Background(), oldHandle, agentdomain.StreamEvent{Type: agentdomain.TextDelta, Delta: "late output"}); !errors.Is(err, ErrRunOwnershipLost) {
...
func runRuntimeManagerReconcilesResponseAfterOwnerControlLoss(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := owner.StartRun(context.Background(), testBotID, testSessionID, "stream-response-reconcile", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start response run: %v", err)
	}
	handle := requireRunHandle(t, owner, testBotID, testSessionID, "stream-response-reconcile")
	if _, err := owner.HandleAgentEvent(context.Background(), handle, agentdomain.StreamEvent{
		Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-response-reconcile",
...
func runRuntimeManagerCloseDeadlineBoundsLeaseRenewal(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := manager.StartRun(context.Background(), testBotID, testSessionID, "stream-close-deadline", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start run: %v", err)
	}
	select {
	case <-backend.started:
...
func runRuntimeManagerExpiredCleanupScopesLocalControlToGeneration(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	oldHandle, err := manager.StartRunHandle(context.Background(), testBotID, testSessionID, "stream-reused-after-expiry", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1))
	if err != nil {
...
func runRuntimeManagerRejectsValidationAfterLocalLeaseDeadline(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	handle, err := manager.StartRunHandle(context.Background(), testBotID, testSessionID, "stream-delayed-validation", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1))
	if err != nil {
		t.Fatalf("start run: %v", err)
	}
...
func runRuntimeManagerScopesStreamRefCleanupToGeneration(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := manager.StartRun(context.Background(), testBotID, testSessionID, streamID, make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start first generation: %v", err)
	}
...
func runRuntimeManagerRejectsDelayedOldGenerationCommand(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	newAbort := make(chan struct{}, 1)
	if err := manager.StartRun(context.Background(), testBotID, testSessionID, streamID, newAbort, func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start second generation: %v", err)
	}
...
func runRuntimeManagerRejectsExpiredLeaseRevivalContract(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := manager.StartRun(context.Background(), testBotID, testSessionID, "stream-expired-revival", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start run: %v", err)
	}
...
func runRuntimeManagerKeepsHookAuthorityForUserAbort(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := owner.StartRunWithAdmissionBuilderAndOwnership(context.Background(), testBotID, testSessionID, testStreamID, func(context.Context) (RunAdmissionView, error) {
		return RunAdmissionView{}, nil
	}, revokeAuthority, make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start run: %v", err)
	}
...
func runRuntimeManagerRetriesTerminalUpdateBeforeDroppingOwnerRoute(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := manager.StartRun(context.Background(), testBotID, testSessionID, "stream-finish-retry", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start run: %v", err)
	}
	const secondSessionID = "session-finish-retry-second"
	if err := manager.StartRun(context.Background(), testBotID, secondSessionID, "stream-finish-retry", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
...
func runRuntimeManagerDoesNotBuildRejectedOperationContract(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := owner.StartRun(context.Background(), testBotID, testSessionID, "stream-admitted", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
...
func runRuntimeManagerRejectsQueuedSteerOnAgentTerminalContract(t *testing.T, suite runtimeBackendContractSuite) {
	...
	for _, tc := range []struct {
		name       string
		event      agentdomain.StreamEvent
		wantStatus string
	}{
		{name: "end", event: agentdomain.StreamEvent{Type: agentdomain.AgentEnd}, wantStatus: RunStatusCompleted},
		{name: "abort", event: agentdomain.StreamEvent{Type: agentdomain.AgentAbort}, wantStatus: RunStatusAborted},
	} {
...
func TestRuntimeManagerPublishesBoundedTextAppendPayloads(t *testing.T) {
	...
	defer sub.Close()
	if err := manager.StartRun(context.Background(), testBotID, "session-bounded-delta", "stream-bounded-delta", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start run: %v", err)
	}
	...
	for i := 0; i < 32; i++ {
		if _, err := manager.HandleAgentEvent(context.Background(), requireRunHandle(t, manager, testBotID, "session-bounded-delta", "stream-bounded-delta"), agentdomain.StreamEvent{
			Type:  agentdomain.TextDelta,
			Delta: chunk,
		}); err != nil {
...
func runRuntimeManagerRoutesAbortAndSteerAcrossManagersContract(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	steerInjectCh := make(chan agentdomain.InjectMessage, 1)
...
func runRuntimeManagerRoutesActiveResponsesAcrossManagersContract(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := owner.StartRun(context.Background(), testBotID, testSessionID, testStreamID, make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start response run: %v", err)
	}
	for _, event := range []agentdomain.StreamEvent{
		{Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-approval", ApprovalID: "approval-1", Status: "pending"},
		{Type: agentdomain.UserInputRequest, ToolName: "ask_user", ToolCallID: "call-input", UserInputID: "input-1", Status: "pending"},
	} {
...
func runRuntimeManagerPreservesRemoteCommandDeadlineError(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := owner.StartRun(context.Background(), testBotID, "session-deadline", "stream-deadline", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start deadline run: %v", err)
	}
	if _, err := owner.HandleAgentEvent(context.Background(), requireRunHandle(t, owner, testBotID, "session-deadline", "stream-deadline"), agentdomain.StreamEvent{
		Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-deadline",
...
func runRuntimeManagerPreservesOwnershipDeadlineError(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := owner.StartRun(context.Background(), testBotID, "session-ownership-deadline", "stream-ownership-deadline", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start ownership deadline run: %v", err)
	}
	if _, err := owner.HandleAgentEvent(context.Background(), requireRunHandle(t, owner, testBotID, "session-ownership-deadline", "stream-ownership-deadline"), agentdomain.StreamEvent{
		Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-ownership-deadline",
...
func runRuntimeManagerAcknowledgesAppliedResponseAfterFinish(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if _, err := owner.HandleAgentEvent(context.Background(), handle, agentdomain.StreamEvent{
		Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-applied",
...
func runRuntimeManagerCommandRoutingOutlivesStartContext(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	abortCh := make(chan struct{}, 1)
	if err := owner.StartRun(context.Background(), testBotID, testSessionID, testStreamID, abortCh, func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
...
func runRuntimeManagerCancelsActiveResponseOnFinish(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	handle := requireRunHandle(t, owner, testBotID, testSessionID, testStreamID)
	if _, err := owner.HandleAgentEvent(context.Background(), handle, agentdomain.StreamEvent{
		Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-finish",
...
func runRuntimeManagerExpiresActiveResponseHandlers(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := owner.StartRun(context.Background(), testBotID, "session-response-expiry", "stream-response-expiry", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start response run: %v", err)
	}
	for _, event := range []agentdomain.StreamEvent{
		{Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-expiry-approval", ApprovalID: "approval-expiry", Status: "pending"},
		{Type: agentdomain.UserInputRequest, ToolName: "ask_user", ToolCallID: "call-expiry-input", UserInputID: "input-expiry", Status: "pending"},
	} {
...
func runRuntimeManagerCancelsActiveResponseOnClose(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if _, err := manager.HandleAgentEvent(context.Background(), requireRunHandle(t, manager, testBotID, "session-response-close", "stream-response-close"), agentdomain.StreamEvent{
		Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-close",
...
func runRuntimeManagerAbortCommandDoesNotRepublishStaleSelfReference(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := manager.StartRun(context.Background(), testBotID, testSessionID, testStreamID, make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
...
func runRuntimeManagerFencesStaleOwnerEventsContract(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := oldOwner.StartRun(context.Background(), testBotID, testSessionID, "stream-stale", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
	...
	if err := newOwner.StartRun(context.Background(), testBotID, testSessionID, "stream-current", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
		t.Fatalf("start replacement run: %v", err)
	}

	messages, err := oldOwner.HandleAgentEvent(context.Background(), requireRunHandle(t, oldOwner, testBotID, testSessionID, "stream-stale"), agentdomain.StreamEvent{
		Type:  agentdomain.TextDelta,
...
func runRuntimeManagerSignalsSubscriberOverflowContract(t *testing.T, suite runtimeBackendContractSuite) {
	...
	defer sub.Close()
	// The baseline snapshot is queued by Subscribe itself, so it arrives while
	// the forwarder is still gated.
	baseline := waitRuntimeEvent(t, sub.C, func(event Event) bool { return event.Type == EventRuntimeSnapshot })
	...
	// Every event above landed in the backend's 64-slot subscriber buffer while
	// the forwarder was gated, so the overflow has already happened. Releasing
	// the forwarder now delivers it, instead of racing the producer against the
	// consumer and hoping the buffer wins.
	close(backend.gate)
	var recovered Event
...
func runRuntimeManagerRetriesDroppedCommandContract(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	abortCh := make(chan struct{}, 1)
	if err := owner.StartRun(context.Background(), testBotID, testSessionID, "stream-command-retry", abortCh, func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
...
func runRuntimeManagerReleasesPendingCommandOnClose(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	handle, err := owner.StartRunHandle(context.Background(), testBotID, testSessionID, testStreamID, make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1))
	if err != nil {
		t.Fatalf("start run: %v", err)
	}
	if _, err := owner.HandleAgentEvent(context.Background(), handle, agentdomain.StreamEvent{
		Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-pending-close",
...
func runRuntimeManagerReleasesOwnedRunOnClose(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	if err := owner.StartRun(context.Background(), testBotID, "session-graceful-release", "stream-graceful-release", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
	...
	if err := remote.StartRun(context.Background(), testBotID, "session-graceful-release", "stream-after-graceful-release", make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
...
func runRuntimeManagerDoesNotBlockCommandResultsBehindSlowHandlers(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	startApprovalRun := func(manager *Manager, sessionID, streamID, approvalID string) {
		t.Helper()
		if err := manager.StartRun(context.Background(), testBotID, sessionID, streamID, make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
			t.Fatalf("start %s: %v", streamID, err)
		}
		handle := requireRunHandle(t, manager, testBotID, sessionID, streamID)
		if _, err := manager.HandleAgentEvent(context.Background(), handle, agentdomain.StreamEvent{
			Type: agentdomain.ToolApprovalRequest, ToolName: "exec", ToolCallID: "call-" + approvalID, ApprovalID: approvalID, Status: "pending",
		}); err != nil {

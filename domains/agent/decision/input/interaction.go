...
func (s *Service) AdvanceInteraction(ctx context.Context, input AdvanceInteractionInput) (AdvanceInteractionResult, error) {
	if s == nil || s.persistence == nil {
		return AdvanceInteractionResult{}, errors.New("user input persistence not configured")
	}
...
func (s *Service) persistInteraction(ctx context.Context, req Request, state TextInteractionState) (Request, bool, error) {
	...
	row, err := s.persistence.UpdateInteraction(ctx, InteractionRecordInput{
		InteractionJSON:     stateJSON,
		ID:                  req.ID,
		InteractionRevision: req.InteractionRevision,
	})
	if err == nil {
...
func applySelectOption(payload UIPayload, state TextInteractionState, op InteractionOp) (TextInteractionState, InteractionOutcome) {
	...
	state.Answers = putTextAnswer(state.Answers, agentdomain.QuestionAnswer{QuestionID: question.ID, OptionIDs: []string{optionID}})
...
func applySetText(payload UIPayload, state TextInteractionState, op InteractionOp) (TextInteractionState, InteractionOutcome) {
	...
	case QuestionKindText:
		state.Answers = putTextAnswer(state.Answers, agentdomain.QuestionAnswer{QuestionID: question.ID, Text: text})
	case QuestionKindSingleSelect:
		if !question.AllowCustom {
			return state, InteractionOutcome{Reject: RejectCustomNotAllowed}
		}
		state.Answers = putTextAnswer(state.Answers, agentdomain.QuestionAnswer{QuestionID: question.ID, CustomText: text})
...
func fillSkippedAnswers(payload UIPayload, answers []agentdomain.QuestionAnswer) []agentdomain.QuestionAnswer {
	...
	for _, question := range payload.Questions {
		if _, ok := have[question.ID]; !ok {
			out = append(out, agentdomain.QuestionAnswer{QuestionID: question.ID, Skipped: true})
		}
	}
	return out
}

func removeTextAnswer(answers []agentdomain.QuestionAnswer, questionID string) []agentdomain.QuestionAnswer {
	out := make([]agentdomain.QuestionAnswer, 0, len(answers))

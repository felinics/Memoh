...
func (s *Source) ListContacts(ctx context.Context, botID string) ([]delivery.Contact, error) {
	routes, err := s.routes.List(ctx, botID)
	if err != nil {
		return nil, err
	}
	contacts := make([]delivery.Contact, 0, len(routes))
	for _, item := range routes {
		contacts = append(contacts, delivery.Contact{
			RouteID:                item.ID,

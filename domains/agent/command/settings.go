...
func (h *Handler) buildSettingsGroup() *CommandGroup {
	...
	g.Register(SubCommand{
		...
		ResultHandler: func(cc CommandContext) (*Result, error) {
			...
			if _, err := h.settingsService.UpsertBot(cc.Ctx, cc.BotID, setting.UpsertRequest{CommandUILanguage: v}); err != nil {
				return nil, err
			}
			s, err := h.settingsService.GetBot(cc.Ctx, cc.BotID)
			if err != nil {
...
func (h *Handler) settingsResult(cc CommandContext, s setting.Settings) *Result {
...
func (h *Handler) getBotSettings(cc CommandContext) (setting.Settings, error) {
	if h.settingsService == nil {
		return setting.Settings{}, errors.New("settings service is not available")
	}

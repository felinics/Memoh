...
// MemberRoleResolver resolves a user's role within a bots.
...
// BotMemberRoleAdapter adapts BotProfileReader to MemberRoleResolver.
type BotMemberRoleAdapter struct {
	BotService     BotProfileReader
...
func NewHandler(
	...
	skillLoader SkillLoader,
	containerFS ContainerFS,
	modelProviderResolvers ...modelcatalog.ProviderResolver,
) *Handler {
	if log == nil {
		log = slog.Default()
	}
	...
	if len(modelProviderResolvers) > 0 {
		h.modelProviderResolver = modelProviderResolvers[0]
	}
...
func (h *Handler) SetCompactionService(s *compaction.Service) {
...
func (h *Handler) ExecuteResult(ctx context.Context, input ExecuteInput) (res *Result, err error) {
	...
	// Resolve the user's role in this bots.
	role := ""

...
func (h *Handler) buildUsageGroup() *CommandGroup {
	...
	g.Register(SubCommand{
		...
		ResultHandler: func(cc CommandContext) (*Result, error) {
			...
			rows, err := h.queries.GetTokenUsageByDayAndType(cc.Ctx, cc.BotID, from, now)
			if err != nil {
				return nil, err
			}
			if len(rows) == 0 {
	...
	g.Register(SubCommand{
		...
		ResultHandler: func(cc CommandContext) (*Result, error) {
			...
			rows, err := h.queries.GetTokenUsageByModel(cc.Ctx, cc.BotID, from, now)
			if err != nil {
				return nil, err
			}
			if len(rows) == 0 {

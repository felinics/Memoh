...
import (
	...
	"github.com/memohai/memoh/domains/agent/command/syntax"
)
...
func Classify(input ClassifyInput) Decision {
	...
	invocation, err := syntax.ParseInvocation(syntax.InvocationInput{
		Text:       text,
		BotAliases: input.BotAliases,
		Directed:   input.Directed,
	})
	if err != nil {
		if input.Surface == SurfaceChannel && errors.Is(err, syntax.ErrCommandForOtherBot) {
...
func isSlashLike(text string, aliases []string) bool {
	_, err := syntax.ParseInvocation(syntax.InvocationInput{Text: text, BotAliases: aliases})
	return err == nil
}

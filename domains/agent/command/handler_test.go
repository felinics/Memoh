...
func (f *fakeCommandQueries) GetLatestSessionIDByBot(_ context.Context, _ string) (string, error) {
	return f.latestSessionID, f.latestSessionErr
}

func (f *fakeCommandQueries) CountMessagesBySession(_ context.Context, _ string) (int64, error) {
...
func (f *fakeCommandQueries) GetSessionCacheStats(_ context.Context, _ string) (CacheStats, error) {
	if f.cacheErr != nil {
		return CacheStats{}, f.cacheErr
	}
...
func (f *fakeCommandQueries) GetTokenUsageByDayAndType(_ context.Context, _ string, _, _ time.Time) ([]UsageByDay, error) {
	return f.usageByDay, nil
}

func (f *fakeCommandQueries) GetTokenUsageByModel(_ context.Context, _ string, _, _ time.Time) ([]UsageByModel, error) {
	return f.usageByModel, nil
}
...
func TestSettingsResultUsesFocusedActions(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			settings:         setting.Settings{AclDefaultEffect: "allow", HeartbeatEnabled: false},
		...
		},
		{
			...
			settings:         setting.Settings{AclDefaultEffect: "deny", HeartbeatEnabled: true},
...
func TestExecute_StatusLatest(t *testing.T) {
	...
	h := newTestHandlerWithQueries(&fakeRoleResolver{role: "owner"}, &fakeCommandQueries{
		latestSessionID: "01020304-0506-0708-0900-000000000000",
		messageCount:    42,
		latestUsage:     1200,
		cacheRow: CacheStats{
			CacheReadTokens:  300,

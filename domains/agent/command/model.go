...
func (h *Handler) buildModelGroup() *CommandGroup {
	...
	g.Register(SubCommand{
		...
		Handler: func(cc CommandContext) (string, error) {
			...
			if _, err := h.settingsService.UpsertBot(cc.Ctx, cc.BotID, setting.UpsertRequest{
	...
	g.Register(SubCommand{
		...
		Handler: func(cc CommandContext) (string, error) {
			...
			_, err = h.settingsService.UpsertBot(cc.Ctx, cc.BotID, setting.UpsertRequest{
...
func modelDisplayName(m modelcatalog.GetResponse) string {
...
func (h *Handler) findModelByProviderAndName(cc CommandContext, providerName, modelName string) (modelcatalog.GetResponse, error) {
	chatModels, err := h.selectableChatModels(cc)
	if err != nil {
		return modelcatalog.GetResponse{}, err
	}
	...
	return modelcatalog.GetResponse{}, fmt.Errorf("%s", cc.T("cmd.model.notFoundUnderProvider", map[string]any{"name": fmt.Sprintf("%q", modelName), "provider": fmt.Sprintf("%q", providerName), "command": CmdRef("model list")}))
}

func (h *Handler) findModelForSelection(cc CommandContext, args []string) (modelcatalog.GetResponse, error) {
	if h.modelsService == nil {
		return modelcatalog.GetResponse{}, fmt.Errorf("%s", cc.T("cmd.model.serviceUnavailable"))
	}
	if len(args) == 0 {
		return modelcatalog.GetResponse{}, fmt.Errorf("%s", cc.T("cmd.model.idRequired"))
	}
	if len(args) == 1 {
...
func (h *Handler) findModelByIDOrName(cc CommandContext, target string) (modelcatalog.GetResponse, error) {
	...
	matches := make([]modelcatalog.GetResponse, 0, 4)
	for _, item := range items {
	...
	switch len(matches) {
	case 0:
		return modelcatalog.GetResponse{}, fmt.Errorf("%s", cc.T("cmd.model.notFound", map[string]any{"name": fmt.Sprintf("%q", target), "command": CmdRef("model list")}))
	case 1:
	...
	default:
		...
		return modelcatalog.GetResponse{}, fmt.Errorf("%s", cc.T("cmd.model.ambiguous", map[string]any{
...
func (h *Handler) selectableChatModels(cc CommandContext) ([]modelcatalog.GetResponse, error) {
	if h.modelsService == nil {
		return nil, fmt.Errorf("%s", cc.T("cmd.model.serviceUnavailable"))
	}
	return h.modelsService.ListEnabledByType(cc.Ctx, modeldomain.ModelTypeChat)
}

func (h *Handler) filterModelsByProvider(cc CommandContext, items []modelcatalog.GetResponse, providerName string) []modelcatalog.GetResponse {
	providerName = strings.TrimSpace(providerName)
	if providerName == "" {
		return items
	}
	filtered := make([]modelcatalog.GetResponse, 0, len(items))
	for _, item := range items {
...
func modelMarkers(modelID string, settingsResp setting.Settings) []string {
	var markers []string
...
func modelSortRank(model modelcatalog.GetResponse, settingsResp setting.Settings) int {

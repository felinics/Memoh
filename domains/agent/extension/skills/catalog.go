...
func RequestedSkillContexts(resolved []ResolvedSkill) []agentdomain.RequestedSkillContext {
	out := make([]agentdomain.RequestedSkillContext, 0, len(resolved))
	for _, item := range resolved {
		out = append(out, agentdomain.RequestedSkillContext{
			Name:           item.Name,

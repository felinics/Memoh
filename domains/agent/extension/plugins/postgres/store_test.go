...
func TestStoreMapsPluginInstallations(t *testing.T) {
	...
	created, err := store.CreateInstallation(t.Context(), plugins.CreateInstallationInput{
	...
	if _, err := store.UpdateInstallationStatus(t.Context(), plugins.InstallationStatusUpdate{
...
func TestStoreMapsPluginResources(t *testing.T) {
	...
	if err := store.UpsertResource(t.Context(), plugins.ResourceUpsert{
...
func TestStoreRejectsInvalidPluginIDs(t *testing.T) {
	...
	if err := store.UpsertResource(t.Context(), plugins.ResourceUpsert{InstallationID: "not-a-uuid"}); err == nil {
...
func TestStoreMapsMissingInstallationRows(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "update",
			run: func(store *Store) error {
				_, err := store.UpdateInstallationStatus(t.Context(), plugins.InstallationStatusUpdate{

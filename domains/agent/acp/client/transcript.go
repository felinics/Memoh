...
func (b *TranscriptRecorder) Add(ev agentdomain.StreamEvent) {
	if b == nil {
		return
	}
...
func (b *TranscriptRecorder) appendToolResult(ev agentdomain.StreamEvent) {
	result := ev.Result
...
func (b *TranscriptRecorder) attachToolMetadata(ev agentdomain.StreamEvent, key string, value map[string]any) {
	toolCallID := strings.TrimSpace(ev.ToolCallID)
	toolName := strings.TrimSpace(ev.ToolName)
...
func (b *TranscriptRecorder) upsertToolCallStart(ev agentdomain.StreamEvent) {
	b.flushReasoning()
	b.flushText()
	toolCallID := strings.TrimSpace(ev.ToolCallID)
	toolName := strings.TrimSpace(ev.ToolName)
...
func userInputTranscriptMetadata(ev agentdomain.StreamEvent) map[string]any {
...
func approvalTranscriptMetadata(ev agentdomain.StreamEvent) map[string]any {
	status := strings.TrimSpace(ev.Status)
	if status == "" {
...
func TranscriptFromEvents(events []agentdomain.StreamEvent, fallbackText string) []sdk.Message {

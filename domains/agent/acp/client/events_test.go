...
func TestACPGenericExecuteToolMapsToNativeExecEvents(t *testing.T) {
	...
	if start[0].Type != agentdomain.ToolCallStart || start[0].ToolName != "exec" || start[0].ToolCallID != "call-1" {
	...
	if end[0].Type != agentdomain.ToolCallEnd || end[0].ToolName != "exec" || end[0].Error != "" {
...
func TestACPGenericExecuteCompletionWithoutStartEmitsStartThenEnd(t *testing.T) {
	...
	if events[0].Type != agentdomain.ToolCallStart || events[0].ToolName != "exec" {
		t.Fatalf("first event = %#v, want exec start", events[0])
	}
	if events[1].Type != agentdomain.ToolCallEnd || events[1].ToolName != "exec" {
...
func TestACPGenericEditWithContentMapsToNativeWriteEvents(t *testing.T) {
	...
	if start[0].Type != agentdomain.ToolCallStart || start[0].ToolName != "write" || start[0].ToolCallID != "write-1" {
	...
	if end[0].Type != agentdomain.ToolCallEnd || end[0].ToolName != "write" || end[0].Error != "" {
...
func TestACPGenericEditDiffMapsToNativeEditEvents(t *testing.T) {
	...
	if events[0].Type != agentdomain.ToolCallStart || events[0].ToolName != "edit" {
	...
	if events[1].Type != agentdomain.ToolCallEnd || events[1].ToolName != "edit" {
...
func TestACPGenericEditDiffWithoutOldTextMapsToNativeWriteEvents(t *testing.T) {
	...
	if events[0].Type != agentdomain.ToolCallStart || events[0].ToolName != "write" {
	...
	if events[1].Type != agentdomain.ToolCallEnd || events[1].ToolName != "write" {
...
func TestACPAgentThoughtMapsToReasoningDelta(t *testing.T) {
	...
	if events[0].Type != agentdomain.ReasoningDelta || events[0].Delta != "I should inspect the workspace first." {
...
func TestACPPlanMapsToReasoningDelta(t *testing.T) {
	...
	if events[0].Type != agentdomain.ReasoningDelta {
		t.Fatalf("event = %#v, want reasoning delta", events[0])
	}
...
func TestEventCollectorBoundsStoredEvents(t *testing.T) {
	...
	for i := 0; i < maxCollectedStreamEvents+10; i++ {
		collector.record(agentdomain.StreamEvent{

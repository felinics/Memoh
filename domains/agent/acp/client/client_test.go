...
func TestRunnerStartSessionStreamsEvents(t *testing.T) {
	...
	var streamedEvents []agentdomain.StreamEvent
	...
	}, EventSinkFunc(func(ev agentdomain.StreamEvent) {
		streamedMu.Lock()
		defer streamedMu.Unlock()
		streamedEvents = append(streamedEvents, ev)
		if ev.Type == agentdomain.TextDelta {
	...
	streamedEventsSnapshot := append([]agentdomain.StreamEvent(nil), streamedEvents...)
	streamedMu.Unlock()
	if !strings.Contains(streamedText, "read: hello") {
		t.Fatalf("streamed text = %q", streamedText)
	}
	for _, want := range []string{"read", "write", "exec"} {
		if !hasStreamedToolEvent(streamedEventsSnapshot, agentdomain.ToolCallEnd, want) {
			t.Fatalf("streamed events missing %s tool end: %#v", want, streamedEventsSnapshot)
		}
	}
	for _, want := range []string{"read", "write", "exec"} {
		if !hasStreamedToolEvent(result.Events, agentdomain.ToolCallEnd, want) {
			t.Fatalf("result events missing %s tool end: %#v", want, result.Events)
		}
	}
	writeEvent := findStreamedToolEvent(streamedEventsSnapshot, agentdomain.ToolCallEnd, "write")
	if writeEvent == nil {
...
func hasStreamedToolEvent(events []agentdomain.StreamEvent, typ agentdomain.StreamEventType, toolName string) bool {
	return findStreamedToolEvent(events, typ, toolName) != nil
}

func findStreamedToolEvent(events []agentdomain.StreamEvent, typ agentdomain.StreamEventType, toolName string) *agentdomain.StreamEvent {
	for i := range events {
...
func TestRequestPermissionUsesMemohToolApproval(t *testing.T) {
	...
	for i, status := range []string{toolapproval.StatusPending, toolapproval.StatusApproved} {
		if events[i].Type != agentdomain.ToolApprovalRequest ||
...
func TestRequestPermissionRejectedByMemohToolApprovalSelectsRejectOption(t *testing.T) {
	...
	for _, ev := range collector.result().Events {
		if ev.Type == agentdomain.ToolApprovalRequest &&
...
func TestACPWorkspaceEffectsRejectStaleRedisOwner(t *testing.T) {
	...
	rawOwnerBackend, err := session.NewRedisBackend(ctx, session.RedisOptions{URL: redisURL, KeyPrefix: prefix, StateTTL: time.Minute})
	if err != nil {
		t.Fatalf("create stale owner backend: %v", err)
	}
	t.Cleanup(func() { _ = rawOwnerBackend.Close() })
	owner := session.NewManager(nonClosingDistributedBackend{DistributedBackend: rawOwnerBackend}, session.Options{
	...
	backendB, err := session.NewRedisBackend(ctx, session.RedisOptions{URL: redisURL, KeyPrefix: prefix, StateTTL: time.Minute})
	if err != nil {
		t.Fatalf("create takeover backend: %v", err)
	}
	takeover := session.NewManager(backendB, session.Options{
	...
	ownerHandle, err := owner.StartRunHandle(ctx, botID, sessionID, streamA, make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1))
	if err != nil {
		t.Fatalf("start stale owner run: %v", err)
	}
	ownerSnapshot, ok, err := rawOwnerBackend.Load(ctx, session.Key{BotID: botID, SessionID: sessionID})
	...
	if err := takeover.StartRun(ctx, botID, sessionID, streamB, make(chan struct{}, 1), func() {}, make(chan agentdomain.InjectMessage, 1)); err != nil {
	...
	for name, call := range map[string]func() error{
		...
		if err := call(); !errors.Is(err, session.ErrRunOwnershipLost) {
...
func TestWriteTextFileWithoutToolSessionIsRejectedWhenApprovalEnabled(t *testing.T) {
	...
	for _, ev := range events {
		if ev.Type == agentdomain.ToolCallEnd && ev.ToolName == "write" && strings.Contains(ev.Error, "not approved") {
			sawRejectedEnd = true
		}
	}
	if !sawRejectedEnd {
...
func assertSingleApprovalWithStartEnd(t *testing.T, events []agentdomain.StreamEvent, toolCallID, toolName, approvalID string) {
	t.Helper()
...
func newTestBridgeClient(t *testing.T, root string) *bridge.Client {
	...
	bridgepb.RegisterContainerServiceServer(server, bridgeserver.New(bridgeserver.Options{
		DefaultWorkDir:    root,
		WorkspaceRoot:     root,
		DataMount:         runtimedomain.DefaultDataMount,
...
func (s *startupCancelBridgeServer) Exec(stream grpc.BidiStreamingServer[bridgepb.ExecInput, bridgepb.ExecOutput]) error {
	...
	s.mu.Unlock()
	if execNumber == 1 {
		return stream.Send(&bridgepb.ExecOutput{Stream: bridgepb.ExecOutput_EXIT, ExitCode: 0})
	}
...
func newStartupCancelBridgeClient(t *testing.T, testServer *startupCancelBridgeServer) *bridge.Client {
	...
	server := grpc.NewServer()
	bridgepb.RegisterContainerServiceServer(server, testServer)
	go func() {
		_ = server.Serve(listener)
	}()
	t.Cleanup(func() {

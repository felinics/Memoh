...
func renderScript(t *testing.T, events []agentdomain.StreamEvent) []chatview.UIMessage {
	t.Helper()
...
func acpProducedEvents(t *testing.T) []agentdomain.StreamEvent {
	t.Helper()
	events := make([]agentdomain.StreamEvent, 0, 6)
...
func nativeShapedEvents() []agentdomain.StreamEvent {
	pending := toolapproval.Request{
		ID:         "approval-1",
		ShortID:    1,
		ToolCallID: "call-1",

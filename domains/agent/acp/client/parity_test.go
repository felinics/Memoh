...
// parityApproval implements both tool.NativeToolApprovalService and
...
func (p parityToolProvider) Tools(context.Context, tool.SessionContext) ([]sdk.Tool, error) {
...
func parityNativeSource(approval tool.NativeToolApprovalService, toolEvents tool.NativeToolEventSink, toolName string, executed *bool) *tool.NativeToolSource {
	if toolEvents == nil {
		toolEvents = &parityToolEvents{deliver: true}
	}
	return tool.NewNativeToolSource(nil, []tool.ToolProvider{parityToolProvider{tool: sdk.Tool{
		Name:       toolName,

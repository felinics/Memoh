...
func newFakeScriptPoolForBot(t *testing.T, record bot.Bot) (*SessionPool, string) {
	t.Helper()
	root := t.TempDir()
	...
	pool := newSessionPool(nil, runner, fakeBotGetter{bot: record})
...
func TestPromptToolEventSinkPreservesACPAndHTTPToolEventOrder(t *testing.T) {
	...
	sink.EmitStreamEvent(agentdomain.StreamEvent{Type: agentdomain.TextDelta, Delta: "before"})
	...
	sink.EmitStreamEvent(agentdomain.StreamEvent{Type: agentdomain.TextDelta, Delta: "after"})

	events := sink.Events()
	if len(events) != 5 {
		t.Fatalf("events = %#v", events)
	}
	if events[0].Type != agentdomain.TextDelta || events[1].Type != agentdomain.ToolCallStart || events[2].Type != agentdomain.ToolApprovalRequest || events[3].Type != agentdomain.ToolCallEnd || events[4].Type != agentdomain.TextDelta {
...
func (g fakeBotGetter) Get(context.Context, string) (bot.Bot, error) {
	return g.bot, g.err
}
...
func (s *caBundleStatServer) Stat(_ context.Context, req *bridgepb.StatRequest) (*bridgepb.StatResponse, error) {
	s.mu.Lock()
	s.path = req.GetPath()
	s.mu.Unlock()
	return &bridgepb.StatResponse{Entry: &bridgepb.FileEntry{Path: filepath.Base(req.GetPath())}}, nil
}
...
func enabledACPBot(id, mode string, managed map[string]any) bot.Bot {
	return enabledACPAgentBot(id, acpprofile.AgentCodexID, mode, managed)
}

func enabledACPAgentBot(id, agentID, mode string, managed map[string]any) bot.Bot {
	if managed == nil {
...
func newSessionPoolBridgeClient(t *testing.T, root string) *bridge.Client {
	...
	bridgeServer := bridgeserver.New(bridgeserver.Options{
		DefaultWorkDir:    root,
		WorkspaceRoot:     root,
		DataMount:         runtimedomain.DefaultDataMount,
		AllowHostAbsolute: true,
	})
	bridgepb.RegisterContainerServiceServer(server, &sessionPoolBridgeServer{
		Server: bridgeServer,
		binDir: filepath.Join(root, "bin"),
	})
	go func() {
...
func (s *sessionPoolBridgeServer) Exec(stream bridgepb.ContainerService_ExecServer) error {
...
type sessionPoolExecStream struct {
	bridgepb.ContainerService_ExecServer
	binDir string
	first  bool
}

func (s *sessionPoolExecStream) Recv() (*bridgepb.ExecInput, error) {
...
func newCABundleStatClient(t *testing.T) (*bridge.Client, *caBundleStatServer) {
	...
	bridgepb.RegisterContainerServiceServer(server, statServer)
	go func() {
		_ = server.Serve(listener)
	}()
	t.Cleanup(func() {

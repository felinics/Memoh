...
type botGetter interface {
	Get(ctx context.Context, botID string) (bot.Bot, error)
}
...
func NewSessionPool(log *slog.Logger, runner *client.Runner, botService botGetter, sessionServices ...SessionDescriptorReader) *SessionPool {
...
func (p *SessionPool) startRuntime(ctx context.Context, h *runtimeHandle, opts startOptions) error {
	...
	if err != nil {
		...
		p.logger.WarnContext(ctx, "dynamic ACP adapter unavailable; falling back to bundled version",
			slog.String("bot_id", h.botID),
			slog.String("agent_id", h.agentID),
			slog.String("runtime_id", h.id),
			slog.Any("error", err))
...
func (p *SessionPool) resolveAgentSetup(ctx context.Context, botID, agentID string) (bot.Bot, acpprofile.Profile, acpprofile.AgentSetup, client.SetupMode, bridge.WorkspaceInfo, error) {
	agentID = acpprofile.NormalizeAgentID(agentID)
	profile, ok := acpprofile.Lookup(agentID)
	if !ok {
		return bot.Bot{}, acpprofile.Profile{}, acpprofile.AgentSetup{}, "", bridge.WorkspaceInfo{}, feedback.New(
	...
	record, err := p.bots.Get(ctx, botID)
	if err != nil {
		return bot.Bot{}, acpprofile.Profile{}, acpprofile.AgentSetup{}, "", bridge.WorkspaceInfo{}, fmt.Errorf("load bot ACP setup: %w", err)
	}
	setup := acpprofile.ParseAgentSetup(record.Metadata, agentID)
	if !setup.Enabled {
		return bot.Bot{}, acpprofile.Profile{}, acpprofile.AgentSetup{}, "", bridge.WorkspaceInfo{}, feedback.New(
	...
	if err != nil {
		return bot.Bot{}, acpprofile.Profile{}, acpprofile.AgentSetup{}, "", bridge.WorkspaceInfo{}, fmt.Errorf("resolve workspace: %w", err)
	}
	...
	return record, profile, setup, mode, workspaceInfo, nil
}
...
func (s *promptToolEventSink) EmitStreamEvent(ev agentdomain.StreamEvent) {
	if s == nil {
		return
	}
...
func (s *promptToolEventSink) Events() []agentdomain.StreamEvent {
	if s == nil {
		return nil
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	return append([]agentdomain.StreamEvent(nil), s.events...)
}
...
func appendBoundedPromptEvents(events []agentdomain.StreamEvent, incoming ...agentdomain.StreamEvent) []agentdomain.StreamEvent {
	if len(incoming) == 0 {
		return events
	}
	...
	return append([]agentdomain.StreamEvent(nil), events[len(events)-maxCollectedPromptToolEvents:]...)
}

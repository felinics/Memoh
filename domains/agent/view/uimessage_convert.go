...
func ConvertModelMessagesToUIAssistantMessages(messages []agentdomain.ModelMessage) []UIMessage {
...
func decodeUIModelMessage(message agentdomain.ModelMessage) uiDecodedModelMessage {
...
func decodePersistedModelMessage(raw messagepkg.Message) uiDecodedModelMessage {
	if firstJSONByte(raw.Content) != '{' {
		return uiDecodedModelMessage{ModelMessage: agentdomain.ModelMessage{
			Role:    raw.Role,
			Content: raw.Content,
		}}
	}

	var message agentdomain.ModelMessage
...
func extractTextFromPersistedContent(raw json.RawMessage) string {
	...
	decoded := decodeUIModelMessage(agentdomain.ModelMessage{Content: raw})
	return decoded.textContent()
}

func skillActivationPromptFromPersistedText(text string, activation *agentdomain.SkillActivation) string {
...
func uiSkillActivationFromMessage(raw messagepkg.Message) *agentdomain.SkillActivation {
	explicitKind := uiUserMessageKind(raw) == agentdomain.UserMessageKindSkillActivation
...
func skillActivationMetadataSkills(value any) []agentdomain.SkillActivationSkill {
	var skills []agentdomain.SkillActivationSkill
	for _, item := range skillActivationMetadataItems(value) {
		name := stringFromAny(item["name"])
		if name == "" {
			continue
		}
		skills = append(skills, agentdomain.SkillActivationSkill{
			Name:        name,

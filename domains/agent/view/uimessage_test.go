	...
	messages := []messagepkg.Message{
		{
			...
			Content: mustUIMessageJSON(t, agentdomain.ModelMessage{
				Role:    "user",
...
func TestConvertModelMessagesToUIAssistantMessagesIncludesUserInputMetadata(t *testing.T) {
	messages := ConvertModelMessagesToUIAssistantMessages([]agentdomain.ModelMessage{{
		Role: "assistant",
		Content: mustUIRawJSON(t, []map[string]any{{
			"type":       "tool-call",
			"toolCallId": "call-ask",
...
func TestConvertMessagesToUITurnsKeepsSkillActivationWithoutPrompt(t *testing.T) {
	...
	turns := ConvertMessagesToUITurns([]messagepkg.Message{{
		...
		DisplayContent: "",
		Metadata: map[string]any{
			"user_message_kind": agentdomain.UserMessageKindSkillActivation,
	...
	if turns[0].UserMessageKind != agentdomain.UserMessageKindSkillActivation {
...
func TestConvertMessagesToUITurnsStripsLegacyRawSlashActivationText(t *testing.T) {
	...
	for _, uiTurn := range turns {
		if uiTurn.UserMessageKind != agentdomain.UserMessageKindSkillActivation {
...
func TestConvertRawModelMessagesToUIAssistantMessagesBuildsTerminalSnapshots(t *testing.T) {
	raw := mustUIRawJSON(t, []agentdomain.ModelMessage{
		{
			Role: "assistant",
			Content: mustUIRawJSON(t, []map[string]any{
				{"type": "reasoning", "text": "thinking"},
...
func mustUIMessageJSON(t *testing.T, message agentdomain.ModelMessage) json.RawMessage {
	t.Helper()

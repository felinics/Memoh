// Package postgres implements MCP-owned PostgreSQL persistence.
package postgres
...
var _ mcp.ConnectionStore = (*ConnectionStore)(nil)
...
func (s *ConnectionStore) ListConnections(ctx context.Context, botID string) ([]mcp.Connection, error) {
	...
	items := make([]mcp.Connection, 0, len(rows))
...
func (s *ConnectionStore) GetConnection(ctx context.Context, botID, connectionID string) (mcp.Connection, error) {
	bot, err := db.ParseUUID(botID)
	if err != nil {
		return mcp.Connection{}, err
	}
	...
	if err != nil {
		return mcp.Connection{}, mapNotFound(err)
	}
	return connection(row)
}

func (s *ConnectionStore) CreateConnection(ctx context.Context, input mcp.ConnectionWrite) (mcp.Connection, error) {
...
func (s *ConnectionStore) CreateManagedConnection(ctx context.Context, input mcp.ManagedConnectionWrite) (mcp.Connection, error) {
...
func (s *ConnectionStore) UpdateConnection(ctx context.Context, input mcp.ConnectionWrite) (mcp.Connection, error) {
	bot, config, err := connectionPayload(input)
	if err != nil {
		return mcp.Connection{}, err
	}
...
func (s *ConnectionStore) UpsertConnectionByName(ctx context.Context, input mcp.ConnectionWrite) (mcp.Connection, error) {
	bot, config, err := connectionPayload(input)
	if err != nil {
		return mcp.Connection{}, err
	}
...
func (s *ConnectionStore) SaveConnectionProbe(ctx context.Context, input mcp.ConnectionProbeWrite) error {
...
func connectionPayload(input mcp.ConnectionWrite) (pgtype.UUID, []byte, error) {
	bot, err := db.ParseUUID(input.BotID)
	if err != nil {
...
func connection(row dbsqlc.AgentMcpConnection) (mcp.Connection, error) {

...
var _ mcp.OAuthStore = (*OAuthStore)(nil)
...
func (s *OAuthStore) SaveDiscovery(ctx context.Context, connectionID string, result mcp.DiscoveryResult) error {
...
func (s *OAuthStore) GetOAuthToken(ctx context.Context, connectionID string) (mcp.OAuthToken, error) {
	id, err := db.ParseUUID(connectionID)
	if err != nil {
	...
	if err != nil {
		return mcp.OAuthToken{}, mapNotFound(err)
	}
	return oauthToken(row), nil
}

func (s *OAuthStore) GetOAuthTokenByState(ctx context.Context, state string) (mcp.OAuthToken, error) {
...
func (s *OAuthStore) SavePKCEState(ctx context.Context, connectionID string, state mcp.PKCEState) error {
	id, err := db.ParseUUID(connectionID)
	if err != nil {
		return err
	}
...
func (s *OAuthStore) SaveTokens(ctx context.Context, connectionID string, tokens mcp.OAuthTokens) error {
	id, err := db.ParseUUID(connectionID)
	if err != nil {
		return err
	}
...
func oauthToken(row dbsqlc.AgentMcpOauthToken) mcp.OAuthToken {

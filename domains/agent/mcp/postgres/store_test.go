...
func TestConnectionStoreCreateMapsDomainInput(t *testing.T) {
	...
	got, err := store.CreateConnection(t.Context(), mcp.ConnectionWrite{
...
func TestConnectionStoreSaveProbeMapsTools(t *testing.T) {
	...
	err := store.SaveConnectionProbe(t.Context(), mcp.ConnectionProbeWrite{
		BotID: testBotID, ConnectionID: testConnectionID, Status: "healthy",
		Tools: []mcp.ToolDescriptor{{Name: "search"}}, StatusMessage: "ok",
	})
	if err != nil {
...
func TestConnectionStoreMapsMissingRows(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "update",
			run: func(store *ConnectionStore) error {
				_, err := store.UpdateConnection(t.Context(), mcp.ConnectionWrite{
...
func TestOAuthStoreMapsTokens(t *testing.T) {
	...
	err := store.SaveTokens(t.Context(), testConnectionID, mcp.OAuthTokens{

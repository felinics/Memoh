...
func (p staticToolProvider) Tools(context.Context, tool.SessionContext) ([]sdk.Tool, error) {
	return p.tools, nil
}
...
func TestAgentStreamStopsOnToolLoopAbort(t *testing.T) {
	...
	if terminal.Type != agentdomain.AgentAbort {
		t.Fatalf("expected agentdomain.AgentAbort, got %q", terminal.Type)
	}
}
...
func TestAgentStreamMarksRetryTextLoopAsAbort(t *testing.T) {
	...
	if terminal.Type != agentdomain.AgentAbort {
		t.Fatalf("expected agentdomain.AgentAbort after retry text-loop abort, got %q", terminal.Type)
	}
}
...
func TestRunMidStreamRetryMarksTextLoopCancellationAsAborted(t *testing.T) {
	...
	retryResult, aborted := a.runMidStreamRetry(
		...
		make(chan agentdomain.StreamEvent, 32),
		RunConfig{
			Model:         &sdk.Model{ID: "mock-model", Provider: modelProvider},

...
// usageTestProvider is a ToolProvider that also implements tool.ToolUsage. It
...
type usageTestProvider struct {
	emitTool      bool
	usage         string
	requireTool   tool.ToolName
	missingMarker string
	sessionSeen   *tool.SessionContext
}

func (p *usageTestProvider) Tools(_ context.Context, session tool.SessionContext) ([]sdk.Tool, error) {
...
func (p *usageTestProvider) Usage(_ context.Context, _ tool.SessionContext, available tool.AvailableTools) string {
...
// plainTestProvider returns a tool but does NOT implement tool.ToolUsage.
type plainTestProvider struct{}

func (plainTestProvider) Tools(_ context.Context, _ tool.SessionContext) ([]sdk.Tool, error) {
	return []sdk.Tool{{Name: tool.ToolRead().String(), Description: "plain"}}, nil
}

func newTestAgent(providers ...tool.ToolProvider) *Agent {
	a := New(Deps{})
...
func TestAssembleToolsInjectsUsageWhenProviderEmitsTools(t *testing.T) {
	...
	gotTools, usage, err := a.assembleTools(context.Background(), RunConfig{}, tool.StreamEmitter(func(tool.ToolStreamEvent) {}), true)
	if err != nil {
...
func TestAssembleToolsDoesNotExposeAskUserWithoutCapability(t *testing.T) {
	...
	if len(gotTools) != 1 || gotTools[0].Name != tool.ToolAskUser().String() {
		t.Fatalf("expected ask_user when capability is present, got %#v", gotTools)
	}
	if !strings.Contains(usage, tool.ToolAskUser().String()) {
...
// be dropped even though another provider did contribute tool.

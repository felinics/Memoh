...
func (s *mockExecContainerService) Exec(stream bridgepb.ContainerService_ExecServer) error {
	// Read config message.
	...
	if b.stdout != "" {
		if err := stream.Send(&bridgepb.ExecOutput{
			Stream: bridgepb.ExecOutput_STDOUT,
			Data:   []byte(b.stdout),
		}); err != nil {
			return err
		}
	...
	if b.stderr != "" {
		if err := stream.Send(&bridgepb.ExecOutput{
			Stream: bridgepb.ExecOutput_STDERR,
			Data:   []byte(b.stderr),
		}); err != nil {
			return err
		}
...
func (s *mockExecContainerService) WriteFile(_ context.Context, req *bridgepb.WriteFileRequest) (*bridgepb.WriteFileResponse, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func setupExecTestInfra(t *testing.T, svc *mockExecContainerService) (bridge.Provider, func()) {
	...
	srv := grpc.NewServer()
	bridgepb.RegisterContainerServiceServer(srv, svc)
...
func TestE2E_ExplicitBackgroundExec(t *testing.T) {
	...
	a.SetToolProviders([]tool.ToolProvider{
		tool.NewContainerProvider(nil, bp, bgMgr, "/data"),
	})

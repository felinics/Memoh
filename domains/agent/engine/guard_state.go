...
func (c *toolEventCollector) Add(evt tool.ToolStreamEvent) bool {
	if c == nil {
		return false
	}
...
func (c *toolEventCollector) CloseAndSnapshot() []tool.ToolStreamEvent {
	if c == nil {
		return nil
	}
	...
	defer c.mu.Unlock()
	c.closed = true
	snapshot := make([]tool.ToolStreamEvent, len(c.events))
...
func (c *toolEventCollector) Snapshot() []tool.ToolStreamEvent {
	if c == nil {
		return nil
	}
	...
	c.mu.Lock()
	defer c.mu.Unlock()
	out := make([]tool.ToolStreamEvent, len(c.events))
	copy(out, c.events)
	return out
}

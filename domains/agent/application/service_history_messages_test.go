...
import (
	...
	compaction "github.com/memohai/memoh/domains/agent/chat/compaction"
	contextfrag "github.com/memohai/memoh/domains/agent/chat/context/fragment"
...
// fakeArtifactStore serves a fixed artifact frontier through the compaction
// ArtifactStore port, which is the seam the application layer depends on.
type fakeArtifactStore struct {
	rows []compaction.ArtifactRecord
}

func (f fakeArtifactStore) ListArtifactsBySession(context.Context, string) ([]compaction.ArtifactRecord, error) {
	return f.rows, nil
}

func (fakeArtifactStore) GetArtifact(context.Context, string) (compaction.ArtifactRecord, error) {
	return compaction.ArtifactRecord{}, compaction.ErrArtifactNotFound
}

func (fakeArtifactStore) ListParentIDs(context.Context, compaction.ArtifactParentsInput) ([]string, error) {
	return nil, nil
}

func compactionLogRow(t *testing.T, summary string, coveredExternalID string, createdAtMs int64) compaction.ArtifactRecord {
	t.Helper()
...
func messagesDebug(messages []agentdomain.ModelMessage) string {
...
func TestTrimPipelineMessagesKeepsPinnedSummaries(t *testing.T) {
	...
	messages := []agentdomain.ModelMessage{
		...
		{Role: "user", Content: agentdomain.NewTextContent(strings.Repeat("a", 400))},
		{Role: "assistant", Content: agentdomain.NewTextContent(strings.Repeat("b", 400))},
		{Role: "user", Content: agentdomain.NewTextContent("recent")},
	}

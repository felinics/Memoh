...
func (s *Service) resolveMemoryProvider(ctx context.Context, botID string) memprovider.Instance {
	_, p := s.resolveMemoryProviderWithID(ctx, botID)
	return p
}

func (s *Service) resolveMemoryProviderWithID(ctx context.Context, botID string) (string, memprovider.Instance) {
...
func (s *Service) loadMemoryContextMessage(ctx context.Context, req ChatRequest) *agentdomain.ModelMessage {
...
func (s *Service) memoryContextMessageFromResult(ctx context.Context, req ChatRequest, builtQuery memoryQuery, result *memprovider.BeforeChatResult, cacheState, fallbackReason string) *agentdomain.ModelMessage {
	contextText := ""
	resultCount := 0
	contextBytes := 0
	retrievalMode := ""
	...
	return &agentdomain.ModelMessage{
		Role:    "user",
		Content: agentdomain.NewTextContent(contextText),
	}
}
...
func (s *Service) storeMemory(ctx context.Context, req ChatRequest, messages []agentdomain.ModelMessage) {
	botID := strings.TrimSpace(req.BotID)
	if botID == "" {
		return
	}
...
func toProviderMessages(messages []agentdomain.ModelMessage) []memprovider.Message {

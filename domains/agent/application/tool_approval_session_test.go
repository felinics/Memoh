...
func TestToolApprovalHandlerLimitsForcedApprovalRejectionReason(t *testing.T) {
	...
	resolver := &Service{
		agent: engine.New(engine.Deps{
			Limits: engine.Limits{ToolOutputMaxBytes: 512, ToolOutputMaxLines: 80},
		}),
	}
	...
	result, err := handler(engine.ContextWithHookForcedApproval(context.Background(), large), sdk.ToolCall{
		ToolCallID: "call-1",
		ToolName:   "write",
...
func TestToolApprovalPolicyDenyWinsOverHookForcedApproval(t *testing.T) {
	...
	result, err := handler(engine.ContextWithHookForcedApproval(context.Background(), "hook asks for review"), sdk.ToolCall{
		ToolCallID: "call-1",
		ToolName:   "read",

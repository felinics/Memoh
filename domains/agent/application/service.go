...
func NewService(
	...
	return &Service{
		...
		latestSessionModels:   latestSessionModels,
		compactionArtifacts:   compactionArtifacts,
		compactionMessageRefs: compactionMessageRefs,
...
func (s *Service) resolve(ctx context.Context, req ChatRequest) (resolvedContext, error) {
	...
	var messages []agentdomain.ModelMessage
	var historyRecords []history.HistoryRecord
	...
	headerInput := agentdomain.UserMessageHeaderInput{
	...
	if userQueryNeedsHeader(req, len(mergedAttachments)) {
		headerifiedQuery = agentdomain.FormatUserHeader(headerInput, req.Query)
	}
	headerifiedModelQuery := headerifiedQuery
	if strings.TrimSpace(modelQuery) != strings.TrimSpace(req.Query) {
		headerifiedModelQuery = agentdomain.FormatUserHeader(headerInput, modelQuery)
	}
	...
	if req.InjectCh != nil {
		agentInjectCh := make(chan engine.InjectMessage, cap(req.InjectCh))
		go func() {
			for msg := range req.InjectCh {
				agentMsg := engine.InjectMessage{
					Text:            msg.Text,
...
func (s *Service) Chat(ctx context.Context, req ChatRequest) (ChatResponse, error) {
	...
	return ChatResponse{
		...
		Provider: string(rc.provider.ClientType),
	}, nil
}
...
func (s *Service) buildBaseRunConfig(ctx context.Context, p baseRunConfigParams) (engine.RunConfig, modelcatalog.GetResponse, modelcatalog.ResolvedProvider, error) {
	botSettings, err := s.loadBotSettings(ctx, p.BotID)
	if err != nil {
		return engine.RunConfig{}, modelcatalog.GetResponse{}, modelcatalog.ResolvedProvider{}, err
	}
	...
	authCtx := oauth.WithUserID(ctx, p.UserID)
	chatModel, provider, err := s.selectChatModel(authCtx, req, botSettings)
	if err != nil {
		return engine.RunConfig{}, modelcatalog.GetResponse{}, modelcatalog.ResolvedProvider{}, err
	}

	baseURL := provider.BaseURL
	chatCompletionsCompat := modelexecution.ResolveChatCompletionsCompat(
		baseURL,
		provider.ChatCompletionsCompat,
	)

	reasoningConfig := resolveReasoningConfig(chatModel, botSettings, p.ReasoningEffort, string(provider.ClientType))
	reasoningEffort := ""
	if reasoningConfig != nil && reasoningConfig.Active {
		reasoningEffort = reasoningConfig.Effort
	}

	sdkModel := modelexecution.NewSDKChatModel(modelexecution.SDKModelConfig{
	...
	var agentSkills []engine.SkillEntry
	...
	cfg := engine.RunConfig{
		...
		SupportsToolCall:      chatModel.HasCompatibility(modeldomain.CompatToolCall),
		Identity: engine.SessionContext{
			BotID:             p.BotID,
			ChatID:            chatID,
		...
		Bot:               botInfo,
		Skills:            agentSkills,
		LoopDetection:     engine.LoopDetectionConfig{Enabled: loopDetectionEnabled},
		BackgroundManager: s.bgManager,
		ContextScope: fragment.Scope{
			BotID:             p.BotID,
			ChatID:            chatID,
			SessionID:         p.SessionID,
			ChannelIdentityID: strings.TrimSpace(p.ChannelIdentityID),
	...
	if s.workspaceTargets != nil {
		if target, targetErr := s.workspaceTargets.ResolveWorkspaceTarget(ctx, p.BotID, ""); targetErr == nil {
			...
			return engine.RunConfig{}, modelcatalog.GetResponse{}, modelcatalog.ResolvedProvider{}, targetErr
		}
	}
...
func supportsImageInputForModel(m modelcatalog.GetResponse) bool {
	return m.HasCompatibility(modeldomain.CompatVision)
}
...
//     bot's default.
func resolveReasoningConfig(chatModel modelcatalog.GetResponse, botSettings setting.Settings, requestedEffort, clientType string) *modelexecution.ReasoningConfig {
	mode := chatModel.ResolveThinkingMode()
	if mode == modeldomain.ThinkingModeNone {
		return nil
	}
	...
	adaptive := mode == modeldomain.ThinkingModeAdaptive
	...
	if !adaptive && clientType == string(modeldomain.ClientTypeAnthropicMessages) && anthropicEffortEra(effortLevels) {
		adaptive = true
	}

	switch {
	case reasoningEffortDisabled(requested):
		return &modelexecution.ReasoningConfig{Disabled: true, OffEffort: offEffort}
	case requested == reasoningEffortAdaptive:
		...
		return &modelexecution.ReasoningConfig{Active: true, Adaptive: adaptive, Effort: pickEffort("", botSettings, effortLevels), OffEffort: offEffort}
	case requested != "":
		return &modelexecution.ReasoningConfig{Active: true, Adaptive: adaptive, Effort: pickEffort(requested, botSettings, effortLevels), OffEffort: offEffort}
...
func anthropicEffortEra(effortLevels []string) bool {
	for _, e := range effortLevels {
		switch e {
		case modeldomain.ReasoningEffortNone, modeldomain.ReasoningEffortMinimal,
...
func pickEffort(requested string, botSettings setting.Settings, effortLevels []string) string {
	...
	if hasEffort(effortLevels, modeldomain.ReasoningEffortMedium) {
		return modeldomain.ReasoningEffortMedium
	}
...
func effectiveReasoningEfforts(effortLevels []string, clientType string) []string {
	levels := effortLevels
	if len(levels) == 0 {
		levels = []string{modeldomain.ReasoningEffortLow, modeldomain.ReasoningEffortMedium, modeldomain.ReasoningEffortHigh}
	}
	out := make([]string, 0, len(levels))
	for _, e := range levels {
		if normalizesMaxReasoningEffort(clientType) && e == modeldomain.ReasoningEffortMax {
			continue
		}
		if !hasEffort(out, e) {
			out = append(out, e)
...
func normalizesMaxReasoningEffort(clientType string) bool {
	...
	case modeldomain.ClientTypeOpenAICompletions, modeldomain.ClientTypeOpenAIResponses:
		return true
	default:
		return false
	}
...
func offEffortFor(effortLevels []string) string {
	if hasEffort(effortLevels, modeldomain.ReasoningEffortNone) {
		return modeldomain.ReasoningEffortNone
	}
	if hasEffort(effortLevels, modeldomain.ReasoningEffortMinimal) {
		return modeldomain.ReasoningEffortMinimal
	}
	return ""
}
...
func offEffortOrEmpty(rc *modelexecution.ReasoningConfig) string {
	if rc == nil {
		return ""
	}
	return rc.OffEffort
...
func (s *Service) buildToolApprovalHandler(p baseRunConfigParams) func(context.Context, sdk.ToolCall) (sdk.ToolApprovalResult, error) {
	return func(ctx context.Context, call sdk.ToolCall) (sdk.ToolApprovalResult, error) {
		...
		forcedApprovalReason, forcedApproval := engine.HookForcedApprovalReason(ctx)
		...
		if err != nil {
			if input.WorkspaceTargeted && errors.Is(err, workspace.ErrWorkspaceTargetNotFound) {
				...
				if s.logger != nil {
					s.logger.WarnContext(ctx, "workspace tool target not found during approval",
...
func (s *Service) prepareRunConfig(ctx context.Context, cfg engine.RunConfig) engine.RunConfig {
	...
	if s.agent != nil {
		...
		fs := engine.NewFSClient(s.agent.BridgeProvider(), cfg.Identity.BotID, nowFn)
	...
	cfg.System = engine.GenerateSystemPrompt(engine.SystemPromptParams{
		SessionType:               cfg.SessionType,
		Bot:                       cfg.Bot,
		Skills:                    cfg.Skills,
		Files:                     files,
...
func normalizeGatewaySkill(entry SkillEntry) (engine.SkillEntry, bool) {
	name := strings.TrimSpace(entry.Name)
	if name == "" {
		return engine.SkillEntry{}, false
	}
...
func normalizeUserMessageContent(msg agentdomain.ModelMessage) agentdomain.ModelMessage {
...
func userQueryNeedsHeader(req ChatRequest, attachmentCount int) bool {
	...
	return attachmentCount > 0 && req.UserMessageKind != agentdomain.UserMessageKindSkillActivation
}

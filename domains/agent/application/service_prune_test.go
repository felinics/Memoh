...
func TestPruneMessagesForGateway_PrunesToolResultContent(t *testing.T) {
	...
	msgs := []agentdomain.ModelMessage{
		{Role: "tool", Content: agentdomain.NewTextContent(huge), ToolCallID: "call-1"},
	}
...
func TestPruneHistoryForGateway_ClearsStaleUsageTokensAfterPrune(t *testing.T) {
	...
	in := []history.HistoryRecord{
		historyRecord("row-1", agentdomain.ModelMessage{Role: "tool", Content: agentdomain.NewTextContent(huge), ToolCallID: "call-1"}, func(record *history.HistoryRecord) {
			record.UsageInputTokens = &firstTokens
		}),
		historyRecord("row-2", agentdomain.ModelMessage{Role: "user", Content: agentdomain.NewTextContent("hi")}, func(record *history.HistoryRecord) {
...
func TestPruneHistoryForGateway_PreservesUsageTokensWhenUnchanged(t *testing.T) {
	...
	tokens := 321
	in := []history.HistoryRecord{
		historyRecord("row-1", agentdomain.ModelMessage{Role: "user", Content: agentdomain.NewTextContent("short")}, func(record *history.HistoryRecord) {

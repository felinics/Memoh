...
func (s *Service) maybeCompact(ctx context.Context, req ChatRequest, rc resolvedContext, inputTokens int) {
	...
	if !botSettings.CompactionEnabled || botSettings.CompactionThreshold <= 0 {
		s.logger.InfoContext(ctx, "compaction: skipped, disabled or no threshold",
...
func (s *Service) buildCompactionConfig(ctx context.Context, req ChatRequest, botSettings setting.Settings, inputTokens int) (compaction.TriggerConfig, error) {
	...
	authCtx := oauth.WithUserID(ctx, req.UserID)
	compactProvider, err := modelcatalog.FetchProviderByID(authCtx, s.modelProviderResolver, compactModel.ProviderID)
	if err != nil {
		return compaction.TriggerConfig{}, err
	}

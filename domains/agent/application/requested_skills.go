...
func (s *Service) rejectRequestedSkillsIfUnsupportedContext(ctx context.Context, req ChatRequest) error {
	...
	if req.UserMessagePersisted && req.UserMessageKind != agentdomain.UserMessageKindSkillActivation {
...
func buildRequestedSkillContextMessage(items []agentdomain.RequestedSkillContext) *agentdomain.ModelMessage {
	if len(items) == 0 {
		return nil
	}

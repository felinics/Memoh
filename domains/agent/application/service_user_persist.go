...
func (s *Service) persistUserTurn(ctx context.Context, req ChatRequest) (messagepkg.Message, error) {
	...
	modelMessage := agentdomain.ModelMessage{
		Role:    "user",
		Content: agentdomain.NewTextContent(persistedUserTurnText(req)),
	}
	...
	if strings.TrimSpace(req.UserVisibleText) != "" || req.UserMessageKind == agentdomain.UserMessageKindSkillActivation {
	...
	return s.messageService.Persist(ctx, messagepkg.PersistInput{
		...
		SenderDisplayName:       strings.TrimSpace(req.DisplayName),
		SenderAvatarURL:         strings.TrimSpace(req.AvatarURL),
...
func persistedUserTurnText(req ChatRequest) string {
	if req.UserMessageKind == agentdomain.UserMessageKindSkillActivation {

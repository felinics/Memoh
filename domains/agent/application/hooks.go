...
func (s *Service) applyUserMessageHook(ctx context.Context, req ChatRequest) (ChatRequest, error) {
	...
	if strings.TrimSpace(res.AppendContext) != "" {
		req.Messages = append(req.Messages, agentdomain.ModelMessage{
			Role:    "user",
			Content: agentdomain.NewTextContent(formatServiceHookContext(hooks.EventUserMessageReceived, res.AppendContext)),
		})
	}

...
func TestPrepareGatewayAttachments_ResolvesStoredFileAccessPath(t *testing.T) {
	...
	merged := resolver.routeAndMergeAttachments(context.Background(), modelcatalog.GetResponse{}, req)
	if len(merged) != 1 {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := tt.resolver.prepareACPAttachments(context.Background(), ChatRequest{
				BotID:       "bot-1",
				Attachments: []agentdomain.Attachment{tt.input},
			})
...
func TestSanitizeMessagesNormalizesUserMultipartImageBytes(t *testing.T) {
	...
	cleaned := sanitizeMessages([]agentdomain.ModelMessage{{
		Role:    "user",
		Content: content,
	}})
	if len(cleaned) != 1 {

...
func hasVisibleAgentStreamOutput(event agentdomain.StreamEvent) bool {
	switch event.Type {
	case agentdomain.TextDelta,
		agentdomain.ReasoningDelta:
	...
	case agentdomain.ToolCallInputStart,
		...
		agentdomain.ToolApprovalRequest,
		agentdomain.UserInputRequest,
		agentdomain.Reaction,
		agentdomain.Speech:
		return true
	case agentdomain.AttachmentDelta:
...
func extractTerminalSnapshot(data []byte) (terminalSnapshot, bool) {
	...
	return terminalSnapshot{
		...
		aborted:        envelope.Type == string(agentdomain.AgentAbort),
	}, true
}
...
func (s *Service) StreamChat(ctx context.Context, req ChatRequest) (<-chan StreamChunk, <-chan error) {
	...
	go func() {
		...
		if ok, err := s.isACPAgentSession(ctx, streamReq); err != nil {
			s.logger.ErrorContext(ctx, "StreamChat: ACP session check failed",
		...
		for event := range eventCh {
			...
			if event.Type == agentdomain.ToolCallStart {
				toolCallCount++
				idleCancel.RecordToolCall()
			}

			if event.Type == agentdomain.Error {
		...
		if idleCancel.DidFire() {
			...
			if !clientGone {
				timeoutEvent := agentdomain.StreamEvent{
...
func (s *Service) streamChatWSResultWithHooks(
	...
	if ok, err := s.isACPAgentSession(ctx, req); err != nil {
		s.logger.ErrorContext(ctx, "StreamChatWS: ACP session check failed",
...
func hasPersistableAssistantOutput(messages []agentdomain.ModelMessage) bool {
...
func interleaveInjectedMessages(round []agentdomain.ModelMessage, injections []InjectedMessageRecord) []agentdomain.ModelMessage {
	if len(injections) == 0 {
		return round
	}
	result := make([]agentdomain.ModelMessage, 0, len(round)+len(injections))
	injIdx := 0
	for i, msg := range round {
		...
		for injIdx < len(injections) && injections[injIdx].InsertAfter == i {
			result = append(result, agentdomain.ModelMessage{
				Role:    "user",
				Content: agentdomain.NewTextContent(injections[injIdx].HeaderifiedText),
			})
			injIdx++
		}
	}

...
func TestStreamChatWSRoutesACPAgentSessionToACPPool(t *testing.T) {
	...
	if err := resolver.StreamChatWS(
		...
		ChatRequest{
			...
			ReplyAttachments: []agentdomain.Attachment{{
				Type: "file",
				Name: "previous.log",
	...
	if !containsStreamEvent(events, agentdomain.AgentStart) || !containsStreamEvent(events, agentdomain.AgentEnd) {
	...
	end := requireStreamEvent(t, events, agentdomain.AgentEnd)
...
func TestACPTerminalStreamEventFallsBackToTranscriptEvents(t *testing.T) {
	...
	ev := acpTerminalStreamEvent(agentdomain.AgentEnd, acpclient.PromptResult{
		Events: []agentdomain.StreamEvent{{Type: agentdomain.TextDelta, Delta: "from transcript"}},
		Usage:  &sdk.Usage{InputTokens: 2, OutputTokens: 4, TotalTokens: 6},
	})

	if ev.Type != agentdomain.AgentEnd {
		t.Fatalf("terminal event type = %s, want %s", ev.Type, agentdomain.AgentEnd)
	}
...
func TestStreamChatWSPersistsACPUserInputProjectionBeforePromptReturns(t *testing.T) {
	...
	streamed := []agentdomain.StreamEvent{
		{
		...
		{Type: agentdomain.TextDelta, Delta: "done"},
	}
	messages := &recordingMessageService{}
	pool := &recordingACPPrompter{
...
func TestStreamChatWSPersistsACPApprovalProjectionTerminalState(t *testing.T) {
	...
	streamed := []agentdomain.StreamEvent{
		...
		{
			Type:       agentdomain.ToolApprovalRequest,
			ToolCallID: "write-1",
			ToolName:   "write",
...
func TestAuthorizeACPToolApprovalRequiresRuntimeOwnerOrManage(t *testing.T) {
	...
	perms := &fakeBotPermissionChecker{
		values: map[string]bool{
			...
			"bot-1:" + otherID + ":" + bot.PermissionWorkspaceExec: true,
...
func TestAuthorizeACPUserInputAllowsChatResponderWhenRuntimeOwnerStillAuthorized(t *testing.T) {
	...
	perms := &fakeBotPermissionChecker{
		values: map[string]bool{
			"bot-1:" + ownerID + ":" + bot.PermissionWorkspaceExec:     true,
			"bot-1:" + chatMemberID + ":" + bot.PermissionChat:         true,
			"bot-1:" + workspaceID + ":" + bot.PermissionWorkspaceExec: true,
			"bot-1:" + managerID + ":" + bot.PermissionManage:          true,
		},
	}
	resolver := &Service{
	...
	delete(perms.values, "bot-1:"+ownerID+":"+bot.PermissionWorkspaceExec)
...
func TestPersistACPRoundStoresACPEventsAsNativeToolMessages(t *testing.T) {
	...
	err := resolver.persistACPRound(
		...
		"/data/app",
		withTranscriptOutput(acpclient.PromptResult{
			Events: []agentdomain.StreamEvent{
				{Type: agentdomain.TextDelta, Delta: "Before"},
				{
					Type:       agentdomain.ToolCallStart,
					ToolCallID: "read-1",
					ToolName:   "read",
				...
				{Type: agentdomain.TextDelta, Delta: "After"},
			},
			StopReason: "end_turn",
		}),
		nil,
...
func TestPersistACPRoundStoresACPThoughtsAsReasoningParts(t *testing.T) {
	...
	err := resolver.persistACPRound(
		...
		withTranscriptOutput(acpclient.PromptResult{
			Events: []agentdomain.StreamEvent{
				{Type: agentdomain.ReasoningDelta, Delta: "I should inspect first."},
				{Type: agentdomain.TextDelta, Delta: "Done"},
			},
			StopReason: "end_turn",
		}),
...
func TestStreamACPAgentWSFailurePersistsRoundAndSkipsMemory(t *testing.T) {
	...
	events := drainAgentEvents(t, eventCh)
	abort := requireStreamEvent(t, events, agentdomain.AgentAbort)
...
func TestStreamACPAgentWSFeedbackErrorSkipsPersistence(t *testing.T) {
	...
	events := drainAgentEvents(t, eventCh)
	if !containsStreamEvent(events, agentdomain.AgentStart) || containsStreamEvent(events, agentdomain.AgentAbort) {
...
func TestStreamACPAgentWSSuccessStoresMemory(t *testing.T) {
	...
	pool := &recordingACPPrompter{
		result: withTranscriptOutput(acpclient.PromptResult{
			Events: []agentdomain.StreamEvent{{Type: agentdomain.TextDelta, Delta: "done"}},
		}),
	}
	resolver := &Service{
		messageService:  messages,
...
type recordingACPPrompter struct {
	...
	err          error
	onPrompt     func()
	streamEvents []agentdomain.StreamEvent
	afterEvents  func()
}

type storeRoundMemoryProvider struct {
	memprovider.Instance
...
func (*storeRoundSettingsQueries) Get(context.Context, string) (settingpersistence.Record, error) {
	return settingpersistence.Record{
		...
		CompactionRatio:   80,
		MemoryProviderID:  storeRoundMemoryProviderID,
	}, nil
}
...
func (p *recordingACPPrompter) Prompt(_ context.Context, input acpagent.PromptInput) (acpclient.PromptResult, error) {
	...
	if input.Sink != nil {
		...
		if len(events) == 0 && p.err == nil {
			events = []agentdomain.StreamEvent{{Type: agentdomain.TextDelta, Delta: "streamed from acp"}}
		}
...
func allowWorkspaceExecForBot(botID, accountID string) *fakeBotPermissionChecker {
	return &fakeBotPermissionChecker{values: map[string]bool{
		strings.TrimSpace(botID) + ":" + strings.TrimSpace(accountID) + ":" + bot.PermissionWorkspaceExec: true,
	}}
}
...
func drainAgentEvents(t *testing.T, eventCh <-chan WSStreamEvent) []agentdomain.StreamEvent {
	t.Helper()
	events := make([]agentdomain.StreamEvent, 0, len(eventCh))
	for len(eventCh) > 0 {
...
func drainStreamChunks(t *testing.T, chunkCh <-chan StreamChunk) []agentdomain.StreamEvent {
	t.Helper()
...
func containsStreamEvent(events []agentdomain.StreamEvent, eventType agentdomain.StreamEventType) bool {
...
func requireStreamEvent(t *testing.T, events []agentdomain.StreamEvent, eventType agentdomain.StreamEventType) agentdomain.StreamEvent {
	t.Helper()
	for _, event := range events {
		if event.Type == eventType {
			return event
...
func terminalAssistantText(t *testing.T, event agentdomain.StreamEvent) string {
	t.Helper()
...
func containsTextDelta(events []agentdomain.StreamEvent, delta string) bool {
	for _, event := range events {
		if event.Type == agentdomain.TextDelta && event.Delta == delta {
			return true
		}
	}
	return false
...
func persistedModelMessage(t *testing.T, content json.RawMessage) agentdomain.ModelMessage {
	t.Helper()

...
type acpPreparedAttachments struct {
	...
	Context                  []agentdomain.Attachment
	References               []string
...
func (s *Service) streamACPAgentWS(ctx context.Context, req ChatRequest, eventCh chan<- WSStreamEvent, abortCh <-chan struct{}) error {
	...
	recordProjectionStatus := func(ev agentdomain.StreamEvent) bool {
	...
	emit := func(ev agentdomain.StreamEvent) {
	...
	emit(agentdomain.StreamEvent{Type: agentdomain.AgentStart})
	...
	if err != nil {
		s.logger.ErrorContext(ctx, "ACP prompt failed",
		...
		if failureDelta != "" {
			emit(agentdomain.StreamEvent{Type: agentdomain.TextDelta, Delta: failureDelta})
		}
		if err := s.persistACPRound(context.WithoutCancel(ctx), req, agentID, projectPath, failedResult, err); err != nil {
			s.logger.ErrorContext(ctx, "ACP failure persist failed", slog.Any("error", err), slog.String("session_id", req.ThreadID))
		}
		emit(agentdomain.StreamEvent{Type: agentdomain.TextEnd})
		emit(acpTerminalStreamEvent(agentdomain.AgentAbort, failedResult))
		return nil
	}
	...
	if err := s.persistACPRound(context.WithoutCancel(ctx), req, agentID, projectPath, result, nil); err != nil {
		s.logger.ErrorContext(ctx, "ACP persist failed", slog.Any("error", err), slog.String("session_id", req.ThreadID))
	}
	emit(acpTerminalStreamEvent(agentdomain.AgentEnd, result))
	return nil
}
...
func (s *Service) prepareACPAttachments(ctx context.Context, req ChatRequest) (acpPreparedAttachments, error) {
	...
	result := acpPreparedAttachments{
		...
		Context:                  make([]agentdomain.Attachment, 0, len(prepared)),
	...
	for i, item := range prepared {
		...
		contextAttachment := agentdomain.Attachment{
			Type:        attachmentType,
...
func acpTerminalStreamEvent(eventType agentdomain.StreamEventType, result acpclient.PromptResult) agentdomain.StreamEvent {
	result = ensureACPPromptOutput(result)
	ev := agentdomain.StreamEvent{Type: eventType}
...
func (s *Service) requireACPRuntimeOwnerWorkspaceExec(ctx context.Context, botID, runtimeOwnerAccountID string) error {
	...
	ok, err := s.botPermissions.HasBotPermission(ctx, strings.TrimSpace(botID), runtimeOwnerAccountID, bot.PermissionWorkspaceExec)
	if err != nil {
		return err
	}
	if ok {
...
func isACPDecisionProjectionEvent(ev agentdomain.StreamEvent) bool {
	switch ev.Type {
	case agentdomain.UserInputRequest, agentdomain.ToolApprovalRequest:
...
func acpDecisionProjectionStatus(ev agentdomain.StreamEvent) string {
...
func (s *Service) persistACPLeadingUserMessage(ctx context.Context, req ChatRequest) (ChatRequest, *messagepkg.Message) {
	...
	content, err := json.Marshal(agentdomain.ModelMessage{
		Role:    "user",
		Content: agentdomain.NewTextContent(contentText),
	})
	if err != nil {
		s.logger.WarnContext(ctx, "persist ACP leading user message: marshal failed", slog.Any("error", err))
		return req, nil
	}
	...
	if err != nil {
		s.logger.WarnContext(ctx, "persist ACP leading user message failed",
			slog.String("bot_id", req.BotID),
			slog.String("session_id", req.ThreadID),
			slog.Any("error", err))
		return req, nil
...
func (s *Service) persistACPDecisionProjection(ctx context.Context, req ChatRequest, ev agentdomain.StreamEvent) bool {
	...
	for _, msg := range output {
		...
		if err != nil {
			s.logger.WarnContext(ctx, "persist ACP decision projection: marshal failed",
		...
		if _, err := s.messageService.Persist(ctx, messagepkg.PersistInput{
			...
			s.logger.WarnContext(ctx, "persist ACP decision projection failed",
				slog.String("bot_id", req.BotID),
				slog.String("session_id", req.ThreadID),
				slog.String("tool_call_id", ev.ToolCallID),
				slog.Any("error", err))
...
func (s *Service) cancelPendingACPApprovals(ctx context.Context, req ChatRequest, reason string) {
	...
	if err != nil {
		s.logger.WarnContext(ctx, "cancel pending ACP approvals failed",
			slog.String("bot_id", req.BotID),
			slog.String("session_id", req.ThreadID),
			slog.Any("error", err))
		return
	...
	if len(cancelled) > 0 {
		s.logger.InfoContext(ctx, "cancelled pending ACP approvals with their turn",
			slog.String("session_id", req.ThreadID),
...
func (s *Service) persistACPRound(ctx context.Context, req ChatRequest, agentID, projectPath string, result acpclient.PromptResult, promptErr error) error {
	...
	round = append(round, agentdomain.ModelMessage{Role: "user", Content: agentdomain.NewTextContent(req.Query)})
...
func acpFailureResult(result acpclient.PromptResult, err error) (acpclient.PromptResult, string) {
	...
	if strings.TrimSpace(result.Text) != "" {
		...
		result.Events = append(result.Events, agentdomain.StreamEvent{Type: agentdomain.TextDelta, Delta: delta})
		result.Output = acpclient.AppendTranscriptText(result.Output, message)
		return result, delta
	}
	result.Text = message
	result.Events = append(result.Events, agentdomain.StreamEvent{Type: agentdomain.TextDelta, Delta: message})
	result.Output = acpclient.AppendTranscriptText(result.Output, message)

...
type fakeAgentStreamer struct {
	lastConfig *engine.RunConfig
}

func (f *fakeAgentStreamer) Stream(_ context.Context, cfg engine.RunConfig) <-chan agentdomain.StreamEvent {
	f.lastConfig = &cfg
	ch := make(chan agentdomain.StreamEvent, 1)
	ch <- agentdomain.StreamEvent{
...
type testAgentStreamer interface {
	Stream(context.Context, engine.RunConfig) <-chan agentdomain.StreamEvent
}
...
func drainDiscuss(t *testing.T, h agentdomain.RunHandle) []agentdomain.Event {
	t.Helper()
...
func discussCommand() agentdomain.StartTurnCommand {
	return agentdomain.StartTurnCommand{
		SchemaVersion: 1,
		TeamID:        "team-1",
		Mode:          agentdomain.ModeDiscuss,
		BotID:         "bot-1",
		ThreadID:      "sess-1",
		DiscussMessages: []agentdomain.DiscussMessage{
...
func TestDiscussInlinesImages(t *testing.T) {
	...
	resolver := &fakeDiscussService{
		resolveResult: ResolveRunConfigResult{
			RunConfig: engine.RunConfig{SupportsImageInput: true},
			ModelID:   "model-1",
		},
	...
	cmd := discussCommand()
	cmd.DiscussImageRefs = []agentdomain.DiscussImageRef{{ContentHash: "img-hash", Mime: "image/jpeg"}}
...
func TestDiscussNoInlineWhenNoVision(t *testing.T) {
	...
	resolver := &fakeDiscussService{
		resolveResult: ResolveRunConfigResult{
			RunConfig: engine.RunConfig{SupportsImageInput: false},
			ModelID:   "model-1",
		},
...
func TestDiscussACPUsesChatStreamer(t *testing.T) {
	...
	cmd.DiscussMessages = []agentdomain.DiscussMessage{{Role: "user", Content: "please inspect the app"}}
...
func TestDiscussACPSkipsWhenNotAddressed(t *testing.T) {
	...
	var sawSkip bool
	for _, e := range events {
		if e.Kind == agentdomain.DiscussEventSkipped {
			sawSkip = true
		}
	}
	if !sawSkip {
...
func TestDiscussRefreshesContextFragAfterLateBinding(t *testing.T) {
	...
	resolver := &fakeDiscussService{
		resolveResult: ResolveRunConfigResult{
			RunConfig: engine.RunConfig{System: "base system"},
			ModelID:   "model-1",
		},
	}
	a := newDiscussTestService(&fakeRunner{}, agent, resolver)
	cmd := discussCommand()
	cmd.DiscussMessages = []agentdomain.DiscussMessage{{Role: "user", Content: `<message id="x">hello</message>`}}
...
func lastMessageFragContains(frags []fragment.ContextFrag, needle string) bool {
	for i := len(frags) - 1; i >= 0; i-- {
		frag := frags[i]
		if frag.Kind != fragment.KindConversationEvent || len(frag.Parts) == 0 || frag.Parts[0].SDKMessage == nil {
			continue
		}
...
func (f *burstAgentStreamer) Stream(ctx context.Context, _ engine.RunConfig) <-chan agentdomain.StreamEvent {
	ch := make(chan agentdomain.StreamEvent)
	go func() {
		defer close(ch)
		for range f.count {
			select {
			case ch <- agentdomain.StreamEvent{Type: agentdomain.TextDelta, Delta: "x"}:
			case <-ctx.Done():
				return
			}
		}

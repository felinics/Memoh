...
func (s *Service) buildMessagesFromPipeline(ctx context.Context, req ChatRequest, contextTokenBudget int) []agentdomain.ModelMessage {
	...
	for _, m := range composed.Messages {
		...
		messages = append(messages, agentdomain.ModelMessage{
			Role:    m.Role,
			Content: contentJSON,
		})
...
func stripToolMessages(messages []agentdomain.ModelMessage) []agentdomain.ModelMessage {
	filtered := make([]agentdomain.ModelMessage, 0, len(messages))
...
func hasToolCallContent(message agentdomain.ModelMessage) bool {
...
func stripNonAskUserToolCalls(message agentdomain.ModelMessage) (agentdomain.ModelMessage, bool) {
	...
	if len(message.ToolCalls) > 0 {
		if text != "" {
			message.Content = agentdomain.NewTextContent(text)
		}
		return message, true
	}
	if text == "" {
		return agentdomain.ModelMessage{}, false
	}
	message.Content = agentdomain.NewTextContent(text)
	return message, true
}

func keepAskUserToolResultMessage(message agentdomain.ModelMessage) *agentdomain.ModelMessage {
...
func keepAskUserLegacyToolCalls(calls []agentdomain.ToolCall) []agentdomain.ToolCall {
	if len(calls) == 0 {
		return nil
	}
	kept := make([]agentdomain.ToolCall, 0, len(calls))

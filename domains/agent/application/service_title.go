...
func (s *Service) maybeGenerateSessionTitle(ctx context.Context, req ChatRequest, userQuery string) {
	...
	titleModel, provider, err := s.fetchChatModel(oauth.WithUserID(ctx, ownerUserID), titleModelID)
	if err != nil {
...
func (s *Service) resolveTitleModel(ctx context.Context, botID string) (modelID, ownerUserID string, err error) {
	...
	ownerUserID = strings.TrimSpace(bot.OwnerUserID)
	if ownerUserID == "" {
		return "", "", errors.New("bot owner is not configured")
	}
	account, err := s.accounts.Get(ctx, ownerUserID)
	if err != nil {
		return "", "", err
	}
...
func (s *Service) generateTitle(ctx context.Context, _ string, model modelcatalog.GetResponse, provider modelcatalog.ResolvedProvider, userQuery string) string {
	...
	modelCfg := modelexecution.SDKModelConfig{
	...
	sdkModel := modelexecution.NewSDKChatModel(modelCfg)

	genCtx, cancel := context.WithTimeout(ctx, titleGenerateTimeout)
	defer cancel()

	cacheTTL := provider.PromptCacheTTL

...
type userInputService interface {
	...
	AdvanceText(ctx context.Context, input agentdomain.AdvanceTextInput) (agentdomain.AdvanceTextResult, error)
...
func (s *Service) AdvancePlainTextUserInput(ctx context.Context, input agentdomain.AdvanceTextInput) (agentdomain.AdvanceTextResult, error) {
	if s.userInput == nil {
		return agentdomain.AdvanceTextResult{}, errors.New("user input service not configured")
	}
...
func (s *Service) authorizeACPUserInputResponse(ctx context.Context, target userinput.Request, input UserInputResponseInput) error {
	...
	if ok, err := s.botPermissions.HasBotPermission(ctx, botID, runtimeOwnerID, bot.PermissionWorkspaceExec); err != nil {
		return err
	} else if !ok {
		return userinput.ErrForbidden
	}
...
func userInputAnswersFromText(payload userinput.UIPayload, text string) ([]agentdomain.QuestionAnswer, error) {
	...
	return []agentdomain.QuestionAnswer{answer}, nil
}
		...
		if !stored && event.IsTerminal() && len(event.Messages) > 0 {
			if snap, ok := extractTerminalSnapshot(data); ok {
				if storeErr := s.persistTerminalSnapshot(
					...
					chatReq,
					resolvedContext{model: modelcatalog.GetResponse{ID: resolved.ModelID}},
					snap,
				); storeErr != nil {
					return storeErr
				}

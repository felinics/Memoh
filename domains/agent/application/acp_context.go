...
func (s *Service) buildACPContextMarkdown(ctx context.Context, req ChatRequest, agentID, projectPath string) string {
	...
	if s != nil && s.agent != nil {
		...
		fs := engine.NewFSClient(s.agent.BridgeProvider(), req.BotID, nowFn)
	...
	if s != nil && s.platformIdentities != nil {
		...
		if err != nil {
			if s.logger != nil {
				s.logger.WarnContext(ctx, "load bot platform identities for ACP context failed",
...
func acpContextSystemFiles(files []engine.SystemFile, maxBytes int) []acpContextFileSection {
	if maxBytes <= 0 {
		maxBytes = engine.DefaultSystemFilesMaxBytes
	}
...
func formatACPContextAttachments(attachments []agentdomain.Attachment) string {

...
func (s *Service) routeAndMergeAttachments(ctx context.Context, model modelcatalog.GetResponse, req ChatRequest) []any {
...
func (s *Service) prepareGatewayAttachments(ctx context.Context, req ChatRequest) []gatewayAttachment {
	...
	for _, raw := range attachments {
		bundle := bundleFromAttachment(raw)
...
func requestAttachmentsForGateway(req ChatRequest) []agentdomain.Attachment {
	if len(req.ReplyAttachments) == 0 {
		return req.Attachments
	}
	attachments := make([]agentdomain.Attachment, 0, len(req.Attachments)+len(req.ReplyAttachments))
...
func (s *Service) inlineInjectAttachments(ctx context.Context, botID string, atts []agentdomain.Attachment) []sdk.ImagePart {

...
func (b memoryQueryBuilder) Build(req ChatRequest, hist []history.HistoryRecord) memoryQuery {
	current := strings.TrimSpace(req.Query)
	if current == "" {
		return memoryQuery{}
	}
	b = b.withDefaults()
	recent := b.recentUserMessages(current, hist)
	if len(recent) == 0 {
...
func (b memoryQueryBuilder) recentUserMessages(current string, hist []history.HistoryRecord) []string {
	if len(hist) == 0 || b.MaxRecentMessages <= 0 {
		return nil
	}
	...
	for i := len(hist) - 1; i >= 0 && len(reversed) < b.MaxRecentMessages; i-- {
		msg := hist[i].ModelMessage

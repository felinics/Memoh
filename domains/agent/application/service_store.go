...
func (s *Service) storeRound(ctx context.Context, req ChatRequest, messages []agentdomain.ModelMessage, modelID string) error {
...
func (s *Service) storeRoundWithOptions(ctx context.Context, req ChatRequest, messages []agentdomain.ModelMessage, modelID string, opts storeRoundOptions) error {
	_, err := s.storeRoundWithOptionsResult(ctx, req, messages, modelID, opts)
	return err
}

func (s *Service) storeRoundWithOptionsResult(ctx context.Context, req ChatRequest, messages []agentdomain.ModelMessage, modelID string, opts storeRoundOptions) ([]messagepkg.Message, error) {
	fullRound := make([]agentdomain.ModelMessage, 0, len(messages))
	...
	filtered := make([]agentdomain.ModelMessage, 0, len(fullRound))
...
func isEmptyAssistantMessage(m agentdomain.ModelMessage) bool {
...
func (s *Service) storeMessages(ctx context.Context, req ChatRequest, messages []agentdomain.ModelMessage, modelID string, opts storeRoundOptions) []messagepkg.Message {
	...
	for i, msg := range messages {
		...
		if err != nil {
			s.logger.WarnContext(ctx, "storeMessages: marshal failed", slog.Any("error", err))
			continue
		}
		...
		if msg.Role == "user" {
			...
			messageSenderDisplayName = strings.TrimSpace(req.DisplayName)
			messageSenderAvatarURL = strings.TrimSpace(req.AvatarURL)
			...
			isOriginalSkillActivation := req.UserMessageKind == agentdomain.UserMessageKindSkillActivation &&
			...
			if isOriginalQuery {
				...
				switch {
				case strings.TrimSpace(req.UserVisibleText) != "" || req.UserMessageKind == agentdomain.UserMessageKindSkillActivation:
		...
		persistInputs = append(persistInputs, messagepkg.PersistInput{
			...
			SenderAvatarURL:         messageSenderAvatarURL,
	...
	if batcher, ok := s.messageService.(messagepkg.ToolTailRoundPersister); ok {
		if persisted, handled, err := batcher.PersistToolTailRound(ctx, persistInputs); handled || err != nil {
			if err != nil {
				s.logger.WarnContext(ctx, "persist tool tail round failed", slog.Any("error", err))
				return nil
			}
			return persisted
		}
...
func outboundAssetRefsToMessageRefs(refs []agentdomain.OutboundAssetRef) []messagepkg.AssetRef {
	if len(refs) == 0 {
		return nil
	}
...
// chatAttachmentsToAssetRefs converts agentdomain.Attachment slice to message AssetRef slice.
// Only attachments that carry a content_hash are included.
func chatAttachmentsToAssetRefs(attachments []agentdomain.Attachment) []messagepkg.AssetRef {
...
func publicSkillActivationMetadata(activation *agentdomain.SkillActivation) map[string]any {
	if activation == nil {
		return nil
	}
	out := map[string]any{}
...
func publicRequestedSkillMetadata(items []agentdomain.RequestedSkillContext) []map[string]any {
	if len(items) == 0 {
		return nil
	}
...
func chatAttachmentMetadata(attachments []agentdomain.Attachment) []map[string]any {
	if len(attachments) == 0 {
		return nil
	}
	result := make([]map[string]any, 0, len(attachments))
	for _, att := range attachments {
		item := bundleFromAttachment(att).ToMap()
		if len(item) > 0 {

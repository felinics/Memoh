...
func NewStore(queries queries, bots schedule.BotReader) *Store {
	return &Store{queries: queries, bots: bots}
}

func NewStoreFromDB(db dbsqlc.DBTX, bots schedule.BotReader) *Store {
...
func (s *Store) ListEnabled(ctx context.Context) ([]schedule.Record, error) {
...
func (s *Store) Create(ctx context.Context, command schedule.CreateCommand) (schedule.Record, error) {
	botID, err := db.ParseUUID(command.BotID)
	if err != nil {
		return schedule.Record{}, err
	}
	...
	if err != nil {
		return schedule.Record{}, mapNotFound(err)
	}
	return scheduleRecord(row), nil
}

func (s *Store) Get(ctx context.Context, id string) (schedule.Record, error) {
...
func (s *Store) ListByBot(ctx context.Context, botID string) ([]schedule.Record, error) {
...
func (s *Store) Update(ctx context.Context, command schedule.UpdateCommand) (schedule.Record, error) {
...
func (s *Store) IncrementCalls(ctx context.Context, id string) (schedule.Record, error) {
	parsed, err := db.ParseUUID(id)
	if err != nil {
		return schedule.Record{}, err
	}
...
func (s *Store) GetBot(ctx context.Context, id string) (schedule.BotRecord, error) {
...
func (s *Store) CreateLog(ctx context.Context, command schedule.CreateLogCommand) (string, error) {
...
func (s *Store) CompleteLog(ctx context.Context, command schedule.CompleteLogCommand) error {
...
func (s *Store) ListLogsByBot(ctx context.Context, page schedule.LogPage) ([]schedule.LogRecord, error) {
	...
	items := make([]schedule.LogRecord, 0, len(rows))
...
func (s *Store) ListLogsBySchedule(ctx context.Context, page schedule.LogPage) ([]schedule.LogRecord, error) {
	parsed, err := db.ParseUUID(page.ID)
	if err != nil {
		return nil, err
	}
...
func scheduleRecords(rows []dbsqlc.AgentSchedule) []schedule.Record {
	items := make([]schedule.Record, 0, len(rows))
	for _, row := range rows {
...
func scheduleRecord(row dbsqlc.AgentSchedule) schedule.Record {
	var maxCalls *int

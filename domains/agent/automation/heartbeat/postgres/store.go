...
func NewStore(queries queries, bots heartbeat.BotReader) *Store {
	return &Store{queries: queries, bots: bots}
}

func NewStoreFromDB(db dbsqlc.DBTX, bots heartbeat.BotReader) *Store {
...
func (s *Store) ListEnabledBots(ctx context.Context) ([]heartbeat.BotRecord, error) {
	return s.bots.ListEnabledBots(ctx)
}

func (s *Store) GetBot(ctx context.Context, id string) (heartbeat.BotRecord, error) {
...
func (s *Store) CreateLog(ctx context.Context, command heartbeat.CreateLogCommand) (string, error) {
...
func (s *Store) CompleteLog(ctx context.Context, command heartbeat.CompleteLogCommand) error {
...
func (s *Store) ListLogsByBot(ctx context.Context, page heartbeat.LogPage) ([]heartbeat.LogRecord, error) {
	...
	items := make([]heartbeat.LogRecord, 0, len(rows))
	for _, row := range rows {
		items = append(items, heartbeat.LogRecord{

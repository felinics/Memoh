...
type botReaderStub struct {
	listEnabled func(context.Context) ([]heartbeat.BotRecord, error)
	getBot      func(context.Context, string) (heartbeat.BotRecord, error)
}

func (s botReaderStub) ListEnabledBots(ctx context.Context) ([]heartbeat.BotRecord, error) {
	return s.listEnabled(ctx)
}

func (s botReaderStub) GetBot(ctx context.Context, id string) (heartbeat.BotRecord, error) {
...
func TestStoreMapsEnabledAndBotRecords(t *testing.T) {
	store := newStore(&queriesStub{}, botReaderStub{
		...
		getBot: func(_ context.Context, id string) (heartbeat.BotRecord, error) {
...
func TestStoreMapsLogLifecycleAndPage(t *testing.T) {
	...
	logID, err := store.CreateLog(t.Context(), heartbeat.CreateLogCommand{BotID: heartbeatBotID, SessionID: heartbeatSessionID})
	if err != nil || logID != heartbeatLogID {
		t.Fatalf("CreateLog() = %q, %v", logID, err)
	}
	if err := store.CompleteLog(t.Context(), heartbeat.CompleteLogCommand{ID: heartbeatLogID, Usage: []byte(`{"tokens":3}`), ModelID: heartbeatModelID}); err != nil {
	...
	rows, err := store.ListLogsByBot(t.Context(), heartbeat.LogPage{BotID: heartbeatBotID, Limit: 10, Offset: 20})
...
func TestStoreRejectsInvalidUUIDBeforeQuery(t *testing.T) {
	called := false
	store := newStore(&queriesStub{}, botReaderStub{getBot: func(context.Context, string) (heartbeat.BotRecord, error) {
		called = true
		return heartbeat.BotRecord{}, nil
	}})

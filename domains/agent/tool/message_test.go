...
func TestChannelAttachmentsToToolAttachments_NormalizesLocalPath(t *testing.T) {
	...
	atts := channelAttachmentsToToolAttachments([]delivery.Attachment{
		{
			Type: delivery.AttachmentImage,
...
func (s *recordingSender) Send(_ context.Context, _ string, _ delivery.Platform, req delivery.SendRequest) error {
	s.called++
	s.req = req
	return nil
}
...
type recordingReactor struct {
	called int
	req    delivery.ReactRequest
}

func (r *recordingReactor) React(_ context.Context, _ string, _ delivery.Platform, req delivery.ReactRequest) error {
	r.called++
	r.req = req
	return nil
}

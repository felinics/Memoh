...
func (p *EmailProvider) execListEmails(ctx context.Context, session SessionContext, args map[string]any) (any, error) {
	...
	emails, total, err := p.service.ListMailbox(ctx, binding.EmailProviderID, page, pageSize)
	if err != nil {
		return nil, err
	}
...
func (p *EmailProvider) execReadEmail(ctx context.Context, session SessionContext, args map[string]any) (any, error) {
	...
	item, err := p.service.ReadMailbox(ctx, binding.EmailProviderID, uint32(uidRaw)) //nolint:gosec // bounds checked above
	if err != nil {
		return nil, err
	}

...
type ImageGenProvider struct {
	...
	resolver   modelcatalog.ProviderResolver
...
func NewImageGenProvider(
	log *slog.Logger,
	settingsSvc BotSettingsReader,
	modelsSvc *modelcatalog.Service,
	resolver modelcatalog.ProviderResolver,
	containers bridge.Provider,
	dataMount string,
) *ImageGenProvider {
...
func (p *ImageGenProvider) Tools(ctx context.Context, session SessionContext) ([]sdk.Tool, error) {
	if p.settings == nil || p.models == nil || p.resolver == nil {
		return nil, nil
	}
...
func (p *ImageGenProvider) execGenerateImage(ctx context.Context, session SessionContext, toolCallID string, args map[string]any) (any, error) {
	...
	if !modelResp.HasCompatibility(modeldomain.CompatImageOutput) {
		return nil, errors.New("configured model does not support image generation")
	}

	provider, err := modelcatalog.FetchProviderByID(ctx, p.resolver, modelResp.ProviderID)
	if err != nil {
	...
	image, err := p.generateImage(ctx, provider, modelResp.ModelID, prompt, size)
	if err != nil {
...
func (*ImageGenProvider) generateImage(ctx context.Context, provider modelcatalog.ResolvedProvider, modelID, prompt, size string) (generatedImage, error) {
	baseURL := provider.BaseURL
	httpClient := modelexecution.NewProviderHTTPClient(modelexecution.DefaultProviderRequestTimeout)

	switch {
	case shouldUseDashScopeImageGeneration(string(provider.ClientType), baseURL, modelID):
		return generateDashScopeImage(ctx, httpClient, baseURL, provider.APIKey, modelID, prompt, size)
	case shouldUseOpenAIImagesGeneration(string(provider.ClientType), baseURL, modelID):
		return generateOpenAIImagesImage(ctx, httpClient, baseURL, provider.APIKey, modelID, prompt, size)
	default:
		return generateChatImage(ctx, provider, modelID, prompt, size)
	}
}
...
func generateChatImage(ctx context.Context, provider modelcatalog.ResolvedProvider, modelID, prompt, size string) (generatedImage, error) {

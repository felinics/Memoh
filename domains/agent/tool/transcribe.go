...
// transcriptionMediaStore is the narrow asset surface TranscriptionProvider needs.
type transcriptionMediaStore interface {
...
type TranscriptionProvider struct {
	...
	media    transcriptionMediaStore
	http     *http.Client
}

func NewTranscriptionProvider(log *slog.Logger, settingsSvc BotSettingsReader, audioSvc *audiopkg.Service, mediaSvc transcriptionMediaStore) *TranscriptionProvider {
	if log == nil {
		log = slog.Default()
	}

...
func NewWebFetchProvider(log *slog.Logger, settingsSvc BotSettingsReader, fetchSvc *fetch.Service) *WebFetchProvider {
	if log == nil {
		log = slog.Default()
	}
...
func (p *WebFetchProvider) execWebFetch(ctx context.Context, session SessionContext, args map[string]any) (any, error) {
	...
	provider := fetch.ProviderRecord{Provider: string(fetch.ProviderNative), Enable: true}
	...
	if provider.Provider == "" {
		provider.Provider = string(fetch.ProviderNative)
	}
	if provider.Provider != string(fetch.ProviderNative) && !provider.Enable {
...
func (p *WebFetchProvider) callFetchProvider(ctx context.Context, providerName string, configJSON []byte, rawURL, format string) (any, error) {
	...
	case "", string(fetch.ProviderNative):
		return p.callNativeWebFetch(ctx, rawURL, format)
	case string(fetch.ProviderJina):
		return p.callJinaReaderFetch(ctx, configJSON, rawURL)
	case string(fetch.ProviderCloudflareMarkdown):
...
func (*WebFetchProvider) callJinaReaderFetch(ctx context.Context, configJSON []byte, rawURL string) (any, error) {
	...
	return map[string]any{
		"success": true, "url": rawURL, "provider": string(fetch.ProviderJina),
...
func (*WebFetchProvider) callCloudflareMarkdownFetch(ctx context.Context, configJSON []byte, rawURL string) (any, error) {
	...
	return map[string]any{
		"success": true, "url": rawURL, "provider": string(fetch.ProviderCloudflareMarkdown),
		"format": "markdown", "contentType": resp.Header.Get("Content-Type"),
		"content": content, "length": len(content),
	}, nil
}
...
func (*WebFetchProvider) processJSON(fetchedURL, contentType string, body []byte) (any, error) {
	...
	return map[string]any{"success": true, "url": fetchedURL, "provider": string(fetch.ProviderNative), "format": "json", "contentType": contentType, "data": data}, nil
}
...
func (*WebFetchProvider) processXML(fetchedURL, contentType string, body []byte) (any, error) {
	...
	return map[string]any{"success": true, "url": fetchedURL, "provider": string(fetch.ProviderNative), "format": "xml", "contentType": contentType, "content": content}, nil
}
...
func (p *WebFetchProvider) processHTML(fetchedURL, contentType string, body []byte) (any, error) {
	...
	return map[string]any{
		"success": true, "url": fetchedURL, "provider": string(fetch.ProviderNative), "format": "markdown", "contentType": contentType,
...
func (*WebFetchProvider) processText(fetchedURL, contentType string, body []byte) (any, error) {
	...
	return map[string]any{"success": true, "url": fetchedURL, "provider": string(fetch.ProviderNative), "format": "text", "contentType": contentType, "content": content, "length": length}, nil
}
...
func registerFetchProviderSecrets(provider fetch.ProviderRecord) {
	...
	if len(secrets) > 0 {
		redact.SetSecrets("fetch:"+provider.ID, secrets...)
	}
}

func fetchProviderDisplayName(provider fetch.ProviderRecord) string {

...
func NewWebProvider(log *slog.Logger, settingsSvc BotSettingsReader, searchSvc *search.Service) *WebProvider {
	if log == nil {
		log = slog.Default()
	}
	return &WebProvider{
...
func registerSearchProviderSecrets(provider search.ProviderRecord) {
	...
	if len(secrets) > 0 {
		redact.SetSecrets("search:"+provider.ID, secrets...)
	}
}

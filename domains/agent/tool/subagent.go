...
func (p *SpawnProvider) Tools(ctx context.Context, session SessionContext) ([]sdk.Tool, error) {
	...
	if catalog, err := p.listModelCatalog(ctx, session.UserID); err == nil {
...
func (p *SpawnProvider) execListModels(ctx context.Context, session SessionContext, _ map[string]any) (any, error) {
	catalog, err := p.listModelCatalog(ctx, session.UserID)
	if err != nil {
		return nil, err
	}
...
func (p *SpawnProvider) listModelCatalog(ctx context.Context, userID string) ([]subagentModelCatalogItem, error) {
	if p.models == nil || p.resolver == nil {
		return nil, errors.New("model catalog services not configured")
	}
	modelList, err := p.models.ListEnabledByType(ctx, modeldomain.ModelTypeChat)
	if err != nil {
		return nil, err
	}
	items := make([]subagentModelCatalogItem, 0, len(modelList))
	authCtx := oauth.WithUserID(ctx, strings.TrimSpace(userID))
	for _, model := range modelList {
		provider, fetchErr := modelcatalog.FetchProviderByID(authCtx, p.resolver, model.ProviderID)
		if fetchErr != nil {
			return nil, fetchErr
		}
		description := ""
...
func (p *SpawnProvider) resolveModel(
	...
	var modelInfo modelcatalog.GetResponse
	var err error
	switch {
	case modelUUID != "":
	...
	case requestedModelID != "":
		catalog, catalogErr := p.listModelCatalog(ctx, session.UserID)
		if catalogErr != nil {
	...
	authCtx := oauth.WithUserID(ctx, strings.TrimSpace(session.UserID))
	provider, err := modelcatalog.FetchProviderByID(authCtx, p.resolver, modelInfo.ProviderID)
	if err != nil {
		return resolvedSubagentModel{}, err
	}
	if !provider.Enable {

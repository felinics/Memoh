...
func NewMessageProvider(log *slog.Logger, sender delivery.Sender, reactor delivery.Reactor, resolver delivery.ChannelTypeResolver, assetResolver delivery.AssetResolver) *MessageProvider {
	if log == nil {
		log = slog.Default()
	}
	exec := &delivery.Executor{
	...
	if promoter, ok := sender.(delivery.AttachmentPromoter); ok {
		...
		tools = append(tools, sdk.Tool{
			...
			Parameters: map[string]any{
				...
				"properties": map[string]any{
					...
					"reply_to": map[string]any{"type": "string", "description": "Message ID to reply to. The reply will reference this message on the iam."},
	...
	return map[string]any{
		...
		"properties": map[string]any{
			"type": map[string]any{
				"type": "string",
				"enum": []any{
					string(delivery.AttachmentImage),
					string(delivery.AttachmentAudio),
					string(delivery.AttachmentVideo),
					string(delivery.AttachmentVoice),
					string(delivery.AttachmentFile),
					string(delivery.AttachmentGIF),
				},
			},
...
func channelAttachmentsToToolAttachments(atts []delivery.Attachment) []Attachment {
	if len(atts) == 0 {
		return nil
	}
...
func toMessagingSession(s SessionContext) delivery.SessionContext {
	return delivery.SessionContext{
		BotID:              s.BotID,
		ChatID:             s.ChatID,

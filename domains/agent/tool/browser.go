...
func NewBrowserProvider(log *slog.Logger, settingsSvc BotSettingsReader, containers bridge.Provider, displayService runtimedisplay.Service, dataRoot string) *BrowserProvider {
	if log == nil {
		log = slog.Default()
	}
...
func (p *BrowserProvider) execComputerAction(ctx context.Context, session SessionContext, args map[string]any) (any, error) {
	...
	case "mouse_move", "pointer":
		...
		if err := p.sendDisplayInputs(ctx, botID, runtimedisplay.ControlInput{Type: "pointer", X: x, Y: y, ButtonMask: mask}); err != nil {
			return nil, err
		}
...
func (p *BrowserProvider) pointerClick(ctx context.Context, botID string, x, y int, mask byte) error {
	return p.sendDisplayInputs(ctx, botID,
		runtimedisplay.ControlInput{Type: "pointer", X: x, Y: y, ButtonMask: mask},
		runtimedisplay.ControlInput{Type: "pointer", X: x, Y: y, ButtonMask: 0},
	)
}

func (p *BrowserProvider) pointerDrag(ctx context.Context, botID string, fromX, fromY, toX, toY int, mask byte) error {
	events := []runtimedisplay.ControlInput{{Type: "pointer", X: fromX, Y: fromY, ButtonMask: mask}}
	for i := 1; i <= 12; i++ {
		t := float64(i) / 12
		x := int(math.Round(float64(fromX) + float64(toX-fromX)*t))
		y := int(math.Round(float64(fromY) + float64(toY-fromY)*t))
		events = append(events, runtimedisplay.ControlInput{Type: "pointer", X: x, Y: y, ButtonMask: mask})
	}
	events = append(events, runtimedisplay.ControlInput{Type: "pointer", X: toX, Y: toY, ButtonMask: 0})
...
func (p *BrowserProvider) typeText(ctx context.Context, botID, text string) error {
	events := make([]runtimedisplay.ControlInput, 0, len(text)*2)
	for _, r := range text {
		ks := keysymForRune(r)
		events = append(events,
			runtimedisplay.ControlInput{Type: "key", Keysym: ks, Down: true},
			runtimedisplay.ControlInput{Type: "key", Keysym: ks, Down: false},
		)
	}
	return p.sendDisplayInputs(ctx, botID, events...)
}
...
func (p *BrowserProvider) typeKeyChord(ctx context.Context, botID, chord string) error {
	...
	events := make([]runtimedisplay.ControlInput, 0, len(parts)*2)
	for _, part := range parts[:len(parts)-1] {
		ks := namedKeysym(part)
		if ks == 0 {
		...
		events = append(events, runtimedisplay.ControlInput{Type: "key", Keysym: ks, Down: true})
	}
	...
	events = append(events,
		runtimedisplay.ControlInput{Type: "key", Keysym: main, Down: true},
		runtimedisplay.ControlInput{Type: "key", Keysym: main, Down: false},
	)
	for i := len(modifiers) - 1; i >= 0; i-- {
		events = append(events, runtimedisplay.ControlInput{Type: "key", Keysym: modifiers[i], Down: false})
	}
	return p.sendDisplayInputs(ctx, botID, events...)
}

func (p *BrowserProvider) sendDisplayInputs(ctx context.Context, botID string, events ...runtimedisplay.ControlInput) error {

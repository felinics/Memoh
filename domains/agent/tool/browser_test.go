...
func (s *computerDisplayExecServer) Exec(stream bridgepb.ContainerService_ExecServer) error {
	req, err := stream.Recv()
	if err != nil {
		return err
	}
	...
	return stream.Send(&bridgepb.ExecOutput{Stream: bridgepb.ExecOutput_EXIT, ExitCode: exitCode})
}

func newComputerDisplayTestClient(t *testing.T, server bridgepb.ContainerServiceServer) *bridge.Client {
	t.Helper()

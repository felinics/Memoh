...
func TestDoCompactionSharesPromptBudgetWithPriorContext(t *testing.T) {
	...
	rows := make([]CandidateRecord, 0, 13)
	for i := 0; i < 12; i++ {
		role := "user"
		if i%2 == 1 {
			role = "assistant"
	...
	run := func(t *testing.T, withPrior bool) int {
		...
		var priorLogs []ArtifactRecord
		if withPrior {
			priorLogs = []ArtifactRecord{priorLogFor(t, cfg, strings.Repeat("p", 1200))}
		}
...
func TestDoCompactionSacrificesPriorContextForOversizedEntries(t *testing.T) {
	...
	q := &fakeQueries{
		uncompacted: rows,
		priorLogs:   []ArtifactRecord{priorLogFor(t, cfg, prior)},
	}
	svc := newMachineryService(q)
	res, err := svc.RunCompactionSync(context.Background(), cfg)
	if err != nil {
...
func TestDoCompactionCountsPriorSeparatorsInSharedBudget(t *testing.T) {
	...
	cfg := machineryConfig(stub, 100)
	cfg.MaxCompactTokens = 1000
	priors := make([]ArtifactRecord, 0, 100)
	for i := 0; i < 100; i++ {
...
func priorLogFor(t *testing.T, cfg TriggerConfig, summary string) ArtifactRecord {
	t.Helper()

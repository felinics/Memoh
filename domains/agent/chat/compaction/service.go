...
func NewService(log *slog.Logger, store CompactionStore, artifacts ArtifactStore) *Service {
	return &Service{
		store:     store,
		artifacts: artifacts,
		logger:    log,
...
func (s *Service) runCompaction(ctx context.Context, cfg TriggerConfig) (Result, *inflightRun, error) {
	...
	preHookRan = true
	if _, err := parseUUID(cfg.BotID); err != nil {
		compactErr = err
		return Result{}, nil, compactErr
	}
	if _, err := parseUUID(cfg.SessionID); err != nil {
		compactErr = err
		return Result{}, nil, compactErr
	}

	compactRes, compactErr = s.doCompaction(ctx, cfg)
...
func (s *Service) ListLogs(ctx context.Context, botID string, limit, offset int) ([]Log, int64, error) {
	...
	total, err := s.store.CountLogs(ctx, botID)
	if err != nil {
		return nil, 0, err
	}

	rows, err := s.store.ListLogs(ctx, ListLogsInput{
		BotID:  botID,
		Limit:  limit,
		Offset: offset,
	})
...
func (s *Service) DeleteLogs(ctx context.Context, botID string) error {
	...
	return s.store.DeleteLogs(ctx, botID)
}

func toLog(r LogRecord) Log {
	l := Log{
		ID:           r.ID,
		BotID:        r.BotID,
...
func parseUUID(value string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("parse UUID %q: %w", value, err)
	}

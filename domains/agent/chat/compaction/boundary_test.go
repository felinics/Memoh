...
func toolCallRow(t *testing.T, tokens int) CandidateRecord {
	t.Helper()
	return mkRow(t, "assistant", `[{"type":"tool-call","toolName":"calc","toolCallId":"c1","input":{}}]`, tokens)
}

func toolResultRow(t *testing.T, tokens int) CandidateRecord {
	t.Helper()
...
func textRow(t *testing.T, role string, tokens int) CandidateRecord {
	t.Helper()
...
func TestTrimCompactMessagesKeepsOldestAndToolExchangeIntact(t *testing.T) {
	...
	if trimmed[len(trimmed)-1].ID != items[2].ID {
		t.Fatalf("newest overflow must be deferred, got tail %s", trimmed[len(trimmed)-1].ID)
	}
}
...
func TestTrimCompactMessagesFloorsTinyRowCosts(t *testing.T) {
	...
	rows := make([]CandidateRecord, 0, 100)
	for i := 0; i < 100; i++ {

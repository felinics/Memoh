...
func (s *Service) doCompaction(ctx context.Context, cfg TriggerConfig) (Result, error) {
	rows, err := s.store.ListCandidates(ctx, cfg.SessionID)
	if err != nil {
		return Result{}, err
	}
	if len(rows) == 0 {
	...
	frontier, err := NewArtifactProjection(s.artifacts).LoadActiveSession(ctx, ArtifactOwner{BotID: cfg.BotID, SessionID: cfg.SessionID, SessionIDKnown: true})
	if err != nil {
		return Result{}, err
	}
	...
	logID, err := s.store.CreateLog(persistCtx, CreateLogInput{
		BotID:         cfg.BotID,
	...
	marked, err := s.store.ClaimCandidates(persistCtx, ClaimCandidatesInput{
		LogID:              logID,
		MessageIDs:         compactedMessageIDs,
		ExpectedCompactIDs: expectedCompactIDs,
	})
	if err != nil {
		_ = s.completeLog(persistCtx, logID, "error", "", err.Error(), 0, nil, "", nil)
		return Result{}, err
	}
	...
	assetRows, err := s.store.ListAssets(persistCtx, compactedMessageIDs)
	if err != nil {
	...
	if cost := entriesPromptCost(entries); cost+priorTokens > maxCompactTokens {
		s.logger.WarnContext(ctx, "compaction: entry floors exceed the budget, prompt may overflow the compaction window",
	...
	model := modelexecution.NewSDKChatModel(modelexecution.SDKModelConfig{
	...
	systemPromptDecorated, sdkMessages, _ := modelexecution.ApplyPromptCache(
	...
	if strings.TrimSpace(result.Text) == "" {
		_ = s.completeLog(persistCtx, logID, "error", "", errEmptySummary.Error(), 0, nil, "", nil)
	...
	modelID := ""
	if parsed, parseErr := parseUUID(cfg.ModelID); parseErr == nil {
		modelID = parsed
	}
	if err := s.completeLog(persistCtx, logID, "ok", result.Text, "", len(compactedMessageIDs), usageJSON, modelID, &artifact); err != nil {
...
func expectedCompactionClaims(rows []CandidateRecord, messageIDs []string) ([]string, error) {
	byID := make(map[string]string, len(rows))
	for _, row := range rows {
		byID[row.ID] = row.CompactID
	}
	expected := make([]string, 0, len(messageIDs))
	for _, id := range messageIDs {
		claim, ok := byID[id]
		if !ok {
			return nil, fmt.Errorf("compaction source %s missing from selected rows", id)
		}
...
func (s *Service) completeLog(ctx context.Context, logID, status, summary, errMsg string, messageCount int, usage []byte, modelID string, artifact *artifactMetadata) error {
	coverage := []byte("[]")
	...
	err := s.store.CompleteLog(ctx, CompleteLogInput{
		ID:            logID,
		Status:        status,
		Summary:       summary,

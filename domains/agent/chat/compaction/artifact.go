...
type CoveredSource struct {
	Ref                    fragment.ContextRef `json:"ref"`
...
func (a Artifact) HistoryRecord(scope fragment.Scope) history.HistoryRecord {
	if scope.BotID == "" {
		scope.BotID = a.BotID
	}
	if scope.SessionID == "" {
	...
	coveredRefs := make([]fragment.ContextRef, 0, len(a.Coverage))
	for _, source := range a.Coverage {
		coveredRefs = append(coveredRefs, source.Ref)
	}
	record := history.SummaryRecord(a.ID, a.Summary, coveredRefs, scope)
...
func (a Artifact) CoversRecord(record history.HistoryRecord) bool {
	createdAtMs := int64(0)
...
func artifactMetadataFor(items []CompactionCandidate, ids []string) (artifactMetadata, error) {
	byID := make(map[string]CompactionCandidate, len(items))
	...
	for _, id := range ids {
		item, ok := byID[id]
		if !ok {
			return artifactMetadata{}, fmt.Errorf("compaction artifact: marked id %s missing from candidates", id)
		}
		createdAtMs := int64(0)
...
func validatePersistedCoverageRef(ref fragment.ContextRef) error {
	if err := fragment.ValidateContextRef(ref); err != nil {
		return err
	}
	if ref.HashAlgo != fragment.HashAlgoSHA256 {
		return fmt.Errorf("hash algo must be %q", fragment.HashAlgoSHA256)
	}
	if ref.HashScope != fragment.HashScopeSourcePayload {
		return fmt.Errorf("hash scope must be %q", fragment.HashScopeSourcePayload)
	}

...
func mkRow(t *testing.T, role, content string, outputTokens int) CandidateRecord {
	t.Helper()
	row := CandidateRecord{
		ID:      testUUID(t),
		Role:    role,
...
func TestEstimateItemTokensAcceptsSnakeCaseUsage(t *testing.T) {
	...
	items, _ := itemsFromRows([]CandidateRecord{row})
...
func TestItemsFromRowsPreservesDirectedSignalMetadata(t *testing.T) {
	...
	row.ExternalMessageID = "msg-123"
	row.SourceReplyToMessageID = "msg-parent"
	row.EventID = testUUID(t)
	row.SenderDisplayName = "Alice"
	row.Platform = "telegram"
	row.ConversationType = "group"
	row.ConversationName = "Ops Room"
	row.ReplyTarget = "thread-9"

	items, barrierCount := itemsFromRows([]CandidateRecord{row})
...
func TestGuardedSelectionCompactsAroundMustKeepAskExchange(t *testing.T) {
	...
	for _, id := range ids {
		...
		for _, protected := range []string{rows[6].ID, rows[7].ID} {
			if id == protected {
...
func TestGuardedSelectionKeepsCompactRunContiguousAcrossMustKeepIsland(t *testing.T) {
	...
	wantIDs := []string{rows[0].ID, rows[1].ID}
	if len(gotIDs) != len(wantIDs) {
		t.Fatalf("selected %d rows, want the contiguous pre-island run [old q, old a]", len(gotIDs))
	}
	for i := range wantIDs {
		if gotIDs[i] != wantIDs[i] {
			t.Fatalf("selected run must be the contiguous pre-island run, got id[%d]=%s", i, gotIDs[i])
		}
	}
...
func TestItemsFromRowsPreservesUnparseableRowsAsSelectionBarriers(t *testing.T) {
	...
	bad := CandidateRecord{
		ID:      "", // empty id -> FromDBMessage rejects it
		Role:    "user",
		Content: json.RawMessage(`"x"`),
	}
	after := mkRow(t, "assistant", `"after"`, 100)
	current := mkRow(t, "user", `"current"`, 100)
	items, barrierCount := itemsFromRows([]CandidateRecord{before, bad, after, current})
	if barrierCount != 1 {
...
func assertPolicy(t *testing.T, item CompactionCandidate, policy CompactPolicy) {
	...
	if !item.HasPolicy(policy) {
		t.Fatalf("candidate %s missing policy %s; got %#v", item.ID, policy, item.Policies)
	}
}
...
func assertNoPolicy(t *testing.T, item CompactionCandidate, policy CompactPolicy) {
	t.Helper()
	if item.HasPolicy(policy) {
		t.Fatalf("candidate %s unexpectedly has policy %s; got %#v", item.ID, policy, item.Policies)
	}
}
...
func TestSplitByRatioBoundaryConditions(t *testing.T) {
	...
	rows := []CandidateRecord{mkRow(t, "user", `"a"`, 100)}
	items, _ := itemsFromRows(rows)
...
func TestTrimCompactMessagesAccountsForDirectedSignalHeaders(t *testing.T) {
	...
	for i := range rows {
		rows[i].ExternalMessageID = longID
		rows[i].SenderDisplayName = longSender
	}
	items, _ := itemsFromRows(rows)

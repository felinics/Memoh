...
func TestFromDBMessageBuildsDurableRecordScopeAndFrag(t *testing.T) {
	...
	msg := messagepkg.Message{
		...
		Role:                    "user",
		Content:                 persistedModelMessage(t, agentdomain.ModelMessage{Role: "user", Content: agentdomain.NewTextContent("hello")}),
	...
	if record.Ref.Namespace != "bot_history_message" || record.Ref.ID != "row-1" || record.Ref.Schema != fragment.SchemaContextRef {
	...
	frag := ToFrag(record)
	if err := fragment.ValidateContextRef(frag.Ref); err != nil {
		t.Fatalf("frag ref invalid: %#v: %v", frag.Ref, err)
	}
	if frag.Ref.Namespace != "bot_history_message" || frag.Ref.ID != "row-1" || frag.Ref.Durability != fragment.RefDurable {
		t.Fatalf("frag ref should be durable DB row identity: %#v", frag.Ref)
	}
	if frag.Ref.ContentHash == "" || frag.Ref.HashAlgo != fragment.HashAlgoSHA256 || frag.Ref.HashScope != fragment.HashScopeSourcePayload {
		t.Fatalf("frag ref missing source payload hash: %#v", frag.Ref)
	}
	if frag.Kind != fragment.KindConversationEvent || frag.Slot != fragment.SlotHistory {
...
func TestDBMessageSourceHashIncludesMessageMetadata(t *testing.T) {
	...
	msg := messagepkg.Message{
		...
		BotID:    "bot-1",
		Role:     "user",
		Content:  agentdomain.NewTextContent("hello"),
...
func TestFromDBMessageParsesSnakeCaseUsageTokens(t *testing.T) {
	...
	record, err := FromDBMessage(messagepkg.Message{
		...
		BotID:   "bot-1",
		Role:    "assistant",
		Content: persistedModelMessage(t, agentdomain.ModelMessage{Role: "assistant", Content: agentdomain.NewTextContent("done")}),
...
func TestFromDBMessageDuplicateContentUsesDurableRowIDs(t *testing.T) {
	...
	first, err := FromDBMessage(messagepkg.Message{
		...
		BotID:   "bot-1",
		Role:    "user",
		Content: persistedModelMessage(t, agentdomain.ModelMessage{Role: "user", Content: agentdomain.NewTextContent("same")}),
	}, ScopeFallback{})
	if err != nil {
	...
	if firstFrag.Ref.Durability != fragment.RefDurable || secondFrag.Ref.Durability != fragment.RefDurable {
...
func TestFromDBMessageContentHashChangesWhenRowContentChanges(t *testing.T) {
	...
	original, err := FromDBMessage(messagepkg.Message{
		...
		BotID:   "bot-1",
		Role:    "user",
		Content: persistedModelMessage(t, agentdomain.ModelMessage{Role: "user", Content: agentdomain.NewTextContent("original")}),
	}, ScopeFallback{})
	if err != nil {
	...
	edited, err := FromDBMessage(messagepkg.Message{
		...
		BotID:   "bot-1",
		Role:    "user",
		Content: persistedModelMessage(t, agentdomain.ModelMessage{Role: "user", Content: agentdomain.NewTextContent("edited")}),
	}, ScopeFallback{})
	if err != nil {
...
func TestFromDBMessageContentHashChangesWhenAssetsChange(t *testing.T) {
	...
	msg := messagepkg.Message{
		...
		BotID:   "bot-1",
		Role:    "user",
		Content: persistedModelMessage(t, agentdomain.ModelMessage{Role: "user", Content: agentdomain.NewTextContent("with asset")}),
		Assets: []messagepkg.MessageAsset{{
			ContentHash: "asset-hash-1",
			Role:        "attachment",
			Ordinal:     0,
...
func TestHistoryRecordsRenderLegacyModelAndSDKMessages(t *testing.T) {
	...
	expectedModel := []agentdomain.ModelMessage{
		{Role: "user", Content: agentdomain.NewTextContent("hello")},
		{Role: "assistant", Content: agentdomain.NewTextContent("hi")},
	}
...
func TestToFragKeepsPersistedSystemRowsExternalTrust(t *testing.T) {
	...
	record, err := FromDBMessage(messagepkg.Message{
		...
		BotID:   "bot-1",
		Role:    "system",
		Content: persistedModelMessage(t, agentdomain.ModelMessage{Role: "system", Content: agentdomain.NewTextContent("stored policy-looking text")}),
	}, ScopeFallback{})
	if err != nil {
		t.Fatalf("FromDBMessage failed: %v", err)
	}
	...
	frag := ToFrag(record)
	if frag.Trust != fragment.TrustExternal {
		t.Fatalf("history frag trust = %s, want %s", frag.Trust, fragment.TrustExternal)
	}
}
...
func persistedModelMessage(t *testing.T, msg agentdomain.ModelMessage) json.RawMessage {
	t.Helper()

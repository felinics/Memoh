	...
	case message.ListActiveSince:
		...
		for _, row := range rows {
			item := messageFromFields(
				...
				row.SourceReplyToMessageID, row.Role, row.Content, row.Metadata, row.Usage,
				row.SessionMode, row.RuntimeType, row.EventID, row.DisplayText, row.CreatedAt)
			item.CompactID = uuidString(row.CompactID)
...
func (s *MessageStore) GetVisibleMessageCursor(ctx context.Context, sessionID, messageID string) (message.MessageCursor, error) {
	...
	return message.MessageCursor{
		...
		CreatedAt: row.CreatedAt.Time, MessageID: uuidString(row.ID),
	}, nil
}
...
func (s *MessageStore) ListMessagesBeforeCursor(ctx context.Context, sessionID string, cursor message.MessageCursor, limit int32) ([]message.Message, error) {
	session, err := dbpkg.ParseUUID(sessionID)
	if err != nil {
		return nil, err
	}
	id, err := dbpkg.ParseUUID(cursor.MessageID)
	if err != nil {
		return nil, err
	}
...
func (s *MessageStore) LocateMessages(ctx context.Context, sessionID, externalID string, beforeLimit, afterLimit int32) (message.LocateResult, error) {
	...
	result := message.LocateResult{Messages: make([]message.Message, 0, len(rows)), TargetID: uuidString(rows[0].TargetID)}
	for _, row := range rows {
...
func (s *MessageStore) ListAssetLinks(ctx context.Context, ids []string) (map[string][]message.MessageAsset, error) {
	...
	for _, row := range rows {
		...
		result[uuidString(row.MessageID)] = append(result[uuidString(row.MessageID)], message.MessageAsset{
...
func messageFromFields(
	...
	result := message.Message{
		ID: uuidString(id), BotID: uuidString(botID), SessionID: uuidString(sessionID),
		SenderChannelIdentityID: uuidString(senderChannelID), SenderUserID: uuidString(senderUserID),
		SenderDisplayName: textString(senderDisplayName), SenderAvatarURL: textString(senderAvatarURL),
		Platform: textString(platform), ExternalMessageID: textString(externalID),
		SourceReplyToMessageID: textString(replyID), Role: role,
		...
		EventID: uuidString(eventID), DisplayContent: textString(displayText), CreatedAt: createdAt.Time,
	}
...
func historyTurnFromCreate(row dbsqlc.CreateHistoryTurnRow) message.HistoryTurn {
	return historyTurnFromFields(
		row.ID, row.BotID, row.SessionID, row.Position, row.RequestMessageID, row.AssistantMessageID,
		row.SupersededByTurnID, row.SupersededAt, textString(row.SupersededReason), row.CreatedAt, row.UpdatedAt,
	)
}
...
func historyTurnFromFields(
	...
	return message.HistoryTurn{
		ID: uuidString(id), BotID: uuidString(botID), SessionID: uuidString(sessionID),
		Position: position, RequestMessageID: uuidString(requestMessageID),
		AssistantMessageID: uuidString(assistantMessageID),
		SupersededByTurnID: uuidString(supersededByTurnID),
...
func historyTurnFromReplace(row dbsqlc.ReplaceHistoryTurnRow) message.HistoryTurn {
	return message.HistoryTurn{
		ID: uuidString(row.ID), BotID: uuidString(row.BotID), SessionID: uuidString(row.SessionID),
		Position: row.Position, RequestMessageID: uuidString(row.RequestMessageID),
		AssistantMessageID: uuidString(row.AssistantMessageID),
		SupersededByTurnID: uuidString(row.SupersededByTurnID),
		SupersededAt:       timestamp(row.SupersededAt), SupersededReason: textString(row.SupersededReason),
...
func twoUUIDs(a, b string) (pgtype.UUID, pgtype.UUID, error) {
	first, err := dbpkg.ParseUUID(a)
	if err != nil {
		return pgtype.UUID{}, pgtype.UUID{}, err
	}
	second, err := dbpkg.ParseUUID(b)
	return first, second, err
}

func mustUUID(value string) pgtype.UUID {
	id, _ := dbpkg.ParseUUID(value)
	return id
}

...
type serviceReaderStub struct {
	...
	models       []usagepersistence.Model
	recordFilter usagepersistence.Filter
	pagination   usagepersistence.Pagination
	page         usagepersistence.Page
}

func (s *serviceReaderStub) GetByModel(_ context.Context, filter usagepersistence.Filter) ([]usagepersistence.Model, error) {
	s.modelFilter = filter
	return s.models, nil
}

func (s *serviceReaderStub) ListRecords(_ context.Context, filter usagepersistence.Filter, pagination usagepersistence.Pagination) (usagepersistence.Page, error) {
...
type modelProjectionReaderStub struct {
	calls       int
	modelIDs    []string
	projections map[string]usagepersistence.ModelProjection
}

func (s *modelProjectionReaderStub) GetModelProjections(_ context.Context, modelIDs []string) (map[string]usagepersistence.ModelProjection, error) {
	s.calls++
...
func TestServiceBatchEnrichesModelsAndPreservesFilterOrder(t *testing.T) {
	...
	raw := &serviceReaderStub{models: []usagepersistence.Model{
		{ModelID: firstID, InputTokens: 30},
		{ModelID: secondID, InputTokens: 20},
		{ModelID: firstID, InputTokens: 10},
	}}
	models := &modelProjectionReaderStub{projections: map[string]usagepersistence.ModelProjection{
		firstID:  {ModelSlug: "model-a", ModelName: "usagepersistence.Model A", ProviderName: "Provider A"},
		secondID: {ModelSlug: "model-b", ModelName: "usagepersistence.Model B", ProviderName: "Provider B"},
	}}
	filter := usagepersistence.Filter{BotID: "bot-id", ModelID: firstID, SessionType: "discuss"}
	...
	if len(got) != 3 || got[0].ModelName != "usagepersistence.Model A" || got[1].ProviderName != "Provider B" || got[2].InputTokens != 10 {
...
func TestServiceUsesUnknownProjectionForMissingModel(t *testing.T) {
	...
	raw := &serviceReaderStub{page: usagepersistence.Page{
		Items: []usagepersistence.Record{{ID: "record-id", ModelID: modelID}},
		Total: 7,
	}}
	models := &modelProjectionReaderStub{projections: map[string]usagepersistence.ModelProjection{}}
	filter := usagepersistence.Filter{BotID: "bot-id", ModelID: modelID}
	pagination := usagepersistence.Pagination{Limit: 20, Offset: 40}
...
func TestServiceSkipsProjectionReadForEmptyResults(t *testing.T) {
	t.Parallel()
	raw := &serviceReaderStub{models: []usagepersistence.Model{}, page: usagepersistence.Page{Items: []usagepersistence.Record{}}}
	models := &modelProjectionReaderStub{}
	service := NewService(raw, models)

	if _, err := service.GetByModel(t.Context(), usagepersistence.Filter{BotID: "bot-id"}); err != nil {
		t.Fatalf("GetByModel() error = %v", err)
	}
	if _, err := service.ListRecords(t.Context(), usagepersistence.Filter{BotID: "bot-id"}, usagepersistence.Pagination{}); err != nil {
		t.Fatalf("ListRecords() error = %v", err)
	}
	if models.calls != 0 {

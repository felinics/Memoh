...
func TestPostgresRuntimeFenceUnfencedReplacementIsAtomic(t *testing.T) {
	...
	service := NewService(nil, newPostgresMessageStore(pool))
	...
	if err := pool.QueryRow(ctx, "SELECT COUNT(*) FROM agent.bot_history_messages WHERE session_id = $1", sessionID).Scan(&beforeFailedReplacement); err != nil {
	...
	if err := pool.QueryRow(ctx, "SELECT COUNT(*) FROM agent.bot_history_messages WHERE session_id = $1", sessionID).Scan(&afterFailedReplacement); err != nil {
	...
	if err := pool.QueryRow(ctx, "SELECT metadata->'forked_from'->>'fork_message_id' FROM agent.bot_sessions WHERE id = $1", sessionID).Scan(&forkMessageID); err != nil {
...
func TestPostgresRuntimeFenceLockSerializesTokenAdvance(t *testing.T) {
	...
	if _, err := agentsqlc.New(tx).LockSessionRuntimeFence(ctx, agentsqlc.LockSessionRuntimeFenceParams{
		SessionID:           sessionID,
		BotID:               botID,
	...
	go func() {
		token, err := queries.ActivateSessionRuntimeFence(ctx, agentsqlc.ActivateSessionRuntimeFenceParams{SessionID: sessionID, BotID: botID, RuntimeFencingToken: token2})
...
func TestPostgresRuntimeFenceLockSerializesConcurrentWritersBeforeUpdate(t *testing.T) {
	...
	params := agentsqlc.LockSessionRuntimeFenceParams{
		SessionID:           sessionID,
		BotID:               botID,
	...
	if _, err := agentsqlc.New(tx1).LockSessionRuntimeFence(ctx, params); err != nil {
	...
	go func() {
		...
		backendPID <- pid
		if _, err := agentsqlc.New(tx2).LockSessionRuntimeFence(ctx, params); err != nil {
			locked <- writerResult{stage: "lock", err: err}
			return
		}
		locked <- writerResult{stage: "locked"}
		<-proceed
		if _, err := tx2.Exec(ctx, "UPDATE agent.bot_sessions SET next_turn_position = next_turn_position + 1 WHERE id = $1", sessionID); err != nil {
	...
	if _, err := tx1.Exec(ctx, "UPDATE agent.bot_sessions SET next_turn_position = next_turn_position + 1 WHERE id = $1", sessionID); err != nil {
...
func acquireRuntimeFenceToken(t *testing.T, ctx context.Context, queries *agentsqlc.Queries, botID, sessionID pgtype.UUID) int64 {
	t.Helper()
	token, err := queries.NextSessionRuntimeFenceToken(ctx)
	if err != nil {
		t.Fatalf("allocate runtime fence: %v", err)
	}
	activated, err := queries.ActivateSessionRuntimeFence(ctx, agentsqlc.ActivateSessionRuntimeFenceParams{SessionID: sessionID, BotID: botID, RuntimeFencingToken: token})
	if err != nil {

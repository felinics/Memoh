...
func TestPostgresMismatchedClaimArraysDoNotReclaimSource(t *testing.T) {
	...
	if err := fixture.pool.QueryRow(ctx, `
		SELECT status FROM agent.bot_history_message_compacts WHERE id = $1
...
func TestPostgresReclaimSerializesWithSourceCompletion(t *testing.T) {
	...
	if _, err := agentsqlc.New(completeTx).CompleteCompactionLog(ctx, successfulCompaction(first.ID)); err != nil {
...
func TestPostgresSourceCompletionCannotPassCommittedReclaim(t *testing.T) {
	...
	go func() {
		_, err := agentsqlc.New(completeTx).CompleteCompactionLog(ctx, successfulCompaction(first.ID))
		result <- err
	}()
...
func TestPostgresCompactionClaimLocksSessionBeforeMessages(t *testing.T) {
	...
	if _, err := sessionTx.Exec(ctx, `SELECT id FROM agent.bot_sessions WHERE id = $1 FOR UPDATE`, fixture.sessionID); err != nil {
...
func TestPostgresCrossOwnerClaimLocksArtifactsInIDOrder(t *testing.T) {
	...
	if _, err := fixture.pool.Exec(ctx, `
		INSERT INTO agent.bot_history_message_compacts (
	...
	if _, err := fixture.pool.Exec(ctx, `
		UPDATE agent.bot_history_messages SET compact_id = $1 WHERE id = $2
	...
	if err := fixture.pool.QueryRow(ctx, `
		SELECT compact_id FROM agent.bot_history_messages WHERE id = $1
...
func successfulCompaction(id pgtype.UUID) agentsqlc.CompleteCompactionLogParams {
	return agentsqlc.CompleteCompactionLogParams{
		ID:           id,
		Status:       "ok",
		Summary:      "summary",
		MessageCount: 2,

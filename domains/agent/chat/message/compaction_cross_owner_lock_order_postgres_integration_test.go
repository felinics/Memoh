...
func TestPostgresCompactionClaimLocksMessagesInIDOrder(t *testing.T) {
	...
	if _, err := blockerTx.Exec(ctx, `
		SELECT id FROM agent.bot_history_messages WHERE id = $1 FOR UPDATE
	...
	if _, err := probeTx.Exec(ctx, `
		SELECT id FROM agent.bot_history_messages WHERE id = $1 FOR UPDATE NOWAIT
...
func TestPostgresCrossOwnerClaimLocksOwnerSessionsInIDOrder(t *testing.T) {
	...
	if _, err := probeTx.Exec(ctx, `
		SELECT id FROM agent.bot_sessions WHERE id = $1 FOR UPDATE NOWAIT
...
func TestPostgresDeletedSourceDoesNotReclaimForeignArtifact(t *testing.T) {
	...
	if err := agentsqlc.New(fixture.pool).DeleteMessagesByIDs(ctx, []pgtype.UUID{messageUUID}); err != nil {
		t.Fatalf("delete claimed source message: %v", err)
	}

	marked, err := agentsqlc.New(fixture.pool).MarkMessagesCompacted(ctx, agentsqlc.MarkMessagesCompactedParams{

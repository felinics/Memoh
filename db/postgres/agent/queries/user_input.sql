...
next_short_id AS (
  SELECT COALESCE(MAX(agent.user_input_requests.short_id), 0) + 1 AS short_id
  FROM locked_session
  LEFT JOIN agent.user_input_requests ON agent.user_input_requests.session_id = locked_session.id
    AND agent.user_input_requests.team_id = iam.memoh_current_team_id()
)
INSERT INTO agent.user_input_requests (
  bot_id,
  session_id,
  route_id,
  channel_identity_id,
...
SET requested_by_channel_identity_id = EXCLUDED.requested_by_channel_identity_id,
    ...
    updated_at = now()
WHERE agent.user_input_requests.status = 'pending'
  AND agent.user_input_requests.runtime_fencing_token IS NOT DISTINCT FROM EXCLUDED.runtime_fencing_token
  AND agent.user_input_requests.input_json = EXCLUDED.input_json
  AND agent.user_input_requests.ui_payload_json = EXCLUDED.ui_payload_json
  AND agent.user_input_requests.provider_metadata = EXCLUDED.provider_metadata
  AND agent.user_input_requests.workspace_target_id = EXCLUDED.workspace_target_id
  AND (agent.user_input_requests.expires_at IS NULL OR agent.user_input_requests.expires_at > now())
RETURNING *;

-- name: GetUserInputRequest :one
SELECT *
FROM agent.user_input_requests
...
UPDATE agent.user_input_requests

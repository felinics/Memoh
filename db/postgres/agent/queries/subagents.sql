...
INSERT INTO agent.subagent_configs (
  session_id,
  model_uuid,
  model_id,
  provider_name,
...
prepared_messages AS MATERIALIZED (
  ...
  LEFT JOIN agent.bot_history_messages source
    ON source.team_id = iam.memoh_current_team_id()
...
copied_assets AS (
  ...
  JOIN agent.bot_history_message_assets asset
    ON asset.team_id = iam.memoh_current_team_id()

...
INSERT INTO agent.bot_sessions (
  bot_id, route_id, channel_type, conversation_type, conversation_name, reply_target,
  type, session_mode, runtime_type, runtime_metadata, title, metadata, parent_session_id, created_by_user_id
)
VALUES (
  sqlc.arg(bot_id),
  ...
  sqlc.narg(conversation_name)::text,
...
WITH source_session AS (
  SELECT s.*
  FROM agent.bot_sessions s
  WHERE s.team_id = iam.memoh_current_team_id()
...
target_turn AS (
  ...
  JOIN agent.bot_visible_history_messages vm ON vm.team_id = iam.memoh_current_team_id()
...
copy_messages AS (
  SELECT
    ...
    vm.turn_message_seq,
    vm.created_at
  FROM agent.bot_visible_history_messages vm
  JOIN target_turn tt ON (
    vm.turn_position < tt.position
    OR vm.turn_position = tt.position
  )
  WHERE vm.team_id = iam.memoh_current_team_id()
...
created_session AS (
  ...
  SELECT
    fp.bot_id,
    fp.channel_type,
    fp.conversation_type,
    fp.conversation_name,
    fp.reply_target,
    fp.type,
    fp.session_mode,
    fp.runtime_type,
...
copied_assets AS (
  ...
  WHERE a.team_id = iam.memoh_current_team_id()
  RETURNING id
)
SELECT cs.*
...
SELECT nextval('agent.session_runtime_fencing_token_seq')::bigint;
...
SELECT
  s.id, s.bot_id, s.route_id, s.channel_type, s.conversation_type, s.conversation_name, s.reply_target,
  s.type, s.session_mode, s.runtime_type, s.runtime_metadata, s.title, s.metadata,
...
-- name: LockSessionForRouteAssignment :one
SELECT id
FROM agent.bot_sessions
WHERE team_id = iam.memoh_current_team_id()
  AND id = sqlc.arg(session_id)
...
WHERE team_id = iam.memoh_current_team_id() AND id = sqlc.arg(id) AND deleted_at IS NULL
RETURNING *;
...
WHERE team_id = iam.memoh_current_team_id()
  ...
  AND EXISTS (
    SELECT 1
    FROM agent.subagent_configs config
    WHERE config.team_id = iam.memoh_current_team_id()
      AND config.session_id = agent.bot_sessions.id
  )
...
UPDATE agent.bot_sessions
SET deleted_at = now(), updated_at = now()
WHERE team_id = iam.memoh_current_team_id() AND bot_id = sqlc.arg(bot_id) AND deleted_at IS NULL;

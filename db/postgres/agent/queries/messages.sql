...
VALUES (
  ...
  sqlc.narg(sender_display_name)::text,
  sqlc.narg(sender_avatar_url)::text,
...
WITH target AS (
  ...
  WHERE existing.team_id = iam.memoh_current_team_id() AND existing.turn_id = sqlc.arg(turn_id)
...
WHERE team_id = iam.memoh_current_team_id() AND id = sqlc.arg(session_id)
...
WITH next_position AS (
  UPDATE agent.bot_sessions
  SET next_turn_position = next_turn_position + 1
  WHERE agent.bot_sessions.team_id = iam.memoh_current_team_id() AND agent.bot_sessions.id = sqlc.arg(session_id)
  RETURNING (next_turn_position - 1)::bigint AS position
),
inserted_message AS (
  INSERT INTO agent.bot_history_messages (
...
WITH input_rows(
  ...
  VALUES
      (
        ...
        sqlc.narg(user_sender_display_name)::text,
        sqlc.narg(user_sender_avatar_url)::text,
      ...
      (
        ...
        sqlc.narg(tool_call_assistant_sender_display_name)::text,
        sqlc.narg(tool_call_assistant_sender_avatar_url)::text,
      ...
      (
        ...
        sqlc.narg(tool_sender_display_name)::text,
        sqlc.narg(tool_sender_avatar_url)::text,
      ...
      (
        ...
        sqlc.narg(final_assistant_sender_display_name)::text,
        sqlc.narg(final_assistant_sender_avatar_url)::text,
...
WITH target AS (
  ...
  WHERE request.team_id = iam.memoh_current_team_id() AND request.session_id = sqlc.arg(session_id)
...
linked AS (
  ...
  SET turn_id = target.turn_id,
      ...
      turn_superseded_reason = NULL
  FROM target
  WHERE assistant.team_id = iam.memoh_current_team_id() AND assistant.id = sqlc.arg(assistant_message_id)
...
SET turn_id = target.turn_id,
    ...
    turn_superseded_reason = NULL
FROM target
WHERE m.team_id = iam.memoh_current_team_id() AND m.id = sqlc.arg(message_id)
...
WHERE m.team_id = iam.memoh_current_team_id() AND m.session_id = sqlc.arg(session_id)
...
WITH latest AS (
  ...
  WHERE visible.team_id = iam.memoh_current_team_id() AND visible.session_id = sqlc.arg(session_id)
...
WITH target AS (
  ...
  FROM agent.bot_history_messages assistant
  WHERE assistant.team_id = iam.memoh_current_team_id() AND assistant.turn_id = sqlc.arg(turn_id)
    AND assistant.role = 'assistant'
    AND assistant.turn_message_seq = 2
    AND assistant.turn_visible = true
  LIMIT 1
...
WITH target AS (
  SELECT m.turn_id, m.session_id, m.turn_position
  FROM agent.bot_history_messages m
  WHERE m.team_id = iam.memoh_current_team_id() AND m.turn_id = sqlc.arg(old_turn_id)
...
replacement_input AS (
  ...
  WHERE (
      ...
      OR EXISTS (
        ...
        WHERE request_message.team_id = iam.memoh_current_team_id() AND request_message.id = sqlc.narg(request_message_id)::uuid
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE assistant_message.team_id = iam.memoh_current_team_id() AND assistant_message.id = sqlc.narg(assistant_message_id)::uuid
...
updated AS (
  ...
  SET turn_superseded_by_turn_id = replacement.id,
      ...
      turn_visible = false
  FROM replacement
  WHERE old.team_id = iam.memoh_current_team_id() AND old.turn_id = sqlc.arg(old_turn_id)
...
FROM agent.bot_history_messages m
LEFT JOIN agent.bot_sessions s ON s.id = m.session_id AND s.team_id = iam.memoh_current_team_id()
WHERE m.team_id = iam.memoh_current_team_id() AND m.bot_id = sqlc.arg(bot_id)
ORDER BY m.created_at ASC, m.id ASC;
...
WHERE m.team_id = iam.memoh_current_team_id() AND m.session_id = sqlc.arg(session_id)
  ...
  AND (m.turn_position, m.turn_message_seq, m.created_at, m.id)
    < (
      ...
      WHERE c.team_id = iam.memoh_current_team_id() AND c.session_id = sqlc.arg(session_id)

...
-- Idempotency ledger for complete native-agent steps. The history messages for
-- a step and this marker are written in one transaction, so a retried callback
-- can distinguish "already committed" from "not committed" without widening
-- every message row with checkpoint metadata.

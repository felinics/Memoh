<template>
  ...
  <PackageProgressDialog
    ...
    :error="active?.error"
    :done-label="$t('packages.viewBotPackages')"
    @update:open="setProgressOpen"
    @retry="retry"
    @done="openBotPackages"
  />
</template>
...
import PackageProgressDialog from '@/pages/bots/components/package-progress-dialog.vue'
import { usePackageOperation } from '@/pages/bots/composables/usePackageOperation'
...
const { active, progressOpen, start, retry, setProgressOpen } = usePackageOperation(selectedBotId, 'supermarket-install')

function handleInstall() {
  const pkg = props.pkg
  if (!selectedBotId.value || !pkg?.registry_id || !pkg.package_id || !pkg.revision) return
  const started = start({
    targetId: selectedTargetId.value,
    registryId: pkg.registry_id,
    packageId: pkg.package_id,
    name: packageDisplayName(pkg, locale.value),
    action: 'install',
    install: {
      registryId: pkg.registry_id,
      packageId: pkg.package_id,
...
function openBotPackages() {
  const botId = selectedBotId.value
  if (!botId) return
  emit('installed', botId)
  void router.push({ name: 'bot-detail', params: { botName: botId }, query: { tab: 'packages' } }).catch(() => {})
}
</script>

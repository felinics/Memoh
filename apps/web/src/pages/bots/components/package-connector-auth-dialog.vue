  ...
  >
    ...
    >
      <DialogHeader>
        ...
        <DialogDescription>
          {{ catalog?.description || t('packages.connector.authDescription', { name: packageName }) }}
        </DialogDescription>
      </DialogHeader>
      ...
      <DialogBody>
        <Alert
          ...
          variant="default"
        >
          <AlertTitle>{{ t('packages.connector.unavailableTitle') }}</AlertTitle>
          <AlertDescription>{{ t('packages.connector.unavailableDescription') }}</AlertDescription>
        </Alert>
        <form
          v-else
          id="package-connector-auth-form"
      ...
      <DialogFooter>
        ...
        <Button
          v-if="catalog"
          form="package-connector-auth-form"
          type="submit"
          :loading="phase !== 'idle'"
        >
...
// Authorizes one connector a Package references. The connection is created
// through the Package endpoints so the Server links it to the installation;
...
import {
  beginPackageConnectorOAuth,
  createPackageConnectorCredential,
...
const props = defineProps<{
  ...
  packageName: string
  connector: PackageConnectorItem | null
...
async function connect() {
  ...
  try {
    ...
    if (method.type === 'oauth2') {
      const result = await beginPackageConnectorOAuth(props.botId, props.installationId, connectorType, method.key)
      ...
      await createPackageConnectorCredential(
        props.botId,

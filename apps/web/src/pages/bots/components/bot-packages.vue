<template>
  ...
  >
    ...
    <PackageProgressDialog
      ...
      :action="activePackage?.action ?? 'install'"
      :steps="activePackage?.steps ?? []"
      :lines="activePackage?.lines ?? []"
      :status="activePackage?.status ?? 'running'"
      :result="activePackage?.result"
      :error="activePackage?.error"
      @update:open="setPackageProgressOpen"
      @retry="retryPackage"
    />

    <PackageRemoveDialog
      :open="!!removeTarget"
      :name="removeTarget ? packageDisplayName(removeTarget, locale) : ''"
    ...
    <PackageConnectorAuthDialog
      ...
      :package-name="authTarget?.packageName ?? ''"
...
import {
  checkPackageUpdates,
  fetchPackageRemovalPreview,
  invalidateBotPackages,
  packageDisplayName,
  packageInProgress,
  packageKey,
  useBotPackagesQuery,
  ...
  switch (workspaceState.value) {
    case 'not_running':
      return {
        ...
        description: t('packages.workspaceNotRunningDescription'),
...
// ---- Package operations (streamed) -----------------------------------------

const {
  active: activePackage,
  progressOpen: packageProgressOpen,
  running,
  ownsStream: ownsPackageStream,
  start: startPackage,
  retry: retryPackage,
  viewProgress: viewPackageProgress,
...
const removeTarget = ref<PackageItem | null>(null)
const removePreview = ref<PackageRemovalPreview | null>(null)
const removePreviewLoading = ref(false)
const removePreviewError = ref('')

async function openRemove(item: PackageItem) {
  if (!item.installation_id) return
  ...
  try {
    removePreview.value = await fetchPackageRemovalPreview(props.botId, item.installation_id)
  } catch (err) {
...
async function onConnector(item: PackageItem, connector: PackageConnectorItem, action: PackageConnectorAction) {
  if (!item.installation_id) return
  if (action === 'authorize') {
    authTarget.value = { installationId: item.installation_id, packageName: packageDisplayName(item, locale.value), connector }
    return
  }
...
async function onConnectorAuthorized() {
  await Promise.all([
    invalidateBotPackages(queryCache, props.botId),
    queryCache.invalidateQueries({ key: ['bot-connectors', props.botId] }),
  ])
}

async function reauthorize(connector: PackageConnectorItem) {
  ...
  try {
    await runMutation(() => rollbackDependency(props.botId, selectedTargetId.value, item.id ?? ''), {
      ...
      onSuccess: async () => {
        ...
        await Promise.all([invalidateBotPackages(queryCache, props.botId), invalidateBotDependencies(queryCache, props.botId)])
...
async function checkUpdates() {
  ...
  checking.value = true
  try {
    await runMutation(() => checkPackageUpdates(props.botId, selectedTargetId.value), {
      fallbackMessage: t('packages.checkUpdatesFailed'),
      onSuccess: async (refreshed) => {
        queryCache.setQueryData(['bot-packages', props.botId, selectedTargetId.value], refreshed)
        await Promise.all([invalidateBotPackages(queryCache, props.botId), invalidateBotDependencies(queryCache, props.botId)])
        toast.success(t('packages.checkUpdatesDone'))
      },
    })
  } finally {
...
const hasForeignProgress = computed(() => items.value.some(item => packageInProgress(item) && !ownsPackageStream(item.registry_id, item.package_id)))

...
// Picks what to update on one Package before anything runs: the release
...
export interface PackageUpdateChoice {
  release: boolean
  dependencies: string[]
}
...
const emit = defineEmits<{
  ...
  confirm: [choice: PackageUpdateChoice]
}>()
...
const name = computed(() => (props.item ? packageDisplayName(props.item, locale.value) : ''))

const candidates = computed<Candidate[]>(() => {
  const item = props.item
  if (!item) return []
  const list: Candidate[] = []
  if (packageUpdateAvailable(item)) {
    list.push({
      key: RELEASE_KEY,
      label: t('packages.update.release'),
      from: item.version || '',
      to: item.available_version || item.available_revision?.slice(0, 8) || '',
    })
  }
  for (const dep of packageDependencyUpdates(item)) {
    ...
    >
      <DialogHeader class="min-w-0">
        <DialogTitle class="break-words">
          {{ t('packages.update.title', { name }) }}
        </DialogTitle>
        <DialogDescription class="break-words">
          {{ t('packages.update.description') }}
        </DialogDescription>
      </DialogHeader>
      ...
      <DialogFooter>
        ...
        <Button
          ...
          @click="confirm"
        >
          {{ t('packages.update.confirm', { count: selectedCount }, selectedCount) }}
        </Button>
      </DialogFooter>
    </DialogPanel>
  </Dialog>

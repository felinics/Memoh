<template>
  <!-- Full-bleed drop feedback, Teleported to <body>.
       ...
       No transition on enter. The user is holding a file over the window asking
       "will this land?" — any ramp, even one frame's worth, reads as the app
       thinking about it. Leave keeps a short fade so the drop doesn't end in a
       flicker before the upload's own feedback appears.
  ...
  <Teleport to="body">
    <Transition
      leave-active-class="transition-opacity duration-[70ms] ease-in"
      ...
      >
        ...
        >
          <component
            :is="icon"
            :stroke-width="1.5"
            class="size-12 text-accent-blue"
          />
          <span class="text-display font-semibold text-foreground">{{ label }}</span>
        </div>
      </div>
    </Transition>
  </Teleport>
...
// background at 85%, NO blur, no black dim. Blur is out by standing instruction
...
// white sheet); 88% still mutes black text too much. 85% → ghosts #DADADA.
...
// dead" look.
const scrimClass = 'pointer-events-none fixed inset-0 z-(--z-top) bg-background/88' /* ui-allow-alpha: overlay scrim — a full-bleed cover layer is its own role (packages/ui/AGENTS.md § Alpha policy allowlist) */
...
const anchorClass = computed(() => props.bounds
  ? 'absolute flex flex-col items-center justify-center gap-4 px-4 text-center'

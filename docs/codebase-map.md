...
- **Workspace dependencies (`internal/workspacedeps/`)** — per-bot installation of agent CLIs and other managed dependencies. Metadata and confirmed recovery targets remain persistent; payloads and caches use the fixed workspace path `/data/.memoh/deps`. Each dependency has one confirmed target, with staging, failure recovery, and deferred cleanup of retired payloads. The permission invariants (read-only launcher resolution, Manage-authorized install confirmation, no remote runtime support) are normative rules — see `AGENTS.md` → Container / Workspace Management. The Server/image upgrade boundary is documented in `docs/workspace-dependencies-upgrade.md`.

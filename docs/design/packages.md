...
- Supermarket 里 package 只是 `packages/<id>/skills/<skill>/` 的容器，可选的 `package.yaml` 只有 `postinstall`。package 的名称、描述、标签、图标全部由 skills 聚合而来，没有版本号。
...
1. 用户面只有 package 一种可浏览、可安装、可删除、可更新的单元。
2. package 可以组合 skills、dependencies、connectors 三类组件。
3. package 拥有版本、作者、主页、许可证、分类、图标与多语言描述，并支持检查更新与更新。
4. deps 与 connector 在 registry 与 Memoh 两侧都保持独立实体，package 只按 id 引用，同一 bot 上不会重复安装。

## 2. 已定决策

| 决策 | 结论 |
| --- | --- |
| 名字 | 沿用 package，中文 UI 显示为“扩展包” |
| deps 的位置 | 保留 `registries/memoh/dependencies/<id>/`，package 只引用 id；不搬进 package 目录 |
| deps 的独立发布 | 保留不可变发布、`dependencies.lock.json` 与 `/api/dependencies*`，只撤掉独立浏览入口 |
| connector | package 引用 connect-it 的 `connector_type`，绑定与凭据仍由 connectors 服务与 connect-it 管理 |
| 数据迁移 | 不做，当前版本尚未发布 |
| 部分安装 | 允许，package 有 `partial` 状态 |
| 哪些 registry 可含 deps 与 connector | 第一期仅 `memoh` registry；远程 registry 的 package 只有 skills |
| dep 的 revision 是否被 package 锁定 | 不锁定，安装与更新时总是解析 dep 当前定义，与现有 workspacedeps 行为一致 |
| 工作区目标粒度 | package 安装记录与 dep 引用按 `(bot, workspace_target)`；connector 引用挂在安装记录上，但 connection 是 bot 级共享 |
| 子项删除 | skills、deps、connector 不能在 package 之外单独删除；dep 子项保留 update、reinstall、rollback、查看脚本，connector 子项保留授权、重新授权、启停 |
...
│   └── packages/<pkg>/
│       ├── package.yaml                  # memoh registry 必需，schema 2
│       ├── icon.svg                      # 可选
│       └── skills/<skill-id>/SKILL.md    # 可选，package 至少要有一种组件
└── openai/                               # 不变，adapter 派生 package
```

### 3.2 `package.yaml` schema 2
...
- package 至少要有一个 skill、一个 dependency 或一个 connector。
- `translations` 只接受 `en`、`zh`、`ja`，结构与 dependency 的 `translations` 一致。
- skill 的 `metadata.author`、`metadata.homepage` 缺省时回落到 package。
...
远程 registry 的自由文本分类先经 `normalizeSkillCategory` 归一化，再按 `aliases` 匹配，匹配不到落到 `other`。`memoh` registry 的 package 与 dependency 的 `category` 必须是表中的 id。

### 3.4 规范包

每个 dependency 都必须有一个同 id 的 package 作为它在商店里的载体，构建时校验。第一期新增五个包：`node`、`python`、`uv`、`codex`、`claude-code`，各自只引用同名 dependency，`python` 的 `requires: [uv]` 关系仍由 dependency.yaml 表达，不在 package 层重复。

### 3.5 发布产物

`SkillPackageRelease` 与 `SkillPackageDescriptor` 新增字段：`version`、`author`、`homepage`、`repository`、`license`、`icon`、`category`、`category_name`、`translations`、`dependencies: [{ id }]`、`connectors: [{ type, required }]`。`SkillPackageSummary` 新增 `version`、`category`、`dependency_count`、`connector_count`。package 的 revision 仍是 release JSON 字节的 SHA-256，因此引用列表、元数据或任一 skill 变化都会产生新 revision；dependency 定义变化不影响 package revision。

### 3.6 API 变化

| 变化 | 说明 |
| --- | --- |
| `GET /api/packages` | 新增 `component=skills\|dependencies\|connectors` 过滤；`category` 改为按 categories.yaml 的 id 过滤 |
...
| `GET /api/artifacts/icon/:digest` | 不变，package 图标复用 |

### 3.7 涉及文件

- `registry/package-manifest.ts`：schema 2 解析与校验。
- `registry/adapters/memoh.ts`：读取 package.yaml，允许无 skills 的包，收集 dependencies 与 connectors 引用。
- `registry/adapters/common.ts`、`skill-directory.ts`、`codex-marketplace.ts`：合成远程 registry 的 package 元数据，version 缺省。
...
- `server/api/packages/index.get.ts`、新增 `server/api/categories.get.ts`、删除 `server/api/registries/[id]/categories.get.ts`。
- `registries/categories.yaml`、七个现有包的 `package.yaml`、五个规范包。
...
新增 `internal/packages`，编排三个既有服务，不复制它们的逻辑：

- skills 部分沿用 `internal/supermarket` 的 `FetchPackageRelease`、`preparePackage` 与 `internal/skills` 的 `PublishPackage`、`PreparePackageRemoval`。
...
`internal/skillpackages` 改名 `internal/packages/store`，承载新表的读写。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/bots/:bot_id/packages` | 安装列表，含子项状态与发现的规范包，参数 `workspace_target_id` |
| POST | `/bots/:bot_id/packages` | 安装，SSE |
| GET | `/bots/:bot_id/packages/:installation_id` | 详情 |
...
- `db/postgres/migrations/0149_packages.{up,down}.sql`、`db/postgres/queries/packages.sql`、sqlc 重新生成。
- `internal/packages/`：service、store、install、remove、update、list、events。
- `internal/supermarket/protocol.go`、`package_installer.go`（拆出可复用的 release 拉取与 skills 发布）。
...
- `internal/handlers/packages.go` 新增，`supermarket.go`、`supermarket_skills.go`、`workspace_dependencies.go`、`connectors.go`、`containerd.go` 调整路由。
- `internal/agent/runtime/codex`、`claudecode` 的依赖启用路径改为规范包。
- `cmd/internal/core/module.go`、`providers.go` 注册新服务。
- `docs/design/workspace-dependencies.md` 补一节说明与 package 的关系。
...
| 阶段 | 内容 | 验收 |
| --- | --- | --- |
| 1 Supermarket | schema 2、categories.yaml、adapter 与候选构建校验、发布产物字段、API 变化、五个规范包与七个现有包的 package.yaml、README | `bun test`、`bun run registry:validate`、`bun run registry:publish` 通过；本地 `GET /api/packages` 返回新字段；`GET /api/categories` 三语可用 |
| 2 Memoh 后端 | 迁移、`internal/packages`、handler、SDK 重新生成、删除旧路由 | Go 单测覆盖安装、删除、更新、部分安装、引用计数与规范包合成；`go test ./internal/packages/... ./internal/workspacedeps/... ./internal/connectors/... ./internal/supermarket/...` 通过；swagger 与 SDK 无 diff 残留 |
| 3 前端 | Supermarket 单列表与详情页、安装对话框、bot 详情 packages tab、操作 store、i18n | vitest 通过；手工验证安装含 dep 与 connector 的包、部分安装后继续、删除预览与执行、检查更新与更新 |
| 4 收尾 | memoh-docs 的 supermarket、connectors、bot 指南更新，新增 packages 指南；删除前端与后端残留代码与 i18n key | 文档与实际行为一致 |
...
- 远程 registry 的 package 未来若要映射 `mcpServers` 为 connector，需要 connect-it 先提供对应 type，不在本期。

...
这是Inbox与A2A之间唯一需要连接的地方，对应README第6节的O3：

> 异步A2A调用的结果在调用方会话已经结束时，落在哪里？

两个候选：

1. **丢弃**——符合「同步委托」的语义，实现最简单。倾向此项。
2. **投递到调用方的Inbox**——如果希望`run_in_background`的跨Bot委托能可靠地把结果送回，就必须走这条。

选择第二项会让Inbox多出一个事件源，第1节的分工规则需要相应修订。该决策在Phase 2收尾时确定。

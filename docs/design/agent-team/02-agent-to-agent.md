...
1. B调用`ask_user`，请求写入`user_input_requests`（`internal/agent/decision/input/`），标记预期回应方是A而非人类。
2. A此前的工具调用**不返回最终答案，而是返回一个待澄清状态**，携带B的问题与句柄。
3. A通过`send_message(id, ...)`在同一句柄上作答，走既有的追加消息路径。
4. B收到回答后继续执行。
...
- **回答方消失时快速失败。** A的会话已结束时不会有人作答；按D18该次委托的结果本就会被丢弃，因此B应当在等待超时后结束本轮，而不是无限挂起。
...
- **`owner_user_id`为空时fail closed**：拒绝该次工具调用，并把原因回传给A，不得默认放行。
- **审批超时后委托失败**，失败原因必须明确指出是审批未完成，而不是笼统的超时。
- 审批请求必须能追溯到发起委托的A及其原始发起人（第7.3节），否则审批人无法判断这次调用的来由。

### 8.3 迟到结果直接丢弃（D18）

调用方会话已结束时，异步委托的结果**直接丢弃，不投递到Inbox**。

需要明确的是：**丢弃的是投递，不是执行记录。** B的这次会话及其全部输出仍然完整持久化在B名下，人类可以在B的会话列表中查看。丢弃只意味着没有任何自动路径把它送回给A。
